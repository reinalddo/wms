                ...
                <div class="ibox-title">
                    <div class="row">
                        <div class="col-lg-4" id="_title">
                            <h3>Administrador de Pedidos CrossDocking</h3>
                        </div>
                    </div>
                </div>
                <form id="myform">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-group date" id="data_1">
                                        <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input id="fechai" type="text" class="form-control" value="<?php //if($_SERVER['HTTP_HOST'] == 'dicoisa.assistprowms.com' || $_SERVER['HTTP_HOST'] == 'www.dicoisa.assistprowms.com')echo $fecha_actual; ?>">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-group date" id="data_2">
                                        <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input id="fechaf" type="text" class="form-control" value="<?php //if($_SERVER['HTTP_HOST'] == 'dicoisa.assistprowms.com' || $_SERVER['HTTP_HOST'] == 'www.dicoisa.assistprowms.com') echo $fecha_actual; ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <table id="dt-detalles" class="table" style="table-layout: auto;width: 100%;"> <!--lilo-->
                    <thead>
                        <tr style="height: 50px;">
                            ...
                            <th id="col-semaforo" scope="col" style="width: 70px !important; display: none; min-width: 70px !important;">Status</th>
                            <th id="col-asignar" scope="col" style="width: 70px !important; display: none; min-width: 70px !important;">Asignar</th>
                            <th id="col-planificar" scope="col" style="width: 130px !important; display: none; min-width: 130px !important;">Planificar Ola | XD</th>
                ...
                <div class="ibox-content"></br></br>
                    <div class="form-group">
                        <div class="input-group-btn">
                            ...
                            <button id="btn-planificarXD" type="button" class="btn btn-m btn-primary" onclick="VerificarTiposPedidosPlanificar('XD')" style="padding-right: 20px;margin-left: 10px;">Planificar XD</button>
                            -->
                            <?php 
                            /*
                            ?>
...
**********************************************************************************************************
    ...
    <div class="modal inmodal" id="modalItems" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg" id="modal_surtido_responsive" style="width: 90%;">
...
//imageFormat(item.orden,item.surtido, item.TieneOla, '', item.sufijo, is_backorder, '', item.cliente, item.asignado, item.folio_ws, item.status, item.status_pedido, item.fecha_pedido, item.es_ot, item.ruta_pedido)
        function imageFormat(id, surtido, tieneola, tienda, sufijo, is_backorder, cliente, surtidor, articulo_ot, folio_ws, status, status_pedido, fecha_pedido, es_ot, ruta_pedido) {
           ...
           var cve_cia = <?php echo $_SESSION['cve_cia']; ?>;
            var html = '';
            html += '<a href="#" id="detalles_articulos_true'+id+'" onclick="articulos(\'' + id + '\', \'' + sufijo + '\', \'' + is_backorder + '\', \'' + cliente + '\', \'' + surtidor + '\', \'' + articulo_ot + '\', \'' + folio_ws + '\', \'' + ruta_pedido + '\')"><i class="fa fa-search" title="Ver Detalles"></i></a>&nbsp;<a href="#" id="detalles_articulos_false'+id+'" style="display: none;"><i class="fa fa-search" title="Ver Detalles"></i></a>&nbsp;';
            ...
            if(folio_ws)
            {
                html += '<a href="/api/koolreport/export/reportes/pedidos/reporte-ws?id='+id+'&cve_cia='+cve_cia+'&folio_ws='+folio_ws+'&ruta_pedido='+ruta_pedido+'" target="_blank" title="Reporte Consolidado"><i class="fa fa-file-pdf-o"></i></a>';
                html += '<a href="/api/koolreport/excel/pedidosws/export.php?id='+id+'" target="_blank" title="Reporte Consolidado"><i class="fa fa-file-excel-o"></i></a>';
            }

            if (status == 'L' || status == 'R' || status == 'P' || status == 'M' || status == 'C' || status == 'E' || status == 'T' || status == 'F') 
            {
                html += '<a href="#" onclick="imprimirRemision(\'' + id + '\')" id="btnImprimirRemision"><i class="fa fa-file-pdf-o" title="Imprimir Remisión"></i></a>&nbsp;';

                html += '<a href="/api/koolreport/export/reportes/pedidos/reporte_surtido?folio='+id+'&sufijo='+sufijo+'&cve_cia='+cve_cia+'&fecha_pedido='+fecha_pedido+'" target="_blank" id="btn_reporte_surtido" title="Reporte Productos Surtidos"><i class="fa fa-file-pdf-o"></i></a>&nbsp;';
            }
            ...
            {
            ?>
                if(status == 'C' || status == 'E' || status == 'F' || status == 'T')
                    ...
                    if (data.status == 200) {
                        ...
                        $.each(data.data, function(index, item){
                            ...
                            //{
                                ...
                                var style_asignar = "", style_planificar = "", titleXD="", entrada_xd = "";
                                ...
                                if(($("#tipopedido").val() == '' && $("#ruta_pedido_list").val() != '') || ($("#ruta_pedido_list").val() == '' && $("#tipopedido").val() != '') || $("#tipopedido").val() == '4')
                                 style_planificar = " style='display: none;' "; 

                                row += '<tr>'+'<td align="center">'+imageFormat(item.orden, item.surtido, item.TieneOla, '', item.sufijo, is_backorder, item.cliente, item.asignado, '', item.folio_ws, item.status, item.status_pedido, item.fecha_pedido, item.es_ot, item.ruta_pedido)+'</td>'+
                                    //'<td align="center"><input type="checkbox" /></td>'+
                                    '<td align="center" style="display:none;" id="semaforo_'+item.id+"_"+item.sufijo+'" class="column-semaforo">'+'<i class="fa fa-circle '+color+'" aria-hidden="true"></i>'+'</td>'+
                                    '<td align="center" style="display:none;" class="column-asignar" data-id="'+item.orden+'" data-subpedido="'+item.sufijo+'"><input type="checkbox" '+style_asignar+' '+titleXD+' data-id="'+item.orden+'" data-idpedido="'+item.id+'" data-subpedido="'+item.sufijo+'" data-disponible="'+item.disponible+'" class="check-asignar" /></td>'+
                                    '<td align="center" style="display:none;" class="column-planificar" data-id="'+item.orden+'"><input type="checkbox" data-id="'+item.orden+'" '+style_planificar+' /></td>'+
...
*/
              ...
              window.detalle_pedido["articulos"].forEach(function (item,index){
                ...
                if(item["surtidas"] == 0 && item.idy_ubica != '')
                {
                  obj = {
                      ...
                      acciones: '<button  onclick="guardarSurtidoPorUbicacion('+index+', '+item.existencia+')" type="button" class="btn btn-m btn-primary btn-block" id ="btn_surtir_'+((item["clave"]).replace("/", "").replace(".", "") + (item.lote).replace("/", "").replace(".", "") + (item.serie).replace("/", "").replace(".", "") + (item.idy_ubica).replace("/", "").replace(".", "")).replace(" ", "") +'" style="width:70px; display:'+((!item["idy_ubica"])?'none':'block')+'">Surtir</button>'
                  };
        ...
        function guardarSurtidoPorUbicacion(item, existencia_surtir) 
        {
         /* var myData = $("#grid-table-surtido").jqGrid('getRowData');
          ...
          var id_boton = articulo.clave+articulo.lote+articulo.serie+articulo.idy_ubica;
          id_boton = id_boton.replace("/", "").replace(".", "");
          id_boton = id_boton.replace(" ", "");
          console.log("BOTON SURTIR ID = #btn_surtir_"+id_boton);
          ...
          var ultimo = true;
            ...
            {
                    console.log("BOTON SURTIR ID = #btn_surtir_"+ittem["clave"]+ ittem["lote"] + ittem["serie"] + ittem["idy_ubica"]);
                    id_boton2 = ittem["clave"]+ ittem["lote"] + ittem["serie"] + ittem["idy_ubica"];
          ...
          surtir = parseInt($("#surtir-item-"+id_boton).val());
...
*/
        ...
        function finalizar_surtido(porc, es_ot)
        {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $("#generarExcel").click(function(){
            ...
            $(this).attr("href", "/api/koolreport/excel/detalle_pedidos/export.php?almacen="+almacen+"&criterio="+criterio+"&tipopedido="+tipopedido+"&ruta_pedido_list="+ruta_pedido_list+"&fechaInicio="+fechaInicio+"&fechaFin="+fechaFin+"");
              ...
              i++;
                ...
                if(($("#tipopedido").val() == '' && $("#ruta_pedido_list").val() != '') || ($("#ruta_pedido_list").val() == '' && $("#tipopedido").val() != '') || $("#tipopedido").val() == '4')
                 style_planificar = " style='display: none;' "; 
                  ...
                  '<td align="center" class="column-asignar"  data-id="'+item.orden+'" data-subpedido="'+item.sufijo+'"><input type="checkbox" class="check-asignar" data-id="'+item.orden+'" '+style_asignar+' '+titleXD+' data-idpedido="'+item.id+'" data-subpedido="'+item.sufijo+'" data-disponible="'+item.disponible+'" /></td>'+
                  '<td align="center" class="column-planificar" data-id="'+item.orden+'"><input type="checkbox" data-id="'+item.orden+'" '+style_planificar+' /></td>'+
                  '<td>'+item.ruta_pedido+'</td>'+
...
//                  {
        ...
        function articulos(_codigo, sufijo, is_backorder, cliente, surtidor, articulo_ot, folio_ws, ruta_pedido)
        {
          //EDG117
            $("#folio_detalle").html("");
...
/////////////////////////////////////Aqui se contruye el Grid de articulos/////////////////////////////////////////
        $(function($) {
            ...
            $(grid_selector).jqGrid({
                ...
                height: 'auto',
                mtype: 'POST',
                colNames: ['Pedido', 'Subpedido', 'Cliente', 'Clave', 'Articulo', 'Orden de Surtido', 'BL', 'Lote', 'Caducidad', 'Serie', 'Pallet', 'Cajas', 'Piezas', 'Solicitadas','Disponible', 'Surtir', 'Unidad Medida', 'Back Order','Surtidas', 'Volumen (m3)', 'Peso', 'Secuencia', 'Surtidor', 'Ruta'],
                colModel: [
...
/////////////////////////////////////Aqui se contruye el Grid de articulos/////////////////////////////////////////
        $(function($) {
            ...
            $(grid_selector).jqGrid({
                ...
                height: 'auto',
                mtype: 'POST',
                colNames: ['Pedido', 'Cliente', 'Clave', 'Articulo', 'Lote', 'Caducidad | Elaboración', 'Serie', 'Pallet', 'Cajas', 'Piezas', 'Cantidad Pedida', 'Unidad Medida','Disponible','BackOrder', 'Volumen (m3)', 'Peso'],
                colModel: [{
                        name: 'folio',
                        index: 'folio',
                        width: 80,
...
//                 filtro: $("#filtro").val(),
    ...
    $("#status, #tipopedido, #ruta_pedido_list").change(function(){
