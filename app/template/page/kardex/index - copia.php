...
function Kardex(){*/
    ...
    function fillTable(node){
        ...
        body.innerHTML += entrada(node.entrada);
        body.innerHTML += acomodo(node.acomodo);
        body.innerHTML += traslado(node.traslado);
        body.innerHTML += salida(node.salida);
        body.innerHTML += entradaR(node.entradaR);
        body.innerHTML += salidaR(node.salidaR);
        body.innerHTML += entradaA(node.entradaA);
        body.innerHTML += salidaA(node.salidaA);
        ...
        function entrada(node){
            ...
            for(var i = 0; i < node.length ; i++){
                ...
                if(htmlEntities(node[i].prove_clave)) 
                {
                    origen = htmlEntities(node[i].prove_clave);
                    destino = '('+htmlEntities(node[i].almacen_clave)+') '+htmlEntities(node[i].ubi_nombre)+''+htmlEntities(node[i].CodigoCSD);
                }
                else 
                {
                    origen = htmlEntities(node[i].origen);
                    destino = htmlEntities(node[i].destino);
                }
        ...
        function acomodo(node){
            ...
            for(var i = 0; i < node.length ; i++){
                ...
                _body += '<tr>'+
                            ...
                            '<td>Acomodo</td>'+
                            '<td>'+htmlEntities(node[i].Des_Motivo)+'</td>'+
                            '<td>'+htmlEntities(node[i].cve_ubicacion)+'</td>'+
                            '<td>'+htmlEntities(node[i].ubi_nombre)+'</td>'+
                            '<td>'+htmlEntities(node[i].CodigoCSD)+'</td>'+

                            '<td title="'+htmlEntities(node[i].almacen_nombre)+'"><b>'+htmlEntities(node[i].almacen_des)+" ("+htmlEntities(node[i].almacen_clave)+')</b></td>'+
                            '<td align="right">'+htmlEntities(node[i].cantidad)+'</td>'+
                            '<td>'+htmlEntities(node[i].cve_usuario)+'</td>'+
                            '<td>'+htmlEntities(node[i].fecha)+'</td>'+
                         '</tr>';
        ...
        function traslado(node){

            var _body = "", origen = "", destino = "", almacen = "", origen_t = "", destino_t = "";

            for(var i = 0; i < node.length ; i++){

                if(htmlEntities(node[i].almacen_clave_o) != 'undefined')
                {
                    almacen = htmlEntities(node[i].almacen_clave_o);
                    origen = htmlEntities(node[i].almacen_descrip_o);
                    destino = htmlEntities(node[i].almacen_descrip_d)+"("+htmlEntities(node[i].almacen_clave_d)+")";
                    origen_t = htmlEntities(node[i].almacen_nombre_o);
                    destino_t = htmlEntities(node[i].almacen_nombre_d);
                }
                else
                {
                    almacen = "";
                ...
                _body += '<tr>'+
                            ...
                            '<td></td>'+
                            '<td>'+almacen+'</td>'+
                            '<td title="'+origen_t+'"><b>'+origen+'</b></td>'+
                            '<td>'+htmlEntities(node[i].CodigoCSD)+'</td>'+

                            '<td title="'+destino_t+'"><b>'+destino+'</b></td>'+
                            '<td align="right">'+htmlEntities(node[i].cantidad)+'</td>'+
                            '<td>'+htmlEntities(node[i].cve_usuario)+'</td>'+
                            '<td>'+htmlEntities(node[i].fecha)+'</td>'+
                         '</tr>';
        ...
        function salida(node){
            ...
            for(var i = 0; i < node.length ; i++){
                ...
                _body += '<tr>'+
                            ...
                            '<td>'+htmlEntities(node[i].Des_Motivo)+'</td>'+
                            '<td>'+htmlEntities(node[i].almacen_clave)+'</td>'+
                            '<td title="'+htmlEntities(node[i].almacen_nombre_o)+'"><b>'+htmlEntities(node[i].almacen_des)+'</b></td>'+
                            '<td>'+htmlEntities(node[i].CodigoCSD)+'</td>'+

                            '<td title="Folio"><b>'+htmlEntities(node[i].destino)+'</b></td>'+
                            '<td align="right" style="color:red;">-'+htmlEntities(node[i].cantidad)+'</td>'+
                            '<td>'+htmlEntities(node[i].cve_usuario)+'</td>'+
                            '<td>'+htmlEntities(node[i].fecha)+'</td>'+
                         '</tr>';
            }
        ...
        function salidaR(node){
            ...
            for(var i = 0; i < node.length ; i++){
                ...
                _body += '<tr>'+
                            ...
                            '<td>'+htmlEntities(node[i].contenedor)+'</td>'+
                            '<td>'+htmlEntities(node[i].LP)+'</td>'+
                            '<td>Salida por Reabastecimiento</td>'+
                            '<td>'+htmlEntities(node[i].Des_Motivo)+'</td>'+
                            '<td>'+htmlEntities(node[i].almacen_clave)+'</td>'+
                            '<td title="'+htmlEntities(node[i].almacen_nombre_o)+'"><b>'+htmlEntities(node[i].almacen_des)+'</b></td>'+
                            '<td>'+htmlEntities(node[i].CodigoCSD)+'</td>'+
        ...
        function entradaR(node){
            ...
            for(var i = 0; i < node.length ; i++){
                ...
                _body += '<tr>'+
                            ...
                            '<td>'+htmlEntities(node[i].contenedor)+'</td>'+
                            '<td>'+htmlEntities(node[i].LP)+'</td>'+
                            '<td>Entrada por Reabastecimiento</td>'+
                            '<td>'+htmlEntities(node[i].Des_Motivo)+'</td>'+
                            '<td>-</td>'+
                            '<td>-</td>'+
                            '<td>'+htmlEntities(node[i].CodigoCSD)+'</td>'+
