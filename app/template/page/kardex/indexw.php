                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group date">
                                    ...
                                    foreach( $movimientos AS $m ): 
                                        if($m["nombre"] == 'Acomodo' || $m["nombre"] == 'Traslado' || $m["nombre"] == 'Traspaso' || $m["nombre"] == 'Entrada' || $m["nombre"] == 'Salida')
                                        {
                                            if($m["nombre"] == 'Acomodo' || $m["nombre"] == 'Traslado' || $m["nombre"] == 'Traspaso')
                                            {
                                              $mov_existencias[] = $m["id_TipoMovimiento"];
                                            }
                                            else
                                            {
                                    ?>
                                    ...
                                    endforeach; 
                                    $id_mov = implode(",", $mov_existencias);
                                    ?>
                                        <option value="<?php echo $id_mov; ?>">Existencias</option>
                                    </select>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: 'POST',
            colNames: ['Fecha', "Clave", 'Articulo', 'Lote|Serie', 'Caducidad', 'Pallet|Contenedor', 'License Plate (LP)', 'Movimiento', 'Clave Origen', 'Almacén Origen', 'Origen', 'Almacén Destino', 'Destino', 'BL','Stock Inicial', 'Ajuste', 'Stock Final', 'Usuario', 'OC | Factura', 'Pedimento Well', 'Referencia Well'],
            colModel: [
                ...
                {name: 'oc',index: 'oc',width: 120,editable: false,sortable: false},
                {name: 'pedimento_well',index: 'pedimento_well',width: 120,editable: false,sortable: false},
                {name: 'ref_well',index: 'ref_well',width: 120,editable: false,sortable: false},
            ],
            rowNum: 30,
            rowList: [30, 40, 50],
        ...
        function GenerarReporte()
        {
            ...
            if(cve_articulo == '' && lote == '' && fechaI == '' && fechaF == '' && criterio == '' && OCBusq == '' && pedimentoW == '' && refWell == '')
            {
           ...
           var cve_cia = <?php echo $_SESSION['cve_cia']; ?>;
            ...
            $("#reporte_kardex").attr("href", "/api/koolreport/export/reportes/kardex/reporte-kardex?almacen="+almacen+"&criterio="+criterio+"&cve_articulo="+cve_articulo+"&lote="+lote+"&fechaI="+fechaI+"&fechaF="+fechaF+"&cve_cia="+cve_cia+"&cve_proveedor="+cve_proveedor+"&OCBusq="+OCBusq+"&refWell="+refWell+"&pedimentoW="+pedimentoW+"&reporte_w=true");
        ...
        function GenerarReporteExcel()
        {
           ...
           //console.log("/api/koolreport/excel/kardex/exportSpreadsheet.php?almacen="+almacen+"&criterio="+criterio+"&cve_articulo="+cve_articulo+"&lote="+lote+"&fechaI="+fechaI+"&fechaF="+fechaF);
            ...
            $("#reporte_kardex_excel").attr("href", "/kardex/reporte_kardex?almacen="+almacen+"&criterio="+criterio+"&cve_articulo="+cve_articulo+"&lote="+lote+"&fechaI="+fechaI+"&fechaF="+fechaF+"&cve_proveedor="+cve_proveedor+"&OCBusq="+OCBusq+"&movimiento="+movimiento+"&refWell="+refWell+"&pedimentoW="+pedimentoW+"&reporte_w=true");
