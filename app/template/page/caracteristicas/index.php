...
$tipo_caracteristicas = \db()->prepare("SELECT * FROM c_tipo_car WHERE Activo = 1");
            ...
            <form id="myform">
                <div class="modal-body">
                    <div class="form-group">
                        <label>ID</label>
                        <input id="id_caracteristica" type="text" required="true" class="form-control" readonly>
                    </div>
                    <div class="form-group">
                        <label>Clave * </label>
                        <input id="clave_caracteristica" type="text" placeholder="Código" class="form-control">
                    </div>
                    
                    <div class="form-group">
                        <label>Tipo de Característica * </label>
                        <select id="tipo_caracteristica" class="form-control">
                            <option value="">Seleccione</option>
                            <?php 
                                foreach ($tipo_caracteristicas as $row)
                                {
                            ?>
                                    <option value="<?php echo $row['Id_Tipo_car']; ?>"><?php echo "[".$row['Id_Tipo_car']."] - ".$row['TipoCar_Desc'] ?></option>
                            <?php 
                                }
                            ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input id="descripcion_caracteristica" type="text" placeholder="Descripción" class="form-control"><br>
                        <input type="hidden" id="hiddenAction">
                        <input type="hidden" id="hiddenCAT_ID">
                    </div>
                </div>
...
?>
                            ...
                            <button class="btn btn-primary pull-right" type="button" id="inactivos"><i class="fa fa-search"></i>&nbsp;&nbsp;Motivos de devolución inactivos</button>
<?php 
*/
?>
                        </div>
        ...
        <div class="modal-dialog vertical-align-center modal-lg" style="width: 1200px!important;">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Recuperar Motivo de Devolución</h4>
                </div>
                <div class="modal-body">
                    <div class="col-md-4">
                        <div class="input-group">
    ...
    <div class="vertical-alignment-helper">
        <div class="modal-dialog vertical-align-center">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Importar motivos de devolución</h4>
                </div>
                <div class="modal-body">
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            url: '/api/caracteristicas/lista/index.php',
            datatype: "json",
            shrinkToFit: false,
            height: 'auto',
            postData: {
            ...
            mtype: 'POST',
            colNames: ["Acciones", 'ID','Clave', 'Tipo', 'Descripci&oacute;n'],
            colModel: [
    ...
    function BuscarID()
    {
        $.ajax({
            ...
            dataType: "json",
            data: {
                action: "buscar_id_caracteristica"
            },
            ...
            success: function(data) {
                if (data.success == true) {
                    ...
                    $('#id_caracteristica').val(data.max_id);
                    //$('#hiddenCAT_ID').val(data.max_id);
                }
            }
        });
    }

    function VerificarClaveRepetida()
    {
        console.log("#clave_caracteristica = ", $("#clave_caracteristica").val());
        console.log("#tipo_caracteristica = ", $("#tipo_caracteristica").val());

        $.ajax({
            type: "POST",
            dataType: "json",
            data: {
                clave:  $("#clave_caracteristica").val(),
                tipo:   $("#tipo_caracteristica").val(),
                action: "VerificarClaveRepetida"
            },
            beforeSend: function(x) {
                if (x && x.overrideMimeType) {
                    x.overrideMimeType("application/json;charset=UTF-8");
    ...
    function borrar(_codigo) 
    {
        ...
        swal({
                title: "¿Está seguro que desea borrar esta característica?",
                text: "Está a punto de borrar una característica y esta acción no se puede deshacer",
                type: "warning",
            ...
            function() {
                ...
                $.ajax({
                    ...
                    url: '/api/caracteristicas/update/index.php',
                    success: function(data) {
                        if (data.success == true) {
                            //$('#codigo').prop('disabled', true);
                            ReloadGrid();
    ...
    function editar(_codigo) {
        $("#modaltitle").html('<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><i class="fa fa-laptop modal-icon"></i><h4 class="modal-title">Editar Característica</h4>');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.success == true) {
                    //$('#codigo').prop('disabled', true);
                    $('#hiddenCAT_ID').val(_codigo);
                    $('#id_caracteristica').val(_codigo);
                    $("#clave_caracteristica").val(data.Cve_Carac);
                    $("#tipo_caracteristica").val(data.Id_Tipo_car);
                    $("#descripcion_caracteristica").val(data.descripcion);
                    l.ladda('stop');
                    $("#btnCancel").show();
                    $modal0 = $("#myModal");
                    $modal0.modal('show');
                    $("#hiddenAction").val("edit_caracteristica");
                }
            }
        });
    }
    ...
    function agregar() {
        ...
        $("#hiddenAction").val("add_caracteristica");
        $("#btnCancel").show();
        $("#MOT_DESC").val("");
        $("#ClaveMotivo").prop('disabled', false);
        $("#ClaveMotivo").val("");
        $("#CodeMessage").html("");
        $("#modaltitle").html('<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><i class="fa fa-laptop modal-icon"></i><h4 class="modal-title">Agregar Característica</h4>');
    ...
    l.click(function() {
        ...
        if ($("#clave_caracteristica").val() == "") {
            swal("Clave Característica Vacía", "Campo Clave Característica está vacío, llene este campo por favor", "warning");
            return;
        }

        if ($("#tipo_caracteristica").val() == "") {
            swal("Tipo Característica Vacío", "Seleccione un Tipo de Característica por favor", "warning");
            return;
        }

        if ($("#descripcion_caracteristica").val() == "") {
           swal("Descripción Característica Vacía", "El Campo Descripción Característica está vacío, llene este campo por favor", "warning");
            return;
        }
        ...
        $.post('/api/caracteristicas/update/index.php', {
                id: $("#id_caracteristica").val(),
                descripcion: $("#descripcion_caracteristica").val(),
                clave: $("#clave_caracteristica").val(),
                tipo: $("#tipo_caracteristica").val(),
        ...
        .always(function(response) {
            ...
            if (response.success == false) {
               swal("Clave Característica Repetida", "La clave de la característica solo se puede repetir si pertenece a un tipo diferente.", "warning");
                l.ladda('stop');
                $("#btnCancel").show();
                return;
            }
    ...
    $("#ClaveMotivo").keyup(function(e) {
        ...
        if (claveCodeRegexp.test(claveCode)) {
            ...
            $.ajax({
                ...
                url: '/api/motivodevolucion/update/index.php',
                success: function(data) {
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            url: '/api/motivodevolucion/lista/index_i.php',
            datatype: "json",
            shrinkToFit: false,
            height: 'auto',
            postData: {
