...
<div class="wrapper wrapper-content  animated fadeInRight" id="rut">
    <h3>Administración de Registro de Movimientos con Cargo</h3>
    <div class="row">
        <div class="col-md-12">
            <div class="ibox ">
                <div class="ibox-title">
...
?>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control chosen-select" id="lista_clientes" name="lista_clientes">
                                    ...
                                    <?php 
                                        ...
                                        foreach ($listaCliente->getAllManiobras(" AND s.Cve_Almac = $id_almacen ") as $a): ?>
                    ...
                    <div class="row" style="position: relative;">
                        <div class="col-md-2" style="position: absolute; right: 0;">
                            <a href="#" id="proforma" class="btn btn-primary" style="margin: 10px;position: absolute; right: 0;">
                                            <span class="fa fa-file-pdf-o"></span> Proforma
                            </a>
                        </div>
                        <?php 
                        /*
                        ...
                        <div class="col-md-2" style="margin-top:14px;">
                            <button type="button" id="enviartransaccion" class="btn btn-primary" style="margin: 10px;">
                                <span class="fa fa-file-pdf-o"></span> Enviar Transacción
                            </button>
                        </div>
                        <?php 
                        */
                                ...
                                <div style="width: 100%; overflow-y: auto; overflow-x: auto;">
                                    <table class="table table-striped table-bordered table-hover dataTables-example"
                                        ...
                                        <tr>
                                            ...
                                            <th style="text-align: center">Pedimento </th>
                                            ...
                                            <th style="text-align: center">Uso de CFDI</th>
...
<script type="text/javascript">
    ...
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            ...
            height: 'auto',
            mtype: 'POST',
            colNames: ['Clave', 'Descripción', 'Cantidad', 'UM', 'Precio', 'Moneda'],
            colModel: [
                {name: 'Cve_Servicio', index: 'Cve_Servicio', width: 80, editable: false, sortable: false, align: 'left'},
                {name: 'Des_Servicio', index: 'Des_Servicio', width: 80, editable: false, sortable: false, align: 'left'},
                {name: 'cantidad', index: 'cantidad', width: 80, editable: false, sortable: false, align: 'center'},
                {name: 'id_unimed', index: 'id_unimed', width: 100, editable: false, sortable: false, align: 'right'},
                {name: 'precio', index: 'precio', width: 100, editable: false, sortable: false, align: 'right'},
                {name: 'Des_Moneda', index: 'Des_Moneda', width: 100, editable: false, sortable: false, align: 'right'},
            ],
            rowNum: 10,
            rowList: [10, 20, 30],
            pager: pager_selector,
            ...
            loadComplete: function (data) {
                ...
                let dataReporte2 = [];
                    for (let i = 0; i < data.rows.length; i++) {
                        ...
                        dataReporte2.push({
                            Cve_Servicio: data.rows[i].cell['Cve_Servicio'],
                            Des_Servicio: data.rows[i].cell['Des_Servicio'],
                            cantidad: data.rows[i].cell['cantidad'],
                            id_unimed: data.rows[i].cell['id_unimed'],
                            precio: data.rows[i].cell['precio'],
                            Des_Moneda: data.rows[i].cell['Des_Moneda']
                        });
                    }
    ...
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            //url: '/api/vasadministrador/lista/index.php',
            datatype: "json",
            height: 250,
            shrinkToFit: false,
            height: 'auto',
            ...
            mtype: 'POST',
            colNames: ["Acciones", 'Id', 'Fecha', 'Fecha de Vencimiento ', 'Folio', 'Referencia ', 'Pedimento', 'Clave Cliente', 'Nombre de Cliente', 'Costo', 'IVA', 'Total ', 'Tipo de Moneda', 'Régimen Fiscal', 'Uso de CFDI', 'Status'], 
            colModel: [
                {name: 'myac',index: 'myac',width: 50,fixed: true,sortable: false,resize: false,formatter: imageFormat},//0
                {name: 'id_pedido',index: 'id_pedido',width: 70,editable: false,sortable: false,hidden: true,align: 'right'},//1
                {name: 'fecha', index: 'fecha', width: 100, editable: false, sortable: false, align: 'right'},//2
                {name: 'fecha_de_vencimiento ', index: 'fecha_de_vencimiento', width: 70, editable: false, sortable: false},//3
                {name: 'folio', index: 'folio', width: 120, editable: false, sortable: false},//4
                {name: 'referencia ', index: 'referencia', width: 120, editable: false, sortable: false, align: 'center'},//5
                {name: 'pedimento ', index: 'pedimento', width: 120, editable: false, sortable: false, align: 'center'},//5
                {name: 'clave_cliente', index: 'clave_cliente', width: 100, editable: false, sortable: false},//6
                {name: 'nombre_de_cliente', index: 'nombre_de_cliente', width: 150, editable: false, sortable: false, align: 'right'},//7
                {name: 'costo', index: 'costo', width: 150, editable: false, sortable: false},//8
                {name: 'iva', index: 'iva', width: 150, editable: false, sortable: false, hidden: true},//9
                {name: 'total ', index: 'total', width: 150, editable: false, sortable: false},//10
                {name: 'tipo_de_moneda', index: 'tipo_de_moneda', width: 110, editable: false, sortable: false},//11
                {name: 'regimen_fiscal', index: 'regimen_fiscal', width: 100, editable: false, sortable: false, align: 'right'},//12
                {name: 'uso_de_cfdi', index: 'uso_de_cfdi', width: 100, editable: false, sortable: false, align: 'right'},//13
                {name: 'Status', index: 'Status', width: 100, editable: false, sortable: false, align: 'right', hidden: true},//14
            ],
            rowNum: 30,
            rowList: [30, 40, 50],
            pager: pager_selector,
                ...
                if (data.total > 0) {
                    ...
                    for (let i = 0; i < data.rows.length; i++) {
                        ...
                        for (let j = 0; j < dataGrid.length; j++) {
                            ...
                            if (dataGrid[j].folio === data.rows[i].cell[4]) 
                            {
    ...
    //ver(id, folio, cliente, responsable, operacion, sucursal, dia_operativo, ruta, vendedor, tienepromo, saldo_restante, limite_credito, total)
                        dataReporte.push({
                            acciones: acciones,
                            //acciones: data.rows[i].cell[0],
                            fecha: data.rows[i].cell[2],
                            fecha_de_vencimiento: data.rows[i].cell[3],
                            folio: data.rows[i].cell[4],
                            referencia: data.rows[i].cell[5],
                            pedimento: data.rows[i].cell[6],
                            clave_cliente: data.rows[i].cell[7],
                            nombre_de_cliente: data.rows[i].cell[8],
                            costo: data.rows[i].cell[9],
                            iva: data.rows[i].cell[10],
                            total: data.rows[i].cell[11],
                            tipo_de_moneda: data.rows[i].cell[12],
                            regimen_fiscal: data.rows[i].cell[13],
                            uso_de_cfdi: data.rows[i].cell[14],
                            Status: data.rows[i].cell[15]
                        });
                ...
                $('#grid-table-3').DataTable(
                    {
                        ...
                        columns: [
                            ...
                            {data: 'fecha', className: 'text-center', width: 100},
                            {data: 'fecha_de_vencimiento', className: 'text-center'},
                            {data: 'folio', className: 'text-center'},
                            {data: 'referencia', className: 'text-center'},
                            {data: 'pedimento', className: 'text-center'},
                            {data: 'clave_cliente', className: 'text-center'},
                            {data: 'nombre_de_cliente', className: 'text-center'},
                            {data: 'costo', className: 'text-center'},
                            {data: 'iva', className: 'text-center'},
                            {data: 'total', className: 'text-center'},
                            {data: 'tipo_de_moneda', className: 'text-center'},
                            {data: 'regimen_fiscal', className: 'text-center'},
                            {data: 'uso_de_cfdi', className: 'text-center'},
                        ],
                    }
                );
                /*
                ...
                $('#grid-table-3').DataTable(
                    {
                        ...
                        columns: [
                            ...
                            {data: 'Fecha de Vencimiento ', className: 'text-center'},
                            {data: 'Folio', className: 'text-center'},
                            {data: 'Referencia ', className: 'text-center'},
                            {data: 'Pedimento ', className: 'text-center'},
                            ...
                            {data: 'IVA', className: 'text-center'},
                            {data: 'Total ', className: 'text-center'},
                            {data: 'Tipo de Moneda', className: 'text-center'},
                            {data: 'Régimen Fiscal', className: 'text-center'},
                            {data: 'Uso de CFDI', className: 'text-center'},
                        ],
                    }
                );
        ...
        function imageFormat(cellvalue, options, rowObject) {
            ...
            if(status == 'A')
            html += '&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" class="check_lp permiso_consultar" title="Seleccionar Pedido" value="'+folio+'">';
            //html += '<a class="btn btn-danger" href="/api/koolreport/export/reportes/ventas/ventas_sfa/?id_venta=' + id_venta + '&cve_cia=' + clave_cia + '&folio=' + folio+ '&ruta=' + ruta + '&cobranza" target="_blank"><i class="fa fa-file-pdf-o" title="Reporte de Cobranza"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
            html += '</div>';
            return html;
    ...
    $("#generar_proforma").click(function()
    {
        var folios = [], k = 0;
    ...
    function ReloadGrid2() {
        $('#grid-table_detalles-cobranza').jqGrid('clearGridData')
            .jqGrid('setGridParam', {
                ...
                }, datatype: 'json', page: 1, mtype: 'GET', url: '/api/vasadministrador/lista/index.php'
            })
    ...
    $("#proforma").click(function () {
        ...
        if($("#lista_clientes").val() == '')
        {
            ...
            $(".check_lp").each(function(i,j){
                ...
                if($("#btn-asignarTodo").is(":checked"))
                    seleccionarTodo = 1;
                console.log("clientes folios = ", folios);
                console.log("seleccionarTodo = ", seleccionarTodo);

                if(seleccionarTodo == 0 && folios.length == 0)
                {
                    swal("Error", "No tiene pedidos seleccionados para guardar en la proforma", "error");
                    return;
                }

            $modal0 = $("#ModalProforma");
            $modal0.modal('show');

                //Imprimir_LPs(folios);
            //window.open("/api/koolreport/export/reportes/etiquetas/clientes?cve_clientes="+folios+"&compania=<?php echo $_SESSION['cve_cia']; ?>");
        ...
        $(this).attr("href", "/api/koolreport/export/reportes/pedidos/consolidado-servicios?almacen=" + almacen + "&cliente=" + cliente + "&fechaini=" + fechaini + "&fechafin=" + fechafin + "&criterio=" + criterio+ "&cve_cia=" + <?php echo $_SESSION['cve_cia']; ?>);
