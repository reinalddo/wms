            ...
            <div class="ibox ">
                ...
                <div class="ibox-title" id="encabezado">
                    <div class="row">
                      ...
                      <div class="col-lg-1">
                            <button id="button-auditar" style="margin-top: 25px;" class="btn btn-primary btn-sm navbar-minimalize" onclick="action_auditar()">Auditar</button>
                        </div>  
                    </div>
               ...
               <div class="ibox-title">
                   ...
                   <!-- <div class="row">
                    ...
                    -->
                        ...
                        <div class="col-lg-1">
                            <button id="terminar-auditoria" class="btn btn-primary btn-sm" <?php /* ?>onclick="actionButtonCerrar(0)" <?php */ ?>>Terminar Auditoría</button>
                        </div>
                    </div>
                </div>
              <!--Aqui termina-->
                        ...
                        <form id="demo-form2" role="form" method="post" autocomplete="off">
                            ...
                            <div class="form-group">
                                ...
                                <?php 
                                    ...
                                    $sqlCount = "SELECT * FROM c_tipocaja WHERE Activo = 1";
                                    //AND Packing = 'S'
                            ...
                            <div class="form-group" id="zona-embarque" style="display: none;">
                                ...
                                <?php 
                                    $res = "";
                                    $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
                                    $sqlCount = "SELECT ID_Embarque, cve_ubicacion, descripcion FROM t_ubicacionembarque WHERE Activo = 1";
                                    if (!($res = mysqli_query($conn, $sqlCount))) {
                                        echo "Falló la preparación: (" . mysqli_error($conn) . ") ";
                                    }
...
//
    ...
    select_unidadmedida.onchange = function(){changeLote();};
    select_lote_serie.onchange = function(){changeLote();};
    input_cant.addEventListener("keypress", validateNumber, false);
    //input_cajas.addEventListener("keypress", validateNumber, false);
    input_codigo.addEventListener("keyup", function(e){fillFieldAudi(false, e);});
    ...
    function searchSelects(almacen, fill_solo_pallets){
    ...
    function fillSelectAlmacen(node){
        ...
        if(node){
            ...
            for(var i = 0; i < node.length; i++){
                options += "<option value = "+node[i].clave+">"+htmlEntities(node[i].nombre)+"</option>";
            }
        }
    ...
    function fillSelectArea(node){
        ...
        if(node){
            ...
            for(var i = 0; i < node.length; i++){
                folio_mesa = "";
                if(htmlEntities(node[i].fol_folio)!="") folio_mesa = " - "+htmlEntities(node[i].fol_folio+"-"+node[i].sufijo);
    ...
    function fillSelectPedidos(node){
        ...
        if(node){
            ...
            for(var i = 0; i < node.length; i++){
                options += "<option value = "+node[i].id_pedido+">"+htmlEntities(node[i].Fol_folio+"-"+node[i].id_pedido)+"</option>";
            }
        }
    ...
    function changeLote(e){
        ...
        input_arti.value = node.des_articulo;
        max = parseInt(node.Num_cantidad) - parseInt(node.Num_revisadas);
        console.log("node.Num_cantidad = ", node.Num_cantidad);
        console.log("node.num_multiplo = ", node.num_multiplo);
        console.log("node.Num_revisadas = ", node.Num_revisadas);
        if(node.num_multiplo != '' && node.num_multiplo > 0 && $("#unidadMedida").val() == 2)
        max = Math.trunc((node.Num_cantidad/node.num_multiplo) - (node.Num_revisadas/node.num_multiplo));
...
//            console.log("2_action_auditar folio_selected = ", folio_selected);
        ...
        console.log("pedido_area[1] = ", pedido_area[1]);
...
*/
    ...
    function actionButtonEmpacar(){
        ...
        //if(parseInt(ITEM_SELECT.Num_cantidad)){
            ...
            var cant = parseInt(ITEM_SELECT.Num_cantidad), 
                ...
                if(ITEM_SELECT.num_multiplo != '' && ITEM_SELECT.num_multiplo > 0 && $("#unidadMedida").val() == 2)
    ...
    var zona_embarque_obligatorio = 0;
...
/*
            ...
            function(){
*/
                var folio_send = input_factura.value;
                if(folio_auto)
               ...
               //folio_send = folio_send.split("-");
                ...
                // asegurarse de que el sufijo se tome desde el último guión y no el primero.
                //*****************************************************************************************
                var folio_arr = folio_send.split("");
                for(var i = folio_arr.length; i > 0; i--)
                {
                    if(folio_arr[i] == '-')
                       folio_arr[i] = folio_arr[i].replace('-', '***');
...
//            });
    ...
    function fillTable(node){
        ...
        for(var i = 0; i < node.length ; i++)
        {
            color = RED; cant = parseInt(node[i].Num_cantidad); revi = parseInt(node[i].Num_revisadas);
            //lote = 'SIN LOTE';
            lote = ''; 
            serie = ''; caducidad = '';
            ...
            if(cant === revi)
            {
                ...
                {
                    if(node[i].cve_articulo == cod_articulos_empacados[g])
                    {
            ...
            else if(revi > 0)
            {
                ...
                {
                    if(node[i].cve_articulo == cod_articulos_empacados[j])
                    {
                        is_in_array = true;
                        pos_array = j;
                        break;
...
*/
            ...
            var revisadas = node[i].Num_revisadas;
            ...
            body.innerHTML += '<tr style="background-color:'+color+'; color:black">'+
                        ...
                        '<td align="right">'+htmlEntities(surtidas)+'</td>'+
                        '<td align="right">'+htmlEntities(surtidas-revisadas)+'</td>'+
                        '<td align="right">'+htmlEntities(revisadas)+'</td>'+
                        '<td align="right">'+htmlEntities(empacados)+'</td>'+
        ...
        TABLE = tableData.init("table-info",false);
    }
...
*/
    ...
    function changeClass(state){
        ...
        if(state){
            ...
            $("#label-validate").text("Codigo no valido");
        }
        else{
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success == true) {
                    ...
                    $(select_almacen).trigger("chosen:updated");
                    searchSelects(data.codigo.id, fill_solo_pallets);
                }
            },
            error: function(res){
                window.console.log(res);
