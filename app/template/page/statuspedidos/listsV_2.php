...
<script>
    ...
    <script>
        ...
        function imageFormat(id, surtido, tieneola, tienda, sufijo, is_backorder, cliente, surtidor, articulo_ot, folio_ws, status, status_pedido, fecha_pedido, es_ot) {
                    ...
                    if (data.status == 200) {
                        ...
                        $.each(data.data, function(index, item){
                            ...
                            //{
                                ...
                                row += '<tr>'+
                                    '<td align="center">'+imageFormat(item.orden, item.surtido, item.TieneOla, item.destinatario, item.sufijo, is_backorder, item.cliente, item.asignado, item.articulo_ot, item.folio_ws, item.status, item.status_pedido, item.fecha_pedido, item.es_ot)+'</td>'+
              ...
              window.detalle_pedido["articulos"].forEach(function (item,index){
                ...
                if(item["surtidas"] == 0)
                {
                  obj = {
                      ...
                      acciones: '<button  onclick="guardarSurtidoPorUbicacion('+index+', '+item.existencia+')" type="button" class="btn btn-m btn-primary btn-block" id ="btn_surtir_'+(item["clave"]).replace("/", "") + (item.lote).replace("/", "") + (item.serie).replace("/", "") + (item.idy_ubica).replace("/", "") +'" style="width:70px; display:'+((!item["idy_ubica"])?'none':'block')+'">Surtir</button>'
                  };
...
//**********************************************************************************************************************
        ...
        if(data.articulos[i].piezasxcajas > 0)
           valor1 = data.articulos[i].pedidas/data.articulos[i].piezasxcajas;
        ...
        var valor2 = 0;
        var cantidad_restante = data.articulos[i].pedidas - (Pallet*data.articulos[i].piezasxcajas*data.articulos[i].cajasxpallets);
...
//**********************************************************************************************************************
        ...
        var valor1 = 0;
        if(data.articulos[i].piezasxcajas > 0)
           valor1 = surtidas_cant/data.articulos[i].piezasxcajas;
        ...
        var valor2 = 0;
        var cantidad_restante = surtidas_cant - (Pallet*data.articulos[i].piezasxcajas*data.articulos[i].cajasxpallets);
        if(!isNaN(valor1) || valor1 == 0)
        {
            if(data.articulos[i].piezasxcajas > 0)
               valor2 = (cantidad_restante/data.articulos[i].piezasxcajas);// - ($Pallet*$pedidas);
...
/////////////////////////////////////Aqui se contruye el Grid de articulos/////////////////////////////////////////
        $(function($) {
            ...
            $(grid_selector).jqGrid({
                ...
                height: 'auto',
                mtype: 'POST',
                colNames: ['Pedido', 'Subpedido', 'Cliente', 'Clave', 'Articulo', 'Orden de Surtido', 'BL', 'Lote', 'Caducidad', 'Serie', 'Pallet', 'Cajas', 'Piezas', 'Solicitadas', 'Unidad Medida','Disponible', 'Surtir', 'Back Order','Surtidas', 'Volumen (m3)', 'Peso', 'Secuencia', 'Surtidor', 'Ruta'],
                colModel: [
