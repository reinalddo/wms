...
<div class="wrapper wrapper-content animated fadeInRight" id="verEmpresa">
  ...
  <input type="hidden" id="cve_cliente" value="<?php echo $cve_cliente; ?>">
                        ...
                        <div class="col-md-4" <?php echo $cliente_almacen_style; ?>>
                            <label>Almacén</label>
...
almacenPrede();
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            ...
            height:'auto',
            mtype: 'POST',
            colNames:[ 'Acción', 'Folio', 'Ruta', 'Transporte', 'Clave','Placas','Entregas','Pedidos', 'Pallets', 'Cajas','Cap. Volumen','Volumen Utilizado','Peso Maximo','Peso Utilizado','Fecha de Embarque','Fecha Entrega', 'Status', 'Transporte Externo'],
            colModel:[
                {name:'accion',index:'accion', align:'center', width:250, fixed:true, sortable:false, resize:false, formatter: getActions},
                {name:'folio',index:'folio', align:'center', editable:false, sortable:false,width:80},
                {name:'ruta',index:'ruta', align:'center', editable:false, sortable:false,width:120},
                {name:'transporte',index:'chofer',  sortable:false,width:100, align:"center"},
                {name:'clave',index:'clave',  sortable:false,width:100, align:"center"},
                {name:'placas',index:'placas',  sortable:false,width:100, align:"center"},
                {name:'entregas',index:'entregas', editable:false, sortable:false,width:100,align:"center"},
                {name:'pedidos',index:'pedidos', editable:false, sortable:false,width:150,align:"center"},
                ...
                {name:'volmax',index:'volumen', editable:false, sortable:false,width:100,align:"right"},
                {name:'volumen',index:'volumen', editable:false, sortable:false,width:120,align:"right"},
                {name:'pesomax',index:'peso', editable:false, sortable:false,width:100,align:"right"},
                {name:'peso',index:'volumen', editable:false, sortable:false,width:100,align:"right"},
                ...
                {name:'status',index:'status', editable:false, sortable:false,width:100, align:"right"},
        ...
        function getActions(cell, options, row)
        {
            ...
            var html = `<a href="#" onclick="verDetalle(${id},'${folio}')" title="Ver Detalle"><i class="fa fa-search"></i></a>
                ...
                <a href="#" onclick="printPDF(${id},'${folio}')" title="Lista de Embarque"><i class="fa fa-file-pdf-o"></i></a>
                <a href="#" onclick="printPDFAuditoria(${id},'${folio}')" title="Auditoría de Embarque"><i class="fa fa-file-pdf-o"></i></a>
                <a href="#" onclick="printPDFDiscrepancias(${id},'${folio}')" title="Discrepancias Embarque"><i class="fa fa-file-pdf-o"></i></a>
                <a href="#" onclick="printExcelASN(${id})" title="Imprimir ASN"><i class="fa fa-file-excel-o"></i></a>
                <a href="#" onclick="verGps(${id})" title="GPS"><i class="fa fa-map-marker"></i></a>
                `;
//                <a href="/api/koolreport/export/reportes/auditoria/etiquetas?folio=`+id+`&cve_cia=`+cve_cia+`" target="_blank" title="Código de Barras"><i class="fa fa-barcode"></i></a>
                ...
                if(transporte_externo == 0 && status != 'ENTREGADO' && $("#permiso_editar").val() == 1)//&& $("#cve_proveedor").val() == ""
                  html += `<a href="#" onclick="MarcarEntregado(${id})" title="Marcar Como Entregado"><i class="fa fa-check"></i></a>`;
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            ...
            height:'auto',
            mtype: 'POST',
            colNames:['Acción','Pedido', 'Cliente', 'Nombre', 'Clave Dest.', 'Destinatario', 'Clave', 'Descripción', 'Cant. Enviada', 'Lote', 'Caducidad', 'Serie', 'Pallet', 'Caja', 'Piezas|Kg', 'Volumen (m3)', 'Peso(Kg)', 'Caja Empaque', 'Guia Embarque', 'Peso Artículo', 'Partida'],
            colModel:[
