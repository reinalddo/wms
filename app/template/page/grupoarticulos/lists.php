...
<div class="wrapper wrapper-content  animated fadeInRight">
    ...
    <h3>Grupo de Art&iacute;culos</h3>
    ...
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            <div class="modal-header" id="modaltitle">
                ...
                <h4 class="modal-title">Agregar Grupo de Art&iacute;culos</h4>
            </div>
            <form id="myform">
...
<div class="modal inmodal" id="myModal_copiar" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            <div class="modal-header" id="modaltitle_copiar">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                <i class="fa fa-laptop modal-icon"></i>
                <h4 class="modal-title">Copiar Grupo de Art&iacute;culos</h4>
            </div>
            <form id="myform">
            <div class="modal-body">
                <label>Clave *</label>
                <input id="codigo_copiar" type="text" placeholder="C&oacute;digo" maxlength="20" class="form-control" required="true"><!--<label id="CodeMessage" style="color:red;"></label>--><br>
                <label>Descripci&oacute;n *</label>
                <input id="descripcion_copiar" type="text" placeholder="Descripci&oacute;n" class="form-control" required="true" readonly>
                <input type="hidden" id="hiddenAction">
                <br>
                <div class="form-group">
                    <label>Grupo de Artículo a Copiar</label>
                    <select class="chosen-select form-control" id="grupo_c" name="grupo_c">
                        <option value="">Seleccione un grupo</option>
                        <?php foreach( $ga->getAllCopiar($_SESSION['id_almacen']) AS $p ): ?>
                            <option value="<?php echo $p->id; ?>"><?php echo "( ".$p->cve_gpoart." ) ".$p->des_gpoart." | (".$p->clave.") - ".$p->nombre; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-white" data-dismiss="modal" id="btnCancel_copiar">Cerrar</button>
                <button type="button" class="btn btn-primary ladda-button" id="btnSave_copiar">Copiar</button>
            </div>
        </form>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            url:'/api/grupoarticulos/lista/index.php',
            datatype: "json",
            shrinkToFit: false,
			height:'auto',
            postData: {
            ...
            colModel:[
                ...
                {name:'cve_gpoart',index:'cve_gpoart',width:198, editable:false, sortable:false},
        ...
        function imageFormat( cellvalue, options, rowObject ){
            ...
            {
            ?>
                html += '<a href="#" onclick="copiar(\''+serie+'\')"><i class="fa fa-copy" title="Copiar Grupo a otro almacen" alt="Editar"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
            <?php 
            }
            ?>
    ...
    function borrar(_codigo) {
        $.ajax({
            ...
            url: '/api/grupoarticulos/update/index.php',
            success: function(data) {
                if (data.success == true) {
                    swal(
                        '¡Alerta!',
                        'El grupo de articulos esta siendo usado en este momento',
                        'warning'
                    );
                    ...
                    //ReloadGrid();
                        ...
                        swal({
                            title: "¿Está seguro que desea borrar el grupo de articulos?",
                            text: "Está a punto de borrar un grupo de articulos y esta acción no se puede deshacer",
                            type: "warning",
    ...
    $("#btnSave_copiar").click(function(){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                swal("Éxito", "Grupo de Artículos Copiado con éxito", "success");
    ...
    function agregar() {
        $("#modaltitle").html('<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><i class="fa fa-laptop modal-icon"></i><h4 class="modal-title">Agregar Grupo de Art&iacute;culos</h4>');
        $modal0 = $("#myModal");
		 $("#CodeMessage").html("");
        $modal0.modal('show');
        l.ladda('stop');
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success == false) {
                        $.post('/api/grupoarticulos/update/index.php',
                        {
                            almacen: <?php echo $_SESSION['id_almacen']; ?>,
                            codigo : $("#codigo").val(),
...
$(function($) {
        ...
        $(grid_selector).jqGrid({            
			url:'/api/grupoarticulos/lista/index_i.php',
            datatype: "json",
            shrinkToFit: false,
			height:'auto',
            postData: {
