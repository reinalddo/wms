      ...
      <div class="ibox">
        ...
        <div class="ibox-content">
          <form id="form-import" action="/importar/pedidos" method="post"  enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="almacenes" name="almacenes">
                    ...
                    <?php endforeach; ?>
                            ...
                            <div class="form-group" style="text-align: center;">
                                <div class="checkbox">
                                    <label for="pedido_masivo" style="font-weight: 600;font-size: 14px; -moz-user-select: none; -webkit-user-select: none;-ms-user-select: none; user-select: none;">
                                    <input type="checkbox" name="pedido_masivo" id="pedido_masivo" value="0">Importar Pedidos Masivos</label>
                                    <input type="hidden" name="pedido_masivo_input" id="pedido_masivo_input" value="0">
                                </div>
                            </div>
                                </div>
            ...
            <div class="form-group pedido_masivo">
            ...
            <div class="form-group pedido_masivo" style="padding-right: 0;">
            ...
            <div class="row">
            <div class="form-group pedido_masivo col-md-6">
            ...
            <div class="form-group col-md-6" style="padding-left: 0;z-index: 200;">
            ...
            <div class="form-group col-md-6" style="padding-right: 0;z-index: 200;">
...
?>          <div class="row">
            ...
            <div class="form-group" style="text-align: center;">
                <div class="checkbox">
                    <label for="surtir_pedido" style="font-weight: 600;font-size: 14px; -moz-user-select: none; -webkit-user-select: none;-ms-user-select: none; user-select: none;">
                    <input type="checkbox" name="surtir_pedido" id="surtir_pedido" value="0">Surtir Pedido</label>
                    <input type="hidden" name="surtir_pedido_input" id="surtir_pedido_input" value="0">
                </div>
                <div class="datos_surtir" style="text-align: left; display: none;">
                    <label for="areaembarque">Usuario Surtidor</label>
                    <select id="usuarios_surtidores" class="chosen-select form-control">
                    <option value="">Seleccione usuario</option>
                    <?php foreach( $usuarios as $value ): ?>
                        <option value="<?php echo $value["cve_usuario"]; ?>"><?php echo "( ".$value["cve_usuario"]." ) ".$value["nombre_completo"]; ?></option>
                    <?php endforeach; ?>
                    </select>
                    <br><br>
          ...
          <div class="col-md-4" style="text-align: left">
            <button id="btn-layoutLP" type="button" class="btn btn-primary pedido_masivo">Descargar Layout LP</button>
          </div>
...
$('#btn-layout').on('click', function(e) {
  ...
  //window.location.href = 'http://www.tinegocios.com/proyectos/wms/Layout_Pedidos.xlsx';
    if($("#pedido_masivo_input").val() == 0)
        window.location.href = '/Layout/Layout_Pedidos.xlsx';
    else
        window.location.href = '/Layout/Layout_PedidosMasivos.xlsx';
...
$("#surtir_pedido").click(function(){

    if($(this).is(':checked'))
    {
        $("#surtir_pedido_input").val("1");
        $(".datos_surtir").show(100);
    }
    else
    {
        $("#surtir_pedido_input").val("0");
        $(".datos_surtir").hide(100);
    }


    if($("#pedido_masivo").is(':checked'))
    {
        $("#surtir_pedido").prop("checked", false);
        $("#surtir_pedido_input").val("0");
        $(".datos_surtir").hide();
        swal("Ha seleccionado Pedidos Masivos", "Por los momentos no se pueden surtir Pedidos Masivos", "error");
    }
    ...
    if($("#tipo_traslado").is(':checked'))
    {
        ...
        $("#surtir_pedido_input").val("0");
        $(".datos_surtir").hide();
        swal("Ha seleccionado Pedido tipo Traslado", "Este pedido debe surtirse en el almacén destino", "error");
    }

});
    $("#pedido_masivo").click(function(){


        if($("#pedido_masivo").is(':checked'))
        {
            if($("#surtir_pedido").is(':checked'))
            {
                $("#pedido_masivo").prop("checked", false);
                swal("Tiene Seleccionado Surtir Pedidos", "Por los momentos no se pueden surtir Pedidos Masivos", "error");
                return;
            }
            $("#pedido_masivo_input").val("1");
            $(".pedido_masivo").hide();

            $("#tipo_traslado").prop("checked", false);
    ...
    $("#tipo_traslado").click(function()
    {
        if($("#tipo_traslado").is(':checked'))
        {
            ...
            {
                $("#tipo_traslado").prop("checked", false);
                swal("Tiene Seleccionado Surtir Pedidos", "Este pedido debe surtirse en el almacén destino", "error");
                return;
            }
            $(".pedido_masivo").show();
...
$('#btn-import_prueba').on('click', function (){
  ...
  function prueba_importacion() {
    ...
    if($("#fecha_pedido").val() == "" && !$("#pedido_masivo").is(':checked'))
    {
        swal("Error", "Debe Seleccionar una Fecha del Pedido", "error");
        return false;
    }

    if($("#fecha_entrega").val() == "" && !$("#pedido_masivo").is(':checked'))
    {
...
*/
    if(($("#desc_cliente").val() == "" || $("#desc_cliente").val() == null)  && $("#rutas_ventas").val() == '' && !$("#tipo_traslado").is(':checked') && !$("#pedido_masivo").is(':checked'))
    {
        swal("Error", "Debe Seleccionar un Cliente", "error");
        return false;
    }

    if((($("#destinatario").val() == "" || $("#destinatario").val() == null)  && $("#rutas_ventas").val() == '' )  && !$("#tipo_traslado").is(':checked') && !$("#pedido_masivo").is(':checked'))
    {
        swal("Error", "Debe Seleccionar un Destinatario", "error");
        return false;
    }

    if($("#surtir_pedido").is(":checked") && $("#usuarios_surtidores").val() == '')
    {
        swal("Error", "Debe Seleccionar un usuario para surtir el pedido", "error");
        return false;
    }

    if($("#surtir_pedido").is(":checked") && $("#areaembarque").val() == '')
    {
        swal("Error", "Debe Seleccionar un Área de Embarque", "error");
        return false;
    }
...
<style>
    .chosen-results{text-align: left;}
</style>