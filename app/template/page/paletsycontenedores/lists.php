...
<div class="modal inmodal" id="devolver_modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            <div class="modal-header" id="modaltitle">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                <h4 style="font-size: 18px;" class="modal-title" id="modal_devolver">Devolver Pallets | Contenedores de la Ruta: <span></span></h4>
            </div>
            <div class="modal-body" style="height: 160px;">
                  <div class="col-sm-12">
                  <label><b>Pallets</b></label>
                  <input type="number" class="form-control" name="dev_pallets" id="dev_pallets" min="0" max="" style="text-align: end;">
                  </div>

                  <div class="col-sm-12">
                  <label><b>Contenedores</b></label>
                  <input type="number" class="form-control" name="dev_contenedores" id="dev_contenedores" min="0" max="" style="text-align: end;">
                  </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-white" data-dismiss="modal" id="btnCancel">Cerrar</button>
                <button type="button" class="btn btn-primary ladda-button" id="btn-devolver">Devolver</button>
            </div>
        </div>
    </div>
</div>
...
/*
?>
<div class="modal fade" id="devolver_modal" tabindex="-1" aria-labelledby="modal_devolver" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modal_devolver">Devolver Pallets | Contenedores de la Ruta: <span></span></h5>
      ...
      <div class="modal-body">
          ...
          <div class="col-sm-12">
          <label><b>Pallets</b></label>
          <input class="form-control" name="dev_pallets" id="dev_pallets" min="0" max="">
          </div>

          <div class="col-sm-12">
          <label><b>Contenedores</b></label>
          <input class="form-control" name="dev_contenedores" id="dev_contenedores" min="0" max="">
          </div>
            ...
            <div class="ibox ">
                <div class="ibox-title">
                            ...
                            <div class="form-group">
                              ...
                              <div class="col-sm-2">
                                  <div class="form-group">
                                     ...
                                     <input id="pallet-contenedor" class="form-control"></input>
                                  </div>
                              </div>
                              <div class="col-sm-2">
                                    <div class="form-group">
                                       <label>BL</label>
                                       <input id="slect-BL" class="form-control"></input>
                                    </div>
                              </div>
                                <div class="col-sm-2">
                                    <div class="form-group">
                                        <label for="agentes">Ruta:</label> 
                                        <select class="form-control chosen-select" id="rutas_list" name="rutas_list">
                                            <option value="">Seleccione Ruta</option>
                                            <?php 
                                            foreach( $listaRuta->getAll($cve_almacen) AS $r ): 
                                            ?>
                                            <option value="<?php echo $r->ID_Ruta; ?>" data-clave_ruta="<?php echo $r->cve_ruta; ?>" data-consig_pallets="<?php echo $r->consig_pallets; ?>" data-consig_cont="<?php echo $r->consig_cont; ?>"><?php echo "( ".$r->cve_ruta." ) - ".$r->descripcion; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="row text-center">
                        ...
                        <button type="button" id="devolver_pallets_contenedores" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#devolver_modal" style="float: left; display: none;">Devolver Pallets | Contenedores</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(grid_selector).jqGrid(
        {
            ...
            mtype: 'POST',
            colNames: ['ID','Acciones', 'Pallet|Contenedor', 'Descripción', 'Status', 'Tipo', 'License Plate (LP)', 'BL | ID Cliente','Cliente','Razón Social','Destino','Dirección','Fecha Consignación','Días en Prestamo','Almacén'],
            colModel: [
                ...
                {name: 'razon',index: 'razon',width: 300,editable: false,sortable: false, align:"center"},
            ...
            viewrecords: true,
            sortorder: "desc",
            loadComplete: function(data){console.log("SUCCESS: ", data);},//almacenPrede()
  ...
  //////////////////////////////////////////////////////////////Nueva Tabla //////////////////////////////////////////////////////////////////
    function traer_cant_ac(){
        $.ajax({
          ...
          success: function(data) {
              console.log("SUCCESS CONTADORES: ", data);

              $("#cconsignacion").val(data.tarimas_consignacion_contenedor);
              $("#cocupado").val(data.tarimas_ocupadas_contenedor);
              $("#clibre").val(data.tarimas_libres_contenedor);
              $("#ctotal").val(data.total_tarimas_contenedor);
              $("#pconsignacion").val(data.tarimas_consignacion_pallet);
              $("#pocupado").val(data.tarimas_ocupadas_pallet);
              $("#plibre").val(data.tarimas_libres_pallet);
              $("#ptotal").val(data.total_tarimas_pallet);
            
          },
          error:function(res){
              console.log("ERROR CONTADORES: ",res);
          }
      });
    }
    ...
    function ReloadGrid() {
        ...
        console.log("lp:", $("#input-lp").val());
        console.log("almacen:", $("#almacenes").val());
        console.log("clave:", $("#pallet-contenedor").val());
        console.log("cliente:", $("#cliente").val());
        console.log("tipo:", $("#select-tipo").val());
        console.log("bl:", $("#slect-BL").val());
        console.log("ruta:", $("#rutas_list").val());
        console.log("vacio:", $("#select-existencia").val());
    ...
    console.log("cliente",cliente);
        $.ajax({
            ...
            success: function(data) 
            {
                console.log("SUCESS DETALLE = ", data);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $("#devolver_pallets_contenedores").click(function(){
    ...
    $("#btn-devolver").click(function(){
        //console.log("OK BOTON");
        //$("#devolver_modal").modal('show');
        //console.log("N Pallet = ", $("#dev_pallets").val() , " MAX = ", $("#dev_pallets").attr("max"));
        //console.log("N Contenedor = ", $("#dev_contenedores").val() , " MAX = ", $("#dev_contenedores").attr("max"));

        if(parseInt($("#dev_pallets").val()) <= parseInt($("#dev_pallets").attr("max")) && parseInt($("#dev_contenedores").val()) <= parseInt($("#dev_contenedores").attr("max")))
        {
                $.ajax({
                   type:"POST",
                   dataType:"json",
                   data: {
                     action:"devolver_pallets_contenedores",
                     id_ruta: $("#rutas_list").val(),
                     num_pallets: parseInt($("#dev_pallets").val()),
                     num_contenedores: parseInt($("#dev_contenedores").val())
                   },
                   beforeSend: function(x) { if(x && x.overrideMimeType) { x.overrideMimeType("application/json;charset=UTF-8"); }},
                   url:"/api/palletsycontenedores/lista/index.php",
                  success: function(data){
                    console.log(data);
                    swal("Éxito", "Devolución Realizada exitosamente", "success");
        ...
        {
            if($("#dev_pallets").val() == "" || $("#dev_contenedores").val() == "")
                swal("Error", "Debe Ingresar una cantidad válida", "error");
            else
                swal("Error", "No puede devolver más Pallets | Contenedores de los que hay consignados", "error");
        }
    });
    ...
    $("#rutas_list").change(function()
    {
        if($(this).val())
        {
            $("#devolver_pallets_contenedores").show();
            $("#cconsignacion").val($(this).find(':selected').data("consig_cont"));
            $("#pconsignacion").val($(this).find(':selected').data("consig_pallets"));
            $("#modal_devolver span").text($(this).find(':selected').data("clave_ruta"));
            $("#dev_pallets").val($(this).find(':selected').data("consig_pallets"));
            $("#dev_contenedores").val($(this).find(':selected').data("consig_cont"));
            $("#dev_pallets").attr("max", $(this).find(':selected').data("consig_pallets"));
            $("#dev_contenedores").attr("max", $(this).find(':selected').data("consig_cont"));
        }
        else
            $("#devolver_pallets_contenedores").hide();
    });
