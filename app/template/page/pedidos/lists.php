                    ...
                    <div class="ibox-content">
                        <div class="row">
                            <div class="col-lg-6 b-r">
                                ...
                                <div class="form-group"><label>Pedido</label> <input id="Pedido" onkeyup="return ValNumero(this);" type="text" placeholder="Número de Folio" class="form-control"></div>

                                <div class="form-group">
                                    <label>Facturar a</label>
                                    <select class="form-control" id="Facturar">
                                        <option value="">Seleccione</option>
                                        <?php foreach( $listClientes->getAll() AS $a ): ?>
                                            <option value="<?php echo $a->Cve_Clte; ?>"><?php echo $a->RazonSocial; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>

                                <div class="form-group"><label>Ejecutivo</label> <input id="Ejecutivo" type="text" placeholder="Ejecutivo" class="form-control"></div>
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-group"><label>Enviar a PTL</label> <input id="PTL" type="text" placeholder="Enviar a PTL" class="form-control"></div>

                                <div class="form-group"><label>Instrucciones</label> <textarea placeholder="Instrucciones" class="form-control" id="Instrucciones"></textarea></div>

                                <div class="form-group">
                                    <label>Almacen</label>
                                    <select class="form-control" id="Almacn">
                                        <option value="">Seleccione</option>
                                ...
                                <div class="form-group">
                                    <label>Area de Embarque</label>
                                    <select class="form-control" id="AreaEmb">
                                        <option value="">Seleccione</option>
                                    </select>
                                </div>
                            ...
                            <div class="col-lg-12">
                                <center><a href="#" id="addrow"><button type="button" class="btn btn-primary" id="btnCancel">Agregar Artículo</button></a>&nbsp;&nbsp;</center>
                            </div>
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            url:'/api/pedidos/lista/index.php',
            datatype: "json",
            height: 250,
            postData: {
            ...
            mtype: 'POST',
            colNames:['Numero de Folio','Facturado al Cliente','Instrucciones', ""],/*'Calle y Numero','Colonia','Ciudad','Estado','Pais','Codigo Postal','RFC','Telefono1','Telefono2'*/
            colModel:[
                //{name:'id',index:'id', width:60, sorttype:"int", editable: false},

                {name:'Fol_folio',index:'Fol_folio',width:50, editable:false, sortable:false},
                {name:'RazonSocial',index:'RazonSocial',width:150, editable:false, sortable:false},
                {name:'Observaciones',index:'Observaciones',width:150, editable:false, sortable:false},
    ...
    function editar(_codigo) {
        $("#_title").html('<h3>Editar Pedido</h3>');
        $.ajax({
            type: "POST",
            dataType: "json",
            data: {
            ...
            success: function(data) {
                if (data.success == true) {
                    ...
                    $("#Facturar").val(data.Cve_clte);
                    $("#Ejecutivo").val(data.cve_Vendedor);
                    $("#fechaentrada").val(data.Fec_Entrada);
                    $("#Pick").val(data.Pick_Num);
                    if (data.Pick_Num == '1') {
                        $('#Pick').iCheck('check');
                    } else {
                        $('#Pick').iCheck('uncheck');
                    }
                    $("#Instrucciones").val(data.Observaciones);
                    $("#Almacn").val(data.cve_almac);

                    $("#grid-tabla").jqGrid("clearGridData");

                    if (data.detalle) {
                        for (var i = 0; i < data.detalle.length; i++) {
                            emptyItem=[{codigo:data.detalle[i].Cve_articulo,descripcion:data.detalle[i].des_articulo,CantPiezas:data.detalle[i].Num_cantidad}];
                            $("#grid-tabla").jqGrid('addRowData',0,emptyItem);
                        }
                    }
    ...
    function agregar() {
        $("#_title").html('<h3>Agregar Pedido</h3>');
        $(':input','#myform')
            .removeAttr('checked')
            .removeAttr('selected')
            .not(':button, :submit, :reset, :hidden, :radio, :checkbox')
    ...
    l.click(function() {
        ...
        /*if ($("#Pedido").val()=="") {
            alert("Por favor, Agregue el Número de Pedido");
            $('#Pedido').focus();
            return;
        }
        if ($("#Facturar").val()=="") {
            alert("Por favor, Seleccione a quien se va a Facturar");
            $('#Facturar').focus();
            return;
        }
        if ($("#Ejecutivo").val()=="") {
            alert("Por favor, Seleccione el Ejecutivo");
            $('#Ejecutivo').focus();
            return;
        }
        if ($("#fechaentrada").val()=="") {
        ...
        if ($("#Pick").val()=="") {
            alert("Por favor, Seleccione el Pick Num");
            $('#Pick').focus();
            return;
        }
        if ($("#PTL").val()=="") {
            alert("Por favor, Seleccione el PTL");
            $('#PTL').focus();
            return;
        }
        if ($("#Instrucciones").val()=="") {
            alert("Por favor, Indique las Instrucciones");
            $('#Instrucciones').focus();
            return;
        }

        if ($("#Almacn").val()=="") {
            alert("Por favor, Indique el Almacén");
            $('#Almacn').focus();
            return;
        }*/

        /*if ($("#AreaEmb").val()=="") {
            alert("Por favor, Indique el Area de Embarque");
            $('#AreaEmb').focus();
            return;
        }*/

        /*if ($("#Articul").val()=="") {
            alert("Por favor, Indique el Artículo");
            $('#Articul').focus();
            return;
        }*/
        ...
        $.ajax({
            ...
            data: {
                ...
                Cve_clte: $("#Facturar").val(),
                cve_Vendedor: $("#Ejecutivo").val(),
                Fec_Entrada: $("#fechaentrada").val(),
                Pick_Num:  $("#Pick").val(),
                Observaciones: $("#Instrucciones").val(),
                cve_almac: $("#Almacn").val(),
