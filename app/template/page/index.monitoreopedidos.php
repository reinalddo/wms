...
$sql = "
        SELECT pedido_tabla.usuario, pedido_tabla.pedido, IF(pedido_tabla.pedido != '', pedido_tabla.cliente, '') AS cliente, pedido_tabla.cantidad_pedidos_surtidos FROM (
        SELECT DISTINCT u.nombre_completo AS usuario, 
        IFNULL((SELECT GROUP_CONCAT(DISTINCT fol_folio SEPARATOR ', ') FROM th_subpedido WHERE th_subpedido.status = 'S' AND th_subpedido.Cve_Usuario = s.Cve_Usuario AND th_subpedido.cve_almac = $id_almacen #AND DATE_FORMAT(Fec_Entrada, '%Y-%m-%d')= CURDATE() 
            #AND (SELECT COUNT(*) FROM td_surtidopiezas WHERE fol_folio = th_subpedido.fol_folio AND Sufijo = th_subpedido.Sufijo) > 0
            ), '') AS pedido, 
        IFNULL((
        SELECT GROUP_CONCAT(DISTINCT c_pedido.RazonComercial SEPARATOR ', ') 
        FROM th_pedido th
        LEFT JOIN c_cliente c_pedido ON c_pedido.Cve_Clte = th.Cve_Clte
        LEFT JOIN th_subpedido ON th_subpedido.fol_folio = th.Fol_folio
        WHERE th.status = 'S' AND th.cve_almac = $id_almacen 
        #AND DATE_FORMAT(th_subpedido.Fec_Entrada, '%Y-%m-%d') = CURDATE() 
        #AND (SELECT COUNT(*) FROM td_surtidopiezas WHERE fol_folio = th_subpedido.fol_folio AND Sufijo = th_subpedido.Sufijo) > 0
        AND th_subpedido.fol_folio = IFNULL((SELECT GROUP_CONCAT(DISTINCT fol_folio SEPARATOR ', ') FROM th_subpedido WHERE th_subpedido.status = 'S' AND th_subpedido.Cve_Usuario = s.Cve_Usuario AND th_subpedido.cve_almac = $id_almacen 
            #AND DATE_FORMAT(Fec_Entrada, '%Y-%m-%d')= CURDATE() #AND (SELECT COUNT(*) FROM td_surtidopiezas WHERE fol_folio = th_subpedido.fol_folio AND Sufijo = th_subpedido.Sufijo) > 0
            ), '')
        AND IFNULL(th.Cve_clte, '') != ''
        ), '') AS cliente, 
                (SELECT COUNT(*) FROM th_subpedido WHERE th_subpedido.status NOT IN ('A', 'S') AND th_subpedido.Cve_Usuario = s.Cve_Usuario AND th_subpedido.cve_almac = $id_almacen 
                    #AND DATE_FORMAT(th_subpedido.Fec_Entrada, '%Y-%m-%d') = CURDATE() 
                    #AND (SELECT COUNT(*) FROM td_surtidopiezas WHERE fol_folio = th_subpedido.fol_folio AND Sufijo = th_subpedido.Sufijo) > 0
                    ) AS cantidad_pedidos_surtidos
        FROM th_subpedido s
        LEFT JOIN th_pedido p ON p.Fol_folio = s.fol_folio
        LEFT JOIN c_usuario u ON u.cve_usuario = s.Cve_Usuario
        LEFT JOIN c_cliente c ON c.Cve_Clte = p.Cve_clte
        WHERE s.cve_almac = $id_almacen AND u.cve_usuario IS NOT NULL 
        AND p.TipoPedido = 'P' 
        #AND p.Fec_Pedido = CURDATE()
        GROUP BY usuario
...
*/
$sql = "SELECT COUNT(*) as npedidos FROM th_pedido WHERE status = 'A' AND cve_almac = $id_almacen #AND Fec_Pedido = CURDATE()";
if (!($res = mysqli_query($conn, $sql)))
    echo "Falló la preparación Charset: (" . mysqli_error($conn) . ") ";
$ListosPorAsignarN = mysqli_fetch_array($res)["npedidos"];

$sql = "SELECT COUNT(*) as npedidos FROM th_subpedido WHERE status = 'S' AND cve_almac = $id_almacen #AND DATE_FORMAT(Fec_Entrada, '%Y-%m-%d') = CURDATE()";
if (!($res = mysqli_query($conn, $sql)))
    echo "Falló la preparación Charset: (" . mysqli_error($conn) . ") ";
$PedidosSurtiendoN = mysqli_fetch_array($res)["npedidos"];

$sql = "SELECT COUNT(*) as npedidos, DATE_FORMAT(CURDATE(), '%d-%m-%Y') as fecha_actual FROM th_pedido WHERE status IN ('L', 'R', 'P', 'C') AND cve_almac = $id_almacen AND Fec_Pedido = CURDATE()";
if (!($res = mysqli_query($conn, $sql)))
    echo "Falló la preparación Charset: (" . mysqli_error($conn) . ") ";
$row_fact = mysqli_fetch_array($res);
$PorFacturarN = $row_fact["npedidos"];
$fecha_actual = $row_fact["fecha_actual"];
//echo $sql;
//var_dump($res_lotes);
        ...
        <div class="col-md-3">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content" style="height: 150px;">
                    ...
                    <div class="stat-percent font-bold text-success" style="font-size: 60pt;">
                        <?php echo $ListosPorAsignarN;//echo !$almacenSeleccionado ? 0 :  number_format($lotesVencidosPorcen, 2, ',', '.') ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="ibox float-e-margins">
                ...
                <span class="label label-info pull-right">
                  Total
                    ...
                    <h5 style="font-size: 20pt;">
                        Por Facturar (<?php echo $fecha_actual; ?>)
                    </h5>
                </div>
                <div class="ibox-content" style="height: 150px;">
                    <h3 class="no-margins">
        ...
        <div class="col-md-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <?php else: ?>
                        <table class="table table-striped table-bordered no-margins thead-dark">
                            <thead class="thead-dark">
                            <tr>
                                <th style="background-color: black !important;color: white;font-size: 35pt;" scope="col" width="20%">Usuario</th>
                                <th style="background-color: black !important;color: white;font-size: 35pt;" scope="col" width="35%">Pedido</th>
                                <th style="background-color: black !important;color: white;font-size: 35pt;" scope="col" width="35%">Cliente</th>
                                <th style="background-color: black !important;color: white;font-size: 35pt;" scope="col" width="10%">Completados</th>
                            </tr>
                            </thead>
                            <tbody>
                            <?php foreach($productoMayorMovimiento as $producto): ?>
                                <tr>
                                    <td style="font-size: 20pt;"><?php echo $producto["usuario"]?></td>
                                    <td style="font-size: 20pt;"><?php echo $producto["pedido"]?></td>
                                    <td style="font-size: 20pt;"><?php echo $producto["cliente"]; ?></td>
                                    <td style="font-size: 20pt;" align="right"><?php echo $producto["cantidad_pedidos_surtidos"]; ?></td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                        </table>
