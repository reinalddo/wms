    ...
    <div class="modal-dialog">
        <div class="modal-content animated bounceInRight">
            <div class="modal-header" id="modaltitle">
                ...
                <h4 class="modal-title">Agregar Motivo de Ajuste de Inventario</h4>
            </div>
            ...
            <form id="myform">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input id="MOT_DESC" type="text" placeholder="Motivo de Devoluci&oacute;n" class="form-control" required="true"><br>
                        <input type="hidden" id="hiddenAction">
                        <input type="hidden" id="hiddenMOT_ID">

                        <div class="checkbox" style="margin: 0;">
                            <label for="check_devolucion">
                            <input type="checkbox" name="check_devolucion" id="check_devolucion" value="1">Devolver a Proveedor</label>
                        </div>

                        <div class="checkbox" style="margin: 0;">
                            <label for="cierre_incidencia">
                            <input type="checkbox" name="cierre_incidencia" id="cierre_incidencia" value="1">Cierre de Incidencia</label>
                        </div>
...
<div class="wrapper wrapper-content  animated fadeInRight">
    ...
    <h3>Registro de Motivos de Ajustes de Existencia e Incidencias</h3>
                ...
                <div class="ibox-title">
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            <button class="btn btn-primary pull-right" type="button" id="inactivos"><i class="fa fa-search"></i>&nbsp;&nbsp;Motivos de ajuste de inventario</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog vertical-align-center modal-lg" style="width: 1200px!important;">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Recuperar Motivo de Ajuste de Inventario</h4>
                </div>
                <div class="modal-body">
                    <div class="col-md-4">
                        <div class="input-group">
    ...
    <div class="vertical-alignment-helper">
        <div class="modal-dialog vertical-align-center">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Importar motivos de ajustes de ineventario</h4>
                </div>
                <div class="modal-body">
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            url: '/api/motivoajuste/lista/index.php',
            datatype: "json",
            shrinkToFit: false,
            height: 'auto',
            postData: {
            ...
            mtype: 'POST',
            colNames: ['Acciones', 'Id', 'Descripción', 'Dev Proveedor', 'Registro/Cierre Incidencia'],
            colModel: [
                {name: 'myac',index: '',width: 100,fixed: true,sortable: false,resize: false,formatter: imageFormat},
                {name:'id',index:'id', width:60, sorttype:"int", editable: false},
                {name: 'Des_Motivo',index: 'Des_Motivo',width: 220,sorttype:"int",editable: false,sortable: false}, 
                {name: 'dev_proveedor',index: 'dev_proveedor', align: 'center', width: 150,sorttype:"int",editable: false,sortable: false}, 
                {name: 'reg_cierre',index: 'reg_cierre', align: 'center', width: 250,sorttype:"int",editable: false,sortable: false}, 
            ],
            rowNum: 30,
            rowList: [30, 40, 50],
    ...
    function borrar(_codigo) {
        $.ajax({
            ...
            url: '/api/motivoajuste/update/index.php',
            success: function(data) {
    ...
    function editar(_codigo) {
        ...
        $("#modaltitle").html('<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><i class="fa fa-laptop modal-icon"></i><h4 class="modal-title">Editar Motivo de Ajustes | Incidencias</h4>');
        //$("#myModal").modal('show');
        $("#Tipo_Cat").prop('disabled', true);
        $("#CodeMessage").html("");
        $.ajax({
            type: "POST",
            dataType: "json",
            ...
            success: function(data) {
                ...
                if (data.success == true) {
                    //$('#codigo').prop('disabled', true);Des_Motivo
                    $('#hiddenMOT_ID').val(_codigo);
                    $("#MOT_DESC").val(data.MOT_DESC);
                    $("#Tipo_Cat").val(data.Clave_motivo);
                    l.ladda('stop');

                    //console.log("data.dev_proveedor = ", data.dev_proveedor);
                    $("#check_devolucion").prop("checked", "");
                    if(data.dev_proveedor == 1)
                        $("#check_devolucion").prop("checked", "checked");

                    $("#cierre_incidencia").prop("checked", "");
                    if(data.Clave_motivo == "C")
                        $("#cierre_incidencia").prop("checked", "checked");
    ...
    function agregar() {
        ...
        $("#btnCancel").show();
        $("#MOT_DESC").val("");
        $("#Tipo_Cat").prop('disabled', false);
        $("#Tipo_Cat").val("");
        $("#CodeMessage").html("");
        $("#modaltitle").html('<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><i class="fa fa-laptop modal-icon"></i><h4 class="modal-title">Agregar Motivo de Ajuste de Inventario</h4>');
    }
    ...
    l.click(function() {
        ...
        var check_devolucion = document.getElementById("check_devolucion").checked ? 1 : 0;
        var cierre_incidencia = document.getElementById("cierre_incidencia").checked ? "C" : "A";

        console.log("check_devolucion = ", check_devolucion);

        $.post('/api/motivoajuste/update/index.php', {
                    id: $('#hiddenMOT_ID').val(),
                    Des_Motivo: $("#MOT_DESC").val(),
                    check_devolucion: check_devolucion,
                    cierre_incidencia: cierre_incidencia,
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            url: '/api/motivoajuste/lista/index_i.php',
            datatype: "json",
            shrinkToFit: false,
            height: 'auto',
            postData: {
