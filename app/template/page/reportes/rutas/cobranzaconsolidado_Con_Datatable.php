                ...
                <div class="modal-body">
                    <div class="col-md-4">
                        <div class="input-group">
                            <input type="text" class="form-control input-sm" name="txtCriterio1" id="txtCriterio1"
                                   placeholder="Buscar por Ruta...">
                            <div class="input-group-btn">
                                <a href="#" onclick="ReloadGrid1()">
                                    <button type="submit" id="buscarAR" class="btn btn-sm btn-primary">Buscar
                                    </button>
                                </a>
                            </div>
                        </div>
...
<!--                </h3>-->
<!--                <br>-->
<!--                <h3 style="display: none;"><b>Operación:</b> <span id="val_operacion"></span> | <b>Sucursal:</b> <span-->
...
<!--                           id="grid-table-3" style="width: 100% !important;">-->
<!--                        <thead>-->
<!--                        <tr>-->
<!--                            <th>Fecha</th>-->
<!--                            <th>Folio</th>-->
...
<div class="modal" id="ver_detalles" tabindex="-1" role="dialog">
                            ...
                            <div id="busqueda">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="txtCriterio" style="margin: 10px;"
                                           id="txtCriterio" placeholder="Búsqueda...">
                                    <div class="input-group-btn">
                                        <button type="button" id="buscarA"
                    ...
                    <div class="tabbable" id="tabs-131708">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="panel-928563">
                                <div hidden="hidden" class="koolreport-dashboard-table table-responsive">
                                    <table class="table table-outline" id="grid-table"></table>
                                    <div id="grid-pager"></div>
                                </div>
                                <div style="width: 100%; overflow-y: auto; overflow-x: auto;">
                                    <!--                                    centra los botones-->
                                    <div class="btn-group" style="text-align: center; display: block; margin: auto;">
                                    ...
                                    <table class="table table-striped table-bordered table-hover dataTables-example"
                                        ...
                                        <tfoot>
                                        <!--                                        solo va a haber 2 th, donde el primero abarcara 7 columnas y dira Adeudo Final, y en el otro sera el total-->
                                        <tr>
                                            <th colspan="7" style="text-align: right">Adeudo Final</th>
                                            <th style="text-align: right" id="totalAdeudo">$0</th>
                                        </tr>
                                        </tfoot>
                                    </table>
...
<script type="text/javascript">
    ...
    function printGrid() {
        ...
        if (dataTablaGeneral.length > 0) {
            console.log('datos Reporte:' + dataTablaGeneral);
            $.ajax({
                url: '/api/koolreport/export/reportes/ventas/ventas_sfa/ExportPDFService.php',
                type: 'POST',
                // dataType: "json",
                data: {
                    datosReporte: dataTablaGeneral,
                    cveCia: clave_cia,
                },
                success: function (data) {
                    //decodifica el base64 (erxcel) para abrirlo en una nueva pestannia
                    var link = document.createElement('a');
                    link.href = 'data:application/pdf;base64,' + data;
                    link.target = '_blank';
                    link.download = 'reporte_cuentas_por_cobrar_consolidado.pdf';
            ...
            swal("No hay datos para imprimir", "", "warning");
        }
    }
    ...
    function printExcel() {
        ...
        if (dataTablaGeneral.length > 0) {
            console.log('datos Reporte:' + dataTablaGeneral);
            $.ajax({
                url: '/api/koolreport/export/reportes/ventas/ventas_sfa/ExportExcelService.php',
                type: 'POST',
                // dataType: "json",
                data: {
                    datosReporte: dataTablaGeneral,
                ...
                success: function (data) {
                    ...
                    link.target = '_blank';
                    link.download = 'reporte_cuentas_por_cobrar_consolidado.xlsx';
                    link.dispatchEvent(new MouseEvent('click'));
                ...
                if (data.total > 0) {
                    ...
                    for (let i = 0; i < data.rows.length; i++) {
                        dataReporte.push({
                            ...
                            folio: data.rows[i].cell['folio'],
                            saldo: data.rows[i].cell['saldo'],
                            total: data.rows[i].cell['total']
                        });
                    }
                }
    ...
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            ...
            sortorder: "desc",
            loadComplete: function (data) {
                console.log("Ventas aqui: ", data);
                //imprime la data de las columnas de la tabla grid-table
                //console.log($(this).jqGrid('getGridParam', 'colModel'));
                //imprime la data de las filas de la tabla grid-table
                let dataGrid = $(this).jqGrid('getRowData');
                let dataReporte = [];
                var totalCreditosPendientes = 0;
                if (data.total > 0) {
                    dataTablaGeneral = [];
                    //console.log(data.rows);
                    for (let i = 0; i < data.rows.length; i++) {
                        ...
                        for (let j = 0; j < dataGrid.length; j++) {
                            ...
                            //console.log(dataGrid[j].cvecliente, data.rows[i].cell[2], dataGrid[j].nombre_comercial, data.rows[i].cell[3]);
                            if (dataGrid[j].cvecliente === data.rows[i].cell[2]) {
                                acciones = dataGrid[j].myac;
                                break;
                            }
                        }
                        //console.log(data.rows[i].cell[5]);
                        dataReporte.push({
                            acciones: acciones,
                            numCliente: data.rows[i].cell[1],
                            nombreCliente: data.rows[i].cell[2],
                            saldoDeudor: '$' + (data.rows[i].cell[3] ? data.rows[i].cell[3] : 0),
                            pagos: '$' + (data.rows[i].cell[4] ? data.rows[i].cell[4] : 0),
                            saldoFinal: '$' + (data.rows[i].cell[5] ? data.rows[i].cell[5] : 0),
                            limiteCredito: '$' + (data.rows[i].cell[6] ? data.rows[i].cell[6] : 0),
                            creditoDisponible: '$' + (data.rows[i].cell[7] ? data.rows[i].cell[7] : 0),
                            almacen: data.rows[i].cell[9],
                        });
                        //console.log("dataReporte: ", dataReporte);
                    }
                }
                for (let i = 0; i < dataReporte.length; i++) {
                    totalCreditosPendientes += parseFloat(dataReporte[i].saldoFinal.replace('$', '').replace(',', ''));
                }
                //console.log(totalCreditosPendientes);
                //console.log('totalCreditosPendientes: ', totalCreditosPendientes);
                ...
                $('#totalAdeudo').text('$' + totalCreditosPendientes.toFixed(2));
        ...
        $('#grid-table-3').on('length.dt', function (e, settings, len) {
            $('#pg_grid-pager_detalles .ui-pg-selbox').val(len).trigger('change');
        });
        //get event entries to jqgrid table grid-table input_grid-pager
        $('#pg_grid-pager_detalles .ui-pg-selbox').on('change', function () {
            //selecciona el valor del select anterior
            var selected = $(this).val();
            console.log("selected: ", selected);
        });
        // Setup buttons
        $("#grid-table-2").jqGrid('navGrid', '#grid-pager-2', {
            edit: false,
            add: false,
            del: false,
            search: false
        ...
        function imageFormat(cellvalue, options, rowObject) {
            ...
            var cliente = rowObject[8],
                responsable = rowObject[2],
                cveCliente = rowObject[1],
                sucursal = $("#almacenes option:selected").text(),
                almacen = $("#almacenes").val(),
                tienepromo = rowObject[17];
                //console.log('rowObject: ', rowObject);
                //console.log('cliente:', cliente);
                //console.log('cveCliente: ', cveCliente);
                //console.log('responsable: ', responsable);
                //console.log('tienepromo: ', tienepromo);
                //console.log('almacen: ', almacen);
                //console.log('clave_cia: ', clave_cia);
            var html = '';
            html = '<a title="Ver Reporte Consolidado" class="btn btn-info" href="#" onclick="ver(\'' + cliente + '\', \'' + cveCliente + '\',\'' + responsable + '\', \'' + tienepromo + '\', \'' + almacen + '\')"><i class="fa fa-search" title="Ver Detalles"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
            html += '<a class="btn btn-danger" href="/api/koolreport/export/reportes/ventas/ventas_sfa/?cve_cia=' + clave_cia + '&cliente=' + cliente + '&almacen=' + almacen + '&consolidado=1" target="_blank"><i class="fa fa-file-pdf-o" title="Imprimir Reporte Consolidado"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
            /*
    ...
    //detecta el select de datatable jquery para saber que opcion se selecciono
    // $('grid-table-2
    ...
    function ReloadGrid() {
        console.log("almacen INITB: ", $("#almacenes").val());
        setTimeout(function () {
            console.log("setTimeout INITB: ", $("#almacenes").val());
    ...
    function ver(cveCliente, cliente, responsable, tienepromo, almacen) {
        console.log('cliente: ', cliente);
        /*
                if(id)
        ...
        */
        $("#ver_detalles #num_cliente").text(cveCliente);
        ...
        */
        ...
        loadDetalles(tienepromo, cveCliente, almacen);
    }
