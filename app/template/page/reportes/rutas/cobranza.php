...
<script type="text/javascript">
    ...
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            ...
            height: 'auto',
            mtype: 'POST',
            colNames: ['Folio', 'Ruta', 'Fecha', 'Saldo Anterior', 'Abono', 'Saldo Restante', 'Saldo Disponible'],
            colModel: [
                {name: 'Id', index: 'Id', width: 80, editable: false, sortable: false, align: 'left'},
                {name: 'Ruta', index: 'Ruta', width: 80, editable: false, sortable: false, align: 'left'},
    ...
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: 'POST',
            colNames: ["Acciones", 'Id Venta', 'DO', 'Ruta', 'Operación', 'Fecha', 'Folio', 'Cliente', 'Nombre Comercial', 'Responsable', 'Status', 'Método de Pago', 'Total', 'Abono', 'Saldo Final', 'Vendedor', 'limite crédito'],
            colModel: [
                {name: 'myac',index: 'myac',width: 80,fixed: true,sortable: false,resize: false,formatter: imageFormat},//0
                ...
                {name: 'cvecliente', index: 'cvecliente', width: 100, editable: false, sortable: false, align: 'right'},//7
                ...
                {name: 'status', index: 'status', width: 80, editable: false, sortable: false},//10
                {name: 'metodo_pago', index: 'metodo_pago', width: 110, editable: false, sortable: false},//11
                {name: 'total', index: 'total', width: 100, editable: false, sortable: false, align: 'right'},//12
                {name: 'abono', index: 'abono', width: 100, editable: false, sortable: false, align: 'right'},//13
                {name: 'saldofinal', index: 'saldofinal', width: 100, editable: false, sortable: false, align: 'right'},//14
                {name: 'vendedor', index: 'vendedor', width: 120, editable: false, sortable: false},//15
                {name: 'limite_credito',index: 'limite_credito',width: 120,editable: false,sortable: false,align: 'right'}//16
            ],
            rowNum: 30,
            rowList: [30, 40, 50],
            pager: pager_selector,
                ...
                if (data.total > 0) {
                    ...
                    for (let i = 0; i < data.rows.length; i++) {
                        ...
                        for (let j = 0; j < dataGrid.length; j++) {
                            ...
                            if (dataGrid[j].folio === data.rows[i].cell[6]) 
                            {
    ...
    //ver(id, folio, cliente, responsable, operacion, sucursal, dia_operativo, ruta, vendedor, tienepromo, saldo_restante, limite_credito, total)
                        dataReporte.push({
                            ...
                            Folio: data.rows[i].cell[6],
                            Cliente: data.rows[i].cell[7],
                            NombreComecial: data.rows[i].cell[8],
                            Status: data.rows[i].cell[10],
                            MetodoPago: data.rows[i].cell[11],
                            Total: data.rows[i].cell[12],
                            Abono: data.rows[i].cell[13],
                            SaldoFinal: data.rows[i].cell[14],
                            Vendedor: data.rows[i].cell[15],
                            limite_credito: data.rows[i].cell[16]
                        });
            ...
            loadError: function (data) {
                ...
                if($("#txtCriterio").val() != '' || $("#rutas_list").val() != '' || $("#fechaini").val() != '' || $("#fechafin").val() != '')
        ...
        function imageFormat(cellvalue, options, rowObject) {
            ...
            var folio = rowObject[6],
                ...
                ruta = rowObject[3],
                tienepromo = 0,
                saldo_restante = rowObject[14],
                total = 0,
                limite_credito = 0,
                vendedor = rowObject[15];
    ...
    $("#aplicar-pago-grupo").click(function () {
        ...
        if (folios.length > 1 && mensaje_diferentes == false) {
            ...
            loadDetallesGrupo(folios, id_cliente);
    ...
    function loadDetalles(id, tienepromo, folio, limite_credito, total) {
        ...
        console.log("limite_credito:", limite_credito);
        console.log("total:", total);
    ...
    function loadDetallesGrupo(folios, cve_cliente) {
