...
<div class="wrapper wrapper-content  animated " id="list">
 <h3>Comparativo de Existencias</h3>
    <div class="row">
        <div class="col-lg-12">
            <div class="ibox ">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3">
                            <div class="form-group">
                                ...
                                <select class="form-control chosen-select" name="filtro_diferencias_select" id="filtro_diferencias_select" >
                                    <option value="">Todas</option>
                                    <option value="AND (concentrado.existencia - concentrado.Existencia_SAP) != 0 SEPARADOR ORDER BY (concentrado.existencia - concentrado.Existencia_SAP) ASC">Con Diferencias Ascendente</option>
                                    <option value="AND (concentrado.existencia - concentrado.Existencia_SAP) != 0 SEPARADOR ORDER BY (concentrado.existencia - concentrado.Existencia_SAP) DESC">Con Diferencias Descendente</option>
                                    <option value="AND (concentrado.existencia - concentrado.Existencia_SAP) = 0">Sin Diferencias</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-lg-3">
                        ...
                        <div class="col-lg-3" style="margin-top: 24px;">
                            ...
                            <div class="input-group-btn">
                                <button class="btn btn-sm btn-primary pull-right"  data-toggle="modal" data-target="#importar"><span class="fa fa-download"></span> Importar Match</button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-dialog vertical-align-center">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Importar Match</h4>
                    </div>
...
$('#btn-layout').on('click', function(e) {
    ...
    window.location.href = '/Layout/Layout_Match.xlsx';
}); 
...
$('#btn-import').on('click', function() 
{
    ...
    function buscar() 
    {
      ...
      //.trigger('reloadGrid',[{current:true}]);
        ...
        var data = {
                    ...
                    lotes : $("#lotes").val(),
                    filtro_concentrado : $("#filtro_concentrado_select").val(),
                    filtro_diferencias_select : $("#filtro_diferencias_select").val(),
    ...
    $("#almacen, #filtro_diferencias_select").change(function()
    {
        if(firstTimeAlmacenPrede || (firstTimeAlmacenPrede && $("#filtro_diferencias_select").val()) )
            buscar();
        else
            firstTimeAlmacenPrede = true;
    });
    ...
    function totales()
    {
        ...
        $.ajax({
            url:"/api/reportes/lista/comparativoexistencia.php",
            type: "GET",
            data: {
                almacen: $("#almacen").val(),
                cve_proveedor : $("#cve_proveedor").val(),
...
//**************************************************************************************************
       ...
       console.log("almacen = ", $("#almacen").val(), ", articulo = ", $("#articulo").val(), ", proveedor = ", $("#proveedor").val(), ", grupo = ", $("#grupo").val());
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: 'GET', //lilo
            colNames:['Clave','Descripción', 'Lote | Serie', 'Caducidad', 'Caducidad SAP', 'Existencia', 'Existencia SAP', 'Diferencia',/*, 'Pallet','Caja','Piezas | Kgs','Prod OC','Prod RTM','Reserva Picking','Prod en QA','Obsoletos','RTS','Prod | Kitting'*/'Proveedor', 'Almacén','Grupo'],
            colModel:[
                ...
                {name:'lote',index:'lote',width:100, editable:false, align:"left", sorttype: "text"},
                {name:'caducidad',index:'caducidad',width:100, editable:false, align:"center", sorttype: "text"},
                {name:'caducidad_sap',index:'caducidad_sap',width:100, editable:false, align:"center", sorttype: "text"},
                {name:'existencia',index:'existencia',width:100, editable:false, align:"right", sorttype: "text"},
                {name:'existencia_sap',index:'existencia_sap',width:100, editable:false, align:"right", sorttype: "text"},
                {name:'diferencia',index:'diferencia',width:100, editable:false, align:"right", sorttype: "text"},
                /*
                {name:'Pallet',index:'Pallet',width:100,editable:false, align:"right"},
                {name:'Caja',index:'Caja',width:100,editable:false, align:"right"},
                {name:'Piezas',index:'Piezas',width:100,editable:false, align:"right"},
                {name:'Prod_OC',index:'Prod_OC', width:100, editable:false, sorttype: "text", align:"right"},
                {name:'Prod_RTM',index:'Prod_RTM', width:100, editable:false, align:"right"},
                {name:'Res_Pick',index:'Res_Pick', width:100, editable:false, align:"right", sorttype: "int"},
                {name:'Prod_QA',index:'Prod_QA', width:100, editable:false, align:"right", sorttype: "int"},
                {name:'Obsoletos',index:'Obsoletos', width:100, editable:false, sortable:false, align:"right"},
                {name:'RTS',index:'RTS', width:100, editable:false, sortable:false, align:"right"},
                {name:'Prod_kit',index:'Prod_kit', width:100, editable:false, sortable:false, align:"right"},
                */
            ...
            loadComplete:function(data){
              ...
              console.log(data);
                ...
                $("#totalproductos").val(data.productos);
                $("#totalunidades").val(data.unidades);
                $("#almacen, #articulo, #proveedor, #grupo, #filtro_concentrado_select, #filtro_diferencias_select, #lotes").chosen();
...
*/
       ...
       $("#boton_excel").click(function(e){
        ...
        $(this).attr("href", "/comparativo/exportar_excel?almacen="+$("#almacen").val()+"&cve_proveedor="+$("#cve_proveedor").val()+"&filtro_concentrado="+$("#filtro_concentrado_select").val()+"&proveedor="+$("#proveedor").val()+"&grupo="+$("#grupo").val()+"&articulo="+$("#articulo").val()+"&filtro_diferencias_select="+$("#filtro_diferencias_select").val()+"&lotes="+$("#lotes").val());
        });
                ...
                $.ajax({
                    ...
                    success: function(data, textStatus, xhr)
                    {
                        ...
                        var head_content = '<tr><th style="border: 1px solid #ccc;">Proveedor</th>'+
                                               ...
                                               '<th style="border: 1px solid #ccc;">Existencia SAP</th>'+
                        ...
                        for(var i = 0; i < data.data.length; i++)
                        {
                            body_content += '<tr>'+
                                        ...
                                        '<td style="border: 1px solid #ccc;text-align: right;">'+parseFloat(data.data[i].existencia).toFixed(2)+'</td>'+
                                        '<td style="border: 1px solid #ccc;text-align: right;">'+(parseFloat(data.data[i].existencia).toFixed(2) - parseFloat(data.data[i].Existencia_SAP).toFixed(2))+'</td>'+
                                    '</tr>  ';
...
//    });
    ...
    //*******************************************************************************************
    $("#almacen, #articulo, #proveedor, #grupo, #filtro_concentrado_select, #filtro_diferencias_select, #lotes").change(function(){
    ...
    $("#almacen").trigger("chosen:updated");
    $("#filtro_concentrado_select").trigger("chosen:updated");
    $("#filtro_diferencias_select").trigger("chosen:updated");
    $("#lotes").trigger("chosen:updated");
