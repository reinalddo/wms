...
<input type="hidden" id="tipo_inventario" value="">
                            ...
                            <div class="row">                             
                                <div class="form-group col-md-3">                                    
                                    ...
                                    <select class="form-control" id="almacen2">
                                        <?php /* ?><option value="">Almacen</option><?php */ ?>
                                        <?php foreach( $almacen->getAll() AS $almacen ): ?>
                                        <option value="<?php echo $almacen->clave; ?>"><?php echo "($almacen->clave) $almacen->nombre"; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="col-sd-6 col-md-3 col-lg-3">
                                ...
                                <div class="form-group col-md-3">
                                    <label for="busq">Buscar Inventario</label>
                                    <input type="text" name="busq" class="form-control" id="busq" placeholder="Buscar #Inventario" value="">
                                </div>
                                <div class="form-group col-md-3">
    ...
    <div class="vertical-alignment-helper">
        <div class="modal-dialog vertical-align-center modal-lg">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cerrar</button>
                    <?php  ?><button id="btn-guardar-stock-fisico" style="display:none;" type="button" class="btn btn-primary pull-right" onclick="guardarValoresYUsuariosStockFisico()" >Cerrar Inventario</button><?php  ?>
    ...
    <div class="vertical-alignment-helper">
        <div class="modal-dialog vertical-align-center">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Cerrar</button>
                    <button id="btn-guardar-stock" type="button" class="btn btn-primary pull-right boton_conteos" onclick="ReporteIndividualFisico()"><i class="fa fa-file-pdf-o"></i>  Ver Reporte PDF</button>

                    <button id="btn-excel-aux" type="button" class="btn btn-primary pull-right boton_conteos"><i class="fa fa-file-excel-o"></i>  Descargar Reporte EXCEL</button>

                    <a id="btn-guardar-stock-excel" href="#" class="btn btn-primary pull-right" style="display: none;" target="_blank"><i class="fa fa-file-excel-o"></i>  Descargar Reporte EXCEL</a>
            ...
            success: function(data) 
            {
                ...
                if (data.status == 200) 
                {
                    ...
                    $.each(data.data, function(index, item){
                        ...
                        '<td align="right"><span id="txt-stock-teorico-ciclico-'+i+'">'+item.stockTeorico+'</span></td>'+
                        '<td align="right"><input id="stock-ciclico'+i+'" class="stock-ciclico" data-index="'+i+'" data-id="'+item.id+'"  type="text" value="'+value+'" autocomplete="off" '+readonly+' style="max-width: 100%;text-align: right;margin: 1px; '+style_none+'"/>'+value_reg+'</td>'+
                        '<td align="right"><span id="txt-diferencia-ciclico-'+i+'">'+item.diferencia+'</td>'+
                        '<td align="center">'+item.conteo+'</td>'+
                        '<td>'+item.usuario+'</td>'+
...
*/
    ...
    function guardarValoresYUsuariosStockFisico(id_plan, tipo)
    {
        //******************************************************************************
        //*************************** CIERRE DIRECTO ***********************************
        //******************************************************************************
        ...
        //$("#num_inventario").val(serie);
        $("#tipo_inventario").val(tipo);
    ...
    */
    function asignarSupervisor()
    {
        ...
        */
        ...
        var tipo_inv = 'ciclico';
        if($("#tipo_inventario").val() == 'Físico')
            tipo_inv = 'fisico';

        console.log("tipo_inventario = ", tipo_inv);
        $.ajax({
            ...
            console.log("ERROR INV", data);
        });
    }
        
    function ReloadGrid() 
    {
        console.log("Almacen Init = ", $('#almacen2').val());
        $('#grid-table').jqGrid('clearGridData')
            .jqGrid('setGridParam', {
                postData: {
                    status: $('#status').val(),
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            ...
            height: 'auto',
            mtype: 'GET',
            colNames: ['Acciones','Consecutivo', 'Zona|Artículo', 'Tipo', '# BLs', '# Num Productos', '# BLs Contadas', '# BLs Vacíos', '# BLs No Inventariadas', 'Fecha Inicio', 'Fecha Fin', 'Superviso', 'Diferencia','Fiabilidad %', 'Status', 'Tipo', 'Nro. Conteo', 'Almacén', 'Inventario Efectuado'],
            colModel: [
                {name: 'myac',index: '',width: 250,fixed: true,align: 'center',sortable: false,resize: false,formatter: imageFormat}, 
                ...
                {name: 'tipo',index: 'tipo',align: 'center',width: 70,editable: false,sortable: false, hidden: true}, 
                {name: 'num_bls',index: 'num_bls',align: 'center',width: 70,editable: false,sortable: false, hidden: true}, 
                {name: 'num_productos',index: 'num_productos',align: 'center',width: 110,editable: false,sortable: false, hidden: true}, 
                {name: 'num_bls_cont',index: 'num_bls_cont',align: 'center',width: 110,editable: false,sortable: false, hidden: true}, 
                {name: 'num_bls_vacios',index: 'num_bls_vacios',align: 'center',width: 110,editable: false,sortable: false, hidden: true}, 
                {name: 'num_bls_no_inv',index: 'num_bls_no_inv',align: 'center',width: 160,editable: false,sortable: false, hidden: true}, 
                ...
                {name: 'diferencia',index: 'diferencia',align: 'right',width: 100,editable: false,sortable: false, hidden: true}, 
                {name: 'porcentaje',index: 'porcentaje',align: 'right',width: 100,editable: false,sortable: false, hidden: true}, 
                ...
                {name: 'n_inventario',index: 'n_inventario',align: 'center',width: 100,editable: false,sortable: false, hidden: true}, 
                {name: 'almacen',index: 'almacen',width: 120,editable: false,sortable: false}, 
                {name: 'efectuado',index: 'efectuado',align: 'center',width: 150,editable: false,sortable: false, hidden: true}
            ],
            rowNum: 30,
            rowList: [30, 40, 50],
        ...
        function imageFormat(cellvalue, options, rowObject) 
        {
            ...
            else if (tipo === 'Cíclico') 
            {
                ...
                */
                ...
                html += `<a href="/api/koolreport/excel/vista_previa/export.php?id=`+serie+`&tipo=`+tipo+`" title="Vista Previa Inventario"><i class="fa fa-eye"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += `<a href="#" onclick="ReporteTeoricos('` + serie + `', '` + estado + `', '` + fecha + `', '` + tipo + `')" title="Reporte de Teóricos en Inventario"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += '<a href="/api/koolreport/export/reportes/inventario/reporte_general_conteos_BL?id='+serie+'&cve_cia='+cia+'&tipo='+tipo+'" target="_blank" id="btn_reporte_general" title="Reporte General de Conteos BL"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;';

                html += `<a href="#" onclick="SeleccionarUsuarioConteo('` + serie + `', '`+ fecha +`', '`+ tipo +`')" title="Reporte Individual"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += `<a href="#" onclick="SeleccionarRacks('` + serie + `', '`+ fecha +`', 0, '`+ tipo +`')" title="Reporte Consolidado"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += `<a href="#" onclick="SeleccionarRacks('` + serie + `', '`+ fecha +`', 2, '`+ tipo +`')" title="Reporte Consolidado Por Ubicación"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += `<a href="#" onclick="SeleccionarRacks('` + serie + `', '`+ fecha +`', 3, '`+ tipo +`')" title="Reporte de Conteos Abiertos"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                //html += `<a href="#" onclick="ReporteConsolidadoItemFisico('` + serie + `', '` + estado + `', '` + fecha + `', '`+ tipo +`')" title="Reporte Consolidado Item PDF"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += `<a href="/api/koolreport/excel/inventario_consolidado_item/export.php?id=`+serie+`&tipo=`+tipo+`" title="Reporte Consolidado Item EXCEL"><i class="fa fa-file-excel-o"></i></a>&nbsp;&nbsp;&nbsp;`;

                html += `<a href="#" onclick="SeleccionarRacks('` + serie + `', '` + fecha + `', 1, '`+ tipo +`')" title="Reporte de Ubicaciones con Diferencia"><i class="fa fa-file-pdf-o"></i></a>&nbsp;&nbsp;&nbsp;`;


                if(estado == "Abierto") estado_title = "Cerrado";
                if($("#permiso_editar").val() == 1)
                html += `<a href="#" onclick="CambiarStatus('` + serie + `', '` + estado + `', '`+ tipo +`')" title="Cambiar Status a `+estado_title+`"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;`;

                if(estado == "Abierto" && $("#permiso_registrar").val() == 1 && $("#permiso_editar").val() == 1)
                html += `<a href="#" onclick="guardarValoresYUsuariosStockFisico('` + serie + `', '`+ tipo +`')" title="Cerrar Inventario"><i class="fa fa-archive"></i></a>&nbsp;&nbsp;&nbsp;`;
...
/*
        $(this).attr("href", 
        "/api/koolreport/excel/inventario_consolidado/export.php?id="+consecutivo+"&status="+status+"&fecha_inv="+fecha+"&diferencia_inv="+diferencia+"&rack="+ubicacion_rack+"&tipo="+tipo);
*/
        $(this).attr("href", 
        "/inventario/consolidado?id="+consecutivo+"&status="+status+"&fecha_inv="+fecha+"&diferencia_inv="+diferencia+"&rack="+ubicacion_rack+"&tipo="+tipo);
...
/*
    ...
    function ReporteConsolidadoFisico() 
    {
        var form = document.createElement('form'),
            ...
            tipo_inv = document.createElement('input'),
        ...
        tipo_inv.setAttribute('type', 'hidden');
        tipo_inv.setAttribute('name', 'tipo_inv');
        tipo_inv.setAttribute('value', tipo);
    ...
    function SeleccionarUsuarioConteo(n_inv, fecha, tipo)
    {
        $("#tipo_inventario").val(tipo);
        $.ajax(
        {
            ...
            success: function(data) 
            {
                    ...
                    $("#btn-excel-aux, .boton_conteos").prop("disabled", true);
                    $("#btn-excel-aux").show();
                    $("#btn-guardar-stock-excel").hide();
                    //$('.chosen-select').trigger("chosen:updated");
            }, error: function(data)
            {
                console.log("ERROR", data);
    ...
    function SeleccionarRacks(n_inv, fecha, diferencia, tipo)
    {
        $("#tipo_inventario").val(tipo);
        $.ajax(
        {
        ...
        if($(this).val())
        {
            ...
            $.ajax(
            {
                ...
                success: function(data) 
                {
                    ...
                    options.append(data.option_racks);
                    //$('.chosen-select').trigger("chosen:updated");
                    $(".boton_conteos").prop("disabled", false);
                    $("#btn-excel-aux").hide();
                    $("#btn-guardar-stock-excel").show();
                }, error: function(data)
                {
                    console.log("ERROR", data);
                }
    ...
    function ReporteIndividualFisico() 
    {
        ...
        console.log("Consecutivo = ", consecutivo, " - Status = ", status, " - Compania = ", cia, " - Usuario = ", usuario, " - Conteo = ", conteo, " - Fecha Inventario = ", fecha, " - Tipo = ", tipo);
...
/*
        $(this).attr("href", 
        "/api/koolreport/excel/inventario_conteo/export.php?id="+consecutivo+"&conteo_usuario="+conteo+"&fecha_inv="+fecha+"&ubicacion_inv="+ubicacion+"&ubicacion_text_inv="+ubicacion_text+"&ubicacion_rack="+ubicacion_rack+"&usuario="+usuario+"&tipo="+tipo);
*/
        $(this).attr("href", 
        "/inventario/conteos?id="+consecutivo+"&conteo_usuario="+conteo+"&fecha_inv="+fecha+"&ubicacion_inv="+ubicacion+"&ubicacion_text_inv="+ubicacion_text+"&ubicacion_rack="+ubicacion_rack+"&usuario="+usuario+"&tipo="+tipo);
...
*/
    ...
    function ReporteConsolidadoItemFisico(consecutivo, status, fecha, tipo) 
    {
        $("#tipo_inventario").val(tipo);
        var form = document.createElement('form'),
            nobody = document.createElement('input'),
    ...
    function ReporteTeoricos(consecutivo, status, fecha, tipo) 
    {
        $("#tipo_inventario").val(tipo);
        var form = document.createElement('form'),
            nobody = document.createElement('input'),
        ...
        //var status = $("#status_pdf").val();
        var cia = <?php echo $_SESSION['cve_cia']; ?>;
        console.log("Consecutivo = ", consecutivo, " - Status = ", status, " - Compania = ", cia, " - Tipo = ", tipo);
...
<script type="text/javascript">
    ...
    function CambiarStatus(inv, status, tipo) 
    {
        var mensaje = "Esta Operación Abrirá el inventario nuevamente, Si tiene ubicaciones ya planificadas en estado abierto puede entrar en conflicto con los conteos. \nDesea Abrir el Inventario #"+inv+"?";
        if(status == "Abierto")
            mensaje = "Esta Operación Cerrará el Inventario Pero no afectará las existencias. \nDesea Cerrar el Inventario #"+inv+"?";
        swal({
            title: "Advertencia",
            text: mensaje,
            type: "warning",
