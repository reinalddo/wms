...
<div class="wrapper wrapper-content  animated " id="form-prin">
    ...
    <h3>Ubicaciones de Zona de Almacenaje</h3>
                ...
                <div class="ibox-title">
                    <div class="row">
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                               <label>BL</label>
                               <select id="slect-BL" class="form-control chosen-select">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            ...
                            <div class="form-group">
                               <label>Ordenar por:</label>
                                <select id="select-orden" class="chosen-select form-control">
                                    <option value="1" selected>BL</option>
                                    <option value="2">Secuencia</option>
                                </select>
                            </div>
                        ...
                        <div class="col-sm-6">
                            ...
                            <button id="button-nuevo" class="btn btn-primary permiso_registrar" onclick="initAgregar()"><span class="fa fa-plus"></span> Nuevo</button>
                          ...
                          <button class="btn btn-primary permiso_registrar" data-toggle="modal" data-target="#importar"><span class="fa fa-download"></span> Importar</button>
                            <a href="/api/v2/ubicacion-de-almacenaje/exportar" target="_blank" class="btn btn-primary" id="exportarUbicaciones"><span class="fa fa-upload"></span> Exportar</a>
                        </div>
                ...
                <div class="row">
                  <div class="col-sm-6">
                   ...
                   <div class="row">
                    ...
                    <div class="col-sm-6">
                      <div class="form-group" id="a_stagging" style="display:none;">
                        <input type="checkbox" name="areastagging" id="check-areastagging">
                        <label for="areastagging">Área de Stagging</label>
                      </div>
                    </div>
                  </div>
                      <div class="form-group">
                        <input type="checkbox" name="control_reabasto" id="control_reabasto">
                        <label for="control_reabasto">Control de Reabasto</label>
                        <br><br>

                          <div class="col-sm-4 tipo_reabasto" style="display:none;">
                            <div class="form-group">
                              <input type="radio" name="tipo_reabasto" id="tipo_reabasto-cajas" value="C">
                              <label>Cajas</label>
                            </div>
                          </div>
                          <div class="col-sm-4 tipo_reabasto" style="display:none;">
                            <div class="form-group">
                              <input type="radio" name="tipo_reabasto" id="tipo_reabasto-piezas" value="P">
                              <label>Piezas</label>
                            </div>
                          </div>
                      ...
                      <div class="form-group">
                        <input type="checkbox" name="recibir_traslado" id="check-Traslado">
                        <label for="check-Traslado">BL Rec. Traslado entre Almacenes</label>
                      </div>
                        ...
                        <form id="form-import" action="import" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <div class="checkbox">
                                    ...
                                    <input type="checkbox" name="check-importar" id="check-importar" value="1">Reemplazar Registros</label>
                                    <span id="mensaje_modificar" style="font-weight: bold; color: red;display: none;">
                                        <br>Para reemplazar los registros de las ubicaciones exitosamente, obligatoriamente debe llenar las columnas: Clave Zona de Almacenaje y Código BL. Se reemplazará el valor de la columna que no esté vacía. <br> Se Recomienda llenar solo las celdas que necesita reemplazar para optimizar el tiempo de respuesta</span>
                                </div>
...
$('#btn-layout').on('click', function(e) {
  ...
  window.location.href = '/Layout/Layout_Ubicaciones.xlsx';
    ...
    $("#exportarUbicaciones").click(function(e)
    {
        var almacen            = <?php echo $_SESSION['id_almacen']; ?>;
        var zona               = $("#select-zonaA-1").val();
        var tipo_ubicacion     = $("#select-tipoU").val();
        var codigoBL           = $("#slect-BL").val();
        var rack               = $("#select-rack").val();
        var busqueda           = $("#input-criterio").val();
        var con_sin_existencia = $("#select-existencia").val();
        var instancia          = $("#instancia").val();

        $(this).attr("href", "/api/v2/ubicacion-de-almacenaje/exportar?almacen="+almacen+"&zona="+zona+"&tipo_ubicacion="+tipo_ubicacion+"&codigoBL="+codigoBL+"&rack="+rack+"&busqueda="+busqueda+"&con_sin_existencia="+con_sin_existencia+"&instancia="+instancia);
    });
...
$("#check-importar").click(function(){
    ...
    {
        $("#mensaje_modificar").show();
    }
    else
    {
        $("#mensaje_modificar").hide();
    }
...
<script type="text/javascript">
    ...
    var input_alto = document.getElementById('input-alto'),
        ...
        check_areastagging = document.getElementById('check-areastagging'),
        check_Traslado = document.getElementById('check-Traslado'),
        control_reabasto = document.getElementById('control_reabasto'),
        tipo_reabasto_cajas = document.getElementById('tipo_reabasto-cajas'),
        tipo_reabasto_piezas = document.getElementById('tipo_reabasto-piezas'),
    ...
    $("#check-area").change(function(){
      if($(this).is(":checked"))
      {
        $("#check-areastagging").prop("checked", false);
        $("#a_stagging").show();
      }
      else
      {
    ...
    function buscar()
    {
      console.log("BL Buscar 2:", select_BL.value);
    ...
    $("#control_reabasto").click(function()
    {
         if($(this).is(":checked"))
            $(".tipo_reabasto").show();
        else
        {
            $(".tipo_reabasto").hide();
            tipo_reabasto_cajas.checked = false;
            tipo_reabasto_piezas.checked = false;
        }
    });

    function exportarPDFCodigoBarras(lp, idy_ubica)
    {
            ...
            console.log("LP = ", lp);
                ...
                var 
                        ...
                        input_idy_ubica = document.createElement('input'),
                ...
                input_idy_ubica.setAttribute('name', 'idy_ubica');
                input_idy_ubica.setAttribute('value', idy_ubica);

                form.appendChild(input_nofooter);
                form.appendChild(input_folio);
                form.appendChild(input_oc);
                form.appendChild(input_proveedor);
                form.appendChild(input_idy_ubica);
    ...
    function initAgregar()
    {
      ...
      check_areastagging.checked = false;
      check_Traslado.checked = false;
      control_reabasto.checked = false;
      tipo_reabasto_cajas.checked = false;
      tipo_reabasto_piezas.checked = false;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(node.AreaProduccion === "S")
                {
                    ...
                    $("#a_stagging").show();
                    if(node.AreaStagging === "S")
                    {
                        check_areastagging.checked = true;
                    }
                }
                if(node.Status === "T")
                    check_Traslado.checked = true;
                if(node.Reabasto === "C")
                {
                    control_reabasto.checked = true;
                    tipo_reabasto_cajas.checked = true;
                    $(".tipo_reabasto").show();
                }
                ...
                {
                    control_reabasto.checked = true;
                    tipo_reabasto_piezas.checked = true;
                    $(".tipo_reabasto").show();
                }
...
*/
    ...
    function loadDetalles(id, cvelp) 
    {
          ...
          page : 1, mtype: 'GET', url:'/api/ubicacionalmacenaje/lista/index.php'
        }).trigger('reloadGrid',[{current:true}]);
...
*/
    ...
    function configCodiBL(){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var text = data.codigoBL[0].codigo;
                text = text.replace("Seccion", "Sección");
                text = text.replace("cve_rack", "Rack");
                text = text.replace("cve_nivel", "Nivel");
                text = text.replace("Ubicacion", "Posición");
                text = text.replace("cve_pasillo", "Pasillo");
                $("#input-codigo-bl").val(text);
            },
            error:function(res){
        ...
        function config(node){
            ...
            if(array.length > 0){
                ...
                if(check_pasillo.checked == false){$("#div_de_pasillo").hide();}
                if(check_rack.checked == false){$("#div_de_rack").hide();}
                if(check_nivel.checked == false){$("#div-nivels-1, #div-nivels-2").hide();}
                if(check_seccion.checked == false){$("#div-seccions-1, #div-seccions-2").hide();}
                if(check_posicion.checked == false){$("#ubicacionseccion2").hide();}
        }
    }
    ...
    function changeAlmacens(element, select, valor){
      ...
      this.changeBL();
      //setTimeout(function(){$('#select-rack, #slect-BL').trigger("chosen:updated");}, 1000);
      //$('#select-rack, #slect-BL').trigger("chosen:updated");
    }
      ...
      console.log("changeBL = ", $("#select-zonaA-1").val());
        $.ajax({
           ...
           success: function(data){
              ...
              $('#slect-BL').append(options);
              $('#slect-BL').chosen("destroy").chosen();//.trigger("chosen:updated");
              //select_BL.trigger("chosen:updated");
              console.log("trigger(chosen:updated)");
          },
          error: function(res){
            window.console.log(res);
          }
        ...
        $.ajax({
           ...
           success: function(data){
              ...
              $('#select-rack').append(options_rack);

            $('#select-rack').chosen("destroy").chosen();//.trigger("chosen:updated");
            //select_rack.trigger("chosen:updated");
            console.log("trigger(chosen:updated)2");
    ...
    function seachTableRecu(){
        ...
        $(grid_selector).jqGrid({
            ...
            colModel:[
                ...
                {name:'dim',index:'dim',width:330, editable:false, sortable:false, align:"center", hidden:true},
            ...
            loadComplete:function(data){
              ...
              setTimeout(function(){ 
                if(data.total != 0)
                {
                  ...
                  $("#vol_total").val(volumen_total.toFixed(8));
                  $("#vol_dispo").val(($("#vol_max").val() - volumen_total).toFixed(8));
...
*/
        ...
        {
            arePro = 'S';
            if(check_areastagging.checked)
                areStag = 'S';
        }

        if(check_Traslado.checked)
            Traslado = 'T';

        if(control_reabasto.checked)
        {
            if(tipo_reabasto_cajas.checked)
                c_reabasto = 'C';
            if(tipo_reabasto_piezas.checked)
                c_reabasto = 'P';
        }
        ...
        console.log("c_reabasto = ", c_reabasto);
        //return;
        $.ajax({
            type: "POST",
            dataType: "json",
    ...
    function saveAdd()
    {
      ...
      if(/* $('#radio-ubi-rack').prop('checked') && */ $('#input-rack').val() == '' && check_rack.checked)
      {
        swal({
          title: "Advertencia!",
    ...
    if(check_pasillo.checked && $("#input-pasillo").val() == "")
    {
        swal({
          title: "Advertencia!",
          text: "Debe ingresar el Pasillo",
          type: "warning",
          showCancelButton: false
        });
        return false;
      ...
      if( !$('#radio-ubi-piso').prop('checked') && check_nivel.checked 
          && (
                (($('#input-nivel').val() == '' || $('#input-nivel').val() == '0') && $('#radio-nivels').prop('checked'))
            || (
                  (
                    ($('#input-nivelsR-D').val() == '' || $('#input-nivelsR-A').val() == '0' || $('#input-nivelsR-A').val() == '') && $('#radio-nivelsR').prop('checked')
                  )
               )
             )
        )
      {
        swal({
          title: "Advertencia!",
          text: "Debe ingresar el Nivel o Rango de Niveles.",
          type: "warning",
          showCancelButton: false
        });
        return false;
      ...
      if( check_seccion.checked 
          && (
                ((($('#input-secc').val() == '' || $('#input-secc').val() == '0')) && $('#radio-secc').prop('checked'))
            || (
                  (
                    ($('#input-seccR-D').val() == '' || $('#input-seccR-A').val() == '0' || $('#input-seccR-A').val() == '') && $('#radio-seccR').prop('checked')
                  )
               )
             )
        )
      {
        swal({
          title: "Advertencia!",
          text: "Debe ingresar Las Secciones o Rango de Secciones.",
          type: "warning",
          showCancelButton: false
        });
        return false;
    ...
    if(check_posicion.checked && $("#input-seccU").val() == "")
    {
        swal({
          title: "Advertencia!",
          text: "Debe ingresar las Posiciones por Sección",
          type: "warning",
          showCancelButton: false
        });
        return false;
...
//return;
      ...
      if(!msj)
      {
        ...
        $.ajax({
          ...
          data: {
            ...
            tecno : tecno,
            controlABC: controlABC,
            orden_save : $("#input-codigo-bl").val(),
            code : code
          },
          beforeSend: function(x) { if(x && x.overrideMimeType) { x.overrideMimeType("application/json;charset=UTF-8"); }
          },
          success: function(data) 
          {
            ...
            if(data.msj === "success")
            {
              swal({
                title: "¡Excelente!",
                text: "Ubicaciones Registradas con Éxito.",
                type: "warning",
                showCancelButton: false
              });
              cerrarAgregar();
    ...
    function remove(id){
        ...
        swal({
            title: "¿Está seguro que desea Desactivar esta ubicacion?",
            type: "warning",
    ...
    { 
        ...
        console.log("BL Buscar:", $("#slect-BL").val());
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: 'POST', //lilo
            colNames:['','','Acciones', 'Sec Surtido', 'Ruta Surtido',nombre_BL, 'ABC','Zona','Pallet|Contenedor', 'License Plate (LP)'/*, 'Productos Ubicados'*/,'Pasillo','Rack','Nivel','Sección','Posición','Peso Máx.','Volumen (m3)'/*,'Peso %','Volumen %','Dimensiones (Alt. X Anc. X Lar. )'*/, 'Tipo','Picking', 'PTL',/*'Maximo','Minimo',*/'Libre', 'Reservada', 'Cuarentena', 'Acomodo Mixto', 'Área de Producción', 'Stagging', 'Traslado', 'Tiene Existencia' ],
            colModel:[
                ...
                {name:'ruta_surtido',index:'ruta_surtido', width:150, fixed:true, sortable:false, resize:false, align:"left"},
                ...
                {name:'clave_contenedor',index:'clave_contenedor',width:150,editable:false,align:"right"},
                {name:'CveLP',index:'CveLP',width:150,editable:false,align:"right"},
                ...
                */
                {name:'tipo',index:'', width:80, fixed:true, sortable:false, resize:false, align:"center", formatter:imageCheck7},
                ...
                {name:'areaproduccion',index:'stagging', width:140, fixed:true, sortable:false, resize:false, align:"center", formatter:imageCheck6},
                {name:'stagging',index:'areaproduccion', width:140, fixed:true, sortable:false, resize:false, align:"center", formatter:imageCheck06},
                {name:'traslado',index:'traslado', width:140, fixed:true, sortable:false, resize:false, align:"center", formatter:imageCheck8},
                {name:'tieneexistencia',index:'tieneexistencia', width:140, fixed:true, sortable:false, resize:false, align:"center", hidden: true},
        ...
        function imageCheck8(cellvalue, options, rowObject){
            var html;
            if (rowObject[26] === "T"){
        ...
        function imageCheck06(cellvalue, options, rowObject){
            var html;
            if (rowObject[25] === "S"){
                html =  '<div class="text-success"><i class="fa fa-check"></i></div>';
            }else{
        ...
        function imageFormat( cellvalue, options, rowObject ){
            ...
            var peso_max = rowObject[15];
            var vol_max = rowObject[16];
            ...
            var tieneexistencia = rowObject[29];
            ...
            if($("#permiso_eliminar").val() == 1 && tieneexistencia == 0)
            html += '<a href="#" onclick="remove(\''+idy_ubica+'\')" alt="Desactivar" title="Desactivar"><i class="fa fa-eraser"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
            html += '<a href="#" onclick="exportarPDFCodigoBarras(\''+CveLP+'\', \''+idy_ubica+'\')"><i class="fa fa-barcode" title="Código de barras"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';            /*html += '<a href="#" onclick="search_alm_produ('+serie+','+idy_ubica+')" alt="Buscar" title="Buscar"><i class="fa fa-search"></i></a>';*/
