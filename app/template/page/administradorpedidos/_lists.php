...
/*
require ('Resend/Resend.php');
$resend = Resend::client('re_GMfxRUt7_9i6AqWdC349joAJwaayCKcPZ');

$resend->emails->send([
  'from' => 'Acme <aolivares@adventech-logistica.com>',
  'to' => ['reinalddo@gmail.com'],
  'subject' => 'hello world',
 ...
 */
                                ...
                                if($("#instancia").val() == "welldex") 
                                    rows_welldex = '<td>'+item.Ref_Wel+'</td>'+
                                                   ...
                                                   '<td>'+item.Ped_Imp+'</td>';
;
                                row += '<tr>'+'<td align="center">'+imageFormat(item.orden, item.surtido, item.TieneOla, '', item.sufijo, is_backorder, item.cliente, item.asignado, '', item.folio_ws, item.status, item.status_pedido, item.fecha_pedido, item.es_ot, item.ruta_pedido, item.statusaurora, item.TipoOT, item.usuario_pedido)+'</td>'+
//function imageFormat(id, surtido, tieneola, tienda, sufijo, is_backorder, cliente, surtidor, articulo_ot, folio_ws, status, status_pedido, fecha_pedido, es_ot, ruta_pedido, statusaurora, TipoOT)
                                    ...
                                    '<td align="center" class="column-dividir" data-id="'+item.orden+'"><input type="checkbox" data-id="'+item.orden+'" '+style_planificar+' /></td>'+
            ...
            if($("#con_recorrido").val() != "1")
            {
                ...
                for(var i = 0; i < detalle_subpedido.length; i++)
                {
                  ...
                  window.detalle_pedido["articulos"].forEach(function (item,index){
                    ...
                    if(item.clave == detalle_subpedido[i].Cve_articulo && item.lote == detalle_subpedido[i].Cve_Lote)
                    {
                        cantidad_enviada += parseFloat($("#surtido-item-"+index+"").val());
                        cantidad_total_a_surtir += parseFloat($("#surtido-item-"+index+"").val());
                    }
                  });
                  articulos_a_surtir.push({fol_folio: detalle_subpedido[i].fol_folio, Sufijo: detalle_subpedido[i].Sufijo, Cve_Articulo: detalle_subpedido[i].Cve_articulo, Cve_Lote: detalle_subpedido[i].Cve_Lote, Num_cantidad: cantidad_enviada});
               }
               ...
               console.log("cantidades_disponibles",cantidades_disponibles);
                 ...
                 for(var i = 0; i < detalle_subpedido.length; i++)
                 {
                    ...
                    if(articulos_a_surtir[i].Num_cantidad < parseFloat(detalle_subpedido[i].Num_cantidad))
                    {
                        backorder_sin_recorrido_surtido.push({fol_folio: detalle_subpedido[i].fol_folio, Sufijo: detalle_subpedido[i].Sufijo, Cve_Articulo: detalle_subpedido[i].Cve_articulo, Cve_Lote: detalle_subpedido[i].Cve_Lote, Cantidad_Pedido: parseFloat(detalle_subpedido[i].Num_cantidad), Cantidad_BO: (parseFloat(detalle_subpedido[i].Num_cantidad)-articulos_a_surtir[i].Num_cantidad)});
                    }
...
//function imageFormat(id, surtido, tieneola, tienda, sufijo, is_backorder, cliente, surtidor, articulo_ot, folio_ws, status, status_pedido, fecha_pedido, es_ot, ruta_pedido, statusaurora, TipoOT)
                  ...
                  '<td align="center" class="column-planificar" data-id="'+item.orden+'" data-sufijo="'+item.sufijo+'"><input type="checkbox" data-id="'+item.orden+'" data-sufijo="'+item.sufijo+'" '+style_planificar+' /></td>'+
                  '<td align="center" class="column-dividir" data-id="'+item.orden+'"><input type="checkbox" data-id="'+item.orden+'" '+style_dividir+' /></td>'+
                  '<td>'+item.ruta_pedido+'</td>'+
                ...
                $.ajax({
                    ...
                    success: function(item) 
                    {
                        ...
                        */
                        ...
                        if(color == 'amarillo')
                        {
                            swal("El pedido se asignará parcialmente", "El pedido cumple con parte de los requerimientos para poder asignarlo, Revise el Reporte de Existencias de Pedido para saber cuales productos faltan por existencias o por asignaciones en ruta de surtido", "warning");
                        }
...
//**********************************************************************************************************************
                        ...
                        obj = {
                          ...
                          volumen: data.articulos[i].volumen,
                          peso: data.articulos[i].peso,
                          solicitadas: //(solicitadas_cant*a_piezas), //**(1)**//
                          unidad_medida: data.articulos[i].unidad_medida,
