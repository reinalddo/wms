...
$listaEmpresas = new \Proveedores\Proveedores();
...
$uso_producto = \db()->prepare("SELECT * FROM c_tipo_producto");
$uso_producto->execute();
$uso_producto = $uso_producto->fetchAll(PDO::FETCH_ASSOC);
                ...
                <div class="ibox ">
                    <div class="ibox-title">
                        ...
                        <div class="row" class="col-md-12">
                            ...
                            <?php } ?>
                            <div class="col-md-6">
                                <a href="/api/v2/articulos/exportar" target="_blank" id="boton_excel" class="btn btn-primary pull-right" style="margin-left:15px; margin-top: 22px;"><span class="fa fa-upload"></span> Exportar</a>
                            ...
                            <div class="form-group">
                                ...
                                <div class="checkbox">
                                    ...
                                    <span id="mensaje_modificar" style="font-weight: bold; color: red;display: none;">
                                        <br>Para reemplazar los registros de los artículos exitosamente, obligatoriamente debe llenar las columnas: Clave Artículo y Clave Almacén. Se reemplazará el valor de la columna que no esté vacía. <br> Se Recomienda llenar solo las celdas que necesita reemplazar para optimizar el tiempo de respuesta</span>
                                </div>
                        ...
                        <form id="myform" role="form">
                            <div class="row">
                                <div class="col-md-6 b-r">
                                    ...
                                    <div class="form-group"><label>Clave Alterna </label> <input id="cve_alt" name="cve_alt" type="text" placeholder="Clave Alterna Artículo" class="form-control"><?php /* ?><label id="codigo_msg" style="color:red;"></label><?php */ ?></div>
                                    ...
                                    <div class="form-group"><label>Descripción Corta *</label> <input id="des_articulo" name="des_articulo" type="text" maxlength="100" placeholder="Nombre de Artículo" class="form-control" required="true"></div>
                                    ...
                                    <div class="col-md-6 form-group">
                                        ...
                                        <select class="form-control" id="tipo_producto" name="tipo_producto">
                                            ...
                                            <?php foreach ($uso_producto as $row): ?>
                                            <option value="<?php echo $row['clave']; ?>">
                                                <?php $clave_producto = $row['clave']; if($row['clave'] == "ProductoNoSurtible") $clave_producto = 'NS'; echo "[".$clave_producto."] ".$row['descripcion']; ?></option>
                                            <?php endforeach; ?>
                                ...
                                <div class="col-md-6">
                                    ...
                                    */
                                    ...
                                    <div class="form-group">
                                        <label>Grupo de Artículo </label>
                                        <select class="chosen-select form-control" id="grupo" name="grupo">
                                    ...
                                    <div class="form-group">
                                        <label>Clasificación de Artículo </label>
                                        <select class="chosen-select form-control" id="clasificacion" name="clasificacion">
                                            <option value="">Clasificación de Artículo</option>
                                            <?php foreach( $listaSubgp->getAll($_SESSION['id_almacen']) AS $p ): ?>
                                                <option value="<?php echo $p->cve_sgpoart; ?>"><?php echo "( ".$p->cve_sgpoart." ) "." - ".$p->des_sgpoart; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>Tipo de Artículo </label>
                                        <select class="chosen-select form-control" id="tipo" name="tipo">
                                            <option value="">Tipo de Artículo</option>
                                            <?php foreach( $listaSubsub->getAll($_SESSION['id_almacen']) AS $p ): ?>
                                                <option value="<?php echo $p->cve_ssgpoart; ?>"><?php echo "( ".$p->cve_ssgpoart." ) "." - ".$p->des_ssgpoart; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>

                                <div class="form-group">
                                    <label>Emmpresa </label>
                                    <select class="form-control chosen-select" id="cboEmpresa">
                                    <option value="">Seleccione</option>
                                    <?php foreach( $listaEmpresas->getAll("AND es_cliente = 1") AS $p ): ?>
                                    <option value="<?php echo $p->ID_Proveedor; ?>"><?php echo $p->Nombre; ?></option>
                                    <?php endforeach; ?>
                                </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="cboProveedor" name="cboProveedor[]" multiple>
                                ...
                                */
                                ...
                                ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="button" onclick="ver_fotos()" id="boton-documentos" value="Subir Documentos" class="btn btn-primary">
...
$('#btn-import').on('click', function() 
{
  ...
  ///////////////////////////////////////////////SUBIR FOTOS TH///////////////////////////////////////////////
    $('#btn-fotos-th').on('click', function() {
        ...
        $("#cve_articulo_documento").val($("#codigo").val());
        ...
        if(!$("#cve_articulo_documento").val())
        {
            swal("Advertencia!", "Es necesario una clave de artículo", "warning");
            return false;
        }
        ...
        $.ajax({
            // Your server script to process the upload
            url: '/articulos/archivosdocumentos',
            type: 'POST',
            ...
            success: function(data) {
                ...
                setTimeout(
                    function(){if (data.status == 200 && data.statusType == 1) {
                        ...
                        //ReloadGrid();
                        $("#file_th").val("");
                        ImprimirFotos_th($("#cve_articulo_documento").val());
                    }
                    else {
                        $('#fotos_oc_th').modal('hide');
                        swal("Error", data.statusText, "error");
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: 'POST',
            colNames: ['Acciones','ID','Clave','CB Pieza','CB Caja','CB Pallet','Descripción','KIT', 'Usa Envase','Dimensiones (mm)','Volumen (m3)','Peso U (Kgs)','Costo','Piezas por Caja','Cajas por Pallet','Grupo','Clasificación (SAT)','Tipo', 'Empresa|Proveedor','Tipo de Producto','UMAS','Unida de Medida', 'Imagen','Almacen', 'Descripción Detallada', 'Barcode', 'Tiene Documentos', 'Clave Alterna'],
            colModel: [
                {name: 'myac',index: '',width: 120,fixed: true,sortable: false,resize: false,formatter: imageFormat,frozen: true},
                {name: 'id',index: 'id',width: 150,editable: false,hidden: true,sortable: false,resizable: true},
                {name: 'cve_articulo',index: 'cve_articulo',width: 150,editable: false,sortable: false,resizable: true},
                {name: 'cve_codprov',index: 'cve_codprov',width: 150,editable: false,sortable: false,resizable: true},
                {name: 'barras2',index: 'barras2',width: 150,editable: false,sortable: false,resizable: true},
                {name: 'barras3',index: 'barras3',width: 150,editable: false,sortable: false,resizable: true},
                {name: 'des_articulo',index: 'des_articulo',width: 250,editable: false,sortable: false,resizable: true},
                ...
                {name: 'clave_alterna',index: 'clave_alterna',width: 150,editable: false,sortable: false,resizable: true},
            ],
            rowNum: 30,
            rowList: [30, 40, 50],
        ...
        function imageFormat(cellvalue, options, rowObject) 
        {
            ...
            var tiene_documentos = rowObject[26];
            ...
            html += '<a href="#" onclick="descargar_documentos(\'' + cve_articulo + '\')" title="Ver Documentos"><i class="fa fa-download" ></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
    ...
    function ReloadGrid() 
    {
        $('#grid-table').jqGrid('clearGridData')
            ...
            $("#caracteristicas_art, #cboProveedor").select2();
            //$(".chosen-choices .search-field .default").val("Seleccione");
        ...
        $.ajax({
            ...
            success: function(data) 
            {
                ...
                if (data.success == true) 
                {
                    ...
                    <?php 
                    if(strtolower($_SESSION['cve_usuario']) != 'wmsmaster' && strtolower($_SESSION['cve_usuario']) != 'cgonzalez' && strtolower($_SESSION['cve_usuario']) != 'agonzalez' && strtolower($_SESSION['cve_usuario']) != 'jlopez' && strtolower($_SESSION['cve_usuario']) != 'vicrfsdelgado' && strtolower($_SESSION['cve_usuario']) != 'jgomez' && strtolower($_SESSION['cve_usuario']) != 'mluna' && strtolower($_SESSION['cve_usuario']) != 'asierra' && strtolower($_SESSION['cve_usuario']) != '100' && strtolower($_SESSION['cve_usuario']) != 'l01')
                    {
                    ?>
                    //************************************************
                    ...
                    var ID_Proveedor_val = data.ID_Proveedor;

                    var ID_Proveedor_vec = '';

                    if(ID_Proveedor_val)
                    {
                       ID_Proveedor_vec = ID_Proveedor_val.split(",");
                    }

                    $("#cboProveedor").select2().val(ID_Proveedor_vec).trigger('change');
                    //['1', '2']
                    ...
                    $('#grupo, #clasificacion, #tipo, #cboEmpresa').trigger("chosen:updated");
                }
            }, error: function(data) 
            {
...
*/
        ...
        else 
        {
            ...
            console.log("cboProveedor = ", $("#cboProveedor").val());
            //return;
            $.ajax({
                type: "POST",
                dataType: "json",
...
*/
    ...
    $("#codigo,#cve_alt,#cve_codprov,#barras2,#barras3").keyup(function(e)
    {
    ...
    function codigo_de_barras_funcion(codex)
    {
        ...
        if (claveCodeRegexp.test(claveCode) || claveCode == "") 
        {
            ...
            if(codigo_barras1 != "" || codigo_barras2 != "" || codigo_barras3 != "")
            {
                ...
                var a = (codigo_barras1==codigo_barras2)&&(codigo_barras1!="");
                if(a){$('#CodeMessage2,#CodeMessage3').html(msg); $("#btnSave").prop('disabled', true);console.log("DISABLED 4");}

                var b = (codigo_barras1==codigo_barras3)&&(codigo_barras1!="");
                if(b){$('#CodeMessage2,#CodeMessage4').html(msg); $("#btnSave").prop('disabled', true);console.log("DISABLED 5");}

                var c = (codigo_barras2==codigo_barras3)&&(codigo_barras2!="");
                if(c){$('#CodeMessage3,#CodeMessage4').html(msg); $("#btnSave").prop('disabled', true);console.log("DISABLED 6");}
                return 0;
            }
            else
            {
...
*/
    ...
    $("#boton_excel").click(function(e)
    {
        ...
        var grupo = $("#grupo_b").val();
        var clasificacion = $("#clasificacion_b").val();
        var tipo = $("#tipo_b").val();
        var compuesto = $("#tipo_c").val();
        var instancia = $("#instancia").val();
        var almacen = $("#almacenes").val();

        $(this).attr("href", "/api/v2/articulos/exportar?almacen="+almacen+"&proveedor="+proveedor+"&criterio="+criterio+"&grupo="+grupo+"&clasificacion="+clasificacion+"&tipo="+tipo+"&compuesto="+compuesto+"&instancia="+instancia);
    });
