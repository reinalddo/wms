        ...
        <div class="modal-content animated bounceInRight">
            ...
            <form id="myform">
                <div class="modal-body">
                    ...
                    <select class="form-control" id="cve_sgpoart" name="cve_sgpoart">
                        <option value="">Clasificación de Artículo</option>
                        <?php foreach( $listaSubgp->getAll($_SESSION['id_almacen']) AS $p ): ?>
                        <option value="<?php echo $p->id; ?>"><?php echo str_pad($p->cve_sgpoart, 9, "_", STR_PAD_LEFT)." - ".$p->des_sgpoart; ?></option>
                        <?php endforeach; ?>
                    </select><br>
                    </div>
...
$('#btn-layout').on('click', function(e) {
    window.location.href = '/Layout/Layout_TiposArticulos.xlsx';
}); 
...
<script type="text/javascript">
    ...
    $(function($) {
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: 'POST',
            colNames:["Acciones", "Clave",'Clasificación','Descripción', 'id'],
            colModel:[
                ...
                {name:'id',index:'id',width:735, editable:false, sortable:false, hidden: true},
            ],
            rowNum:30,
            rowList:[30,40,50],
        ...
        function imageFormat( cellvalue, options, rowObject ){
            ...
            html += '<a href="#" onclick="editar(\''+id+'\',\''+serie+'\', \''+des+'\')"><i class="fa fa-edit" alt="Editar"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
    ...
    function editar(id, _codigo, descripcion) {
        $("#hiddenCodigo").val(_codigo);
        $("#codigo").prop('disabled',true);
        console.log("codigo = ", _codigo, " id = ", id);
        ...
        $.ajax({
            ...
            url: '/api/ssubgrupodearticulos/update/index.php',
            success: function(data) {
                console.log("codigo SUCCESS = ", data);
                if (data.success == true) {
                    //$('#codigo').prop('disabled', true);
                    $("#codigo").val(_codigo);
                    $("#cve_sgpoart").val(data.cve_sgpoart);					
