    ...
    function save($_post)
    {
      ...
      try
      {
        ...
        $to_insert_th_aduana = array(
          ...
          "numeroDeExpediente"  => $_post['numeroDeExpediente']
...
*/
    ...
    $sql = "Select count(*) as x from th_entalmacen where id_ocompra = '".$data["Fol_Folio"]."'";
    ...
    if($existe_entrada == 0)
    {
      ...
      $data_Cve_Proveedor    = $data["Cve_Proveedor"];
    ...
    else
    {
      ...
      $this->tools->dbUpdate("th_entalmacen",$data_entrada_header,array("id_ocompra"=>$data["oc"]));
    ...
    if($data["tipo"] != 'RL')
    {
      $sql = "Select Fol_Folio from th_entalmacen where id_ocompra = '".$data["Fol_Folio"]."'";
...
//      { 
      ...
      $sql = "SELECT (IF(control_lotes = 'S', 'lote', IF(control_numero_series = 'S', 'serie', ''))) AS bandera FROM c_articulo WHERE cve_articulo = '$cve_articulo'";
      $rs = mysqli_query($conexion, $sql);
...
*/
      ...
      if($existe_entalmacen)
      {
          $sql = "UPDATE td_entalmacen SET CantidadRecibida = CantidadRecibida + ".$item['cantidad_por_recibir'].",
                                           CantidadDisponible = CantidadDisponible +".$cantidad_disponible."
        ...
        VALUES (".$id_entrada.", '".$cve_articulo."', '".$lote."', ".$cantidad_pedida.", ".$item['cantidad_por_recibir'].", ".$cantidad_disponible.", '".$serie."', 'E', '".$data['usuario']."', '".$item["zona_recepcion"]."', '".date('Y-m-d H:i:s', strtotime($item['hora_de_recepcion']))."', '".date('Y-m-d H:i:s', strtotime($data['fechafin']))."', 1, ".$item['costo'].", ".$data_oc.", '".$item["num_pedimento"]."', '".$item["fecha_pedimento"]."')";
...
//********************************************************************
   ...
   //////insertar datos con contenedor en td_entalmacenxtarima y cajas///////7 
    $clave_contenedor_selected = "";
    foreach ($data["arrDetalle"] as $item)
    {
        if($item['contenedor'] != "" && $item['multiplo'] > 1)
        {
            $pallet = $item['contenedor'];

            $sql = "SELECT * FROM c_charolas WHERE clave_contenedor = '$pallet'";
...
//******************************************************************************************
          ...
          if($item['lote'] == NULL){$item['lote'] = " ";}
             ...
             $insert_xtarimasin = array(
               ...
               "cve_articulo"   => $item['clave_articulo'],
               "cve_lote"       => $item['lote'],
               "ClaveEtiqueta"  => $label_lp, //$item['contenedor'],
...
*/
  ...
  function traer_ordenes($data) 
  {
    $sql = "SELECT DISTINCT th_aduana.*,
            ...
            WHERE (th_aduana.status = 'C' OR th_aduana.status = 'I')
    ...
    foreach($ordenes as $ord)
    {
      ...
      */
      ...
      IFNULL(axt.Cantidad,(a.cantidad-IFNULL((SELECT SUM(CantidadRecibida) FROM td_entalmacen WHERE num_orden = '{$id_td}' AND cve_articulo = a.cve_articulo AND IFNULL(cve_lote, '') = IFNULL(a.cve_lote, '')),0))) AS cantidad, 
      ...
      IFNULL(a.cve_lote, '') AS val_lote,
              ...
              LEFT JOIN td_entalmacen e ON e.cve_articulo = a.cve_articulo AND IFNULL(e.cve_lote, '') = IFNULL(a.cve_lote, '') AND e.num_orden = a.num_orden
              LEFT JOIN c_unimed um ON um.id_umed = c_articulo.unidadMedida
              WHERE a.num_orden = '{$id_td}'
              AND c_articulo.cve_articulo NOT IN (SELECT cve_articulo FROM td_entalmacen WHERE td_entalmacen.num_orden = a.num_orden AND e.CantidadPedida = e.CantidadRecibida)";
      $sth = \db()->prepare($sql);
      $sth->setFetchMode( \PDO::FETCH_CLASS, '\OrdenCompra\OrdenCompra' );
      $sth->execute();
  ...
  function traer_todos_los_articulos_proveedores($data) 
  {
    ...
    $sql = "SELECT DISTINCT a.*
            FROM c_articulo a
              ...
              LEFT JOIN th_aduana th ON th.num_pedimento = td.num_orden
              INNER JOIN td_ajusteexist tex ON tex.cve_articulo = ts.cve_articulo
            WHERE th.ID_Proveedor = $id_prov $and_orden";
    $sth = \db()->prepare($sql);
    $sth->setFetchMode( \PDO::FETCH_CLASS, '\OrdenCompra\OrdenCompra' );
    $sth->execute();
    return $sth->fetchAll();
  ...
  {
    $articulo_search = ""; $sql_search = "";
    if(isset($data['articulo_search']))
    {
        $articulo_search = $data['articulo_search'];
        $sql_search = " WHERE (a.cve_articulo LIKE '%$articulo_search%' OR a.des_articulo LIKE '%$articulo_search%') ";
    }
    $sql = "SELECT a.*, u.mav_cveunimed as cve_unidad_medida FROM c_articulo a LEFT JOIN c_unimed u ON u.id_umed = a.unidadMedida {$sql_search}";
    $sth = \db()->prepare($sql);
    $sth->setFetchMode( \PDO::FETCH_CLASS, '\OrdenCompra\OrdenCompra' );
    $sth->execute();
    return $sth->fetchAll();
