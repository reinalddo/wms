    ...
    mysqli_set_charset(\db2() , $charset);
      ...
      if($tipo == "OC" || $tipo == 1)
      {
					...
					$join = "LEFT JOIN th_entalmacen on th_entalmacen.id_ocompra = th_aduana.num_pedimento
							LEFT JOIN c_articulo on td_aduana.cve_articulo = c_articulo.cve_articulo
                            LEFT JOIN c_compania ON c_compania.cve_cia = IFNULL((SELECT cve_cia FROM c_usuario WHERE id_user = th_aduana.cve_usuario), (SELECT cve_cia FROM c_usuario WHERE id_user = (SELECT id_user FROM c_usuario WHERE cve_usuario = th_aduana.cve_usuario)))
                            ";
                        ...
                        $sql = "
                            SELECT distinct
                                ...
                                (SELECT CONCAT(IFNULL(pr.Nombre, ''),' ',IFNULL(pr.RUT, ''),' ',IFNULL(pr.direccion, ''), ' ',IFNULL(pr.colonia, ''),' ',IFNULL(pr.cve_dane, ''),' ',IFNULL(pr.ciudad, ''),', ',IFNULL(pr.estado, ''),', ',IFNULL(pr.pais , '')) AS p FROM c_proveedores pr WHERE pr.ID_Proveedor = th_aduana.ID_Proveedor) datos_proveedor
                            from {$from}
                            {$join}
...
<?php $imgBarCode = "https://".$_SERVER['HTTP_HOST']."/api/articulos/lista/barcode.php?codetype=Code128&size=20&text=".$row["cve_articulo"]."&print=true"; 
...
//$pdf->Image('@' . $img, 55, 19, '', '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
    ...
    public function generarReporteEntradasCodigoDeBarras($folio,$proveedor, $oc, $lp)
    {
        $res = ""; $res2 = "";
...
*/
        ...
        $imgLogosSP = $base_url.$datos["logo"];
        //ob_start(); 
        if($num_pallets == 1 || $num_pallets == 0)
        {
            ob_start(); 
...
<?php 
    ...
    $sql = "SELECT DISTINCT
            ...
            TRUNCATE((IF(IFNULL(a.num_multiplo, 0) = 0, tde.CantidadPedida - IF(th_entalmacen.tipo = 'OC', tde.CantidadRecibida, 0), (tde.CantidadPedida - IF(th_entalmacen.tipo = 'OC', tde.CantidadRecibida, 0))/a.num_multiplo)), 0) AS cantidad_danada,
            u.cve_usuario AS usuario,
