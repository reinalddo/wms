...
class AdministradorPedidos 
{
  ...
  function verificarStatus($folio, $is_backorder, $sufijo)
  {
	...
	function loadArticulos($id,$almacen,$status_pedido, $sufijo, $is_backorder, $folio_rel, $con_recorrido, $TipoDoc) 
  {
                    ...
                    if($folio_rel)
                    {
                        $sql = "
                                SELECT 
                                  ...
                                  IF(a.control_lotes = 'S' AND (a.Caduca = 'S' OR (th_pedido.Ship_Num != '' AND IFNULL(a.Caduca, 'N') = 'N')), IF(DATE_FORMAT(lo.Caducidad, '%d-%m-%Y') = DATE_FORMAT('00-00-0000', '%d-%m-%Y'), '', DATE_FORMAT(lo.Caducidad, '%d-%m-%Y')), '') AS caducidad,
            ...
            #IF(COALESCE((SELECT SUM(Existencia*a.num_multiplo) AS suma 
                        ...
                        $sql = "
                                SELECT 
                                  ...
                                  '' AS ruta,
                                    ...
                                    IFNULL(a.num_multiplo, 0) piezasxcajas, 
                                  IF(a.control_lotes = 'S', tsb.cve_lote, '') AS lote,
                                  IF(a.control_lotes = 'S' AND a.Caduca = 'S', IF(DATE_FORMAT(lo.Caducidad, '%d-%m-%Y') = DATE_FORMAT('00-00-0000', '%d-%m-%Y'), '', DATE_FORMAT(lo.Caducidad, '%d-%m-%Y')), '') AS caducidad,
                                  IF(a.control_numero_series = 'S', tsb.cve_lote, '') AS serie,
                                  #SUM(ROUND((a.peso * tsb.Num_cantidad),4)) AS peso,
                                  #IF(op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0, SUM(ROUND((a.peso * tsb.Num_cantidad),4)), tsb.Num_cantidad) AS peso,
                ...
                else
                {
                        $sql = "
                                SELECT 
                                  ...
                                  #SUM(ROUND((a.peso * tsb.Num_cantidad),4)) AS peso,
                                  #IF(op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0, SUM(ROUND((a.peso * tsb.Num_cantidad),4)), tsb.Num_cantidad) AS peso,
                                  IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(((SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1) LIKE 'OT%'), IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)), SUM(ROUND((a.peso * IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))),4))), IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))) AS peso,
              ...
              $sql = "
                    ...
                    FROM td_subpedido tsb 
                      ...
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo GROUP BY clave 
            ...
            if($con_recorrido == 1)
            {
              $sql = "
                    ...
                    FROM td_subpedido tsb
                      ...
                      LEFT JOIN t_recorrido_surtido t ON t.fol_folio = th_pedido.Fol_folio AND t.Cve_articulo = tsb.Cve_articulo AND t.cve_lote = tsb.cve_lote AND t.Sufijo = $sufijo AND t.ClaveEtiqueta IN (IFNULL(ch.clave_contenedor, 0), IFNULL(ch.clave_contenedor, 1)) 
                      LEFT JOIN t_recorrido_surtido t_sin_lote ON t_sin_lote.fol_folio = th_pedido.Fol_folio AND t_sin_lote.Cve_articulo = tsb.Cve_articulo AND t_sin_lote.Sufijo = $sufijo AND t_sin_lote.ClaveEtiqueta IN (IFNULL(ch.clave_contenedor, 0), IFNULL(ch.clave_contenedor, 1)) 
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo 
                    #GROUP BY t.claverp,t.Cve_articulo, serie, lote, existencia, ubicacion, clave
                    GROUP BY IFNULL(t.claverp,t_sin_lote.claverp), IFNULL(t.cve_lote, t_sin_lote.cve_lote), IFNULL(t.idy_ubica, t_sin_lote.idy_ubica), IFNULL(t.Cve_articulo, t_sin_lote.Cve_articulo), IFNULL(t.fol_folio, t_sin_lote.fol_folio), LP
                    ORDER BY IFNULL(t.orden_secuencia, LP)
              ";//AND t.cve_lote = tsb.cve_lote
          }
          else if($con_recorrido == 0)
          {
              ...
              $sql = "SELECT * FROM (
                    SELECT 
                          ...
                          IF(IFNULL(t.cve_lote, t_sin_lote.cve_lote) != '', IF(a.control_lotes = 'S',  IFNULL(t.cve_lote, t_sin_lote.cve_lote), ''), '') AS lote,
                          IF(a.Caduca = 'S', IF(IFNULL(t.cve_lote, t_sin_lote.cve_lote) != '', IF(((SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = IFNULL(t.cve_lote, t_sin_lote.cve_lote) AND tsb.Cve_articulo = cve_articulo) != '00-00-0000') AND (a.control_lotes = 'S'),  (SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = IFNULL(t.cve_lote, t_sin_lote.cve_lote) AND tsb.Cve_articulo = cve_articulo), '' ), ''), '') AS caducidad,
                          IF(IFNULL(t.cve_lote, t_sin_lote.cve_lote) != '', IF(a.control_numero_series = 'S',  IFNULL(t.cve_lote, t_sin_lote.cve_lote), ''), '') AS serie,
                          IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(('OT' LIKE (SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1)), SUM(ROUND((a.peso * tsb.Num_cantidad),4)), tsb.Num_cantidad), tsb.Num_cantidad) AS peso,
                          SUM(ROUND(((a.alto/1000) * (a.ancho/1000) * (a.fondo/1000) * tsb.Num_cantidad),4)) AS volumen,
                          IF(a.control_peso = 'S', TRUNCATE(tsb.Num_cantidad, 4), TRUNCATE(tsb.Num_cantidad,0) ) AS pedidas,
              ...
              if($status != 'S')
              {
                $sql = "SELECT 
                          ...
                          #'' AS lote,
                          IF(s.LOTE != '', IF(((SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = s.LOTE AND tsb.Cve_articulo = cve_articulo) != '00-00-0000') AND (a.control_lotes = 'S'),  (SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = s.LOTE AND tsb.Cve_articulo = cve_articulo), '' ), '') AS caducidad,
                          #'' AS caducidad,
