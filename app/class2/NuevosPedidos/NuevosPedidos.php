...
class NuevosPedidos {
  ...
  function GenerarDiaOperativo($id_ruta, $clave_almacen)
  {
      ...
      $sql = "SELECT IFNULL(MAX(IFNULL(DiaO, 1)), '') AS DiaO FROM DiasO WHERE RutaId = $id_ruta AND IdEmpresa = '$clave_almacen'";
  ...
  function save( $_post ) {
    ...
    try {
      ...
      //***************************************************************************************************
            ...
            if($tipo_venta == 'venta' && $_post['tipo_traslado'] != 1) 
            {
                  ...
                  foreach ($_post["arrDetalle"] as $detVenta) 
                  {
                      ...
                      $descuento_monto = ($detVenta['desc_importe']/100)*$importe;
                      $sql = "INSERT IGNORE INTO DetalleVet(Articulo, Descripcion, Precio, Pza, Kg, DescPorc, DescMon, Tipo, Docto, Importe, IVA, IEPS, RutaId, IdEmpresa, Comisiones, Utilidad) 
      ...
      if($tipo_lp == '1')
        ...
        $sql = "INSERT INTO " . self::TABLE . " (Fol_folio, Fec_Pedido, Cve_clte, status, TipoPedido, Fec_Entrega, cve_Vendedor, DiaO, TipoDoc, Fec_Entrada, Pick_Num, destinatario, Cve_Usuario, Observaciones, statusaurora, ID_Tipoprioridad, ruta, rango_hora, cve_almac, Cve_CteProv, cve_ubicacion, tipo_venta, tipo_negociacion, Activo)";
            $sql .= "Values (";
            $sql .= "'".$_post['Fol_folio']."',";
            ...
            $sql .= "(SELECT DISTINCT cve_ruta from c_cliente where Cve_clte = '".$_post['Cve_clte']."' LIMIT 1),";
            $sql .= "'".$rango_hora."',";
            $sql .= "'".$cve_almac."',";
            ...
            if (!empty($_post["arrDetalle"]) && $ots_con_lp == "") {
              //peso, precio_unitario, importe_total
                foreach ($_post["arrDetalle"] as $item) {
                    ...
                    $sql .= "('".$_post['Fol_folio']."', '".$item['Cve_articulo']."', '".$item['cve_lote']."', '".$item['Num_cantidad']."', '".$item['id_unimed']."', '".$item['Num_Meses']."', 'A' , '1', '".$item['precio_unitario']."', '".$item['desc_importe']."', '".$item['iva']."');";
                    ...
                    else if($tipo_lp == '1' && $item['CveLP'] != "")
                    {
                        ...
                        $sth = \db()->prepare( $sql );$sth->execute();$res_folio = $sth->fetch();
                        ...
                        $sth = \db()->prepare( $sql );$sth->execute();$res_CveLP = $sth->fetch();
