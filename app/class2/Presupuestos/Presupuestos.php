...
class Presupuestos {
    ...
    private function load() {
        try {
            $sql = 'SELECT * FROM c_presupuestos Where c_presupuestos.id = '.$this->id;
    ...
    function save($_post) 
    {
        try {
          ...
          $sql = "INSERT INTO ".self::TABLE."
                  SET
                  nombreDePresupuesto= '${nombre_presupuesto}',
                  anoDePresupuesto= '${ano_presupuesto}',
                  claveDePartida= '${cve_partida}',
                  conceptoDePartida= '${concepto_partida}',
                  monto= '${monto_presupuesto}'";
    ...
    function actualizarPresupuestos($_post) 
    {
      try
      {

        $to_update_c_presupuestos = array(
          "nombreDePresupuesto"     => $_post['nombre_presupuesto'],
          "anoDePresupuesto"        => $_post['ano_presupuesto'],
          "claveDePartida"          => $_post['cve_partida'],
          "conceptoDePartida"       => $_post['concepto_partida'],
          "monto"                   => $_post['monto_presupuesto']
        ...
        $insert_orden = $this->tools->dbUpdate(self::TABLE,$to_update_c_presupuestos,$where_update);
      } 
      catch(Exception $e) 
      {
