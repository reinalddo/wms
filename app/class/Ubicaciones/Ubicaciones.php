...
class PDODebugger {
    ...
    public function __construct(\PDO $pdo, $logFileName = 'pdo_debug_log.txt') {
        $this->pdo = $pdo;
        $this->logFile = __DIR__ . '/' . $logFileName;
        // Limpia el log al empezar
        file_put_contents($this->logFile, "--- INICIO DEL LOG DE CONSULTAS " . date('Y-m-d H:i:s') . " ---\n");
    }
    ...
    // Pasa cualquier otra llamada directamente al objeto PDO original
    public function __call($method, $args) {
        if (is_callable([$this->pdo, $method])) {
            return call_user_func_array([$this->pdo, $method], $args);
        }
        throw new \Exception("Método no encontrado en la clase PDO: " . $method);
    }
}
*/
...
//        $sql .= "UPDATE c_charolas SET CveLP = '$valor_lp' WHERE clave_contenedor = '$valor_origen'";
    ...
    function moverTraslado($data, $conn_pdo = "") {

      try{
        extract($data);

        //$conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        if($conn_pdo == "") $conn_pdo = \db();
        //$query = mysqli_query($conn, "INSERT IGNORE INTO `t_tipomovimiento` (`nombre`) VALUES ('Traslado');");

    // ✅ ¡AQUÍ ACTIVAMOS EL ESPÍA!
    // Esto reemplaza la variable $conn_pdo con nuestro objeto espía,
    // que a su vez contiene la conexión real.
    //$conn_pdo = new PDODebugger($conn_pdo);


        $query = $conn_pdo->prepare("INSERT IGNORE INTO `t_tipomovimiento` (`nombre`) VALUES ('Traslado');");$query->execute();$query->closeCursor();
        //$query2 = mysqli_query($conn, "SELECT id_TipoMovimiento AS id FROM `t_tipomovimiento` WHERE nombre = 'Traslado';");
        $query2 = $conn_pdo->prepare("SELECT id_TipoMovimiento AS id FROM `t_tipomovimiento` WHERE nombre = 'Traslado';");$query2->execute();
        //$movimiento = mysqli_fetch_assoc($query2)['id'];
        $movimiento = $query2->fetch(\PDO::FETCH_ASSOC)['id'];$query2->closeCursor();
...
/*
        ...
        {
          $almacen = $data['almacen_destino'];
          $query3 = mysqli_query($conn, "SELECT clave FROM c_almacenp WHERE id = {$almacen}");
          $clave_almacen = mysqli_fetch_assoc($query3)['clave'];
        }
        else 
*/
        //{
          //$query3 = mysqli_query($conn, "SELECT cve_almacenp AS id FROM c_almacen WHERE cve_almac = (SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = '$ididestino')");
          $query3 = $conn_pdo->prepare("SELECT cve_almacenp AS id FROM c_almacen WHERE cve_almac = (SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = '$ididestino')");$query3->execute();
          //$almacen = mysqli_fetch_assoc($query3)['id'];
          $almacen = $query3->fetch(\PDO::FETCH_ASSOC)['id'];$query3->closeCursor();

          //$almacen = $data['almacen_destino'];
          //$query3 = mysqli_query($conn, "SELECT clave FROM c_almacenp WHERE id = {$almacen}");
          $query3 = $conn_pdo->prepare("SELECT clave FROM c_almacenp WHERE id = {$almacen}");$query3->execute();
          //$clave_almacen = mysqli_fetch_assoc($query3)['clave'];
          $clave_almacen = $query3->fetch(\PDO::FETCH_ASSOC)['clave'];$query3->closeCursor();
        //}
        ...
        //$query3 = mysqli_query($conn, "SELECT IFNULL(cve_nivel, 0) as nivel FROM c_ubicacion WHERE idy_ubica = '$ididestino'");
        $query3 = $conn_pdo->prepare("SELECT IFNULL(cve_nivel, 0) as nivel FROM c_ubicacion WHERE idy_ubica = '$ididestino'");$query3->execute();
        //$nivel_ubicacion = mysqli_fetch_assoc($query3)['nivel'];
        $nivel_ubicacion = $query3->fetch(\PDO::FETCH_ASSOC)['nivel'];$query3->closeCursor();
...
if($trasladar_ubicacion == 0)
{
        if($palletizar_traslado && !$traslado_almacen)
        {
            ...
            $rs = $conn_pdo->prepare($sql);$rs->execute();
            //$resul = mysqli_fetch_array($rs, MYSQLI_ASSOC);
            $resul = $rs->fetch(\PDO::FETCH_ASSOC);$rs->closeCursor();
            ...
            //$rs = mysqli_query($conn, $sql);
              $rs = $conn_pdo->prepare($sql);$rs->execute();$rs->closeCursor();
            }
            else 
      ...
      if($tipo_pallet_contenedor_articulo == 'Artículo' /*&& !$traslado_almacen*/  && !$valor_lp && !$palletizar_traslado)
      {
        ...
        $sth = $conn_pdo->prepare( $sql2 );
        $sth->execute();$sth->closeCursor();
...
//*************************************************************************************************************************************************
                  ...
                  $res_id = $conn_pdo->prepare($sql);$res_id->execute();
                  //$row_autoid = mysqli_fetch_assoc($res_id);
                  $row_autoid = $res_id->fetch(\PDO::FETCH_ASSOC);
                  ...
                  $row_caja = $res_id->fetch(\PDO::FETCH_ASSOC);
                  ...
                  $res_id = $conn_pdo->prepare($sql);$res_id->execute();
                  //$row_caja = mysqli_fetch_assoc($res_id);
                  $row_caja = $res_id->fetch(\PDO::FETCH_ASSOC);$res_id->closeCursor();
                  ...
                  $res_id = $conn_pdo->prepare($sqlGuardar);$res_id->execute();$res_id->closeCursor();
                  //*************************************************************************************
...
//*************************************************************************************************************************************************
        ...
        //******************************************************************************************************************************************
          ...
          $query_traz->execute();
          $mayor_a_cero = $query_traz->fetch(\PDO::FETCH_ASSOC)['mayor_a_cero'];
          $query_traz->closeCursor();


          $sql_traz = $conn_pdo->prepare("
          SELECT e.Fol_Folio AS folio_entrada, oc.num_pedimento as folio_oc, IFNULL(e.Fact_Prov, '') as factura_ent, IFNULL(oc.Factura, '') as factura_oc, IFNULL(e.Proyecto, '') as proyecto
          FROM t_trazabilidad_existencias t 
          LEFT JOIN th_entalmacen e ON e.Fol_Folio = t.folio_entrada
          LEFT JOIN th_aduana oc ON oc.num_pedimento = IF(e.tipo != 'RL' AND e.id_ocompra IS NOT NULL, e.id_ocompra, NULL)            
          WHERE t.cve_almac = '{$almacen}' AND t.idy_ubica = '".$data["idiorigen"]."' AND CONVERT(t.cve_articulo, CHAR) = '".$data["cve_articulo"]."' AND CONVERT(t.cve_lote, CHAR) = '".$data["cve_lote"]."' AND t.id_proveedor = '$ID_Proveedor' AND t.id_tipo_movimiento = 2 AND CONVERT(IFNULL(e.Fact_Prov, ''), CHAR) = CONVERT(IFNULL(t.factura_ent, ''), CHAR) AND CONVERT(IFNULL(e.Proyecto, ''), CHAR) = CONVERT(IFNULL(t.proyecto, ''), CHAR) AND CONVERT(IFNULL(oc.Factura, ''), CHAR) = CONVERT(IFNULL(t.factura_oc, ''), CHAR) AND IFNULL(t.ntarima, '') = ''");
          $sql_traz->execute();
          $valores_traz = $sql_traz->fetch(\PDO::FETCH_ASSOC);
          $sql_traz->closeCursor();
...
//, (SELECT IFNULL(Cuarentena, 0) FROM ts_existenciapiezas where cve_articulo='".$data["cve_articulo"]."' and cve_lote='".$data["cve_lote"]."' and idy_ubica='".$data["idiorigen"]."')
        ...
        $sth->execute();$sth->closeCursor();
            ...
            if($fusionar_cajas)
            {
              ...
              //$rs = mysqli_query($conn, $sql_tr);
                $rs = $conn_pdo->prepare($sql_tr);$rs->execute();$rs->closeCursor();
            }
      ...
      else if(/*!$traslado_almacen &&*/ !$palletizar_traslado)
      {
        ...
        {
            ///////$query_rev = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciatarima WHERE cve_articulo='".$data["cve_articulo"]."' and lote='".$data["cve_lote"]."' and idy_ubica='".$data["idiorigen"]."'");<-- Este SIN PDO NO VA
            //$query_rev = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");
            $query_rev = $conn_pdo->prepare("SELECT COUNT(*) as existe FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");$query_rev->execute();
            if($tipo_pallet_contenedor_articulo == 'Caja')
            {
              //$query_rev = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciacajas WHERE idy_ubica='".$data["idiorigen"]."' AND Id_Caja = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");
              $query_rev = $conn_pdo->prepare("SELECT COUNT(*) as existe FROM ts_existenciacajas WHERE idy_ubica='".$data["idiorigen"]."' AND Id_Caja = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");$query_rev->execute();
            }
            //$existe_tarima = mysqli_fetch_assoc($query_rev)['existe'];
            $existe_tarima = $query_rev->fetch(\PDO::FETCH_ASSOC)['existe'];
            $query_rev->closeCursor();
          ...
          if($existe_tarima)
          {
            ...
            if($tipo_pallet_contenedor_articulo == 'Artículo')
             ...
             if($convertir_pzs == 1)
             {
                ...
                $rs = $conn_pdo->prepare($sql);$rs->execute();
                //$resul = mysqli_fetch_array($rs, MYSQLI_ASSOC);
                $resul = $rs->fetch(\PDO::FETCH_ASSOC);
                $rs->closeCursor();
              ...
              //$query3 = mysqli_query($conn, "SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$label_lp}'");
              $query3 = $conn_pdo->prepare("SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$label_lp}'");$query3->execute();
              //$pallet_palletizar = mysqli_fetch_assoc($query3)['IDContenedor'];
              $pallet_palletizar = $query3->fetch(\PDO::FETCH_ASSOC)['IDContenedor'];
              $query3->closeCursor();

              //$query4 = mysqli_query($conn, "SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$valor_lp_origen}'");
              $query4 = $conn_pdo->prepare("SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$valor_lp_origen}'");$query4->execute();
              //$pallet_palletizar_origen = mysqli_fetch_assoc($query4)['IDContenedor'];
              $pallet_palletizar_origen = $query4->fetch(\PDO::FETCH_ASSOC)['IDContenedor'];
              $query4->closeCursor();

                //$query_rev_pzs = mysqli_query($conn, "SELECT * FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");
/*
                $query_rev_pzs = $conn_pdo->prepare("SELECT * FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");$query_rev_pzs->execute();

                //while($row_pzs = mysqli_fetch_assoc($query_rev_pzs))
                while($row_pzs = $query_rev_pzs->fetch(\PDO::FETCH_ASSOC))
                {
                    ...
                    //$sth = $conn_pdo->prepare( $sql_convertir_pzs );
                    //$sth->execute();

                    $sql_convertir_pzs = "CALL SPWS_ConvierteContenedorEnPzs(?, ?, ?, ?, ?, ?, ?)";
                    $sth = $conn_pdo->prepare($sql_convertir_pzs);
                    $sth->execute([
                        $id_almacen, 
                        $valor_lp_origen, 
...
*/
              $query_rev_pzs = $conn_pdo->prepare("SELECT * FROM ts_existenciatarima WHERE idy_ubica= ? AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = ?)");
              $query_rev_pzs->execute([$data["idiorigen"], $data["lp_val"]]);

              // 1. Recoge TODOS los resultados en un array PRIMERO.
              // Después de esta línea, la conexión queda libre.
              $resultados_pzs = $query_rev_pzs->fetchAll(PDO::FETCH_ASSOC);

              // 2. AHORA, itera sobre el array que ya tienes en memoria.
              foreach($resultados_pzs as $row_pzs)
              {
                  $cve_articulo_pzs = $row_pzs['cve_articulo'];
                  $lote_pzs = $row_pzs['lote'];

                  // Esta llamada al procedimiento almacenado ahora funcionará sin problemas.
                  $sql_convertir_pzs = "CALL SPWS_ConvierteContenedorEnPzs(?, ?, ?, ?, ?, ?, ?)";
                  $sth = $conn_pdo->prepare($sql_convertir_pzs);
                  $sth->execute([
                      $id_almacen, 
...
/*
              $query_new_lp = mysqli_query($conn, "SELECT CveLP FROM c_charolas WHERE IDContenedor = '{$pallet_palletizar}' ");
...
*/
             ...
             if($tipo_pallet_contenedor_articulo == 'Caja')
             {
                ...
                if($cajas_iguales == 1)
                {
                  ...
                  $sql_piezas=" INSERT INTO ts_existenciapiezas (cve_almac, idy_ubica, cve_articulo, cve_lote, Existencia, ID_Proveedor, Cuarentena) 
                  VALUES ('{$almacen}', '".$data["ididestino"]."', '".$data["cve_articulo"]."', '".$data["cve_lote"]."', '".$cantidad."', '".$ID_Proveedor."', $QA) 
                  ON DUPLICATE KEY UPDATE existencia=existencia+".$cantidad.";";
                  @$sth_piezas = $conn_pdo->prepare( $sql_piezas );
                  @$sth_piezas->execute();$sth_piezas->closeCursor();
                }
             }
             ...
             if($fusionar_cajas)
             {
                ...
                $sth = $conn_pdo->prepare( $sql_cajas );
                $sth->execute();$sth->closeCursor();
                //while(!$sth->execute())
                //{
                //  sleep(1);
...
//****************************************************************************************************************************
                ...
                //$query_existe_tarima = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' and ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$valor_lp}' LIMIT 1);");
...
/*
                if($existe_tarima)
                {
                    ...
                    $sth = $conn_pdo->prepare( $sql3 );
                    $sth->execute();
                }
*/
                //$query_cajas = mysqli_query($conn, "SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$valor_lp}'");
                //$id_caja = mysqli_fetch_assoc($query_cajas)['IDContenedor'];

                //$query_rev_pzs = mysqli_query($conn, "SELECT *, (SELECT IDContenedor FROM c_charolas WHERE CveLP = '$fusionar_cajas') as id_tarima  FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");
                $query_rev_pzs = $conn_pdo->prepare("SELECT *, (SELECT IDContenedor FROM c_charolas WHERE CveLP = '$fusionar_cajas') as id_tarima  FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");$query_rev_pzs->execute();
             ...
             if($pallets_por_piezas == 1)
             {
                ...
                //{
                  //$query3 = mysqli_query($conn, "SELECT cve_almacenp AS id FROM c_almacen WHERE cve_almac = (SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = '$idiorigen')");
                  $query3 = prepare("SELECT cve_almacenp AS id FROM c_almacen WHERE cve_almac = (SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = '$idiorigen')");$query3->execute();
                  //$almacen_orig = mysqli_fetch_assoc($query3)['id'];
                  $almacen_orig = $query3->fetch(\PDO::FETCH_ASSOC)['id'];$query3->closeCursor();
                  ...
                  if($data['lp_dest'] != '')
                  {
                    ...
                    $row_exist = $rs->fetch(\PDO::FETCH_ASSOC);
                    $rs->closeCursor();
             ...
             /*
                ...
                $query_traz = $conn_pdo->prepare("UPDATE t_trazabilidad_existencias SET idy_ubica='".$data["ididestino"]."', cve_almac = '{$almacen}' WHERE idy_ubica = '".$data["idiorigen"]."' AND cve_articulo = '".$data["cve_articulo"]."' AND cve_lote = '".$data["cve_lote"]."' AND id_proveedor = '$ID_Proveedor' AND id_tipo_movimiento = 2 AND IFNULL(factura_ent, '') = '$factura_ent' AND IFNULL(proyecto, '') = '$proyecto' AND IFNULL(factura_oc, '') = '$factura_oc' AND IFNULL(ntarima, '') = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$valor_lp}' LIMIT 1)");
                $query_traz->execute();
                }
                */
                if($tipo_pallet_contenedor_articulo != 'Caja')
                {
                $query_traz = $conn_pdo->prepare("UPDATE t_trazabilidad_existencias SET idy_ubica='".$data["ididestino"]."', cve_almac = '{$almacen}' WHERE idy_ubica = '".$data["idiorigen"]."' AND id_tipo_movimiento = 2 AND IFNULL(factura_ent, '') = '$factura_ent' AND IFNULL(ntarima, '') = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '{$valor_lp}' LIMIT 1)");
                $query_traz->execute();$query_traz->closeCursor();
                }
        ...
        if($fusionar /*&& ($nivel_ubicacion+0) > 0*/)
        {
          ...
          if($fusionar_cajas)
          {
            ...
            $sql_tr = "INSERT IGNORE INTO t_MovCharolas(Cve_Almac, id_kardex, ID_Contenedor, Fecha, Origen, Destino, Id_TipoMovimiento, Cve_Usuario, Status)
            ...
            $sql = "INSERT IGNORE INTO t_cardex (cve_articulo, cve_lote, fecha, origen, destino, cantidad, id_TipoMovimiento, cve_usuario, Cve_Almac, Activo, stockinicial, ajuste) VALUES ('{$cve_articulo_kdx}', '{$lote_kdx}', NOW(), '{$idiorigen}', '{$ididestino}','{$cantidad}', '{$movimiento}', '{$cve_usuario}', '{$almacen}', 1, '{$cantidad}', 0);";

            $sth = $conn_pdo->prepare( $sql );
            $sth->execute();$sth->closeCursor();

            $sql_tr = "INSERT IGNORE INTO t_MovCharolas(Cve_Almac, id_kardex, ID_Contenedor, Fecha, Origen, Destino, Id_TipoMovimiento, Cve_Usuario, Status, EsCaja)
                    VALUES('{$clave_almacen}', (SELECT MAX(id) FROM t_cardex), {$id_caja}, NOW(), '{$idiorigen}', '{$ididestino}', '{$movimiento}', '{$cve_usuario}', 'I', 'S')";
            //$rs = mysqli_query($conn, $sql_tr);
              $rs = $conn_pdo->prepare($sql_tr);$rs->execute();$rs->closeCursor();
        ...
        if(!$fusionar_cajas)
        {
            ...
            $rs_kardex = $conn_pdo->prepare($sql_kardex);$rs_kardex->execute();
            //echo $sql_kardex;exit;
            //while($row_kardex = mysqli_fetch_array($rs_kardex, MYSQLI_ASSOC))
            while($row_kardex = $rs_kardex->fetch(\PDO::FETCH_ASSOC))
            {
...
*/
        ...
        if($QA == 1)
        {
              ...
              //$rs = mysqli_query($conn, $sql_qa);
                $rs = $conn_pdo->prepare($sql_qa);$rs->execute();
...
else //if($trasladar_ubicacion == 1)
{
        ...
        if($palletizar_traslado && !$traslado_almacen)
        {
          ...
          while($result = $rs->fetch(\PDO::FETCH_ASSOC))
          {
            ...
            $rs1 = $conn_pdo->prepare($sql);$rs1->execute();$rs1->closeCursor();
            ...
            $rs2 = $conn_pdo->prepare($sql);$rs2->execute();$rs2->closeCursor();
      ...
      if($traslado_almacen && $valor_lp)
      {
        //usleep(250000);
        $ub_destino = $data["ididestino"];
        //$query_destino = mysqli_query($conn, "SELECT DISTINCT cve_almac FROM V_ExistenciaGralProduccion WHERE cve_ubicacion = '$ub_destino' AND tipo = 'ubicacion' LIMIT 1");
        $query_destino = $conn_pdo->prepare("SELECT DISTINCT cve_almac FROM V_ExistenciaGralProduccion WHERE cve_ubicacion = ? AND tipo = 'ubicacion' LIMIT 1");
        $query_destino->execute([$ub_destino]);
        //$row_destino = mysqli_fetch_assoc($query_destino);
        $row_destino = $query_destino->fetch(\PDO::FETCH_ASSOC);
        $query_destino->closeCursor();
        $alm_destino = $row_destino['cve_almac'];

        //$query_destino = mysqli_query($conn, "UPDATE c_charolas SET cve_almac = $alm_destino WHERE clave_contenedor = '{$valor_lp}'");
        $query_destino = $conn_pdo->prepare("UPDATE c_charolas SET cve_almac = $alm_destino WHERE clave_contenedor = '{$valor_lp}'");$query_destino->execute();$query_destino->closeCursor();
      }

      //*******************************************************************************
      //                          EJECUTAR EN INFINITY
      //*******************************************************************************
      //$query = mysqli_query($conn, "SELECT COUNT(*) as existe FROM t_configuraciongeneral WHERE cve_conf = 'ejecutar_infinity' AND Valor = '1'");
      $query = $conn_pdo->prepare("SELECT COUNT(*) as existe FROM t_configuraciongeneral WHERE cve_conf = 'ejecutar_infinity' AND Valor = '1'");$query->execute();
      //$ejecutar_infinity = mysqli_fetch_assoc($query)['existe'];
      $ejecutar_infinity = $query->fetch(\PDO::FETCH_ASSOC)['existe'];
      $query->closeCursor();

      if($ejecutar_infinity && $traslado_almacen)
      {
        //$query = mysqli_query($conn, "SELECT Url, Servicio, User, Pswd, IFNULL(Puerto, '8080') as Puerto, Empresa, to_base64(CONCAT(USER,':',Pswd)) Codificado, CONCAT(Url,':',Puerto,'/',Servicio) AS url_curl FROM c_datos_ws WHERE Servicio = 'wms_trasp' AND Activo = 1");
        $query = $conn_pdo->prepare("SELECT Url, Servicio, User, Pswd, IFNULL(Puerto, '8080') as Puerto, Empresa, to_base64(CONCAT(USER,':',Pswd)) Codificado, CONCAT(Url,':',Puerto,'/',Servicio) AS url_curl FROM c_datos_ws WHERE Servicio = 'wms_trasp' AND Activo = 1");$query->execute();
        //$row_infinity = mysqli_fetch_assoc($query);
        $row_infinity = $query->fetch(\PDO::FETCH_ASSOC);
        $query->closeCursor();
        $Url_inf = $row_infinity['Url'];
        $url_curl = $row_infinity['url_curl'];
        $Servicio_inf = $row_infinity['Servicio'];
        ...
        //                                              DATOS ORIGEN 
        //**************************************************************************************************************
        //$query_origen = mysqli_query($conn, "SELECT DISTINCT cve_almac FROM V_ExistenciaGralProduccion WHERE cve_ubicacion = '$ub_origen' AND tipo = 'ubicacion' LIMIT 1");
        $query_origen = $conn_pdo->prepare("SELECT DISTINCT cve_almac FROM V_ExistenciaGralProduccion WHERE cve_ubicacion = '$ub_origen' AND tipo = 'ubicacion' LIMIT 1");$query_origen->execute();
        //$row_origen = mysqli_fetch_assoc($query_origen);
        $row_origen = $query_origen->fetch(\PDO::FETCH_ASSOC);
        $alm_origen = $row_origen['cve_almac'];
        $query_origen->closeCursor();

        //$query_destino = mysqli_query($conn, "SELECT DISTINCT cve_almac FROM V_ExistenciaGralProduccion WHERE cve_ubicacion = '$ub_destino' AND tipo = 'ubicacion' LIMIT 1");
        $query_destino = $conn_pdo->prepare("SELECT DISTINCT cve_almac FROM V_ExistenciaGralProduccion WHERE cve_ubicacion = '$ub_destino' AND tipo = 'ubicacion' LIMIT 1");$query_destino->execute();
        //$row_destino = mysqli_fetch_assoc($query_destino);
        $row_destino = $query_destino->fetch(\PDO::FETCH_ASSOC);
        $alm_destino = $row_destino['cve_almac'];
        $query_destino->closeCursor();
      ...
      //$query_ori = mysqli_query($conn, $sql);
      $query_ori = $conn_pdo->prepare($sql);$query_ori->execute();
      $json = '[';
      
      //$num_rows = mysqli_num_rows($query_ori);
      $num_rows = $query_ori->rowCount();

      //while($row_ori = mysqli_fetch_assoc($query_ori))
      while($row_ori = $query_ori->fetch(\PDO::FETCH_ASSOC))
      {
        extract($row_ori);
         $json .= '{"item":"'.$cve_articulo.'", "batch": "'.$cve_lote.'","qty":'.$Cantidad.',"um":"'.$cve_umed.'","typeMov":"T","warehouse":"'.$clave.'","dataOpe":"'.$HoraTraslado.'"},';//NOW()
      }
      $query_ori->closeCursor();
      if($num_rows == 0)
      {
        //$query_cero = mysqli_query($conn, "SELECT NOW() as hora_traslado, u.cve_umed
        //                                    FROM c_articulo a 
        //                                    LEFT JOIN c_unimed u ON u.id_umed = a.unidadMedida
        //                                    WHERE a.cve_articulo = '{$cve_articulo}'
        //                                    LIMIT 1");
        $query_cero = $conn_pdo->prepare("SELECT NOW() as hora_traslado, u.cve_umed
                                            FROM c_articulo a 
                                            LEFT JOIN c_unimed u ON u.id_umed = a.unidadMedida
                                            WHERE a.cve_articulo = '{$cve_articulo}'
                                            LIMIT 1");$query_cero->execute();
        //$row_cero = mysqli_fetch_assoc($query_cero);
        $row_cero = $query_cero->fetch(\PDO::FETCH_ASSOC);
        $hora_traslado = $row_cero['hora_traslado'];
        $cve_umed = $row_cero['cve_umed'];
        $query_cero->closeCursor();

        //$query_cero = mysqli_query($conn, "SELECT clave FROM c_almacenp WHERE id = '$alm_origen' LIMIT 1");
        $query_cero = $conn_pdo->prepare("SELECT clave FROM c_almacenp WHERE id = '$alm_origen' LIMIT 1");$query_cero->execute();
        //$row_cero = mysqli_fetch_assoc($query_cero);
        $row_cero = $query_cero->fetch(\PDO::FETCH_ASSOC);
...
*/
   ...
   // curl_close($curl);
          ...
          curl_close($curl);      
          //echo $response;
          //$query = mysqli_query($conn, "INSERT INTO t_log_ws(Fecha, Referencia, Mensaje, Respuesta, Proceso, Dispositivo) VALUES (NOW(), '$Servicio_inf', '$json', '$response', 'Traspaso entre Almacenes', 'WEB')");
          $query = $conn_pdo->prepare("INSERT INTO t_log_ws(Fecha, Referencia, Mensaje, Respuesta, Proceso, Dispositivo) VALUES (NOW(), '$Servicio_inf', '$json', '$response', 'Traspaso entre Almacenes', 'WEB')");$query->execute();$query->closeCursor();
      ...
      //$query_dest = mysqli_query($conn, $sql);
        $query_dest = $conn_pdo->prepare($sql);$query_dest->execute();
      $json = '[';
      
      //while($row_dest = mysqli_fetch_assoc($query_dest))
      while($row_dest = $query_dest->fetch(\PDO::FETCH_ASSOC))
      {
        extract($row_dest);
        $json .= '{"item":"'.$cve_articulo.'", "batch": "'.$cve_lote.'","qty":'.$Cantidad.',"um":"'.$cve_umed.'","typeMov":"T","warehouse":"'.$clave.'","dataOpe":"'.$HoraTraslado.'"},';//NOW()
      }
      $query_dest->closeCursor();
      $json[strlen($json)-1] = ' ';
      $json .= ']';
    ...
    function XmoverTrasladoVersion01X($data) {
        extract($data);
...
//*************************************************************************************************************************************************
        ...
        //******************************************************************************************************************************************
          $query_traz = $conn_pdo->prepare("
            ...
            WHERE t.cve_almac = '{$almacen}' AND t.idy_ubica = '".$data["idiorigen"]."' AND CONVERT(t.cve_articulo, CHAR) = '".$data["cve_articulo"]."' AND CONVERT(t.cve_lote, CHAR) = '".$data["cve_lote"]."' AND t.id_proveedor = '$ID_Proveedor' AND t.id_tipo_movimiento = 2 AND CONVERT(IFNULL(e.Fact_Prov, ''), CHAR) = CONVERT(IFNULL(t.factura_ent, ''), CHAR) AND CONVERT(IFNULL(e.Proyecto, ''), CHAR) = CONVERT(IFNULL(t.proyecto, ''), CHAR) AND CONVERT(IFNULL(oc.Factura, ''), CHAR) = CONVERT(IFNULL(t.factura_oc, ''), CHAR) AND IFNULL(t.ntarima, '') = ''");
          $query_traz->execute();
          $mayor_a_cero = $query_traz->fetchAll()['mayor_a_cero'];
          ...
          WHERE t.cve_almac = '{$almacen}' AND t.idy_ubica = '".$data["idiorigen"]."' AND CONVERT(t.cve_articulo, CHAR) = '".$data["cve_articulo"]."' AND CONVERT(t.cve_lote, CHAR) = '".$data["cve_lote"]."' AND t.id_proveedor = '$ID_Proveedor' AND t.id_tipo_movimiento = 2 AND CONVERT(IFNULL(e.Fact_Prov, ''), CHAR) = CONVERT(IFNULL(t.factura_ent, ''), CHAR) AND CONVERT(IFNULL(e.Proyecto, ''), CHAR) = CONVERT(IFNULL(t.proyecto, ''), CHAR) AND CONVERT(IFNULL(oc.Factura, ''), CHAR) = CONVERT(IFNULL(t.factura_oc, ''), CHAR) AND IFNULL(t.ntarima, '') = ''");
          $sql_traz->execute();
          $valores_traz = $sql_traz->fetchAll();
...
//, (SELECT IFNULL(Cuarentena, 0) FROM ts_existenciapiezas where cve_articulo='".$data["cve_articulo"]."' and cve_lote='".$data["cve_lote"]."' and idy_ubica='".$data["idiorigen"]."')
      ...
      else if(/*!$traslado_almacen &&*/ !$palletizar_traslado)
      {
        ...
        if($valor_pallet || (($tipo_pallet_contenedor_articulo == 'Contenedor' || $tipo_pallet_contenedor_articulo == 'Pallet' || $tipo_pallet_contenedor_articulo == 'Caja') && !$valor_pallet))
        {
            //$query_rev = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciatarima WHERE cve_articulo='".$data["cve_articulo"]."' and lote='".$data["cve_lote"]."' and idy_ubica='".$data["idiorigen"]."'");
            $query_rev = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciatarima WHERE idy_ubica='".$data["idiorigen"]."' AND ntarima = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");
            if($tipo_pallet_contenedor_articulo == 'Caja')
              $query_rev = mysqli_query($conn, "SELECT COUNT(*) as existe FROM ts_existenciacajas WHERE idy_ubica='".$data["idiorigen"]."' AND Id_Caja = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '".$data["lp_val"]."')");
            $existe_tarima = mysqli_fetch_assoc($query_rev)['existe'];
