...
class Ubicaciones {
    ...
    private function load() {
        $sql = sprintf('SELECT * FROM %s WHERE idy_ubica = ?',self::TABLE);
    ...
    function getUbicacionesNoVaciasxZona($almacen, $excludeInventario = 0) 
    {
      ...
      $sql = "
        ...
        WHERE (SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = c.idy_ubica) IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = '{$almacen}')
          ...
          AND ((c.idy_ubica IN (SELECT idy_ubica FROM ts_existenciapiezas))
              ...
              OR  (c.idy_ubica IN (SELECT idy_ubica FROM ts_existenciacajas)))
        ";
...
WHERE c.cve_almac = '{$zona}' 
  AND c.Activo = 1 AND c.Tipo = 'L' AND c.Tipo <> 'R' AND c.Tipo <> 'Q'
  AND c.idy_ubica NOT IN (SELECT cve_ubicacion FROM V_ExistenciaGral WHERE V_ExistenciaGral.tipo='area')
  ORDER BY ubicacion;
";
      ...
      //PR tabla traslado
        $sql = " 
          SELECT  DISTINCT
            ...
            IF(ch.clave_contenedor = '', 'Artículo', ch.tipo) AS tipo,
            ch.clave_contenedor AS pallet,
            IF(v.Cve_Contenedor != '', ch.CveLP, '') AS LP,
            ...
            IFNULL(CAST(((a.alto / 1000) * (a.ancho / 1000) * (a.fondo / 1000) * v.Existencia) AS DECIMAL(10,6)), 0) AS volumen_ocupado,
          ...
          FROM (
              SELECT cve_ubicacion, cve_articulo, cve_lote, Existencia, Cve_Contenedor FROM V_ExistenciaGral WHERE cve_ubicacion = '{$idy_ubica}'
              UNION SELECT cve_ubicacion, cve_articulo, cve_lote, Existencia, Cve_Contenedor FROM V_ExistenciaProduccion WHERE cve_ubicacion = '{$idy_ubica}'
          ) AS v
            ...
            select * from(
                select * from (
                    select
                        ...
                        IFNULL(CAST(((c_articulo.alto / 1000) * (c_articulo.ancho / 1000) * (c_articulo.fondo / 1000) * V_EntradasContenedores.CantidadRecibida) AS DECIMAL(10,6)), 0) as volumen_ocupado,
                        IFNULL(V_EntradasContenedores.CantidadRecibida, 0) as num_existencia,
                        ifnull(c_articulo.des_articulo,c_charolas.descripcion) as descripcion,
                        ifnull(V_EntradasContenedores.Clave_Contenedor,V_EntradasContenedores.Cve_Articulo) as clave,
                        IFNULL(CAST((c_articulo.peso * V_EntradasContenedores.CantidadRecibida) as DECIMAL(10,2)), 0) as peso_total,
                        c_articulo.id as id_articulo,
                ...
                select
                    ...
                    sum(IFNULL(V_EntradasContenedores.CantidadRecibida, 0)) as num_existencia,
                    if(V_EntradasContenedores.Clave_Contenedor = null, c_articulo.des_articulo,c_charolas.descripcion) as descripcion,
                    ifnull(V_EntradasContenedores.Clave_Contenedor,V_EntradasContenedores.Cve_Articulo) as clave,
                ...
                where 0 = 0 
                    ...
                    and V_EntradasContenedores.CantidadRecibida != V_EntradasContenedores.CantidadUbicada
                GROUP by V_EntradasContenedores.Clave_Contenedor
            )as x
            order by x.folio desc
        ";
        */
/*
        //IFNULL(c_articulo.des_articulo,c_charolas.descripcion) AS descripcion,
        $sql = "
                ...
                SELECT * FROM (
                    SELECT
                        ...
                        IF(c_articulo.control_lotes = 'S', c_lotes.Caducidad, '') AS caducidad,
                        IF(c_articulo.control_numero_series = 'S', V_EntradasContenedores.Cve_lote,'') AS numero_serie,
                        IFNULL(CAST(((c_articulo.alto / 1000) * (c_articulo.ancho / 1000) * (c_articulo.fondo / 1000) * V_EntradasContenedores.Cantidad_C) AS DECIMAL(10,6)), 0) AS volumen_ocupado,
                        ...
                        IFNULL(CAST((c_articulo.peso * V_EntradasContenedores.Cantidad_C) AS DECIMAL(10,2)), 0) AS peso_total,
                        c_articulo.id AS id_articulo,
                        c_proveedores.Nombre AS proveedor,
                        V_EntradasContenedores.Cve_Proveedor AS  id_proveedor
                    FROM V_EntradasContenedores
                        LEFT JOIN c_articulo ON c_articulo.cve_articulo = V_EntradasContenedores.Cve_articulo
                        LEFT JOIN t_pendienteacomodo ON t_pendienteacomodo.cve_articulo = V_EntradasContenedores.Cve_articulo AND t_pendienteacomodo.cve_lote = V_EntradasContenedores.Cve_Lote
                        LEFT JOIN c_lotes ON c_lotes.LOTE = V_EntradasContenedores.Cve_Lote AND c_lotes.cve_articulo = V_EntradasContenedores.Cve_articulo 
                        LEFT JOIN c_proveedores ON c_proveedores.ID_Proveedor = V_EntradasContenedores.Cve_Proveedor
                        LEFT JOIN c_charolas ON c_charolas.clave_contenedor = V_EntradasContenedores.Clave_Contenedor
                        LEFT JOIN td_entalmacen tde ON tde.fol_folio = V_EntradasContenedores.Fol_Folio
                    ...
                    WHERE 0 = 0 
                        ...
                        AND t_pendienteacomodo.Cantidad > 0
                )Y
                WHERE Y.tipo = 'Articulo'
              ...
              AND V_EntradasContenedores.Clave_Contenedor != ''
              AND V_EntradasContenedores.Cantidad_C != V_EntradasContenedores.CantidadUbicada
              GROUP BY V_EntradasContenedores.Fol_Folio,V_EntradasContenedores.Cve_Articulo,a.des_articulo,V_EntradasContenedores.Cantidad_C,V_EntradasContenedores.Clave_Contenedor,c_proveedores.Nombre,V_EntradasContenedores.Cve_Proveedor
            )AS X
            ORDER BY X.folio DESC
            ";
//       echo var_dump($sql);
//       die();
*/      
//IFNULL(IF(l.CADUCIDAD = '0000-00-00', '', DATE_FORMAT(l.CADUCIDAD, '%d-%m-%Y')), '')
/*
      $sql = "
          SELECT DISTINCT
          V_EntradasContenedores.Fol_Folio AS folio,
          ...
          IFNULL(IF(c_articulo.control_lotes = 'S', c_lotes.LOTE,''), '') AS lote,
          IF(c_articulo.control_lotes = 'S', IFNULL(IF(c_lotes.Caducidad = '0000-00-00', '', DATE_FORMAT(c_lotes.Caducidad, '%d-%m-%Y')), ''), '') AS caducidad,
          IF(c_articulo.control_numero_series = 'S', V_EntradasContenedores.Cve_lote,'') AS numero_serie,
          IFNULL(CAST(((c_articulo.alto / 1000) * (c_articulo.ancho / 1000) * (c_articulo.fondo / 1000)) AS DECIMAL(10,6)), 0) AS volumen_ocupado,
          tde.cve_articulo AS clave,
          a.des_articulo AS descripcion,
          #IFNULL(t_pendienteacomodo.Cantidad, 0) AS num_existencia,
          IFNULL(tde.CantidadDisponible, 0) AS num_existencia,
...
SELECT DISTINCT
          ...
          a.des_articulo AS descripcion,
          a.control_peso,
          #IFNULL(tde.CantidadRecibida, 0) AS num_existencia,
          IFNULL(t_pendienteacomodo.Cantidad, 0) AS num_existencia,
          /*c_articulo.des_articulo AS descripcion,*/
          /*IFNULL(V_EntradasContenedores.Clave_Contenedor,V_EntradasContenedores.Cve_Articulo) AS pallet_contenedor,*/
          V_EntradasContenedores.Clave_Contenedor AS pallet_contenedor,
          ...
          AND IFNULL(V_EntradasContenedores.Clave_Contenedor, '') = ''
              ...
              IF(a.control_lotes = 'S', IFNULL(IF(c_lotes.Caducidad = '0000-00-00', '', DATE_FORMAT(c_lotes.Caducidad, '%d-%m-%Y')), ''), '') AS caducidad,
...
/*
        ...
        {
/*
                "SELECT DISTINCT t.Cantidad as maximo FROM V_EntradasContenedores v, t_pendienteacomodo t
                 WHERE t.cve_articulo = v.Cve_articulo 
                 AND t.cve_lote = v.Cve_Lote 
                 AND t.cve_ubicacion = '".$cve_almaci."' 
                 AND t.ID_Proveedor = v.Cve_Proveedor 
                 AND v.Fol_Folio = '".$idiorigen."' 
                 AND t.cve_articulo = '".$cve_articulo."' LIMIT 1" 


                "SELECT 
                    IFNULL((CantidadRecibida - CantidadUbicada), CantidadRecibida) as maximo 
                 FROM V_EntradasContenedores 
...
/*
        ...
        else 
        {
          $query = \db()->prepare
            (
              "SELECT
                  IF(Clave_Contenedor != '', 1, 0) as maximo
               FROM V_EntradasContenedores
                  WHERE V_EntradasContenedores.Fol_Folio ='".$idiorigen."';"
            );

            $query->execute();
            $maximo = $query->fetch()['maximo'];
        }
*/
        ...
        if($tipo != '3')
        {
          ...
          $sql .= 
            "INSERT INTO `t_cardex` (`cve_articulo`, `cve_lote`, `fecha`, `origen`, `destino`, `cantidad`, `id_TipoMovimiento`, `cve_usuario`, `Cve_Almac`, `Activo`) 
             VALUES ('{$cve_articulo}', '{$cve_lote}', NOW(), '{$folio}', '{$ididestino}','{$cantidadTotal}', '{$movimiento}', '{$cve_usuario}', '{$almacen}', 1);";
...
//*****************************
          ...
          $query = \db()->prepare
           (
           "SELECT IDContenedor FROM c_charolas WHERE clave_contenedor = '{$pallet_contenedor}'"
           );
          $query->execute();
          ...
          $sql2 = \db()->prepare
          (
            "SELECT * FROM td_entalmacenxtarima WHERE ClaveEtiqueta = '{$pallet_contenedor}';"
          );
           $sql2->execute();
           $articulos = $sql2-> fetchAll();
          ...
          foreach ($articulos as $articulo)
          {
             ...
             $sql3 =\db()->prepare(
              "UPDATE t_pendienteacomodo 
                    SET Cantidad = Cantidad - {$cantidadTotal} 
                  WHERE cve_articulo = '$cve_articulo'
                    AND cve_ubicacion = '$idy_ubica'
                    AND cve_lote = '$lote';");
              //echo var_dump($sql3);
              $sql3->execute();

             $sql4 =\db()->prepare(
              "INSERT IGNORE INTO t_MovCharolas (Cve_Almac, ID_Contenedor, Fecha, Origen, Destino, Id_TipoMovimiento, Cve_Usuario, Status) 
            VALUES ('$cve_almac', '$cve_articulo', NOW(), '$cve_almaci', '{$ididestino}', 2, '{$cve_usuario}', 'I');");
              //echo var_dump($sql3);
              $sql4->execute();

             $sql5 =\db()->prepare(
              "INSERT INTO `t_cardex` (`cve_articulo`, `cve_lote`, `fecha`, `origen`, `destino`, `cantidad`, `id_TipoMovimiento`, `cve_usuario`, `Cve_Almac`, `Activo`) 
             VALUES ('{$cve_articulo}', '{$cve_lote}', NOW(), '$Fol_Folio', '{$ididestino}','{$cantidadTotal}', '{$movimiento}', '{$cve_usuario}', '{$almacen}', 1);");
              //echo var_dump($sql3);
              $sql5->execute();
          }

           $sql6 =\db()->prepare(
            "UPDATE td_entalmacenxtarima SET Ubicada='S' WHERE ClaveEtiqueta='{$pallet_contenedor}';");
            //echo var_dump($sql3);
            $sql6->execute();
...
/*
          $sql2_result = "SELECT
              ...
              FROM V_EntradasContenedores
                left join c_charolas on c_charolas.clave_contenedor = V_EntradasContenedores.Clave_Contenedor
              WHERE V_EntradasContenedores.Clave_Contenedor = '{$pallet_contenedor}'
                ;";
          ...
          foreach ($articulos as $articulo)
          {
            ...
            $sql3_result .= "UPDATE td_entalmacen 
                 SET CantidadUbicada = '$existencia', CantidadDisponible = CantidadRecibida - IFNULL(CantidadUbicada, 0) 
               WHERE fol_folio = $idiorigen
                 and cve_articulo = '$cve_arti'
                 and cve_lote = '$lote';";
            ...
            $sql .= 
              "UPDATE t_pendienteacomodo 
                    SET Cantidad = Cantidad - {$cantidadTotal} 
                  WHERE cve_articulo = '{$cve_arti}'
                    AND cve_ubicacion = '{$cve_almaci}'
                    AND cve_lote = '{$lote}';";

          }

          $sql .="UPDATE td_entalmacenxtarima SET Ubicada='S' WHERE ClaveEtiqueta='{$pallet_contenedor}';";
          $sql .="
            INSERT INTO t_MovCharolas (Cve_Almac, ID_Contenedor, Fecha, Origen, Destino, Id_TipoMovimiento, Cve_Usuario, Status) 
            VALUES ('{$alma}', '{$cve_articulo}', NOW(), '$cve_almaci', '{$ididestino}', 2, '{$cve_usuario}', 'I');";
          $sql .= 
...
*/
      ...
      //echo "query_result = ".$query_result." --- sql2_result = ".$sql2_result." --- sql1_result = ".$sql1_result." --- sql3_result = ".$sql3_result." ----- sql = ".$sql;
    //echo $sql;
    $query = \db()->prepare
     (
...
//        $sql .= "UPDATE c_charolas SET CveLP = '$valor_lp' WHERE clave_contenedor = '$valor_origen'";
    ...
    function moverTraslado($data) {
      ...
      $sql = "";
      if($tipo_pallet_contenedor_articulo == 'Artículo' && !$traslado_almacen && !$valor_lp)
      {
