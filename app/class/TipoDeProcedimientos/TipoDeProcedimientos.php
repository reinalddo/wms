...
class TipoDeProcedimientos {
    ...
    private function load() {
        try {
            $sql = 'SELECT * FROM c_procedimientos Where c_procedimientos.id = '.$this->id;
    ...
    function save($_post) {
        try {
          ...
          $sql = "INSERT INTO ".self::TABLE."
                  SET
                  nombreDeProcedimiento= '${nombreDeProcedimiento}'";
    ...
    function actualizarProcedimientos($_post) 
    {
      try
      {

        $to_update_c_procedimientos = array(
          "nombreDeProcedimiento"     => $_post['nombreDeProcedimiento'],
        ...
        $insert_orden = $this->tools->dbUpdate(self::TABLE,$to_update_c_procedimientos,$where_update);
      } 
      catch(Exception $e) 
      {
