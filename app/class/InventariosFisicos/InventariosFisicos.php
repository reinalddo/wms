...
//      $conteo_maximo = 0;
      ...
      //{
        $sql .=
            "SELECT 
              CONVERT(IFNULL(cve_articulo, ''), CHAR) as cve_articulo, 
              CONVERT(cve_ubicacion, CHAR) as cve_ubicacion, 
              CONVERT(cve_lote, CHAR) as cve_lote, 
              CONVERT(Existencia, CHAR) as cantidad,
              CONVERT(IFNULL(Cve_Contenedor, ''), CHAR) as Cve_Contenedor, 
              CONVERT(IFNULL(Cuarentena, '0'), CHAR) as Cuarentena, 
              CONVERT(IFNULL(ID_Proveedor, 0), CHAR) as ID_Proveedor
            FROM V_ExistenciaGralProduccion 
            WHERE 1 #Existencia > 0 
            AND CONVERT(cve_ubicacion, CHAR) IN (SELECT CONVERT(COALESCE(cve_ubicacion, idy_ubica), CHAR) FROM t_ubicacioninventario WHERE CONVERT(t_ubicacioninventario.idy_ubica, CHAR) = '$ubicacion')

        UNION 

        SELECT DISTINCT
          CONVERT(IFNULL(ve.cve_articulo, ''), CHAR) AS cve_articulo,
          CONVERT(u.idy_ubica, CHAR) AS cve_ubicacion,
          CONVERT(IFNULL(ve.cve_lote, ''), CHAR) AS cve_lote,
          CONVERT(IFNULL(ve.Existencia, 0), CHAR) AS cantidad,
          CONVERT(IFNULL(ve.Cve_Contenedor, ''), CHAR) as Cve_Contenedor,
          CONVERT(IFNULL(ve.Cuarentena, '0'), CHAR) as Cuarentena, 
          CONVERT(IFNULL(ve.ID_Proveedor, 0), CHAR) as ID_Proveedor
        ...
        , c_almacen a
        WHERE 1 
            AND CONVERT(u.cve_almac, CHAR)=CONVERT(a.cve_almac, CHAR)
            AND CONVERT(a.cve_almacenp, CHAR)=CONVERT(p.id, CHAR)
            AND CONVERT(u.cve_almac, CHAR) = CONVERT(a.cve_almac, CHAR)
            AND CONVERT(u.Activo, CHAR) = '1' 
            AND CONVERT(u.idy_ubica, CHAR) = '$ubicacion'
            ";//WHERE ID_Inventario = {$idSQL}
      //}
      $result["sql1"] = $sql;
      $sth = \db()->prepare($sql);
    ...
    function saveExistenciaFiltros($max_id_inv, $and)
    {
            ...
            "
              ...
              (
                  SELECT 
                        $max_id_inv, 
                        0,
                        IFNULL(cve_articulo, '') AS cve_articulo, 
                        cve_lote, 
                        cve_ubicacion, 
                        0, 
                        Existencia,
                        ...
                        IFNULL(ID_Proveedor, 0) as ID_Proveedor
                      FROM V_ExistenciaGral 
                      WHERE 1 
                      AND cve_ubicacion COLLATE utf8mb4_general_ci IN (SELECT COALESCE(cve_ubicacion, idy_ubica) FROM t_ubicacioninventario WHERE t_ubicacioninventario.idy_ubica IN (SELECT u.idy_ubica FROM c_ubicacion u WHERE 1 {$and} AND u.cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = (SELECT id FROM c_almacenp WHERE clave = (SELECT cve_almacen FROM th_inventario WHERE ID_Inventario = $max_id_inv)))))
                      AND IFNULL(Cve_Contenedor, '') = '' AND tipo = 'ubicacion'
                  ...
                  SELECT DISTINCT
                    $max_id_inv,
                    0,
                    IFNULL(ve.cve_articulo, '') AS cve_articulo,
                    IFNULL(ve.cve_lote, '') AS cve_lote,
                    u.idy_ubica as cve_ubicacion,
                    0, 
                    IFNULL(ve.Existencia, 0) as Existencia,
                    '',
                    NOW(), 
                    NOW(),
                    0,
                    1, 
                    IFNULL(ve.Cuarentena, 0) as Cuarentena, 
                    IFNULL(ve.ID_Proveedor, 0) as ID_Proveedor
                  FROM c_almacenp p, c_ubicacion u
                  LEFT JOIN V_ExistenciaGral ve ON ve.cve_ubicacion = u.idy_ubica
                  LEFT JOIN c_charolas ch ON ch.clave_contenedor = ve.Cve_Contenedor
                  , c_almacen a
                  WHERE 1 
                      ...
                      AND u.Activo = '1' 
                      {$and} 
                      AND u.cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = (SELECT id FROM c_almacenp WHERE clave = (SELECT cve_almacen FROM th_inventario WHERE ID_Inventario = $max_id_inv))) 
                      AND IFNULL(ve.Cve_Contenedor, '') = '' AND ve.tipo = 'ubicacion'
              )";
//echo $sql;
        $sth = \db()->prepare($sql);
        $sth->execute();
            ...
            "
              ...
              (
                  SELECT 
                        ...
                        '', 
                        NOW(), 
                        IFNULL(ch.IDContenedor, '') as IDContenedor,
                        'N',
                        1, 
                        IFNULL(Cuarentena, '0') as Cuarentena, 
                        IFNULL(ID_Proveedor, 0) as ID_Proveedor
                      FROM V_ExistenciaGral e
                      LEFT JOIN c_charolas ch ON ch.clave_contenedor = Cve_Contenedor
                      WHERE 1 
                      AND e.cve_ubicacion COLLATE utf8mb4_general_ci IN (SELECT COALESCE(cve_ubicacion, idy_ubica) FROM t_ubicacioninventario WHERE t_ubicacioninventario.idy_ubica IN (SELECT u.idy_ubica FROM c_ubicacion u WHERE 1 {$and} AND u.cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = (SELECT id FROM c_almacenp WHERE clave = (SELECT cve_almacen FROM th_inventario WHERE ID_Inventario = $max_id_inv)))))
                      AND IFNULL(e.Cve_Contenedor, '') != '' AND e.tipo = 'ubicacion'
...
*/
    ...
    function saveUbicacionFiltros( $max_id_inv, $and ) 
    {
      //INSERT INTO t_ubicacioninventario (ID_Inventario,NConteo,cve_ubicacion,status)
      $sql = sprintf("
        INSERT INTO t_ubicacioninventario (ID_Inventario,NConteo,idy_ubica,status) (SELECT $max_id_inv, 1, idy_ubica, 'A' FROM c_ubicacion WHERE 1 {$and} AND cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = (SELECT id FROM c_almacenp WHERE clave = (SELECT cve_almacen FROM th_inventario WHERE ID_Inventario = $max_id_inv))))
      ");
      //VALUES ((select MAX(ID_Inventario) from th_inventario),1,'".$ubicacion."','A')"
      ...
      $sql_ubicaciones_por_inventariar = sprintf("
          INSERT INTO t_ubicacionesainventariar(ID_Inventario, idy_ubica, status) (SELECT $max_id_inv, idy_ubica, 'A' FROM c_ubicacion WHERE 1 {$and} AND cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = (SELECT id FROM c_almacenp WHERE clave = (SELECT cve_almacen FROM th_inventario WHERE ID_Inventario = $max_id_inv))))
        ");
      //VALUES ((select MAX(ID_Inventario) from th_inventario),'{$ubicacion}','A');
      $this->save = \db()->prepare($sql_ubicaciones_por_inventariar);
      $this->save->execute();
...
FROM (
        ...
        SELECT DISTINCT 
            ...
            FROM t_invpiezas inv
                ...
                LEFT JOIN c_lotes ON c_lotes.LOTE = inv.cve_lote AND c_lotes.cve_articulo = c_articulo.cve_articulo
...
$utf8Sql->execute();
        ...
        $pdf = new \TCPDF(L, PDF_UNIT , LETTER, true, 'utf8mb4', false);
        $pdf->SetCreator('wms');
        $pdf->SetAuthor('wms');
       ...
       $pdf->SetFooterMargin(30);
        $filename = "Inventario Teorico #{$id_inventario}.pdf";
        ob_start();
?>
        <br><br>
...
FROM (
        ...
        SELECT DISTINCT 
                ...
                IF(v.cve_lote = '', '', v.cve_lote) COLLATE utf8mb4_general_ci AS lote,
                IF(c_articulo.Caduca = 'S', IFNULL(c_lotes.CADUCIDAD, '--'), '') COLLATE utf8mb4_general_ci AS caducidad,
                IFNULL(inv.NConteo, 0) COLLATE utf8mb4_general_ci AS conteo, 
                GROUP_CONCAT(DISTINCT CONCAT(inv.NConteo,'-',inv.Cantidad) ORDER BY inv.NConteo SEPARATOR ',') COLLATE utf8mb4_general_ci AS NConteo_Cantidad_reg,
                GROUP_CONCAT(DISTINCT inv.NConteo SEPARATOR ',') COLLATE utf8mb4_general_ci AS Nconteo, 
                #IFNULL((SELECT SUM(ext.ExistenciaTeorica) FROM t_invpiezas ext WHERE ext.ID_Inventario = {$id_inventario} AND ext.NConteo = 0 AND ext.cve_articulo = v.cve_articulo AND ext.idy_ubica = v.cve_ubicacion AND ext.cve_lote = v.cve_lote AND v.tipo = 'ubicacion'), 0) AS stockTeorico,
                IFNULL((SELECT SUM(Existencia) FROM V_ExistenciaGral WHERE tipo = 'ubicacion' AND cve_ubicacion = v.cve_ubicacion AND cve_lote = v.cve_lote AND cve_articulo = v.cve_articulo), 0) COLLATE utf8mb4_general_ci AS stockTeorico,
                GROUP_CONCAT(DISTINCT inv.Cantidad ORDER BY inv.NConteo SEPARATOR ',') COLLATE utf8mb4_general_ci AS Cantidad_reg,
                (SELECT DISTINCT SUM(Cantidad) FROM t_invpiezas iv WHERE iv.ID_Inventario = {$id_inventario} AND iv.cve_articulo = v.cve_articulo AND iv.idy_ubica = v.cve_ubicacion  AND iv.NConteo = MAX(inv.NConteo) AND iv.cve_lote = v.cve_lote) COLLATE utf8mb4_general_ci AS Cantidad,
                IF(COUNT(DISTINCT inv.NConteo) = (SELECT COUNT(DISTINCT Cantidad) FROM t_invpiezas iv WHERE iv.ID_Inventario = {$id_inventario} AND iv.cve_articulo = v.cve_articulo AND iv.idy_ubica = v.cve_ubicacion AND iv.cve_lote = v.cve_lote AND iv.NConteo > 0 ), 0, 1) COLLATE utf8mb4_general_ci AS Cerrar,
                '' AS LP,
                ...
                (SELECT Status FROM th_inventario WHERE ID_inventario = {$id_inventario} LIMIT 1) COLLATE utf8mb4_general_ci AS Status
            FROM V_ExistenciaGralProduccion v
                LEFT JOIN c_articulo ON c_articulo.cve_articulo = v.cve_articulo
                #LEFT JOIN c_almacen AS c ON c.cve_almac=(SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = v.cve_ubicacion) 
                LEFT JOIN c_lotes ON c_lotes.LOTE = v.cve_lote
...
UNION
        ...
        SELECT DISTINCT 
            ...
            FROM t_invtarima inv
                ...
                LEFT JOIN c_proveedores p ON p.ID_Proveedor = IFNULL(inv_pr.ID_Proveedor, rap.Id_Proveedor)
            WHERE (inv.ID_Inventario = {$id_inventario} AND inv.cve_articulo = c_articulo.cve_articulo)
            #AND inv.NConteo > 0 
...
UNION
        ...
        SELECT DISTINCT 
            ...
            FROM t_invtarimaciclico inv
                ...
                LEFT JOIN c_charolas ch ON ch.IDContenedor = inv.ntarima
                LEFT JOIN rel_articulo_proveedor rap ON rap.Cve_Articulo = inv.cve_articulo
                LEFT JOIN t_invtarimaciclico inv_pr ON inv_pr.ID_PLAN = inv.ID_PLAN  AND inv_pr.NConteo = 0 AND inv_pr.cve_articulo = inv.cve_articulo AND inv_pr.Cve_Lote = inv.Cve_Lote AND inv_pr.ntarima = inv.ntarima 
                LEFT JOIN c_proveedores p ON p.ID_Proveedor = IFNULL(inv_pr.ID_Proveedor, rap.Id_Proveedor)
            WHERE (inv.ID_PLAN = {$id_inventario} AND inv.cve_articulo = c_articulo.cve_articulo)
            AND inv.existencia >= 0 
            {$sql_rack}
...
*/
        ...
        if($diferencia == 2)
          $titulo = "REPORTE CONSOLIDADO DE UBICACIONES";
              ...
              <td style="width: 850px">
                ...
                <table border="1">
                    ...
                    //if(($diferencia == 1 && $row["Cerrar"] == 1))  continue;
                      ...
                      if($diferencia == 1 && ($imprimir_diferencia == false || utf8_encode($row["clave"]) == "")) continue;
