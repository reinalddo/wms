...
class FormasPago {
    ...
    public function __construct( $cve_gpoart = false, $key = false ) {
        ...
        ',
            ...
            $sth = \db()->prepare($sql);
            $sth->setFetchMode(\PDO::FETCH_CLASS, '\FormasPago\FormasPago');
            $sth->execute(array($key));

            $FormasPago = $sth->fetch();

            $this->cve_gpoart = $FormasPago->cve_gpoart;
    ...
    function actualizarFormasPago( $data ) {
        $sql = '
        UPDATE
          ' . self::TABLE . '
        SET
    ...
    function borrarFormasPago( $data ) {
        $sql = '
        UPDATE
          ' . self::TABLE . '
        SET
