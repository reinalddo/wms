...
class EntradasPDF {
    ...
    public function generarReporteEntradas($folio,$tipo = 0, $codigobarras = 1)
    {
        /*$sql = "
            SELECT 
                th_aduana.ID_Aduana,
    ...
    mysqli_set_charset(\db2() , $charset);
      ...
      $cve_cia = $_SESSION['cve_cia'];
      if(($tipo == "OC" || $tipo == 1) /*&& $codigobarras != 3*/)
      {
					$titulo ="Orden de Compra ".$folio." | OC ERP";
					$datos_artculos ="
							td_aduana.ID_Aduana,
							td_aduana.cve_articulo,
							td_aduana.cantidad,
					...
					$join = "LEFT JOIN th_entalmacen on th_entalmacen.id_ocompra = th_aduana.num_pedimento
							LEFT JOIN c_articulo on td_aduana.cve_articulo = c_articulo.cve_articulo
                            LEFT JOIN c_compania ON c_compania.cve_cia = $cve_cia
                            ";
//IFNULL((SELECT cve_cia FROM c_usuario WHERE id_user = th_aduana.cve_usuario), (SELECT cve_cia FROM c_usuario WHERE id_user = (SELECT id_user FROM c_usuario WHERE cve_usuario = th_aduana.cve_usuario)))3
                        $sql = "
                            SELECT distinct
                                th_aduana.ID_Aduana,
                                th_aduana.fech_pedimento,
                                ...
                                (SELECT CONCAT(IFNULL(pr.Nombre, ''),' ',IFNULL(pr.RUT, ''),' ',IFNULL(pr.direccion, ''), ' ',IFNULL(pr.colonia, ''),' ',IFNULL(pr.cve_dane, ''),' ',IFNULL(pr.ciudad, ''),', ',IFNULL(pr.estado, ''),', ',IFNULL(pr.pais , '')) AS p FROM c_proveedores pr WHERE pr.cve_proveedor = th_aduana.procedimiento) datos_proveedor
                            from {$from}
                            {$join}
                            ...
                            LEFT JOIN c_usuario u ON u.id_user = th_aduana.cve_usuario
					...
					$join = "LEFT JOIN td_entalmacen on td_entalmacen.fol_folio = th_entalmacen.Fol_Folio
                        ...
                        $sql = "
                            SELECT distinct
                                ...
                                (select concat(c_compania.des_cia,'<br>', c_compania.des_rfc,'<br>',c_compania.des_direcc, ' ',c_compania.des_cp) from c_compania where cve_cia = 1) as datos_facturacion,
                                (SELECT CONCAT(IFNULL(pr.Nombre, ''),' ',IFNULL(pr.RUT, ''),' ',IFNULL(pr.direccion, ''), ' ',IFNULL(pr.colonia, ''),' ',IFNULL(pr.cve_dane, ''),' ',IFNULL(pr.ciudad, ''),', ',IFNULL(pr.estado, ''),', ',IFNULL(pr.pais , '')) AS p FROM c_proveedores pr WHERE pr.ID_Proveedor = th_entalmacen.Cve_Proveedor) datos_proveedor
                            from {$from}
                            {$join}

                            LEFT JOIN c_proveedores on c_proveedores.ID_Proveedor = th_entalmacen.Cve_Proveedor
                            LEFT JOIN c_usuario u ON u.id_user = th_entalmacen.Cve_Usuario
...
//IFNULL((SELECT cve_cia FROM c_usuario WHERE id_user = th_aduana.cve_usuario), (SELECT cve_cia FROM c_usuario WHERE id_user = (SELECT id_user FROM c_usuario WHERE cve_usuario = th_aduana.cve_usuario)))3
                        $sql = "
                            ...
                            from th_aduana
                            LEFT JOIN td_aduana on td_aduana.num_orden = th_aduana.num_pedimento
                            LEFT JOIN th_entalmacen on th_entalmacen.id_ocompra = th_aduana.num_pedimento
                            LEFT JOIN c_articulo on td_aduana.cve_articulo = c_articulo.cve_articulo
                            LEFT JOIN c_compania ON c_compania.cve_cia = $cve_cia
                            ...
                            WHERE th_aduana.num_pedimento = $folio;
                        ";//th_aduana.cve_usuario
      }
      */
//echo $sql;
        ...
        if($tipo == "OC")
        {
            ...
            $sql_usuario = "SELECT nombre_completo FROM c_usuario WHERE id_user = '{$usuario}' OR cve_usuario = '{$usuario}'";
        ...
        $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT , array(152.4, 101.6), true, '$charset', false);
        $pdf->SetCreator('wms');
        $pdf->SetAuthor('wms');
                ...
                <tr style="width:100%;">
                    ...
                    <td style="width:340px;">
                        <table>
                            ...
                            <tr <?php if($codigobarras == 3){ ?> style="display: none;" <?php } ?>>
                            ...
                            <tr>
                                <td colspan="12" style="border: 1px solid black; white-space:nowrap; text-align: center; background-color: #DCDCDC; font-size: 7px;"> <?php if($tipo == "OC" || $tipo == 1) {echo $titulo." ".utf8_encode($datos["factura"]);} else echo $titulo." ".utf8_encode($datos["num_pedimento"]); ?></td>
                            </tr>
                            <tr>
                                <td colspan="12" style="">&nbsp;</td>
                            </tr>
                            ...
                            <tr>
                              <td colspan="2" style="border: 1px solid black; white-space:nowrap; line-height: 14px; text-align: center; font-size: 6px; background-color: #DCDCDC;">CLAVE</td>
                            ...
                            ?>
                                <tr>
                                    <td colspan="2" rowspan="1" style="border: 1px solid black; white-space:nowrap; text-align: left; font-size: 5px;margin:0"><br><br>

<?php 
if($codigobarras == 1)
{
$imgBarCode = "https://".$_SERVER['HTTP_HOST']."/api/articulos/lista/barcode.php?codetype=Code128&size=20&text=".$row["cve_articulo"]."&print=true"; 

?>
                                        <img style="position:  relative; left: 100px;" src="<?=$imgBarCode?>" >
    ...
    echo $row["cve_articulo"];
        ...
        //echo $row["codebar"];
        /*
        #'<img alt=\"articulo\" src=\"barcode.php?codetype=Code128&size=20&text=articulo&print=true\" />' as codebar,
        */
        //$img = file_get_contents($imgBarCode);
...
//$pdf->Image('@' . $img, 55, 19, '', '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
                                          ...
                                          ?></td>
                                    <td colspan="4" rowspan="1" style="border: 1px solid black; white-space:nowrap; text-align: left; font-size: 5px;">&nbsp;&nbsp;<?php echo utf8_encode($row["des_articulo"]) ?></td>
                            ...
                            <?php 
                                ...
                                $iva = 0;
                                //if($datos["iva"]) $iva = $row["subtotal"] - $row["subtotal"]/(1+($datos["iva"]/100));
                                if($datos["iva"]) $iva = ($row["subtotal"]*($datos["iva"]/100));
                            ...
                            <tr>
                                <td colspan="5" style="white-space:nowrap; text-align: center; font-size: 5px;   display: block; text-align: center; position: running(footer);"> <?php echo utf8_encode($datos["nombre_completo"]); ?> </td>
                                <td colspan="2" style=""></td>
    ...
    public function generarReporteEntradasCodigoDeBarras($folio,$proveedor, $oc, $lp, $ubicacion_id = "")
    {
        $res = ""; $res2 = "";
...
/*
          $sql1 = "
              SELECT DISTINCT
                    ...
                    DATE_FORMAT(tde.fecha_inicio, '%d-%m-%Y') AS fecha_inicio,
...
*/
        ...
        if($lp)
        {
            //$lp_in_explode = "";
            //$lp_in = explode(",", $lp);
            //for($i_lp = 0; $i_lp < count($lp_in); $i_lp++)
            //    $lp_in_explode .= ", '".$lp_in[$i_lp]."' ";
            //$lp_in_explode[0] = '';
            //$lp_in_explode = explode(",", $lp_in_explode);
            //$lp_in_implode = implode(",", $lp_in_explode);

            //$sql_lp_charolas = ' AND tdtar.ClaveEtiqueta IN ('.$lp_in_implode.') ';
            $lp = str_replace(",", "','", $lp);
            $sql_lp_charolas = " AND cch.CveLP IN ('{$lp}') ";
        }
        else if($ubicacion_id != '')
        {
            $sql_lp_charolas = " AND ve.cve_ubicacion = '{$ubicacion_id}' ";
        }

        $sql_folio = " tde.fol_folio = '$folio' AND ";
        if(!$folio)
            $sql_folio = "";
          ...
          $sql2 = "
              ...
              FROM td_entalmacen tde
                    ...
                    LEFT JOIN c_charolas cch ON cch.clave_contenedor = tdtar.ClaveEtiqueta or cch.CveLP = tdtar.ClaveEtiqueta
                    #LEFT JOIN c_charolas cch ON cch.clave_contenedor = tdtar.ClaveEtiqueta
                    LEFT JOIN V_ExistenciaGralProduccion ve ON ve.Cve_Contenedor = cch.CveLP
                    LEFT JOIN c_ubicacion u ON ve.cve_ubicacion = u.idy_ubica AND ve.tipo = 'ubicacion'
              WHERE {$sql_folio} cch.CveLP != '' {$sql_lp_charolas} 
              GROUP BY contenedor
          ";
        ...
        //************************************************
        $lp_reg = ""; $pallet_contenedor = ""; $pallets_diferentes = true; $fecha_ingreso = ""; $BL_etiqueta = "";
...
*/
        ...
        $imgLogosSP = $base_url.$datos["logo"];
        //ob_start(); 
        if(($num_pallets == 1 || $num_pallets == 0) && $instancia != 'welldex')
        {
            ob_start(); 
            ...
            $BL_etiqueta = $row2['BL'];
            $BL_en_Etiqueta = false;

            //$lp_reg = $row['ClaveEtiqueta'];
            //$pallet_contenedor = $row['ClaveEtiqueta'];
            //$fecha_ingreso = $row['fecha_inicio'];

            $pdf = "";
            $w_title  = 84; $fs_title = 12; $fs_title2 = 8; $width = 152.4; $height = 101.6; $x_title = 10; $x_proveedor = 32;
            $x_title2 = 30; $w_cell_fecha = 30; $w_cell_fecha2 = 18; $w_cell_proveedor = 30; $w_cell_proveedor2 = 50;
            $x_title3 = 70; $x_title4 = 80; $x_title5 = 35; $y_title5 = 30; $x_codebar = 15; $y_codebar = 35;
            $y_title  = 14; $y_title2  = 20; $y_title2_correccion = 0; $w_codebar = '70'; $h_codebar = '20'; $fs_codebar = 10;
            $w_tabla1 = 100; $w_tabla2 = 200; $fs_tabla = 12; $pallet_una_hoja = 0; $titulo_tabla = true;
            $modo_codigo_barras_producto = false;
            //if(strpos($_SERVER['HTTP_HOST'], 'grupoasl'))
            if($_SERVER['HTTP_HOST'] == 'grupoasl.assistpro-adl.com')
            {
                $BL_en_Etiqueta = true;
                $width = 102.4; $height = 101.6;
                $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT , array($width, $height), true, 'UTF-8', false);

                //$sql_charset = "SET NAMES 'utf8mb4';";
                //if (!($res_charset = mysqli_query($conn, $sql_charset)))echo "Falló la preparación Charset: (" . mysqli_error($conn) . ") ";
            }
            else if($_SERVER['HTTP_HOST'] == 'avavex.assistpro-adl.com')
            {
                $w_title = 65; $fs_title = 8; $x_title = 5; $fs_title2 = 5; $x_title2 = 20; $x_proveedor = 19;
                $w_cell_fecha = 15; $w_cell_fecha2 = 10; $w_cell_proveedor = 15; $w_cell_proveedor2 = 34;
                $y_title  = 10; $y_title2  = 14; $y_title2_correccion = -1; $x_title3 = 50; $x_title4 = 57;
                $x_title5 = 25; $y_title5 = 18; $x_codebar = 5; $y_codebar = 21; $w_codebar = '70'; $h_codebar = '20'; $fs_codebar = 7;
                $w_tabla1 = 70; $w_tabla2 = 150; $fs_tabla = 9; $pallet_una_hoja = 1; $titulo_tabla = false;
                $modo_codigo_barras_producto = true;
                $width = 76; 
                $height = 51; 
            ...
            else
            {
                //$BL_en_Etiqueta = true;
                $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT , array($width, $height), true, 'UTF-8', false);
                $pallet_una_hoja = 0; $titulo_tabla = true; $modo_codigo_barras_producto = false;
            }
...
*/
        $cve_cia = $_SESSION['cve_cia'];
          $sql = "SELECT IF(LEFT(imagen,1) = '/', SUBSTR(imagen,2), imagen) imagen, des_cia FROM c_compania WHERE cve_cia = '$cve_cia'";
          $res  = mysqli_query($conn, $sql);
          if(!$res){echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
          $imagen_empresa = mysqli_fetch_array($res, MYSQLI_ASSOC);
          $PDF_HEADER_LOGO = $imagen_empresa['imagen'];
          $nombre_compania = $imagen_empresa['des_cia'];

        if($PDF_HEADER_LOGO[0] == '//') $PDF_HEADER_LOGO[0] = '';

            //$pdf->Cell(192, 1, "", 0, 0, 'C', 2, '', 0);
            //$pdf->Image($PDF_HEADER_LOGO, 56, 112, 30, 20, '', '', 'center', false, 50, '', false, false, 0, false, false, false);
//********************************************************************************
//********************************************************************************
            $pdf->SetXY($x_title, 4.5);
            $pdf->SetFillColor(0,0,0);
            $pdf->SetTextColor(255,255,255);
            //$pdf->SetLineWidth(3);
            $pdf->SetFontSize($fs_title);
            //"LP Control ".
            $pdf->Cell($w_title, 4, ($nombre_compania), 1, 1, 'C', 1, 0);

            $pdf->SetXY($x_title, $y_title);
            $pdf->SetFontSize($fs_title2);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell($w_cell_fecha, 4, "Fecha Ingreso: ", 0, 1, 'L', 1, 0);

            $pdf->SetXY($x_title2, $y_title);
            $pdf->SetFontSize($fs_title2);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell($w_cell_fecha2, 4, $fecha_ingreso, 0, 1, 'R', 1, 0);

            $pdf->SetXY($x_title3, $y_title);
            $pdf->SetFontSize($fs_title2);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(10, 4, "Folio: ", 0, 1, 'L', 1, 0);

            $pdf->SetXY($x_title4, $y_title);
            $pdf->SetFontSize($fs_title2);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            ...
            $pdf->SetXY($x_title, $y_title2+$y_title2_correccion);
            $pdf->SetFontSize($fs_title2);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell($w_cell_proveedor, 4, "Proveedor: ", 0, 1, 'L', 1, 0);

            $pdf->SetTextColor(0,0,0);
            $pdf->MultiCell($w_cell_proveedor2, 4, $proveedor, 0, 'L', true, 1, $x_proveedor, $y_title2);
//************************************************************************************************************/
        if($num_pallets)
        {
            $pdf->SetXY($x_title5, $y_title5);
            $pdf->SetFontSize($fs_title2);
            $pdf->SetFillColor(0,0,0);
            $pdf->SetTextColor(255,255,255);
            $pdf->Cell(30, 2, "Pallet/Contenedor", 0, 1, 'C', 1, 0);
          ...
          $style = array(
            ...
            $pdf->write1DBarcode($lp_reg, 'C128', $x_codebar, $y_codebar, $w_codebar, $h_codebar, 0.6, $style, 'N');

            if($BL_en_Etiqueta == true)
            {
                $pdf->SetXY($x_title5, $y_title5+10);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                //$BL_etiqueta = 'BL-RACK-SECCION';
                $pdf->Cell(30, 2, $BL_etiqueta, 0, 1, 'C', 1, 0);
            }
        ...
        $sql_folio = "td.fol_folio = '$folio' AND";
        $where_folio = "WHERE fol_folio = '$folio'";
        if(!$folio)
        {
            $sql_folio = "";
            $where_folio = "";
...
FROM td_entalmacenxtarima td
    ...
    {$sql_ubicacion_id} 
WHERE  {$sql_folio} td.Cantidad > 0 AND (td.ClaveEtiqueta = '$lp_reg' OR td.ClaveEtiqueta = (SELECT clave_contenedor FROM c_charolas WHERE CveLP = '$lp_reg'))
...
FROM td_entalmacen td
    ...
    LEFT JOIN c_articulo a ON a.cve_articulo = td.cve_articulo 
    {$sql_ubicacion_id} 
WHERE {$sql_folio} td.CantidadRecibida > 0  #AND td.CantidadPedida > 0 
AND CONCAT(td.cve_articulo, td.cve_lote) NOT IN (SELECT CONCAT(tde.cve_articulo, tde.cve_lote) FROM td_entalmacenxtarima {$where_folio})";//LEFT JOIN c_articulo a ON a.cve_articulo = tde.cve_articulo AND cl.cve_articulo=a.cve_articulo

        if($ubicacion_id != '')
        {
            #$sql_ubicacion_id = " INNER JOIN V_ExistenciaGralProduccion ve ON ve.cve_ubicacion = '{$ubicacion_id}' AND a.cve_articulo = ve.cve_articulo ";
            $sql_folio = "";
            $where_folio = "";
    ...
    ve.cve_lote AS lote,
    DATE_FORMAT(cl.Caducidad, '%d-%m-%Y') AS caducidad,
    ve.Existencia AS cantidad_recibida,
    ch.CveLP AS ClaveEtiqueta
FROM V_ExistenciaGralProduccion ve
LEFT JOIN td_entalmacenxtarima tde ON tde.cve_articulo = ve.cve_articulo AND tde.cve_lote = ve.cve_lote
LEFT JOIN c_charolas ch ON ch.clave_contenedor = ve.Cve_Contenedor AND ch.CveLP = '$lp_reg'
LEFT JOIN c_articulo a ON a.cve_articulo = ve.cve_articulo 
LEFT JOIN c_lotes cl ON cl.LOTE = ve.cve_lote AND cl.Activo=1 AND cl.cve_articulo = ve.cve_articulo
WHERE ve.cve_ubicacion = '{$ubicacion_id}' AND a.cve_articulo = ve.cve_articulo AND ve.Cve_Contenedor != ''
...
";
        ...
        if($modo_codigo_barras_producto)
        {
            $res  = mysqli_query($conn, $sql1);
...
/****************************************************************************************************
            ...
            //$pdf->SetLineWidth(3);
            $pdf->SetFontSize($fs_title);
            $pdf->Cell($w_title, 4, "License Plate Control   AssistPro ADL", 1, 1, 'C', 1, 0);
...
//****************************************************************************************************/
                //$y_title = 27;
                $pdf->SetXY($x_title, $y_title-7);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, "Clave Artículo", 0, 1, 'L', 1, 0);


                $pdf->SetXY($x_title+20, $y_title-7);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+30, 4, "Descripción", 0, 1, 'L', 1, 0);

                $pdf->SetXY($x_title, $y_title-4);
                $pdf->SetFontSize($fs_title2+1);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, $clave, 0, 1, 'L', 1, 0);


                $pdf->SetXY($x_title+20, $y_title-4);
                $pdf->SetFontSize($fs_title2+1);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+30, 4, utf8_decode($descripcion), 0, 1, 'L', 1, 0);



                $pdf->SetXY($x_title, $y_title-1);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, "Lote | Serie", 0, 1, 'L', 1, 0);


                $pdf->SetXY($x_title+20, $y_title-1);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, "Caducidad", 0, 1, 'L', 1, 0);

                $pdf->SetXY($x_title+40, $y_title-1);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, "Cantidad", 0, 1, 'L', 1, 0);


                $pdf->SetXY($x_title, $y_title+2);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, utf8_decode($lote), 0, 1, 'L', 1, 0);


                $pdf->SetXY($x_title+20, $y_title+2);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, $caducidad, 0, 1, 'L', 1, 0);

                $pdf->SetXY($x_title+40, $y_title+2);
                $pdf->SetFontSize($fs_title2);
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0,0,0);
                $pdf->Cell($w_cell_fecha+20, 4, $cantidad_recibida, 0, 1, 'L', 1, 0);
          ...
          $style = array(
              ...
              'stretchtext'  => 4
                ...
                $pdf->write1DBarcode($clave, 'C128', $x_codebar, $y_codebar-7, $w_codebar, $h_codebar-4, 0.6, $style, 'N');
...
/************************************************************************************************************/
        ?>
        <table style="width:<?php echo ($w_tabla1+$w_tabla2); ?>px;">
            <?php 
            if($titulo_tabla)
            {
            ?>
            <tr style="background-color: #000; color: #fff;line-height: 10px;">
                <td colspan="2" style="width: <?php echo $w_tabla1; ?>px;">Clave</td>
                <td colspan="2" style="width: <?php echo $w_tabla2; ?>px;">Descripción</td>
            </tr>
        <?php 
            }
    ...
    ?>
                    <tr>
                        <td style="font-size: <?php echo ($fs_tabla-1); ?>px;width: <?php echo $w_tabla1; ?>px;" colspan="2"><?php echo $datos["clave"]; ?></td>
                        <td style="font-size: <?php echo $fs_tabla; ?>px;width: <?php echo $w_tabla2; ?>px;" colspan="2"><?php echo utf8_encode($datos["descripcion"]); ?></td>
                    </tr>

                    <tr>
                        <td style="width: <?php echo $w_tabla1; ?>px;">Lote | Serie</td>
                        <td style="width: <?php echo $w_tabla1; ?>px;">Caducidad</td>
                        <td align="center" style="width: <?php echo $w_tabla1; ?>px;">Cantidad</td>
                    </tr>
                    <tr>
                        <td style="font-size: <?php echo $fs_tabla; ?>px;width: <?php echo $w_tabla1; ?>px;"><?php echo utf8_decode($lote); ?></td>
                        <td style="font-size: <?php echo $fs_tabla; ?>px;width: <?php echo $w_tabla1; ?>px;"><?php if($caducidad != '00-00-0000') echo $caducidad; ?></td>
                        <td style="font-size: <?php echo $fs_tabla; ?>px;width: <?php echo $w_tabla1; ?>px;" align="center"><?php echo $datos["cantidad_recibida"]; ?></td>
          ...
          $style = array(
              ...
              'stretchtext'  => 4
                    ...
                    $pdf->write1DBarcode($datos["clave"], 'C128', '', 10, $w_codebar, $h_codebar, 0.6, $style, 'N');
                    ?>
                        </td>
                    </tr>
                    <?php 
                    */
                    //$pdf->AddPage();
                    /*
        if($instancia == 'dev' || $instancia == 'welldex')
        {
            ...
            $pdf->SetXY($x_title5, $y_title5+79);
            $pdf->SetFontSize($fs_title2+10);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(20, 2, "Clave", 0, 1, 'C', 1, 0);

            $pdf->write1DBarcode($datos["clave"], 'C128', $x_title5, $y_codebar+81, $w_codebar5-30, $h_codebar-10, 0.4, $style, 'N');


            $pdf->SetXY($x_title5, $y_title5+100);
            $pdf->SetFontSize($fs_title2+10);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(20, 2, "Lote", 0, 1, 'C', 1, 0);
            ...
            $pdf->write1DBarcode($lote, 'C128', $x_title5, $y_codebar+102, $w_codebar-30, $h_codebar-10, 0.4, $style, 'N');
        }
        */
               }
                ?>
          ...
          else
          {
            ...
            //}
                $BL_en_Etiqueta = false;
                $pdf = "";
                //if(strpos($_SERVER['HTTP_HOST'], 'grupoasl') >= 0)
                if($_SERVER['HTTP_HOST'] == 'grupoasl.assistpro-adl.com')
                {
                    $pdf = new \TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT , array(102.4, 101.6), true, 'UTF-8', false);
                    $BL_en_Etiqueta = true;
                    //$sql_charset = "SET NAMES 'utf8mb4';";
                    //if (!($res_charset = mysqli_query($conn, $sql_charset)))echo "Falló la preparación Charset: (" . mysqli_error($conn) . ") ";
                }
...
//********************************************************************************
                ...
                //$pdf->SetLineWidth(3);
                //"LP Control ".
                $pdf->Cell(84, 4, ($nombre_compania), 1, 1, 'C', 1, 0);
...
if($oc)
{
        ...
        $sql_folio = "td.fol_folio = '$folio' AND tde.fol_folio = '$folio' AND";
        $sql_folio2 = "td.fol_folio = '$folio' AND";
        $where_folio = "WHERE fol_folio = '$folio'";
        if(!$folio)
        {
            $sql_folio = "";
...
FROM td_entalmacen td
    ...
    LEFT JOIN c_articulo a ON a.cve_articulo = td.cve_articulo 
    {$sql_ubicacion_id} 
WHERE {$sql_folio2} td.CantidadRecibida > 0  #AND td.CantidadPedida > 0 
AND CONCAT(td.cve_articulo, td.cve_lote) NOT IN (SELECT CONCAT(tde.cve_articulo, tde.cve_lote) FROM td_entalmacenxtarima {$where_folio})
...
<?php 
                ...
                ?>
                        <tr>
                            <td style="font-size: 11px;width: 100px;" colspan="2"><?php echo $datos["clave"]; ?></td>
                            <td style="font-size: 12px;width: 200px;" colspan="2"><?php echo utf8_encode($datos["descripcion"]); ?></td>
                        </tr>

                        <tr>
                            <td style="width: 100px;">Lote | Serie</td>
                            <td style="width: 100px;">Caducidad</td>
                            <td align="center" style="width: 100px;">Cantidad</td>
                        </tr>
                        <tr>
                            <td style="font-size: 12px;width: 100px;"><?php echo utf8_decode($datos["lote"]); ?></td>
                            <td style="font-size: 12px;width: 100px;"><?php if($datos["caducidad"] != '00-00-0000') echo $datos["caducidad"]; //echo $sql_lp_charolas; 
                            ?></td>
                            <td style="font-size: 12px;width: 100px;" align="center"><?php echo $datos["cantidad_recibida"]; ?></td>
                        </tr>
                    ...
                    <?php 
        //if($_SERVER['HTTP_HOST'] == 'dev.assistpro-adl.com' || $_SERVER['HTTP_HOST'] == 'welldex.assistpro-adl.com')
        if($instancia == 'dev' || $instancia == 'welldex')
        {
            ...
            $pdf->write1DBarcode($datos["lote"], 'C128', $x_title5, $y_codebar+102, $w_codebar-30, $h_codebar-10, 0.4, $style, 'N');
/*
            $pdf->SetXY($x_title5, $y_title5+119);
            $pdf->SetFontSize($fs_title2+10);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(20, 2, "Cantidad", 0, 1, 'C', 1, 0);

            $pdf->SetXY($x_title5, $y_title5+125);
            $pdf->SetFontSize($fs_title2+10);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(20, 2, $datos["cantidad_recibida"], 0, 1, 'C', 1, 0);
*/
/*
              $style = array(
                  'position'     => '',
            ...
            $pdf->write1DBarcode($datos["cantidad_recibida"], 'C128', $x_title5, $y_codebar+114.5, $w_codebar-30, $h_codebar-70, 0.4, $style, 'N');
*/

/*
            $pdf->SetXY($x_title5, $y_title5+110);
            $pdf->SetFontSize($fs_title2+10);
            $pdf->SetFillColor(255,255,255);
            $pdf->SetTextColor(0,0,0);
            $pdf->Cell(20, 2, "Cantidad", 0, 1, 'C', 1, 0);

            $pdf->write1DBarcode($datos["cantidad_recibida"], 'C128', $x_codebar+10, $y_codebar+110, $w_codebar-5, $h_codebar-10, 0.6, $style, 'N');
*/
        }
                  $desProducto = ob_get_clean();
    ...
    mysqli_set_charset($conn , $charset);
          ...
          $sql = "
               ...
               #(SELECT MIN( DATE_FORMAT(th_aduana.fech_pedimento, '%d-%m-%Y')) FROM td_entalmacen WHERE fol_folio = th_entalmacen.Fol_Folio) 
               DATE_FORMAT(th_aduana.fech_pedimento, '%d-%m-%Y') AS fecha_recepcion, 
               c_proveedores.nombre AS proveedor, 
          ...
          $nombre_almacen       = mb_convert_encoding($datos_documento['nombre_almacen'], "utf8");
          ...
          $sql = "SELECT IF(LEFT(imagen,1) = '/', SUBSTR(imagen,2), imagen) imagen FROM c_compania WHERE cve_cia = '$cve_cia'";
          $res  = mysqli_query($conn, $sql);
          if(!$res){echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
          $imagen_empresa = mysqli_fetch_array($res, MYSQLI_ASSOC);
          $PDF_HEADER_LOGO = $imagen_empresa['imagen'];
        ...
        $filename = "Entradas Recepcion #{$folio}.pdf";
        $base_url = $_SERVER['DOCUMENT_ROOT'];
        //$imgLogosSP = $base_url."img/logos_sp.jpg";
        //$imgLogosSP = $base_url.$datos["logo"];
        //ob_start(); 
          ...
          $Hora_Salida         = $datos_transporte['Hora_Salida'];
            ...
            //$pdf->SetCreator('wms');
            //$pdf->SetAuthor('wms');
            $pdf->SetTitle('Reporte de Recepción #'.$folio);
...
<?php 
    ...
    $sql = "SELECT DISTINCT
            ...
            IFNULL(a.control_numero_series, 'N') AS band_serie,
            #tde.CantidadPedida AS cantidad_pedida,
            TRUNCATE((IF(IFNULL(a.num_multiplo, 0) = 0, tde.CantidadPedida, tde.CantidadPedida/a.num_multiplo)), 4) AS cantidad_pedida,
            tde.cve_lote AS lote,
            tde.numero_serie AS serie,
            ...
            TRUNCATE((IF(IFNULL(a.num_multiplo, 0) = 0, tde.CantidadRecibida, tde.CantidadRecibida/a.num_multiplo)), 4) AS cantidad_recibida,
            DATE_FORMAT(tde.fecha_inicio, '%d-%m-%Y') AS fecha_recepcion,
            DATE_FORMAT(tde.fecha_inicio, '%d-%m-%Y %h:%i:%s %p') AS fecha_inicio,
            DATE_FORMAT(tde.fecha_fin, '%d-%m-%Y %h:%i:%s %p') AS fecha_fin,
            ...
            (IF(IFNULL(a.num_multiplo, 0) = 0, tde.CantidadPedida - IF(th_entalmacen.tipo = 'OC', tde.CantidadRecibida, 0), (tde.CantidadPedida - IF(th_entalmacen.tipo = 'OC', tde.CantidadRecibida, 0))/a.num_multiplo)) AS cantidad_faltante,
            ...
            TRUNCATE((IF(IFNULL(a.num_multiplo, 0) = 0, tde.CantidadPedida - IF(th_entalmacen.tipo = 'OC', tde.CantidadRecibida, 0), (tde.CantidadPedida - IF(th_entalmacen.tipo = 'OC', tde.CantidadRecibida, 0))/a.num_multiplo)), 4) AS cantidad_danada,
            u.cve_usuario AS usuario,
      ...
      FROM td_entalmacen tde
          ...
          #LEFT JOIN c_charolas ON c_charolas.CveLP = ta.ClaveEtiqueta
                ...
                <tr>
                    ...
                    <td class="td td3"><div><br><?php 
                        ...
                        if($serie)
                            echo "Serie: ".$serie."<br>"; 
                        ?> </div></td>
                    <td class="td td2"><div><br><?php echo number_format((float) $row['cantidad_pedida'], 4); ?></div></td>
                    <td class="td td2"><div><br><?php echo number_format((float) $row['cantidad_recibida'], 4); $total_cajas += $row['cantidad_recibida']; ?></div></td>
                    <td class="td td2"><div><br><?php
                        ...
                        //} 
                         ...
                         echo number_format((float) $cantidad_danada, 4);
                    ?></div></td>
...
?>
                <tr>
                    ...
                    <td class="td td2"><br><br><?php echo number_format((float) $total_cajas, 4); ?></td>
                    <td></td>
                    <td></td>
                </tr>
                </table>
