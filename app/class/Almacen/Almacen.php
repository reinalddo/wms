...
class Almacen {
  ...
  function save( $_post ) {
    ...
    $sql = sprintf('
        ...
        SET
		des_almac = :des_almac
    ,Cve_TipoZona = :cve_talmacen
		,clave_almacen=:clave_almacen
    ,cve_almacenp=:cve_almacenp
    ,clasif_abc=:controlABC
    ,ID_Proveedor=:empresa_proveedor
		');
    ...
    $this->save->bindValue( ':controlABC', $_post['controlABC'], \PDO::PARAM_STR );
    $this->save->bindValue( ':empresa_proveedor', $_post['empresa_proveedor'], \PDO::PARAM_STR );
  ...
  function actualizarAlmacen( $data ) {
    $sql = '
      ...
      ';
    $this->save = \db()->prepare($sql);
    $this->save->execute( array($data['cve_almacenp'], $data['des_almac'], $data['cve_talmacen'], $data['controlABC'], $data['empresa_proveedor'], $data['cve_almac']));
  }
  ...
  function loadUserAlmacen($almacen) 
  {
   ...
   SELECT 
        ...
        FROM 
          trel_us_alm t 
        INNER JOIN 
          c_usuario c ON c.cve_usuario = t.cve_usuario AND c.cve_usuario != "wmsmaster" AND (c.es_cliente = 0 OR c.es_cliente = 3)
        WHERE 
          t.cve_almac = "'.$almacen.'"
        AND 
          t.Activo = 1 
  ...
  function loadUsers($cve_almac) {
    ...
    AND c.cve_usuario != "wmsmaster" AND (c.es_cliente = 0 OR c.es_cliente = 3);
   ...
   function loadUbicacionesDeZonas($zonas,$rack, $area, $almacen, $vacias, $producto, $pasillo, $rackk, $nivel, $seccion, $ubicacion) 
  {
    $p_vacia = "";
    if($producto != '') 
    {
      ...
      $p_vacia = " and e.cve_articulo = '".$producto."' ";
    }
    ...
    if($vacias == true)
    {
        $sqlAreasVacias ="AND u.idy_ubica NOT IN (
                          SELECT cve_ubicacion FROM V_ExistenciaGralProduccion e WHERE  e.tipo = 'area'
                          {$p_vacia})";
    }
    else 
        $sqlVacias ="AND u.idy_ubica IN (
                          SELECT cve_ubicacion FROM V_ExistenciaGralProduccion e WHERE  e.tipo = 'ubicacion'
                          {$p_vacia})";
    //$sqlAreasVacias ="AND cve_ubicacion ".($vacias?"NOT":"")." IN (
    //                  SELECT cve_ubicacion FROM V_ExistenciaGralProduccion e WHERE  e.tipo = 'area'
    //                  {$p_vacia})";

    $bl_part = ""; $bl_part1 = ""; $bl_part2 = "";
    if($pasillo != "")   {$bl_part .= " AND u.cve_pasillo = '{$pasillo}' AND u.cve_pasillo != '' ";}
    if($rackk != "")     {$bl_part .= " AND u.cve_rack = '{$rackk}' AND u.cve_rack != '' ";}
    if($nivel != "")     {$bl_part .= " AND u.cve_nivel = '{$nivel}' AND u.cve_nivel != '' ";}
    if($seccion != "")   {$bl_part .= " AND u.Seccion = '{$seccion}' AND u.Seccion != '' ";}
    if($ubicacion != "") {$bl_part .= " AND u.Ubicacion = '{$ubicacion}' AND u.Ubicacion != '' ";}
    if($producto != "")  {$bl_part1 .= " AND ve.cve_articulo = '{$producto}' ";}
...
/*
    $sql1 = "SELECT 
              ...
              and u.activo = 1 
              and a.activo =1 
              AND u.idy_ubica IN (SELECT V_ExistenciaGral.cve_ubicacion FROM V_ExistenciaGral)
              $split {$sqlVacias} 
...
/*
    $sql1 = "SELECT * FROM (
        ...
        WHERE 1 {$bl_part}
            ...
            AND u.cve_almac = a.cve_almac
            AND u.Activo = '1' 
            AND u.idy_ubica NOT IN (SELECT DISTINCT v.idy_ubica FROM t_invpiezas v WHERE v.ID_Inventario = (SELECT ID_Inventario FROM th_inventario WHERE STATUS = 'A' AND ID_Inventario = v.ID_Inventario) UNION SELECT DISTINCT t.idy_ubica FROM t_invtarima t WHERE ID_Inventario = (SELECT ID_Inventario FROM th_inventario WHERE STATUS = 'A' AND ID_Inventario = t.ID_Inventario))
            $split 
            ORDER BY u.CodigoCSD DESC
        ) AS t 
        ORDER BY ubicacion ASC";
*/
        ...
        SELECT DISTINCT
            ...
            u.Ubicacion AS ubic,
            u.cve_almac AS zona_almacen,
            p.clave AS clave_almacen,
            u.idy_ubica AS id_ubicacion
        FROM c_ubicacion u
        LEFT JOIN c_almacen a ON u.cve_almac = a.cve_almac
        LEFT JOIN c_almacenp p ON a.cve_almacenp = p.id
        LEFT JOIN th_inventario th on th.cve_almacen = p.clave AND th.Status = 'A' ##EN PRUEBA CUALQUIER COSA BORRAR
        JOIN t_invpiezas tp ON tp.idy_ubica != u.idy_ubica and th.ID_Inventario = tp.ID_Inventario ##EN PRUEBA CUALQUIER COSA BORRAR
        JOIN t_invtarima tt ON tt.idy_ubica != u.idy_ubica and th.ID_Inventario = tt.ID_Inventario ##EN PRUEBA CUALQUIER COSA BORRAR
        WHERE 1 {$bl_part}
            AND u.cve_almac=a.cve_almac
            AND a.cve_almacenp=p.id
            AND u.cve_almac = a.cve_almac
            AND u.Activo = '1' 
             AND p.id = {$almacen}
             {$split} 
             AND IFNULL(u.TECNOLOGIA, '') != 'PTL' 

        ORDER BY ubicacion DESC
) AS tv WHERE tv.id_ubicacion NOT IN (
  SELECT DISTINCT v.idy_ubica 
  FROM t_invpiezas v 
  WHERE v.ID_Inventario IN (SELECT ID_Inventario FROM th_inventario WHERE STATUS = 'A' AND ID_Inventario = v.ID_Inventario AND th_inventario.cve_almacen = tv.clave_almacen) 
  UNION 
  SELECT DISTINCT t.idy_ubica 
  FROM t_invtarima t 
  WHERE ID_Inventario IN (SELECT ID_Inventario FROM th_inventario WHERE STATUS = 'A' AND ID_Inventario = t.ID_Inventario AND th_inventario.cve_almacen = tv.clave_almacen)
)";
  ...
  if($bl_part1 != "")
  {
        ...
        LEFT JOIN c_almacenp p ON ve.cve_almac = p.id
        LEFT JOIN c_ubicacion u ON u.idy_ubica = ve.cve_ubicacion 
        LEFT JOIN c_charolas ch ON ch.clave_contenedor = ve.Cve_Contenedor
        LEFT JOIN c_almacen a ON a.cve_almac = u.cve_almac
        LEFT JOIN th_inventario th on th.cve_almacen = p.clave AND th.Status = 'A'##EN PRUEBA CUALQUIER COSA BORRAR
        JOIN t_invpiezas tp ON tp.idy_ubica != ve.cve_ubicacion and th.ID_Inventario = tp.ID_Inventario##EN PRUEBA CUALQUIER COSA BORRAR
        JOIN t_invtarima tt ON tt.idy_ubica != ve.cve_ubicacion and th.ID_Inventario = tt.ID_Inventario##EN PRUEBA CUALQUIER COSA BORRAR
        WHERE 1 {$bl_part} {$bl_part1}
            AND u.cve_almac=a.cve_almac
            AND a.cve_almacenp=p.id
            AND u.cve_almac = a.cve_almac
            AND u.Activo = '1' 
             AND ve.cve_almac = {$almacen}
             {$split} 
             AND IFNULL(u.TECNOLOGIA, '') != 'PTL' 
        ORDER BY ubicacion DESC
) AS tv WHERE tv.id_ubicacion NOT IN (
...
//            AND u.cve_rack != '11' AND u.cve_rack != '12' AND u.cve_rack != '17' AND LEFT(u.CodigoCSD,2) != '01' AND LEFT(u.CodigoCSD,2) != '05' AND LEFT(u.CodigoCSD,2) != '04' AND LEFT(u.CodigoCSD,2) != '07' AND LEFT(u.CodigoCSD,2) != '06' AND LEFT(u.CodigoCSD,2) != '08' AND LEFT(u.CodigoCSD,2) != '09' AND LEFT(u.CodigoCSD,2) != '10' AND LEFT(u.CodigoCSD,2) != '13' AND LEFT(u.CodigoCSD,2) != '14' AND LEFT(u.CodigoCSD,2) != '15'
  ...
  function loadRackDeZonas($zonas, $vacias, $pasillo, $rack, $nivel, $seccion, $ubicacion, $id_almacen) 
  {
    $sqlVacias=""; $bl_part = "";
    if($vacias == true)
    {
    ...
    if($pasillo != "")   {$bl_part .= " AND u.cve_pasillo = '{$pasillo}' AND u.cve_pasillo != '' ";}
    if($rack != "")      {$bl_part .= " AND u.cve_rack = '{$rack}' AND u.cve_rack != '' ";}
    if($nivel != "")     {$bl_part .= " AND u.cve_nivel = '{$nivel}' AND u.cve_nivel != '' ";}
    if($seccion != "")   {$bl_part .= " AND u.Seccion = '{$seccion}' AND u.Seccion != '' ";}
    if($ubicacion != "") {$bl_part .= " AND u.Ubicacion = '{$ubicacion}' AND u.Ubicacion != '' ";}
    ...
    $sql = "
      SELECT DISTINCT u.idy_ubica as id, IFNULL(u.CodigoCSD, '') as rack, IFNULL(u.cve_pasillo, '') as cve_pasillo, IFNULL(u.cve_rack, '') as cve_rack, IFNULL(u.cve_nivel, '') as cve_nivel, IFNULL(u.Seccion, '') as Seccion, IFNULL(u.Ubicacion, '') as Ubicacion
      FROM `c_ubicacion` u
      inner join c_almacen a on a.cve_almac = u.cve_almac
      LEFT JOIN c_almacenp al ON al.id = a.cve_almacenp
      LEFT JOIN th_inventario th on th.cve_almacen = al.clave AND th.Status != 'A'
      JOIN t_invpiezas tp ON tp.idy_ubica != u.idy_ubica and th.ID_Inventario = tp.ID_Inventario
      JOIN t_invtarima tt ON tt.idy_ubica != u.idy_ubica and th.ID_Inventario = tt.ID_Inventario
      {$zonas_limbo}
      and u.activo = 1 and a.activo =1 {$sqlVacias}
      #AND u.idy_ubica IN (SELECT V_ExistenciaGralProduccion.cve_ubicacion FROM V_ExistenciaGralProduccion) 
      AND u.idy_ubica NOT IN (SELECT Idy_Ubica FROM t_movcuarentena)
      WHERE 1 {$bl_part}
      AND al.id = $id_almacen 
      ORDER BY rack asc
      ";
     //echo $sql;exit;
    //echo var_dump($sql);
