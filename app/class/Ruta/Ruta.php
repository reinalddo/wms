<?php
  ...
  class Ruta {
    ...
    function getAll($almacen = "", $venta = "") {
        ...
        $sqlVenta = "";
        if($venta)
          $sqlVenta = " AND (ID_Ruta IN (SELECT DISTINCT RutaId FROM Venta WHERE IdEmpresa = '{$almacen}') OR ID_Ruta IN (SELECT DISTINCT Ruta FROM V_Cabecera_Pedido WHERE Ruta IS NOT NULL AND IdEmpresa = '{$almacen}')) ";
    ...
    function getAllEnvases($almacen) {
        ...
        SELECT
          *
        FROM
          " . self::TABLE . " WHERE Activo = '1' {$sqlAlmacen} AND ID_Ruta IN (SELECT RutaId FROM DevEnvases WHERE IdEmpresa = '{$almacen}')
      ";
    ...
    function getAllInventario($almacen = "", $status = "") 
    {
        ...
        $sqlStatus = "";$sqlStatus2 = "";$sqlStatus3 = "";
        if($status)
        {
            $sqlStatus  = " AND th.status IN ('T', 'F') ";
            $sqlStatus2 = " AND p.status IN ('T', 'F') ";
            $sqlStatus3 = " AND 0 ";
        }
        ...
        $sql = "SELECT DISTINCT * FROM t_ruta 
                ...
                WHERE r.ID_Ruta IS NOT NULL AND p.cve_almac = '{$almacen}' #(SELECT id FROM c_almacenp WHERE clave = '{$almacen}')
                {$sqlStatus2} 
    ...
    function getAllRutasPedidos($almacen = "") 
    {
        $sql = "SELECT DISTINCT * FROM t_ruta WHERE ID_Ruta IN (SELECT Id_Ruta FROM th_ordenembarque WHERE IFNULL(Id_Ruta, '') != '' AND Cve_Almac = '{$almacen}')";
    ...
    $this->save->bindValue( ':id_proveedor', $data['id_proveedor'], \PDO::PARAM_STR );
        ...
        if($data["venta_preventa"] == 2)
        {
          ...
          foreach ($rel_rutas as $ruta_entrega) 
          {
            ...
            $sql = sprintf("INSERT INTO RelClirutas (IdCliente, IdRuta, IdEmpresa, Fecha) (SELECT r.IdCliente, (SELECT MAX(ID_Ruta) FROM t_ruta) AS id_ruta, r.IdEmpresa, CURDATE() AS fecha_actual FROM RelClirutas r INNER JOIN c_destinatarios d ON d.id_destinatario = r.IdCliente AND d.Activo = '1' WHERE r.IdRuta = $ruta_entrega)");
            $this->save = \db()->prepare($sql);
            $this->save->execute();

            $sql = sprintf("INSERT INTO t_clientexruta (clave_cliente, clave_ruta) (SELECT r.IdCliente, (SELECT MAX(ID_Ruta) FROM t_ruta) AS id_ruta FROM RelClirutas r INNER JOIN c_destinatarios d ON d.id_destinatario = r.IdCliente AND d.Activo = '1' WHERE r.IdRuta = $ruta_entrega)");
            $this->save = \db()->prepare($sql);
            $this->save->execute();
...
*/
  ...
  function EliminarChoferRuta( $ruta, $agente ) 
  {
    ...
    if($this->existeAgenteRuta($ruta, $agente))
    {
        ...
        $sql = "UPDATE RelDayCli SET Cve_Vendedor = NULL WHERE Cve_Vendedor = '".$agente."' AND Cve_Ruta = (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '".$ruta."' LIMIT 1)";
        $rs = mysqli_query(\db2(), $sql) or die(mysqli_error(\db2()));
    }
    else
    {
...
*/
        ...
        if($venta_preventa == 2)
        {
            ...
            $sql = sprintf("INSERT INTO RelClirutas (IdCliente, IdRuta, IdEmpresa, Fecha) (SELECT r.IdCliente, r.IdRuta, r.IdEmpresa, CURDATE() AS fecha_actual FROM RelClirutas r INNER JOIN c_destinatarios d ON d.id_destinatario = r.IdCliente AND d.Activo = '1' WHERE r.IdRuta = $idRuta AND CONCAT(r.IdRuta, r.IdCliente, r.IdEmpresa) NOT IN (SELECT CONCAT(IdRuta, IdCliente, IdEmpresa) FROM RelClirutas))");
            $this->save = \db()->prepare($sql);
            $this->save->execute();

            $sql = sprintf("INSERT INTO t_clientexruta (clave_cliente, clave_ruta) (SELECT r.IdCliente, r.IdRuta FROM RelClirutas r INNER JOIN c_destinatarios d ON d.id_destinatario = r.IdCliente AND d.Activo = '1' WHERE r.IdRuta = $idRuta AND CONCAT(r.IdRuta, r.IdCliente, r.IdEmpresa) NOT IN (SELECT CONCAT(IdRuta, IdCliente, IdEmpresa) FROM RelClirutas))");
            $this->save = \db()->prepare($sql);
            $this->save->execute();
	 ...
	 function OperadoresRuta($id_ruta, $asignados_si_no, $cve_almac) 
  {
    ...
    if($asignados_si_no == 1)
        ...
        INNER JOIN Rel_Ruta_Agentes ra ON ra.cve_vendedor = u.id_user
        INNER JOIN t_ruta r ON ra.cve_ruta = r.ID_Ruta AND r.cve_ruta = '{$id_ruta}'
        WHERE u.Activo=1 AND u.es_cliente = 3 
        #AND CONCAT(IFNULL(u.id_user, ''), IFNULL(r.ID_Ruta, '')) NOT IN (SELECT CONCAT(IFNULL(cve_vendedor, ''), IFNULL(cve_ruta, '')) FROM Rel_Ruta_Agentes)
        ";
    else 
    $sql= "
      SELECT DISTINCT 
      ...
      AND u.id_user NOT IN (SELECT cve_vendedor FROM Rel_Ruta_Agentes)
    ";
/*
        $sql= "
          SELECT DISTINCT 
