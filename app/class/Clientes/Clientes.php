...
class Clientes
{
    ...
    ');
            ...
            $this->save->bindValue(':cliente_general', strtoupper($data['cliente_general']), \PDO::PARAM_STR);
            ...
            // Corregir el uso de $data en lugar de $_POST para mantener consistencia
            // $this->save->bindValue(':envio_correo_automatico', (isset($data['envio_automatico']) ? $data['envio_automatico'] : 0), \PDO::PARAM_STR);
            //si existe envio_automatico y es true entonces lo guardamos con 1
              if (isset($data['envio_automatico']) && $data['envio_automatico'] == 'true') {
                $this->save->bindValue(':envio_correo_automatico', 1, \PDO::PARAM_STR);
            } else {
                $this->save->bindValue(':envio_correo_automatico', 0, \PDO::PARAM_STR);
            }

            $this->save->execute();
            $clienteId = $data['Cve_Clte'];

            // DEBUG: Verificar qué datos llegan
            $logFile = __DIR__ . '/debug_correos.log';
            file_put_contents($logFile, date('Y-m-d H:i:s') . " - Datos recibidos:\n" . print_r($data, true) . "\n\n", FILE_APPEND);

            // Verificar específicamente envio_automatico
            if (isset($data['envio_automatico'])) {
                file_put_contents($logFile, date('Y-m-d H:i:s') . " - envio_automatico encontrado: " . $data['envio_automatico'] . "\n", FILE_APPEND);
            } else {
                file_put_contents($logFile, date('Y-m-d H:i:s') . " - envio_automatico NO encontrado\n", FILE_APPEND);
            }

            // Verificar correos
            if (isset($data['correos'])) {
                file_put_contents($logFile, date('Y-m-d H:i:s') . " - correos encontrados: " . print_r($data['correos'], true) . "\n", FILE_APPEND);
            } else {
                file_put_contents($logFile, date('Y-m-d H:i:s') . " - correos NO encontrados\n", FILE_APPEND);
            }

            // CAMBIO PRINCIPAL: Usar $data en lugar de $_POST para consistencia
            if (isset($data['envio_automatico']) && $data['envio_automatico'] == 'true') {
                file_put_contents($logFile, date('Y-m-d H:i:s') . " - Entrando al bloque de guardar correos\n", FILE_APPEND);

                // Verificar si existen correos
                if (!isset($data['correos']) || !is_array($data['correos'])) {
                    file_put_contents($logFile, date('Y-m-d H:i:s') . " - ERROR: No hay correos para procesar\n", FILE_APPEND);
                } else {
                    file_put_contents($logFile, date('Y-m-d H:i:s') . " - Procesando " . count($data['correos']) . " correos\n", FILE_APPEND);

                    // Guardar correos en la tabla envios_correos
                    foreach ($data['correos'] as $correo) {
                        $sql = "INSERT INTO envios_correos (tipo_destinatario, destinatario_id, correo, almacen) VALUES ('cliente', '" . $clienteId . "', '" . $correo['correo'] . "', '" . $data['almacen'] . "')";

                        //escribimos un archivo log con el sql
                        file_put_contents($logFile, date('Y-m-d H:i:s') . " - SQL: " . $sql . "\n", FILE_APPEND);


                        $rs = mysqli_query(\db2(), $sql);
                        if (!$rs) {
                            $error = mysqli_error(\db2());
                            file_put_contents($logFile, date('Y-m-d H:i:s') . " - ERROR en INSERT envios_correos: " . $error . "\n", FILE_APPEND);
                            continue; // Saltar al siguiente correo si hay error
                        }

                        file_put_contents($logFile, date('Y-m-d H:i:s') . " - Correo guardado exitosamente\n", FILE_APPEND);
    ...
    function getAllProformas($filtro = "")
    {
        $sql = "
        SELECT DISTINCT
          c.Cve_Clte, c.RazonSocial
        FROM
          c_cliente c
        INNER JOIN th_proforma s ON s.Cve_Clte = c.Cve_Clte 
	  ...
	  function getAllManiobras($filtro = "")
      {
          $sql = "
        SELECT DISTINCT
          c.Cve_Clte, c.RazonSocial
        FROM
          c_cliente c
        INNER JOIN th_pedservicios s ON s.Cve_Clte = c.Cve_Clte 
      WHERE c.Activo = 1 {$filtro} 
      ORDER BY RazonSocial
      ";
  ...
  function traerClientesDeRutaClave($cve_ruta)
  {
    ...
    function borrarCliente($data)
    {
      ...
      ';
        ...
        $sql = "DELETE FROM t_clientexruta WHERE clave_cliente IN (SELECT id_destinatario FROM c_destinatarios WHERE Cve_Clte = '" . $data['Cve_Clte'] . "')";
        $sth = \db()->prepare($sql);
        $sth->execute();
        ...
        WHERE
          Cve_Clte = :Cve_Clte;';
            ...
            */
            ...
            $this->save->bindValue(':cliente_general', $data['cliente_general'], \PDO::PARAM_STR);
            ...
            */
            ...
            $sql = "DELETE FROM envios_reportes WHERE envio_id IN (SELECT id FROM envios_correos WHERE tipo_destinatario = 'cliente' AND destinatario_id = '" . $data['Cve_Clte'] . "' AND almacen = '" . $data['almacen'] . "')";
            $rs = mysqli_query(\db2(), $sql) or die(mysqli_error(\db2()));

            // Primero eliminar registros existentes para evitar duplicados
            $sql = "DELETE FROM periodicidad WHERE envio_id IN (SELECT id FROM envios_correos WHERE tipo_destinatario = 'cliente' AND destinatario_id = '" . $data['Cve_Clte'] . "' AND almacen = '" . $data['almacen'] . "')";
            $rs = mysqli_query(\db2(), $sql) or die(mysqli_error(\db2()));

            $sql = "DELETE FROM envios_correos WHERE tipo_destinatario = 'cliente' AND destinatario_id = '" . $data['Cve_Clte'] . "' AND almacen = '" . $data['almacen'] . "'";
            $rs = mysqli_query(\db2(), $sql) or die(mysqli_error(\db2()));

            // Procesar cada correo individualmente
            foreach ($data['correos'] as $correo) {
                // Insertar en envios_correos
                $sql = "INSERT INTO envios_correos (tipo_destinatario, destinatario_id, correo,almacen) VALUES ('cliente', '" . $data['Cve_Clte'] . "', '" . $correo['correo'] . "', '" . $data['almacen'] . "')";
                $rs = mysqli_query(\db2(), $sql) or die(mysqli_error(\db2()));
    ...
    function getCorreosProveedor($clave_cliente, $almacen)
    {
        //obtenemos primero los correos de la tabla envios_correos
        $sql = "SELECT * FROM envios_correos WHERE tipo_destinatario = 'cliente' AND destinatario_id = '" . $clave_cliente . "' AND almacen = '" .$almacen . "'";
        //log del insert
        $logFile = __DIR__ . '/debug_correos.log';
        file_put_contents($logFile, date('Y-m-d H:i:s') . " - SQL: " . $sql . "\n", FILE_APPEND);
        ...
        $sth = \db()->prepare($sql);
        $sth->setFetchMode(\PDO::FETCH_CLASS, '\Clientes\Clientes');
        $sth->execute(array($clave_cliente));
	...
	function recoveryCliente($data)
    {
        ...
        $sql = "UPDATE c_destinatarios SET Activo = '1' WHERE Cve_Clte IN (SELECT Cve_Clte FROM c_cliente WHERE id_cliente = '" . $data['id_cliente'] . "');";
        $this->delete = \db()->prepare($sql);
        $this->delete->execute(array(
            $data['id_cliente']
        ));
  ...
  function getClientesAnalisisVentas($cve_almacen)
  {
      /*
              ...
              SELECT DISTINCT * FROM (
                  ...
                  SELECT DISTINCT d.id_destinatario as CodCliente, d.Cve_Clte as Cve_Clte, d.razonsocial as Nombre
                  FROM V_Cabecera_Pedido p
                  LEFT JOIN c_destinatarios d ON d.id_destinatario = p.Cod_Cliente
                  WHERE p.IdEmpresa = '{$cve_almacen}' AND p.Cancelada = 0
                ) as clientes_ventas
                  ";
      */
      $sql = "SELECT d.id_destinatario as CodCliente, d.Cve_Clte as Cve_Clte, d.razonsocial as Nombre
                  FROM c_cliente c 
                  LEFT JOIN c_destinatarios d ON d.Cve_Clte = c.Cve_Clte
                  WHERE c.Cve_Almacenp = $cve_almacen ";
