...
class AdministradorPedidos 
{
  ...
  //EDG117
  function verificarStatus($folio)
  {
	...
	function loadArticulos($id,$almacen,$status_pedido, $sufijo, $is_backorder) 
  {
            ...
            if($status == 'A')
            {
                if($is_backorder == 0)
                {
                  ...
                  $sql = "
                      ...
                      WHERE td_pedido.Fol_folio = '$id' GROUP BY clave
                  ";*/
                  $sql = "
                        SELECT 
    ...
    if($surtiendo)
    {
      ...
      $sql_pendientes = "
          ...
          WHERE X.surtidas = 0
      ";
      $sth = \db()->prepare($sql_pendientes);
      $sth->execute();

      $res[0]=$sth->fetchAll();
      $res[2]="status_pedido = ".$status_pedido."...SSSS".$sql_pendientes;
      $res[3]=$status;
  ...
  {	
    ...
    function consecutivo_folio_backorder() 
    {
      ...
      while(true)
      {
          ...
          $sql = "SELECT COUNT(*) as Consecutivo FROM th_backorder WHERE Fol_Folio = '$folio_next'";
          $sth = \db()->prepare( $sql );
          $sth->execute();
          $data = $sth->fetch();
