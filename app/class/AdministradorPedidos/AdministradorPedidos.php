...
class AdministradorPedidos 
{
  ...
  function verificarStatus($folio, $is_backorder, $sufijo)
  {
	...
	function loadArticulos($id,$almacen,$status_pedido, $sufijo, $is_backorder, $folio_rel, $con_recorrido, $TipoDoc, $TipoOT) 
  {
    ...
    $sql_tipo_pedido = "SELECT TipoPedido, Observaciones, TipoDoc FROM th_pedido WHERE Fol_folio = '{$id}'";
    $sth = \db()->prepare($sql_tipo_pedido);
    $sth->execute();
    $row_pedido = $sth->fetch();
    $tipo_pedido = $row_pedido[0];
    $observaciones = $row_pedido[1];
    $tipoDoc = $row_pedido[2];

    $utf8Sql = \db()->prepare("SET NAMES 'utf8mb4';");
    $utf8Sql->execute();

/*
    $SQLNoSurtible = " AND IFNULL(a.tipo_producto, '') != 'ProductoNoSurtible' ";
    if($tipo_pedido == 'RI' || $tipo_pedido == 'P' || $tipo_pedido == 'R')
    {
        $SQLNoSurtible = "";
    }
*/
...
*/
    ...
    $sqlExistencia = " VS_ExistenciaParaSurtido ";
    $andUbica = " V_ExistenciaGral.idy_ubica ";
    if($tipo_pedido == 'R' || $tipo_pedido == 'RI' || $con_recorrido == 0)
    {
        $sqlExistencia = " V_ExistenciaGralProduccion ";
        $andUbica = " V_ExistenciaGral.cve_ubicacion ";
    }
        ...
        $sql = "";
            if($status == 'A')
            {
                if($is_backorder == 0 && $sufijo == 0)
                {
                    if($folio_rel)
                    {
                        $sql = "
                                SELECT 
                                  ...
                                  IF(a.control_lotes = 'S' AND (a.Caduca = 'S' OR (th_pedido.Ship_Num != '' AND IFNULL(a.Caduca, 'N') = 'N')) AND IFNULL(tsb.cve_lote, '') != '', IF(DATE_FORMAT(lo.Caducidad, '%d-%m-%Y') = DATE_FORMAT('00-00-0000', '%d-%m-%Y'), '', DATE_FORMAT(lo.Caducidad, '%d-%m-%Y')), '') AS caducidad,
                                  ...
                                  IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(((SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1) LIKE 'OT%'), IFNULL(a.peso, 0) * tsb.Num_cantidad, SUM(ROUND((IFNULL(a.peso, 0) * tsb.Num_cantidad),4))), IFNULL(a.peso, 0) * tsb.Num_cantidad) AS peso,
                                  SUM(ROUND(((a.alto/1000) * (a.ancho/1000) * (a.fondo/1000) * IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)),4)) AS volumen,
                                  #SUM(tsb.Num_cantidad) AS pedidas,
                                  IFNULL(tdt.Num_cantidad, tsb.Num_cantidad) AS pedidas,
                                  #IFNULL(s.Cantidad, 0) AS surtidas,
                                  IFNULL(tdt.Num_cantidad, tsb.Num_cantidad) AS surtidas,
                    ...
                    AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad >= CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0) >= IFNULL(tdt.Num_cantidad, tsb.Num_cantidad), IFNULL(tdt.Num_cantidad, tsb.Num_cantidad), COALESCE((SELECT SUM(Existencia) AS suma FROM  V_ExistenciaGral LEFT JOIN c_ubicacion u ON u.idy_ubica = V_ExistenciaGral.cve_ubicacion WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo 
                    AND u.picking = 'S'
                    #AND V_ExistenciaGral.cve_lote = IFNULL(tsb.cve_lote, '') 
                    ...
                    IFNULL(tsb.Proyecto, '') as proyecto
            ...
            #IF(COALESCE((SELECT SUM(Existencia*a.num_multiplo) AS suma 
                    ...
                    #AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad > CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0) >= tsb.Num_cantidad, tsb.Num_cantidad, COALESCE((SELECT SUM(Existencia) AS suma FROM VS_ExistenciaParaSurtido V_ExistenciaGral WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo AND V_ExistenciaGral.cve_almac = '{$almacen}' AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad > CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0))) AS existencia
                            ...
                            FROM td_pedido tsb
                              ...
                              LEFT JOIN td_pedidoxtarima tdt ON tdt.Fol_folio = tsb.Fol_folio AND tdt.Cve_articulo = tsb.Cve_articulo AND IFNULL(tdt.cve_lote, '') = IFNULL(tsb.cve_lote, '')
                            ...
                            WHERE tsb.Fol_folio = '{$id}' {$SQLNoSurtible}  
                            AND th_pedido.cve_almac = '{$almacen}' 
                            GROUP BY clave, lote, serie, LP 
                            ORDER BY LP, clave 
                        ";
                }
                else if($TipoDoc == 'lp_ot')
                {
                        $sql = "
                                SELECT 
                                  tsb.Fol_folio AS folio,
                                  ...
                                  '' AS ruta,
                                    ...
                                    IFNULL(a.num_multiplo, 0) piezasxcajas, 
                                  IF(a.control_lotes = 'S', tsb.cve_lote, '') AS lote,
                                  IF(a.control_lotes = 'S' AND a.Caduca = 'S' AND IFNULL(tsb.cve_lote, '') != '', IF(DATE_FORMAT(lo.Caducidad, '%d-%m-%Y') = DATE_FORMAT('00-00-0000', '%d-%m-%Y'), '', DATE_FORMAT(lo.Caducidad, '%d-%m-%Y')), '') AS caducidad,
                                  IF(a.control_numero_series = 'S', tsb.cve_lote, '') AS serie,
                                  #SUM(ROUND((a.peso * tsb.Num_cantidad),4)) AS peso,
                                  #IF(op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0, SUM(ROUND((a.peso * tsb.Num_cantidad),4)), tsb.Num_cantidad) AS peso,
                                  IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(((SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1) LIKE 'OT%'), IFNULL(a.peso, 0) * tsb.Num_cantidad*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)), SUM(ROUND((IFNULL(a.peso, 0) * tsb.Num_cantidad*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))),4))), IFNULL(a.peso, 0) * tsb.Num_cantidad*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))) AS peso,
                    ...
                    AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad >= CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0) >= (tsb.Num_cantidad*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))), (tsb.Num_cantidad*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))), COALESCE((SELECT SUM(Existencia) AS suma FROM VS_ExistenciaParaSurtido V_ExistenciaGral LEFT JOIN c_ubicacion u ON u.idy_ubica = V_ExistenciaGral.idy_ubica WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo 
                        ...
                        IFNULL(tsb.Proyecto, '') as proyecto
                            ...
                            FROM td_pedido tsb
                              ...
                              LEFT JOIN c_lotes lo ON lo.cve_articulo = a.cve_articulo AND lo.Lote = tsb.cve_lote
                              LEFT JOIN c_serie se ON se.cve_articulo = a.cve_articulo AND se.numero_serie = tsb.cve_lote
                              LEFT JOIN c_charolas ch ON ch.IDContenedor = tsb.id_ot
                              LEFT JOIN PRegalado pr ON (pr.Docto = tsb.Fol_folio OR pr.Docto IN (SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = '$id')) AND pr.SKU = tsb.Cve_articulo
                              #LEFT JOIN td_surtidopiezas s ON s.fol_folio = th_pedido.Fol_Folio AND s.Cve_articulo = tsb.Cve_Articulo 
                              #LEFT JOIN t_recorrido_surtido t ON t.fol_folio = th_pedido.Fol_folio AND t.Cve_articulo = tsb.Cve_articulo 
                              LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                            WHERE tsb.Fol_folio = '{$id}' {$SQLNoSurtible} 
                ...
                {
                    $sql_left = "";
                    $sql_where = " tsb.Fol_folio = '{$id}' ";
                    //if($TipoOT == 'WS2')
                    //{
                    //    $sql_left = " LEFT JOIN t_consolidado tc ON tc.Fol_PedidoCon = tsb.Fol_folio ";
                    //    $sql_where = " tc.Fol_Consolidado = '{$id}' ";
                    //}
                        $sql = "
                                SELECT 
                                  tsb.Fol_folio AS folio,
                                  ...
                                  #SUM(ROUND((a.peso * tsb.Num_cantidad),4)) AS peso,
                                  #IF(op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0, SUM(ROUND((a.peso * tsb.Num_cantidad),4)), tsb.Num_cantidad) AS peso,
                                  IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(((SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1) LIKE 'OT%'), IFNULL(a.peso, 0) * IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)), SUM(ROUND((IFNULL(a.peso, 0) *  IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))),4))), IFNULL(a.peso, 0) * IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))) AS peso,
                                  SUM(ROUND(((a.alto/1000) * (a.ancho/1000) * (a.fondo/1000) * IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1))),4)) AS volumen,
                                  #SUM(tsb.Num_cantidad) AS pedidas,
                                  (IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))) AS pedidas,
                                  #IFNULL(s.Cantidad, 0) AS surtidas,
                                  (IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))) AS surtidas,
                                  IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) AS id_medida,
                                  IFNULL(umed2.des_umed, umed.des_umed) AS unidad_medida,
                                  #tsb.Num_cantidad AS existencia
                                  #IF(COALESCE((SELECT SUM(Existencia) AS suma FROM VS_ExistenciaParaSurtido V_ExistenciaGral WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo AND V_ExistenciaGral.cve_almac = '{$almacen}' AND V_ExistenciaGral.cve_lote = (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad > CURDATE())), 0) >= tsb.Num_cantidad, tsb.Num_cantidad, COALESCE((SELECT SUM(Existencia) AS suma FROM VS_ExistenciaParaSurtido V_ExistenciaGral WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo AND V_ExistenciaGral.cve_almac = '{$almacen}' AND V_ExistenciaGral.cve_lote = (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad > CURDATE())), 0)) AS existencia
                        ...
                        IF(IFNULL(tsb.cve_lote, '') = '',
                                IF(COALESCE((SELECT SUM(Existencia) AS suma 
                    FROM {$sqlExistencia} V_ExistenciaGral 
                    LEFT JOIN c_ubicacion u ON u.idy_ubica = $andUbica
                    WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo #AND V_ExistenciaGral.cve_lote = IFNULL(tsb.cve_lote, '') 
                    AND u.picking = 'S'
                    AND V_ExistenciaGral.cve_almac = '{$almacen}' 
                    AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad >= CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0) >= (IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))), (IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))), COALESCE((SELECT SUM(Existencia) AS suma FROM {$sqlExistencia} V_ExistenciaGral LEFT JOIN c_ubicacion u ON u.idy_ubica = $andUbica WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo 
                        #AND V_ExistenciaGral.cve_lote = IFNULL(tsb.cve_lote, '') 
                        AND u.picking = 'S'
                        AND V_ExistenciaGral.cve_almac = '{$almacen}' AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad >= CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0))
                        , 
                    ...
                    AND ((V_ExistenciaGral.cve_lote IN (SELECT Lote FROM c_lotes WHERE cve_articulo = V_ExistenciaGral.cve_articulo AND Caducidad >= CURDATE())) OR (V_ExistenciaGral.cve_articulo = a.cve_articulo AND IFNULL(a.Caduca, 'N') = 'N'))), 0) >= (IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))), (IFNULL(tdt.Num_cantidad, tsb.Num_cantidad)*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))), COALESCE((SELECT SUM(Existencia) AS suma FROM {$sqlExistencia} V_ExistenciaGral LEFT JOIN c_ubicacion u ON u.idy_ubica = $andUbica WHERE V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo 
                        ...
                        IFNULL(tsb.Proyecto, '') as proyecto
                            ...
                            AND (th_pedido.cve_almac = '{$almacen}' OR th_pedido.statusaurora = '{$almacen}') AND (tsb.Num_cantidad*(IF(IFNULL(umed2.mav_cveunimed, umed.mav_cveunimed) = 'XBX', a.num_multiplo, 1)) - IF(pr.Tipmed = 'Caja', IFNULL(pr.Cant*a.num_multiplo, 0), IFNULL(pr.Cant, 0))) > 0
                            {$SQLNoSurtible} 
                            GROUP BY clave, lote, serie, LP 
                            ORDER BY LP, clave  
                        ";
              ...
              $sql = "
                    SELECT 
                          ...
                          IF(t.cve_lote != '', IF(((SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = t.cve_lote AND tsb.Cve_articulo = cve_articulo) != DATE_FORMAT('00-00-0000', '%d-%m-%Y')) AND (a.control_lotes = 'S'),  (SELECT Caducidad FROM c_lotes WHERE LOTE = t.cve_lote AND tsb.Cve_articulo = cve_articulo), '' ), '') AS caducidad,

                          IF(t.cve_lote != '', IF(a.control_numero_series = 'S',  t.cve_lote, ''), '') AS serie,
                          #SUM(ROUND((a.peso * tsb.Num_cantidad),4)) AS peso,
                          #IF(op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0, SUM(ROUND((a.peso * tsb.Num_cantidad),4)), tsb.Num_cantidad) AS peso,
                          IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(((SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1) LIKE 'OT%'), IFNULL(a.peso, 0) * tsb.Num_cantidad, SUM(ROUND((a.peso * tsb.Num_cantidad),4))), tsb.Num_cantidad) AS peso,
                    ...
                    FROM td_subpedido tsb 
                      LEFT JOIN c_articulo a ON a.cve_articulo = tsb.Cve_articulo
                      LEFT JOIN td_pedido td ON td.Cve_articulo = a.cve_articulo AND IFNULL(td.cve_lote, '') = IFNULL(tsb.Cve_Lote, '') AND td.Fol_folio = tsb.fol_folio 
                      LEFT JOIN c_unimed umed ON umed.id_umed = a.unidadMedida
                      LEFT JOIN c_unimed umed2 ON umed2.id_umed = td.id_unimed
                      LEFT JOIN th_pedido ON th_pedido.Fol_folio = tsb.fol_folio
                      LEFT JOIN c_cliente c ON c.Cve_Clte = th_pedido.Cve_clte
                      LEFT JOIN td_surtidopiezas s ON s.fol_folio = th_pedido.Fol_Folio AND s.Cve_articulo = tsb.Cve_Articulo 
                      LEFT JOIN (
                      ...
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo {$SQLNoSurtible} 
                    GROUP BY clave 
                    ORDER BY IFNULL(t.orden_secuencia, a.cve_articulo)
              ";//AND t.cve_lote = tsb.cve_lote
                    ...
                    $sql = "
                        SELECT 
                              ...
                              (SELECT Descripcion FROM V_RutaSurtido_Usuario WHERE orden_secuencia = t.orden_secuencia AND cve_usuario = t.cve_usuario AND idy_ubica = t.idy_ubica) AS ruta,
                              IF(t.cve_lote != '', IF(a.control_lotes = 'S',  t.cve_lote, ''), '') AS lote,
                              IF(t.cve_lote != '', IF(((SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = t.cve_lote AND td_backorder.Cve_Articulo = cve_articulo) != DATE_FORMAT('00-00-0000', '%d-%m-%Y')) AND (a.control_lotes = 'S'),  (SELECT Caducidad FROM c_lotes WHERE LOTE = t.cve_lote AND td_backorder.Cve_Articulo = cve_articulo), '' ), '') AS caducidad,
                              IF(t.cve_lote != '', IF(a.control_numero_series = 'S',  t.cve_lote, ''), '') AS serie,
                          ...
                          WHERE th_backorder.Fol_Folio = '{$id}' {$SQLNoSurtible} 
...
$sth_charset = \db()->prepare($sql_charset);
$sth_charset->execute();
*/
      //#LEFT JOIN t_recorrido_surtido on t_recorrido_surtido.fol_folio = td_pedido.Fol_folio
            //echo "status = ".$status."status_pedido = ".$status_pedido."...AAAA".$sql;
            //echo \db()->prepare($sql);
//echo $sql;
      $sth = \db()->prepare($sql);
      $sth->execute();
    ...
    if($surtiendo)
    {
      ...
      */
        ...
        if($is_backorder == 0 && $sufijo > 0)
        {
            ...
            if($con_recorrido == 1 || $tipo_pedido == 'R' || $tipo_pedido == 'RI' || $TipoDoc == 'tipo_lp')
            {
                    $sqlExistenciaGral = "V_ExistenciaGral v";
                    if($tipo_pedido == 'R' || $tipo_pedido == 'RI')
                        $sqlExistenciaGral = "V_ExistenciaGralProduccion v";
              ...
              $sql = "
                SELECT * FROM (
                    SELECT 
                          ...
                          tsb.Cve_articulo AS clave,
                          a.des_articulo AS articulo,
                          #IFNULL(IFNULL(t.cve_usuario, t_sin_lote.cve_usuario),'') AS surtidor,
                          IFNULL(IFNULL(IFNULL(t.cve_usuario, t_sin_lote.cve_usuario),ths.Cve_Usuario), '') AS surtidor,
                          IFNULL(IFNULL(t.claverp, t_sin_lote.claverp),'') AS ubicacion,
                          IFNULL(IFNULL(t.idy_ubica, t_sin_lote.idy_ubica), '') AS idy_ubica,
                          IFNULL(IFNULL(t.orden_secuencia, t_sin_lote.orden_secuencia), '') AS secuencia,
                          IFNULL(a.control_peso, 'N') as control_peso,
                            ...
                            IFNULL(a.cajas_palet, 0) cajasxpallets,
                            IFNULL(a.num_multiplo, 0) piezasxcajas, 
                            IFNULL(ch.IDContenedor, IFNULL(t.ClaveEtiqueta, t_sin_lote.ClaveEtiqueta)) AS LPNtarima,
                          ...
                          IF((op.id_umed = a.unidadMedida OR IFNULL(op.id_umed, 0) = 0), IF(('OT' LIKE (SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = tsb.fol_folio LIMIT 1)), SUM(ROUND((a.peso * TRUNCATE(IFNULL(t.Cantidad,t_sin_lote.Cantidad), 4)),4)), ROUND((a.peso * TRUNCATE(IFNULL(t.Cantidad,t_sin_lote.Cantidad), 4)),4)), ROUND((a.peso * TRUNCATE(IFNULL(t.Cantidad,t_sin_lote.Cantidad), 4)),4)) AS peso,
                          SUM(ROUND(((a.alto/1000) * (a.ancho/1000) * (a.fondo/1000) * tsb.Num_cantidad),4)) AS volumen,
                          #SUM(tsb.Num_cantidad) AS pedidas,
                          ...
                          #IFNULL(s.Cantidad, 0) AS surtidas,
                          IF(IFNULL(t.claverp, t_sin_lote.claverp) IS NOT NULL, 0, IF(IFNULL(ch.IDContenedor, '')='' OR IFNULL(s.Cantidad, '')='', IFNULL(s.Cantidad, 0), tdt.Num_cantidad-IFNULL((SELECT DISTINCT Cantidad FROM t_recorrido_surtido WHERE fol_folio = tsb.fol_folio AND Sufijo = tsb.Sufijo AND ClaveEtiqueta = ch.IDContenedor AND cve_articulo = a.cve_articulo AND IFNULL(cve_lote, '') = IFNULL(s.LOTE, '')), 0))) AS surtidas,
                    ...
                    FROM td_subpedido tsb
                      ...
                      LEFT JOIN c_unimed umed ON umed.id_umed = a.unidadMedida
                      LEFT JOIN th_pedido ON th_pedido.Fol_folio = tsb.Fol_folio
                      LEFT JOIN th_subpedido ths ON th_pedido.Fol_folio = ths.Fol_folio AND ths.sufijo = $sufijo
                      LEFT JOIN c_cliente c ON c.Cve_Clte = th_pedido.Cve_clte
                      LEFT JOIN td_pedidoxtarima tdt ON tdt.Fol_folio = tsb.Fol_folio AND tdt.Cve_articulo = tsb.Cve_articulo AND IFNULL(tdt.cve_lote, '') = IFNULL(tsb.cve_lote, '')
                      #LEFT JOIN t_recorrido_surtido tlp ON tlp.fol_folio = th_pedido.Fol_folio AND tlp.Cve_articulo = tsb.Cve_articulo AND tlp.cve_lote = tsb.cve_lote AND tlp.Sufijo = $sufijo 
                      #LEFT JOIN c_charolas ch ON ch.IDContenedor IN (tdt.nTarima, tlp.ClaveEtiqueta)
                      LEFT JOIN c_charolas ch ON ch.IDContenedor = tdt.nTarima 
                      LEFT JOIN td_surtidopiezas s ON s.fol_folio = th_pedido.Fol_Folio AND s.Cve_articulo = tsb.Cve_Articulo 
                      LEFT JOIN (
                          SELECT 
                          v.cve_articulo,
                          c_ubicacion.CodigoCSD,
                          MIN(v.cve_ubicacion) AS MIN 
                          FROM {$sqlExistenciaGral} 
                          LEFT JOIN c_ubicacion ON v.cve_ubicacion = c_ubicacion.idy_ubica 
                          GROUP BY cve_articulo
                      ) X ON X.cve_articulo = tsb.Cve_articulo
                      LEFT JOIN t_recorrido_surtido t ON t.fol_folio = th_pedido.Fol_folio AND t.Cve_articulo = tsb.Cve_articulo AND t.cve_lote = tsb.cve_lote AND t.Sufijo = $sufijo AND (t.ClaveEtiqueta IN (IFNULL(ch.IDContenedor, 0), IFNULL(ch.IDContenedor, 1)) )  # OR t.ClaveEtiqueta > 0
                      LEFT JOIN t_recorrido_surtido t_sin_lote ON t_sin_lote.fol_folio = th_pedido.Fol_folio AND t_sin_lote.Cve_articulo = tsb.Cve_articulo AND t_sin_lote.Sufijo = $sufijo AND (t_sin_lote.ClaveEtiqueta IN (IFNULL(ch.IDContenedor, 0), IFNULL(ch.IDContenedor, 1)) ) #OR t_sin_lote.ClaveEtiqueta > 0
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    #LEFT JOIN c_charolas tarima ON tarima.IDContenedor = IFNULL(t.ClaveEtiqueta, t_sin_lote.ClaveEtiqueta)
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo AND IFNULL(IFNULL(t.claverp, t_sin_lote.claverp),'') != '' {$SQLNoSurtible} 
                    #GROUP BY t.claverp,t.Cve_articulo, serie, lote, existencia, ubicacion, clave
                    GROUP BY IFNULL(t.claverp,t_sin_lote.claverp), IFNULL(t.cve_lote, t_sin_lote.cve_lote), IFNULL(t.idy_ubica, t_sin_lote.idy_ubica), IFNULL(t.Cve_articulo, t_sin_lote.Cve_articulo), IFNULL(t.fol_folio, t_sin_lote.fol_folio), IFNULL(t.ClaveEtiqueta, t_sin_lote.ClaveEtiqueta), LP
                    ...
                    FROM td_subpedido tsb
                      ...
                      LEFT JOIN t_recorrido_surtido t ON t.fol_folio = th_pedido.Fol_folio AND t.Cve_articulo = tsb.Cve_articulo AND t.cve_lote = tsb.cve_lote AND t.Sufijo = $sufijo AND t.ClaveEtiqueta > 0
                      LEFT JOIN t_recorrido_surtido t_sin_lote ON t_sin_lote.fol_folio = th_pedido.Fol_folio AND t_sin_lote.Cve_articulo = tsb.Cve_articulo AND t_sin_lote.Sufijo = $sufijo AND t_sin_lote.ClaveEtiqueta > 0 
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    #LEFT JOIN c_charolas tarima ON tarima.IDContenedor = IFNULL(t.ClaveEtiqueta, t_sin_lote.ClaveEtiqueta)
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo {$SQLNoSurtible} AND tdt.Fol_folio IS NULL
                    #GROUP BY t.claverp,t.Cve_articulo, serie, lote, existencia, ubicacion, clave
                    GROUP BY IFNULL(t.claverp,t_sin_lote.claverp), IFNULL(t.cve_lote, t_sin_lote.cve_lote), IFNULL(t.idy_ubica, t_sin_lote.idy_ubica), IFNULL(t.Cve_articulo, t_sin_lote.Cve_articulo), IFNULL(t.fol_folio, t_sin_lote.fol_folio), IFNULL(t.ClaveEtiqueta, t_sin_lote.ClaveEtiqueta), LP

                    ORDER BY secuencia, LP ASC
                    ) as s WHERE IFNULL(s.idy_ubica, '') != ''
              ";//AND t.cve_lote = tsb.cve_lote
          }
...
/*
              $sql = "SELECT * FROM (
                    SELECT 
                          ...
                          IF(IFNULL(t.cve_lote, t_sin_lote.cve_lote) != '', IF(a.control_lotes = 'S',  IFNULL(t.cve_lote, t_sin_lote.cve_lote), ''), '') AS lote,
                          IF(a.Caduca = 'S', IF(IFNULL(t.cve_lote, t_sin_lote.cve_lote) != '', IF(((SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = IFNULL(t.cve_lote, t_sin_lote.cve_lote) AND tsb.Cve_articulo = cve_articulo) != DATE_FORMAT('00-00-0000', '%d-%m-%Y')) AND (a.control_lotes = 'S'),  (SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = IFNULL(t.cve_lote, t_sin_lote.cve_lote) AND tsb.Cve_articulo = cve_articulo), '' ), ''), '') AS caducidad,
                          IF(IFNULL(t.cve_lote, t_sin_lote.cve_lote) != '', IF(a.control_numero_series = 'S',  IFNULL(t.cve_lote, t_sin_lote.cve_lote), ''), '') AS serie,
                    ...
                    FROM td_subpedido tsb
                      ...
                      LEFT JOIN (
                        ...
                        LEFT JOIN VS_ExistenciaParaSurtido t ON t.cve_articulo = tsb.Cve_articulo AND t.cve_lote = tsb.cve_lote AND tsb.Sufijo = $sufijo AND t.Cve_Almac = th_pedido.cve_almac
                        LEFT JOIN VS_ExistenciaParaSurtido t_sin_lote ON t_sin_lote.Cve_articulo = tsb.Cve_articulo AND tsb.Sufijo = $sufijo AND t_sin_lote.Cve_Almac = th_pedido.cve_almac
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo AND tsb.Cve_articulo = IFNULL(t.cve_articulo, t_sin_lote.cve_articulo) {$SQLNoSurtible} 
...
*/
            $sql = "SELECT * FROM (
                    SELECT 
                          ...
                          tsb.Cve_articulo AS clave,
                          a.des_articulo AS articulo,
                          (SELECT cve_usuario FROM th_subpedido WHERE fol_folio = '{$id}' AND Sufijo = tsb.Sufijo) AS surtidor,
                          ...
                          IFNULL(a.control_peso, 'N') AS control_peso,
                            ...
                            IFNULL(ch.CveLP, '') AS LP,
                                    IFNULL(a.cajas_palet, 0) cajasxpallets,
                                    IFNULL(a.num_multiplo, 0) piezasxcajas, #IFNULL(t.cve_usuario, t_sin_lote.cve_usuario)
                          (SELECT Descripcion FROM V_RutaSurtido_Usuario WHERE cve_usuario = (SELECT cve_usuario FROM th_subpedido WHERE fol_folio = '{$id}' AND Sufijo = tsb.Sufijo) AND idy_ubica = t.cve_ubicacion AND Cve_Almac = (SELECT cve_almac FROM c_ubicacion WHERE idy_ubica = t.cve_ubicacion)) AS ruta,
                          IF(t.cve_lote != '', IF(a.control_lotes = 'S',  t.cve_lote, ''), '') AS lote,
                    ...
                    FROM td_subpedido tsb
                     ...
                     #     V_ExistenciaGral.cve_articulo,
                     #     c_ubicacion.CodigoCSD,
                     #     V_ExistenciaGral.Cve_Contenedor,
                     #     MIN(V_ExistenciaGral.cve_ubicacion) AS MIN 
                     #     FROM V_ExistenciaGral 
                     #    LEFT JOIN c_ubicacion ON V_ExistenciaGral.cve_ubicacion = c_ubicacion.idy_ubica 
                     #     GROUP BY cve_articulo
                     # ) X ON X.cve_articulo = tsb.Cve_articulo
                        ...
                        LEFT JOIN V_ExistenciaGral t ON t.cve_articulo = tsb.Cve_articulo AND tsb.Sufijo = $sufijo AND t.Cve_Almac = th_pedido.cve_almac AND t.tipo = 'ubicacion' #AND t.cve_lote = tsb.cve_lote
                        LEFT JOIN ts_existenciatarima ts ON ts.cve_articulo = tsb.Cve_articulo AND IFNULL(ts.lote, '') = IFNULL(tsb.cve_lote, '') AND tsb.Sufijo = 1 AND t.Cve_Almac = th_pedido.cve_almac AND t.cve_ubicacion = ts.idy_ubica
                        LEFT JOIN c_charolas ch ON ch.clave_contenedor = t.Cve_Contenedor
                      LEFT JOIN t_ordenprod op ON op.Folio_Pro = tsb.fol_folio
                    WHERE tsb.Fol_folio = '{$id}' AND tsb.Sufijo = $sufijo AND tsb.Cve_articulo = t.cve_articulo  
                    {$SQLNoSurtible} 
                    #AND th_pedido.Cve_Almac = X.cve_almac
                    #AND th_pedido.Cve_Almac in (t_sin_lote.Cve_Almac, t.Cve_Almac)
                    GROUP BY ubicacion, serie, lote, existencia, ubicacion, clave
              ...
              if($status != 'S')
              {
                $sql = "SELECT 
                          ...
                          #'' AS lote,
                          IF(s.LOTE != '', IF(((SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = s.LOTE AND tsb.Cve_articulo = cve_articulo) != DATE_FORMAT('00-00-0000', '%d-%m-%Y')) AND (a.control_lotes = 'S'),  (SELECT DATE_FORMAT(Caducidad, '%d-%m-%Y') FROM c_lotes WHERE LOTE = s.LOTE AND tsb.Cve_articulo = cve_articulo), '' ), '') AS caducidad,
                          #'' AS caducidad,
                          ...
                          #SUM(tsb.Num_cantidad) AS pedidas,
                          tsb.Num_cantidad AS pedidas,
                          IFNULL(s.Cantidad, tsb.Num_cantidad) AS surtidas
                          #tsb.Num_cantidad AS existencia
                          #IF(COALESCE((SELECT SUM(Existencia) AS suma FROM V_ExistenciaGral WHERE V_ExistenciaGral.cve_ubicacion = t.idy_ubica AND V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo AND V_ExistenciaGral.cve_almac = '{$almacen}'), 0) >= tsb.Num_cantidad, tsb.Num_cantidad, COALESCE((SELECT SUM(Existencia) AS suma FROM V_ExistenciaGral WHERE V_ExistenciaGral.cve_ubicacion = t.idy_ubica AND V_ExistenciaGral.cve_articulo = tsb.Cve_Articulo AND V_ExistenciaGral.cve_almac = '{$almacen}'), 0)) AS existencia
                    FROM td_subpedido tsb
                      LEFT JOIN c_articulo a ON a.cve_articulo = tsb.Cve_articulo
                    ...
                    #AND X.cve_almac = th_pedido.Cve_Almac 
                    AND s.cve_almac = th_pedido.Cve_Almac 
                    {$SQLNoSurtible} 
          ...
          $sql = "
              ...
              WHERE td_pedido.Fol_folio = '$id' {$SQLNoSurtible} 
          ";
            }
...
//*********************************************************************************************************
          $sql = "
                SELECT DISTINCT pr.SKU AS cve_articulo, a.des_articulo, CONCAT('(', r.cve_ruta, ') ', r.descripcion) AS ruta, 
                ...
                WHERE  al.id = {$almacen} AND ('$id' = pr.Docto OR pr.Docto IN (SELECT DISTINCT Fol_Folio FROM td_consolidado WHERE Fol_PedidoCon = '$id'));
          ";
...
//*********************************************************************************************************
  ...
  {
      $sql = "
          SELECT fol_folio, Sufijo, Cve_articulo, Cve_Lote, Num_cantidad FROM td_subpedido WHERE fol_folio = '{$id_pedido}'
      ";
      $sth = \db()->prepare($sql);
      $sth->execute();
      $res = $sth->fetchAll();
