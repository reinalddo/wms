      ...
      try 
      {
        $sql = sprintf('
          SELECT DISTINCT
            ...
            c_gpoarticulo.des_gpoart as des_grupo,
            c_sgpoarticulo.des_sgpoart as des_clasificacion,
            c_ssgpoarticulo.des_ssgpoart as des_tipo,
            ...
            ra.StockMax as stock_maximo,
            ra.StockMin as stock_minimo,
            ...
            c_articulo.ID_Proveedor as empresa,
            GROUP_CONCAT(DISTINCT rap.Id_Proveedor ORDER BY rap.Id_Proveedor) AS ID_Proveedor,
          ...
          LEFT JOIN rel_articulo_proveedor rap ON rap.cve_articulo = c_articulo.cve_articulo
          ...
          LEFT JOIN c_gpoarticulo ON c_gpoarticulo.cve_gpoart = c_articulo.grupo AND c_gpoarticulo.id_almacen = ra.Cve_Almac
          LEFT JOIN c_sgpoarticulo ON c_sgpoarticulo.cve_sgpoart = c_articulo.clasificacion AND c_sgpoarticulo.id_almacen = ra.Cve_Almac
          LEFT JOIN c_ssgpoarticulo ON c_ssgpoarticulo.cve_ssgpoart = c_articulo.tipo AND c_ssgpoarticulo.id_almacen = ra.Cve_Almac
          WHERE c_articulo.id = ?
          AND ra.Cve_Almac = ?
        ',
        self::TABLE
...
"*/
    ...
    function save( $_post ) 
    {
      ...
      if($cve_proveedor[0] != "")
      {
        $sql_proveedor = "DELETE FROM rel_articulo_proveedor WHERE cve_articulo = '${cve_articulo}'";
        $sth = \db()->prepare($sql_proveedor);
        $sth->execute();

        foreach ($_post['cve_proveedor'] as $selectedOptionP)
        {
          $sql_proveedor = "INSERT INTO rel_articulo_proveedor(Cve_Articulo, Id_Proveedor) VALUES('${cve_articulo}', ${selectedOptionP})";
          $sth = \db()->prepare($sql_proveedor);
          $sth->execute();
        }
      }
...
*/
      ...
      */
      ...
      if($_post["empresa"]!="") 
      {
          ...
          $sql_proveedor = "UPDATE c_articulo SET ID_Proveedor = $empresa WHERE cve_articulo= '${cve_articulo}'";
          $sth = \db()->prepare($sql_proveedor);
          $sth->execute();
      }
    ...
    function save_Rel_Art_Almacen($data)
    {
      ...
      $sql =  "INSERT IGNORE INTO Rel_Articulo_Almacen(Cve_Almac, Cve_Articulo, Grupo_ID, Clasificacion_ID, Tipo_Art_ID, StockMax, StockMin) VALUES ('${cve_almac}', '${cve_articulo}', '$id_grupo', '$id_clasificacion', '$id_tipo_art', '${stock_maximo}', '${stock_minimo}') ON DUPLICATE KEY UPDATE Grupo_ID = '$id_grupo', Clasificacion_ID = '$id_clasificacion' , Tipo_Art_ID = '$id_tipo_art'";
    ...
    function getAllArticulosQ($id_almacen = "") {
        try {
            ...
            if($id_almacen != '')
               $sql = " SELECT a.* 
                        FROM c_articulo a, Rel_Articulo_Almacen r 
                        WHERE a.Activo = 1 AND r.Cve_Almac = $id_almacen AND r.Cve_Articulo = a.cve_articulo
                ";
    ...
    function actualizarArticulos( $_post ) {
        try {
          ...
          empq_cveumed= :unidadMedida_empaque,
            ...
            $this->save->bindValue( ':cve_alt', $_post['cve_alt'], \PDO::PARAM_STR );
            ...
            $this->save->bindValue( ':empresa', $_post['empresa'], \PDO::PARAM_STR );
...
if($_post["fotos"][0] != 'noimage.jpg')
{
            ...
            if($_post['cve_proveedor'][0] != '')
            {
                $sql_proveedor = "DELETE FROM rel_articulo_proveedor WHERE Cve_Articulo = '${cve_articulo}'";
                $sth = \db()->prepare($sql_proveedor);
                $sth->execute();
                //foreach ($_post['cve_proveedor'] as $selectedOptionP)
                for($i = 0; $i < count($_post['cve_proveedor']); $i++)
                {
                    $sql = sprintf('
                          INSERT INTO rel_articulo_proveedor
                          SET Cve_Articulo = :cve_articulo,
                              Id_Proveedor = :proveedor_art');

                    $this->save = \db()->prepare($sql);
                    $this->save->bindValue( ':cve_articulo', $_post['cve_articulo'], \PDO::PARAM_STR );
                    $this->save->bindValue( ':proveedor_art', $_post['cve_proveedor'][$i], \PDO::PARAM_STR );
                    $this->save->execute();
                }
            }
...
*/
      ...
      $id_tipo_art = $id[0];
            ...
            $this->save->bindValue( ':stockmax', $_post['stock_maximo'], \PDO::PARAM_STR );
            $this->save->bindValue( ':stockmin', $_post['stock_minimo'], \PDO::PARAM_STR );
  ...
  /*      
          ...
          $existe_envase = $id[0];
*/
          $sql_envase = "DELETE FROM ProductoEnvase WHERE Producto = '$cve_articulo'";
          $sth = \db()->prepare($sql_envase);
          $sth->execute();
...
/*
        ...
        {
          //$sql_envase = "DELETE FROM ProductoEnvase WHERE Producto = '$cve_articulo' AND Envase = '$envase1'";
          //$sth = \db()->prepare($sql_envase);
          //$sth->execute();
...
*/
        ...
        {
          //$sql_envase = "DELETE FROM ProductoEnvase WHERE Producto = '$cve_articulo' AND Envase = '$envase2'";
          //$sth = \db()->prepare($sql_envase);
          //$sth->execute();
...
*/
    ...
    function ExisteCodigoBarras($data) 
    {
      $clave_art = $data['cve_articulo'];
      $codBarraProducto = $data["cve_codprov"];
      $codBarraCaja     = $data["barras2"];
      $codBarraPallet   = $data["barras3"];

//*****************************************************************************************************************
//*****************************************************************************************************************
      $sql="SELECT COUNT(*) as existe FROM c_articulo WHERE cve_articulo = '{$clave_art}'";
      $sth = \db()->prepare($sql);
      $sth->execute();
      $existe = $sth->fetch();
      ...
      if($codBarraProducto != '') array_push($array_codigos, "'".$codBarraProducto."'");
      if($codBarraCaja != '') array_push($array_codigos, "'".$codBarraCaja."'");
      if($codBarraPallet != '') array_push($array_codigos, "'".$codBarraPallet."'");
      $editando = false;
      if($codBarraProducto != '' || $codBarraCaja != '' || $codBarraPallet != '')
      {
          $SqlCodigosBarra = implode(",", $array_codigos);
          if($existe)//se está editando el artículo
          {
            $sql="SELECT count(*) as x FROM c_articulo WHERE ( (cve_codprov IN ($SqlCodigosBarra)) OR (barras2 IN ($SqlCodigosBarra)) OR (barras3 IN ($SqlCodigosBarra)) ) AND cve_articulo != '{$clave_art}'";
            $editando = true;
          }
          else //se está agregando un nuevo artículo
          {
            $sql="SELECT count(*) as x FROM c_articulo WHERE ( (cve_codprov IN ($SqlCodigosBarra)) OR (barras2 IN ($SqlCodigosBarra)) OR (barras3 IN ($SqlCodigosBarra)) )";
          }
          $sth = \db()->prepare($sql);
          $sth->execute();
          $resultado = $sth->fetch();

          if($editando == true && $resultado["x"] == 1)
            return true;
          else if($resultado["x"] >= 1) 
            return false; //alguno de los códigos de barra ya están ocupados
          else
            return true; //ningún código de barra se está usando
      }
      else
        return true;//los codigos de barra están todos vacíos y si se permite registrar/editar


//*****************************************************************************************************************
//*****************************************************************************************************************      
      //echo $clave_art." ::: ".$codBarraProducto." ::: ".$codBarraCaja." ::: ".$codBarraPallet;
      /*
          ...
          if($id[0] != "")
          {
            $sql="SELECT count(*) as x FROM `c_articulo` WHERE ( (cve_codprov IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja')) OR (barras2 IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja')) OR (barras3 IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja')) ) AND id != '{$id[0]}'";
          }
          else
          {
            $sql="SELECT count(*) as x FROM `c_articulo` WHERE ( (cve_codprov IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja') AND cve_codprov != '') OR (barras2 IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja') AND barras2 != '') OR (barras3 IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja') AND barras3 != '') OR (cve_articulo IN ('$codBarraProducto', '$codBarraPallet', '$codBarraCaja') AND cve_articulo != '') )";
          }
          ...
          if(
                ($resultado["x"] >= 1 && ($codBarraProducto != '' || $codBarraCaja != '' || $codBarraPallet != '')) || 
                ($codBarraProducto == $codBarraCaja && $codBarraProducto != '' && $codBarraCaja != '') || 
                ($codBarraProducto == $codBarraPallet && $codBarraProducto != '' && $codBarraPallet != '') || 
                ($codBarraCaja == $codBarraPallet && $codBarraPallet != '' && $codBarraCaja != '')
            )
          {
            return false;
          }
    ...
    function existe_mismo_almacen($data) 
    {
        $clave_art = $data['clave_producto'];
        $id_almacen = $data["id_almacen"];
