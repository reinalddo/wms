...
SELECT 
    ...
    cab.embarque AS folio, cab.FechaEmbarque, cab.FechaEnvio, GROUP_CONCAT(DISTINCT cab.cliente SEPARATOR '<br>') AS cliente, GROUP_CONCAT(DISTINCT cab.factura SEPARATOR '<br>' ) AS factura, GROUP_CONCAT(DISTINCT cab.pedido SEPARATOR '<br>' ) AS pedido,  IF(cab.cs-SUM(cab.cantidad)>0, cab.cs, SUM(cab.cantidad)) AS cantidad, COUNT(cab.caja) AS total_cajas, SUM(cab.peso) AS peso
FROM (
SELECT 
                  th_ordenembarque.ID_OEmbarque AS embarque,
                  th_pedido.Fol_folio AS folio,
                  DATE_FORMAT(th_ordenembarque.fecha, '%d-%m-%Y') AS FechaEmbarque,
                  IFNULL(DATE_FORMAT(IFNULL(th_ordenembarque.FechaEnvio, ''), '%d-%m-%Y'), '') AS FechaEnvio,
                  IFNULL(c_cliente.RazonSocial, '') AS cliente,
                  IFNULL(GROUP_CONCAT(DISTINCT th_pedido.Pick_Num), '') AS factura,
                  IFNULL(GROUP_CONCAT(DISTINCT th_pedido.Fol_folio), '') AS pedido,
...
#SELECT * FROM (
        SELECT DISTINCT o.embarque, o.folio, o.clave, o.Lote_Serie, o.caducidad, o.articulo, (o.cs-o.cantidad) AS cantidad, o.caja, '' AS lp, '' AS clave_contenedor, o.peso, o.guia, o.lp AS lp_comp, o.clave_contenedor AS cc_comp 
        FROM (
...
SELECT 
              ...
              IFNULL(IF(a.control_lotes = 'S' AND a.Caduca = 'S', DATE_FORMAT(c_lotes.Caducidad, '%d-%m-%Y'), ''), '') AS caducidad,
                  a.des_articulo AS articulo,
                  #tds.Cantidad AS cs,
                  tds.Cantidad/(SELECT IF(COUNT(*)=0, 1, COUNT(*)) FROM t_tarima tw WHERE tt.Fol_Folio = tw.Fol_Folio AND tt.cve_articulo = tw.cve_articulo AND IFNULL(tt.lote, '') = IFNULL(tw.lote, '')) AS cs,
                  #IFNULL(IFNULL(IFNULL(ppt.Num_cantidad, tt.cantidad), IFNULL(IF(a.control_lotes = 'S' AND IFNULL(c_lotes.Lote, '') = '', tds.Cantidad, (td_cajamixta.Cantidad)), (SELECT Num_cantidad FROM td_pedido WHERE Fol_folio = th_pedido.Fol_folio AND td_pedido.Cve_articulo = td_cajamixta.Cve_articulo AND td_pedido.cve_lote = td_cajamixta.Cve_Lote))), tds.Cantidad) AS cantidad,
                  IFNULL(tt.cantidad, 0) AS cantidad,
                  IFNULL(th_cajamixta.cve_tipocaja, 'Pallet') AS caja,
                  IFNULL(ch.CveLP, '') AS lp,
                  IFNULL(ch.clave_contenedor, '') AS clave_contenedor,
                  #'' AS lp,
          ...
          LEFT JOIN c_serie ON c_serie.cve_articulo = a.cve_articulo AND c_serie.numero_serie = tds.LOTE
            WHERE th_ordenembarque.ID_OEmbarque = '$id' AND a.cve_articulo = IFNULL(td_cajamixta.Cve_articulo, tds.Cve_articulo)  
            #AND IFNULL(ch.CveLP, ch.clave_contenedor) NOT IN (SELECT ClaveEtiqueta FROM t_recorrido_surtido WHERE fol_folio = th_pedido.Fol_folio) 
            AND th_pedido.TipoDoc != 'tipo_lp'
            AND IFNULL(IF(IFNULL(td_cajamixta.cve_lote, '') != '', td_cajamixta.cve_lote, ''), IFNULL(c_lotes.Lote, '')) = IFNULL(IF(IFNULL(td_cajamixta.cve_lote, '') != '', tds.LOTE, ''), '')
            AND IFNULL(tt.cantidad, 0) > 0 AND (tds.Cantidad - IFNULL(tt.cantidad, 0)) > 0
            GROUP BY clave, Lote_Serie, folio, lp
...
WHERE IFNULL(resp.lp_comp, resp.cc_comp) NOT IN (SELECT ClaveEtiqueta FROM t_recorrido_surtido WHERE fol_folio = resp.folio) AND resp.cantidad > 0 

            ";
*/
  
$sql = "SELECT DISTINCT o.embarque, o.folio, o.clave, o.Lote_Serie, o.caducidad, o.articulo, o.cantidad, o.caja, o.lp AS lp, o.clave_contenedor AS clave_contenedor, 
        o.peso, o.guia, o.lp AS lp_comp, o.clave_contenedor AS cc_comp 
        FROM (
SELECT DISTINCT
   oe.ID_OEmbarque AS embarque,
   oe.Fol_folio AS folio,
   k.cve_articulo AS clave,
   k.cve_lote AS Lote_Serie,
  IFNULL(IF(a.control_lotes = 'S' AND a.Caduca = 'S', DATE_FORMAT(c_lotes.Caducidad, '%d-%m-%Y'), ''), '') AS caducidad,
  a.des_articulo AS articulo,
  SUM(IF((SELECT COUNT(*) FROM t_surtido_origen WHERE folio = k.destino AND web_apk = 'WEB') = 0, k.cantidad, ABS(IF(IFNULL(k.ajuste, 0) = 0, k.cantidad, k.ajuste)))) AS cantidad,
  #IFNULL(ch.tipo, th_cajamixta.cve_tipocaja) AS caja,
  IFNULL(ch.tipo, '') AS caja,
  IFNULL(ch.CveLP, '') AS lp,
  IFNULL(ch.clave_contenedor, '') AS clave_contenedor,
  IFNULL(a.peso, 0) AS peso,
  #IFNULL(th_cajamixta.Guia, '') AS guia
  IFNULL((SELECT DISTINCT Guia FROM th_cajamixta WHERE fol_folio = oe.Fol_folio LIMIT 1), '') AS guia
  FROM td_ordenembarque oe 
  LEFT JOIN t_cardex k ON k.destino = oe.Fol_folio AND k.id_TipoMovimiento = 8
  LEFT JOIN t_MovCharolas m ON k.id = m.id_kardex AND m.Id_TipoMovimiento = 8
  LEFT JOIN td_pedidoxtarima pt ON pt.Fol_folio = k.destino and k.cve_articulo = pt.Cve_articulo AND pt.cve_lote = k.cve_lote 
  LEFT JOIN c_charolas ch ON ch.IDContenedor = IFNULL(pt.nTarima, m.ID_Contenedor)
  LEFT JOIN c_articulo a ON a.cve_articulo = k.cve_articulo
  #LEFT JOIN th_cajamixta ON th_cajamixta.fol_folio =oe.Fol_folio AND k.destino = th_cajamixta.fol_folio AND oe.Fol_folio = k.destino #AND th_cajamixta.cve_tipocaja != 0 #and th_cajamixta.Cve_CajaMix = min(th_cajamixta.Cve_CajaMix)
  #LEFT JOIN td_cajamixta ON td_cajamixta.Cve_CajaMix = th_cajamixta.Cve_CajaMix AND td_cajamixta.Cve_articulo = k.cve_articulo AND IFNULL(td_cajamixta.Cve_Lote, '') = IFNULL(k.cve_lote, '') 
  LEFT JOIN c_lotes ON c_lotes.Lote = k.cve_lote AND c_lotes.cve_articulo = a.cve_articulo
  LEFT JOIN c_serie ON c_serie.cve_articulo = a.cve_articulo AND c_serie.numero_serie = k.cve_lote
  #LEFT JOIN t_surtido_origen so ON so.folio = oe.Fol_folio
  WHERE oe.ID_OEmbarque = '$id' AND k.destino = oe.Fol_folio AND k.cve_articulo = a.cve_articulo
  GROUP BY folio, clave, Lote_Serie, caja, lp
  ) AS o
  ORDER BY folio, guia";

        $sqlSuma = "SELECT SUM(sm.peso) as peso, SUM(sm.cantidad) as cantidad, COUNT(DISTINCT sm.LP) AS cantidad_pallets FROM (". $sql .") as sm";
        ...
        $sthSuma = \db()->prepare($sqlSuma);
        $sthSuma->execute();
        $suma = $sthSuma->fetchAll();

        return array("header" => $header,"body" => $body, "sqlCabecera" => $sqlCabecera, "suma" => $suma, "sql" => $sql);
    }
...
*/
    ...
    function existenciaubica($almacen,$articulo, $zona, $bl, $contenedor, $cve_proveedor, $proveedor, $grupo, $clasificacion, $lp, $art_obsoletos, $mostrar_folios_excel_existencias, $existencia_cajas, $lote, $factura, $proyecto_existencias, $lote_alterno, $enviar_email)
    {
    $zona_produccion = "";
    $num_produccion = 0;
...
*/
    ...
    if($instancia == 'foam' || $instancia == 'dev')
    {
...
#        ) AS fecha_ingreso,
            ...
            {$sql_existencia_cajas} JOIN ts_existenciacajas ec ON ec.idy_ubica = e.cve_ubicacion AND e.cve_articulo = ec.cve_articulo AND IFNULL(ec.cve_lote, '') = IFNULL(e.cve_lote, '')  AND e.cve_almac = ec.cve_almac {$TempMostrarCajas}
            ...
            LEFT JOIN td_aduana ad ON ad.num_orden = tr.folio_oc AND CONVERT(tr.cve_articulo, CHAR) = CONVERT(ad.cve_articulo, CHAR) 
            {$left_join_folios}
            {$sqlCliente}
