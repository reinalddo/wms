...
class NuevosPedidos {
    ...
    public function __construct( $Fol_folio = false, $key = false ) {
        ...
        ',
            ...
            $sth = \db()->prepare($sql);
            $sth->setFetchMode(\PDO::FETCH_CLASS, '\NuevosPedidos\NuevosPedidos');
  ...
  function GenerarDiaOperativo($id_ruta, $clave_almacen, $pdo = "")
  {
      ...
      $sth = $pdo->prepare( $sql );$sth->execute();$res = $sth->fetch();$sth->closeCursor();
      $DiaO = $res['DiaO'];
      $Fecha = $res['Fecha'];
      ...
      if($DiaO == '') 
      {
        ...
        $sth = $pdo->prepare( $sql );$sth->execute();$sth->closeCursor();
  ...
  function save( $_post, $pdo = "" ) {
    ...
    try {
      ...
      $sql = "SELECT COUNT(*) as existe FROM th_pedido WHERE Fol_folio = '{$fol_verif}'";
      $sth = $pdo->prepare( $sql );$sth->execute();$res_fol = $sth->fetch();$sth->closeCursor();
      $existe_folio  = $res_fol['existe'];

      if($existe_folio) 
      {
            if($_post['tipo_traslado'] == 1) 
                $_post['Fol_folio'] = $this->consecutivo_folio_traslado($pdo);
            else
                $_post['Fol_folio'] = $this->consecutivo_folio($pdo);
      }
      ...
      //***************************************************************************************************
            ...
            if($tipo_venta == 'venta' && $_post['tipo_traslado'] != 1) 
            {
                  ...
                  $sth = $pdo->prepare( $sql );$sth->execute();$res_almacen = $sth->fetch();
                  $clave_almacen  = $res_almacen['clave'];$sth->closeCursor();

                  $cve_ruta = $_post['rutas_ventas'];
                  $sql = "SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$cve_ruta}' LIMIT 1";
                  $sth = $pdo->prepare( $sql );$sth->execute();$res_ruta = $sth->fetch();
                  $id_ruta  = $res_ruta['ID_Ruta'];$sth->closeCursor();

                  $cve_vendedor = $_post['cve_Vendedor'];
                  $sql = "SELECT id_user FROM c_usuario WHERE cve_usuario = '{$cve_vendedor}' LIMIT 1";
                  $sth = $pdo->prepare( $sql ); $sth->execute(); $res_vendedor = $sth->fetch();
                  $id_user  = $res_vendedor['id_user'];$sth->closeCursor();
                  ...
                  $DiaOperativo = $this->GenerarDiaOperativo($id_ruta, $clave_almacen, $pdo);
      ...
      {
          $sql = "SELECT clave FROM c_almacenp WHERE id = {$cve_almac} LIMIT 1";
          $sth = $pdo->prepare( $sql );$sth->execute();$res_almacen = $sth->fetch();$sth->closeCursor();
          $clave_almacen  = $res_almacen['clave'];

          $cve_ruta = $_post['rutas_ventas'];
          $sql = "SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$cve_ruta}' LIMIT 1";
          $sth = $pdo->prepare( $sql );$sth->execute();$res_ruta = $sth->fetch();$sth->closeCursor();
      ...
      if($tipo_lp == '1')
        ...
        $sql = "INSERT INTO " . self::TABLE . " (Fol_folio, Fec_Pedido, Cve_clte, status, TipoPedido, Fec_Entrega, cve_Vendedor, DiaO, TipoDoc, Fec_Entrada, Pick_Num, destinatario, Cve_Usuario, Observaciones, statusaurora, ID_Tipoprioridad, ruta, bloqueado, rango_hora, cve_almac, Cve_CteProv, cve_ubicacion, tipo_venta, tipo_negociacion, Activo, contacto_id)";
            $sql .= "Values (";
            $sql .= "'".$_post['Fol_folio']."',";
            ...
            $sql .= "'".$_post['status']."',";
            $sql .= "'".$tipo_pedido."',";
            $sql .= "'".$_post['Fec_Entrega']."',";
            $sql .= "'".$_post['cve_Vendedor']."',";
            $sql .= $DiaOperativo.",";
            $sql .= "'".$tipoDoc."',";
            $sql .= "'".$_post['Fec_Entrada']."',";
            $sql .= "'".$_post['Pick_Num']."',";
            $sql .= "'".$_post['destinatario']."',";
            $sql .= "(SELECT DISTINCT cve_usuario from c_usuario where id_user = '".$_post['Cve_Usuario']."' LIMIT 1),";
            $sql .= "'".$_post['Observaciones']."',";
            $sql .= "'".$statusaurora."',";
            $sql .= "'".$_post['ID_Tipoprioridad']."',";
            $sql .= "(SELECT DISTINCT IFNULL(cve_ruta, (SELECT DISTINCT ID_Ruta FROM t_ruta WHERE cve_ruta = '".$_post['rutas_ventas']."' LIMIT 1)) from c_cliente where Cve_clte = '".$_post['Cve_clte']."' LIMIT 1),";
            $sql .= "'".$tipo_ptl."',";
            $sql .= "'".$rango_hora."',";
            $sql .= "'".$cve_almac."',";
            $sql .= $Cve_CteProv.", '".$_post['rutas_ventas']."', '".$_post['tipo_venta']."', '".$_post['tipo_negociacion']."', 
            '1', '".$_post['contacto_pedido']."');";
            //echo $sql;
            //$rs = mysqli_query(\db2(), $sql) or die("Error description: " . mysqli_error(\db2()).$sql);
            $rs = $pdo->prepare( $sql ); $rs->execute(); $rs->closeCursor();
            ...
            if (!empty($_post["arrDetalle"]) && $ots_con_lp == "") {
              //peso, precio_unitario, importe_total
                foreach ($_post["arrDetalle"] as $item) {
                        $num_meses = 0;
                        if($item['Num_Meses'] != '')
                        {
                              $n_meses = explode("-", $item['Num_Meses']);
                              $n_fecha_meses = $n_meses[2]."-".$n_meses[1]."-".$n_meses[0];
                              $sql = "SELECT TIMESTAMPDIFF(MONTH, CURDATE(), $n_fecha_meses) AS meses_transcurridos";
                              $sth = $pdo->prepare( $sql );$sth->execute();$res_meses = $sth->fetch();$sth->closeCursor();
                              $num_meses  = $res_meses['meses_transcurridos'];
                        }
                    if($tipo_lp == '0')
                    {
                    $sql = "INSERT INTO td_pedido ( Fol_folio, Cve_articulo, cve_lote, Num_cantidad, id_unimed, Num_Meses, status, Activo, Precio_unitario, Desc_Importe, IVA, Proyecto) Values ";
                    $sql .= "('".$_post['Fol_folio']."', '".$item['Cve_articulo']."', '".$item['cve_lote']."', '".$item['Num_cantidad']."', '".$item['id_unimed']."', '".$num_meses."', 'A' , '1', '".$item['precio_unitario']."', '".$item['desc_importe']."', '".$item['iva']."', '".$item['proyecto']."');";
                    //echo $sql;
                    //$rs = mysqli_query(\db2(), $sql) or die("Error description: " . mysqli_error(\db2()));
                    $rs = $pdo->prepare( $sql ); $rs->execute(); $rs->closeCursor();
                    }
                    else if($tipo_lp == '1' && $item['CveLP'] != "")
                    {

                        $sql = "SELECT COUNT(*) as existe FROM td_pedido WHERE Fol_folio = '".$_post['Fol_folio']."' AND cve_articulo = '".$item['Cve_articulo']."' AND cve_lote = '".$item['cve_lote']."'";
                        $sth = $pdo->prepare( $sql );$sth->execute();$res_folio = $sth->fetch();$sth->closeCursor();
                        ...
                        {
                            $sql = "UPDATE td_pedido SET Num_cantidad = Num_cantidad + '".$item['Num_cantidad']."' WHERE Fol_folio = '".$_post['Fol_folio']."' AND cve_articulo = '".$item['Cve_articulo']."' AND cve_lote = '".$item['cve_lote']."'";
                            //$rs = mysqli_query(\db2(), $sql) or die("Error description: " . mysqli_error(\db2()));
                            $rs = $pdo->prepare( $sql ); $rs->execute(); $rs->closeCursor();
                        }

                        $CveLP = $item['CveLP'];
                        $sql = "SELECT DISTINCT IDContenedor FROM c_charolas WHERE CveLP = '{$CveLP}' LIMIT 1";
                        $sth = $pdo->prepare( $sql );$sth->execute();$res_CveLP = $sth->fetch();$sth->closeCursor();
                        $nTarima  = $res_CveLP['IDContenedor'];

                        $sql = "INSERT INTO td_pedidoxtarima (Fol_folio, nTarima, Cve_articulo, cve_lote, Num_cantidad, status, Activo) Values ";
                        $sql .= "('".$_post['Fol_folio']."', '".$nTarima."', '".$item['Cve_articulo']."', '".$item['cve_lote']."', '".$item['Num_cantidad']."', 'A', 1);";
                        //$rs = mysqli_query(\db2(), $sql) or die("Error description: " . mysqli_error(\db2()));
                        $rs = $pdo->prepare( $sql ); $rs->execute(); $rs->closeCursor();
                    }
            ...
            {
                    $sql_lp = "";
                    $folio_de_ot = $_post['Fol_folio'];
                    if($lp_ot != '') $sql_lp = " AND ch.CveLP = '$lp_ot' ";
                    $sql = "INSERT INTO td_pedido ( Fol_folio, Cve_articulo, cve_lote, id_ot, Num_cantidad, Num_Meses, status, Activo, Precio_unitario, Desc_Importe, IVA) (SELECT '$folio_de_ot', ts.cve_articulo, ts.lote, ts.ntarima, SUM(ts.existencia), 0, 'A', 1, 0, 0, 0 FROM ts_existenciatarima ts LEFT JOIN c_charolas ch ON ts.ntarima = ch.IDContenedor WHERE ts.cve_almac = {$cve_almac} AND CONCAT(ts.cve_articulo, ts.lote) IN (SELECT CONCAT(Cve_Articulo, Cve_Lote) FROM t_ordenprod WHERE Status = 'T' AND cve_almac = {$cve_almac} AND Folio_Pro = '$ots_con_lp') {$sql_lp} GROUP BY cve_articulo, lote, ts.ntarima)";
                    //$rs = mysqli_query(\db2(), $sql) or die("Error description: " . mysqli_error(\db2()). $sql);
                    $rs = $pdo->prepare( $sql ); $rs->execute(); $rs->closeCursor();
            }
    ...
    function consecutivo_folio($pdo = "") 
    {
      if($pdo == "") $pdo = \db();
      ...
      $sth = $pdo->prepare( $sql );
      $sth->execute();
      $fecha = $sth->fetch();$sth->closeCursor();
      ...
      while(true)
      {
          ...
          $sql = "SELECT COUNT(*) as Consecutivo FROM th_pedido WHERE Fol_Folio = '$folio_next' OR '$folio_next' in (SELECT Documento FROM Venta)";
          $sth = $pdo->prepare( $sql );
          $sth->execute();
          $data = $sth->fetch();$sth->closeCursor();
    ...
    function consecutivo_folio_traslado($pdo = "") 
    {
      if($pdo == "") $pdo = \db();
