...
// SIN FILTRO DE LOTES
        ...
        SELECT DISTINCT 
                ...
                GROUP_CONCAT(DISTINCT CONCAT(inv.NConteo,'-',inv.Cantidad) ORDER BY inv.NConteo SEPARATOR ',') AS NConteo_Cantidad_reg,
                GROUP_CONCAT(DISTINCT inv.NConteo SEPARATOR ',') AS Nconteo, 
                (SELECT DISTINCT ext.ExistenciaTeorica FROM t_invpiezas ext WHERE ext.ID_Inventario = {$id_inventario} AND ext.NConteo = 0 AND ext.cve_articulo = v.cve_articulo AND ext.idy_ubica = v.cve_ubicacion AND ext.cve_lote = v.cve_lote) AS stockTeorico,
                GROUP_CONCAT(DISTINCT inv.Cantidad ORDER BY inv.NConteo SEPARATOR ',') AS Cantidad_reg,
