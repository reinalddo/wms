...
//*********************************************************************************
      ...
      foreach ($data as $row) 
      {
        ...
        $sql_cliente = "SELECT DISTINCT IFNULL(RazonComercial, RazonSocial) AS RazonComercial FROM c_cliente WHERE Cve_Almacenp = (SELECT Cve_Almac FROM th_pedido where Fol_folio = '$folio_pedido') AND Cve_Clte = '$cliente'";
...
*/
        ...
        {   
              //$row_sheet = $row;
              //if($linea == 1 && $this->pSQL($row[3]) != "" && $pedido_masivo_input == 0) $ok_pedido = false; 
              // SI HAY UNA COLUMNA ACTIVA EN LA COLUMNA[3] SIGNIFICA QUE ESTÁN USANDO OTRO LAYOUT
              ...
              {//s60
                  ...
                  {//s38
                        ...
                        if($pedido_masivo_input == 1)
                        {
                            ...
                            {
                                $sql = "SELECT id_destinatario, Cve_Clte FROM c_destinatarios WHERE id_destinatario = '$cve_destinatario'"; //Cve_CteProv
                                $rs = mysqli_query($conn, $sql);
                                $resul = mysqli_fetch_array($rs, MYSQLI_ASSOC);
                                $destinatario = $resul['id_destinatario'];
                        ...
                        if($pedido_masivo_input == 1)
                            $cve_articulo_th = $this->pSQL($row[self::CVE_ARTICULO_MSV]);
                        ...
                        $sql = "SELECT COUNT(*) as num_articulo FROM c_articulo WHERE cve_articulo = '{$cve_articulo_th}'";
                        ...
                        {//s27
                            ...
                            //if($cve_articulo_registrado != $cve_articulo)
                            $sql = "SELECT control_lotes, control_numero_series FROM c_articulo WHERE cve_articulo = '$cve_articulo_th'";
                            $rs = mysqli_query($conn, $sql);
                            $resul = mysqli_fetch_array($rs, MYSQLI_ASSOC);
                            $control_lotes         = $resul['control_lotes'];
                            $control_numero_series = $resul['control_numero_series'];
                            ...
                            if($lote_serie != '')
                            {
                                if($control_lotes == 'S')
                                {
                                    ...
                                    $sql = "SELECT COUNT(*) as existe FROM c_lotes WHERE cve_articulo = '$cve_articulo_th' AND Lote = '$lote'";
                                    $rs = mysqli_query($conn, $sql);
                                    $resul = mysqli_fetch_array($rs, MYSQLI_ASSOC);
                                    $existe_lote = $resul['existe'];
                                }
                                ...
                                if($control_numero_series == 'S')
                                {
                                    ...
                                    $sql = "SELECT COUNT(*) as existe FROM c_serie WHERE cve_articulo = '$cve_articulo_th' AND numero_serie = '$serie'";
                                    $rs = mysqli_query($conn, $sql);
                                    $resul = mysqli_fetch_array($rs, MYSQLI_ASSOC);
                                    $existe_serie = $resul['existe'];
                                }
                          ...
                          if((($registrar_lote_serie == true && ($control_lotes == 'S' || $control_numero_series == 'S')) || ($lote == '' && $serie == '')) || $cve_lote == '')
                            {
                                $respuesta = $model->save();
                                $cve_articulo_registrado = $cve_articulo_th;
                        ...
                        {//s28
                            if(!$num_articulo && $cve_articulo_th)
                            {//s29
                                $registro_actual = "(Cve_Almacen = ".$cve_almac.", Cve_Articulo = ".$cve_articulo_th.", Cve_Cliente = ".$cve_clte.", Sku = ".$Sku.")\n";
                                if($registro_actual != $registro_anterior) $articulos_no_existentes .= $registro_actual;
                                $registro_anterior = $registro_actual;
                            }//s29
                            $tracking_codigo .= "[$linea -3X]";
                  ...
                  //}//s40
                    ...
                    $model->Fol_folio        = $folio_pedido;
                    $model->Cve_articulo     = ($pedido_masivo_input == 0)?($this->pSQL($row[self::CVE_ARTICULO])):($this->pSQL($row[self::CVE_ARTICULO_MSV]));
...
*/
        ...
        foreach ($xlsx->rows($i_sheet) as $row)
        {
              ...
              {//s60
                ...
                if($NTarima != '')
                {
                  $sql = "SELECT * FROM ts_existenciatarima WHERE cve_almac = {$almacen_val} AND ntarima = {$NTarima}";
                  $rs = mysqli_query($conn, $sql);
                  ...
                  //*****************************************************************************************
                    ...
                    if(!$existe)
                    {
                        ...
                        $sql .= "('".$folio_pedido."', '".$cve_articulo."', '".$lote."', '".$existencia."', NULL, '0', 'A' , '1', '0', '0', '0');";
                        $rs1 = mysqli_query($conn, $sql);
                    }
                    else
                    {
