...
//*****************************************       PEDIDOS TH      ************************************************************
//****************************************************************************************************************************
    const FOLIO                      = 0;    const PRIORIDAD                  = 10;    const DIRECCION_DESTINATARIO     = 20;
    const FECHA_PEDIDO               = 1;    const BLOQUEADO                  = 11;    const COLONIA_DESTINATARIO       = 21;
    const CLAVE_CLIENTE              = 2;    const NUM_ORDEN_CLIENTE          = 12;    const CODIGO_POSTAL_DESTINATARIO = 22;
    const CLAVE_CLIENTE_PROV         = 3;    const PICK_NUMBER                = 13;    const CIUDAD_DESTINATARIO        = 23;
    const DIR_ENTREGA_PRINCIPAL      = 4;    const CVE_ARTICULO               = 14;    const ESTADO_DESTINATARIO        = 24;
    const FECHA_ENTREGA              = 5;    const NUM_CANTIDAD               = 15;    const CONTACTO                   = 25;
    const CLAVE_VENDEDOR             = 6;    const MESES_2                    = 16;    const TELEF_DESTINATARIO         = 26;
    const MESES                      = 7;    const ITEM_NUM                   = 17;    const EMAIL_DESTINATARIO         = 27;
    const OBSERVACIONES              = 8;    const CVE_DESTINATARIO           = 18;    const LATITUD                    = 28;
    const FECHA_ENTRADA              = 9;    const RAZON_SOCIAL               = 19;    const LONGITUD                   = 29;
//****************************************************************************************************************************
//****************************************************************************************************************************
    ...
    public function paginate()
    {
      ...
      {
            $sql_search = " AND (o.Fol_folio like '%$search%' OR o.Pick_Num like '%$search%' OR p.Descripcion like '%$search%' OR c.RazonSocial like '%$search%' OR u.nombre_completo like '%$search%') ";

            $sql_union_search = " AND (tdsp.Fol_folio LIKE '%$criterio%' OR th.Pick_Num LIKE '%$criterio%' OR p.Descripcion LIKE '%$criterio%' OR c.RazonSocial LIKE '%$criterio%' OR c_usuario.nombre_completo LIKE '%$criterio%') ";
      ...
      SELECT * FROM (
        SELECT 
          ...
          0 AS sufijo, 
          #IFNULL(thsub.Sufijo, 0) AS sufijo,
          ...
          #IFNULL((SELECT DISTINCT IF((SUM(vsx.Existencia) >= SUM(od.Num_cantidad) AND (SELECT COUNT(*) FROM td_pedido tdc WHERE tdc.Fol_folio = o.Fol_folio) = (SELECT COUNT(DISTINCT vsx.cve_articulo) FROM VS_ExistenciaParaSurtido vsx WHERE vsx.cve_articulo IN (SELECT tdx.cve_articulo FROM td_pedido tdx WHERE tdx.Fol_folio = o.Fol_folio) AND vsx.Cve_Almac = '{$almacen}')), 1, 0) 
          #FROM VS_ExistenciaParaSurtido vsx
             #WHERE od.Cve_articulo = vsx.cve_articulo AND od.Fol_folio = o.Fol_folio AND vsx.Cve_Almac = '{$almacen}' 
             #GROUP BY vsx.cve_articulo
             #), 'BO') AS disponible,

          #IFNULL((SELECT DISTINCT IF((SUM(vsx.Existencia) >= SUM(od.Num_cantidad) AND (SELECT COUNT(*) FROM td_pedido tdc WHERE tdc.#Fol_folio = o.Fol_folio) = (SELECT COUNT(DISTINCT vsx.cve_articulo) FROM VS_ExistenciaParaSurtido vsx WHERE vsx.cve_articulo IN #(SELECT tdx.cve_articulo FROM td_pedido tdx WHERE tdx.Fol_folio = o.Fol_folio) AND vsx.Cve_Almac = '{$almacen}')), 1, 
          #(
            #IF(IFNULL((SELECT COUNT(DISTINCT vse.cve_articulo) 
            #FROM VS_ExistenciaParaSurtido vse 
            #WHERE vse.Cve_Almac = '{$almacen}' 
              #AND vse.cve_articulo IN (SELECT tp.Cve_articulo FROM td_pedido tp WHERE tp.Fol_folio = o.Fol_folio 
              #AND tp.Num_cantidad <= (SELECT SUM(vs.Existencia) FROM VS_ExistenciaParaSurtido vs 
              #WHERE vs.Cve_Almac = '{$almacen}' 
              #AND vs.cve_articulo IN (SELECT tp1.Cve_articulo FROM td_pedido tp1 WHERE tp1.Fol_folio = o.Fol_folio)))), 0) = 0, 0, 2)
                  #)) 
             #FROM VS_ExistenciaParaSurtido vsx
             #WHERE od.Cve_articulo = vsx.cve_articulo AND od.Fol_folio = o.Fol_folio AND vsx.Cve_Almac = '{$almacen}' 
             #GROUP BY vsx.cve_articulo
...
#CASE 
#WHEN ((SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE# h.Fol_folio = th.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= h.Num_Cantidad)) = (SELECT COUNT(DISTINCT cve_articulo) FROM #td_pedido WHERE Fol_folio = th.Fol_folio) AND (SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (#SELECT cve_articulo FROM td_pedido WHERE Fol_folio = th.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= Num_Cantidad)) > 0) THEN 1
#WHEN ((SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE# h.Fol_folio = th.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= h.Num_Cantidad)) != (SELECT COUNT(DISTINCT cve_articulo) FROM #td_pedido WHERE Fol_folio = th.Fol_folio)) THEN 2
#WHEN (SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE h.Fol_folio = th.Fol_folio)) = 0 THEN 0
#END AS disponible,
CASE 
WHEN ((SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE h.Fol_folio = o.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= h.Num_Cantidad)) = (SELECT COUNT(DISTINCT cve_articulo) FROM td_pedido WHERE Fol_folio = o.Fol_folio) AND (SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT cve_articulo FROM td_pedido WHERE Fol_folio = o.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= Num_Cantidad)) > 0) THEN 1
WHEN (((SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE h.Fol_folio = o.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= h.Num_Cantidad)) != (SELECT COUNT(DISTINCT cve_articulo) FROM td_pedido WHERE Fol_folio = o.Fol_folio)) AND (SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE h.Fol_folio = o.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= h.Num_Cantidad))>0) THEN 2
WHEN (SELECT COUNT(DISTINCT cve_articulo) FROM VS_ExistenciaParaSurtido WHERE cve_articulo IN (SELECT h.cve_articulo FROM td_pedido h WHERE h.Fol_folio = o.Fol_folio AND VS_ExistenciaParaSurtido.Existencia >= h.Num_Cantidad)) = 0 THEN 0
END AS disponible,
...
*/
      ...
      SELECT DISTINCT 
        thsp.Fol_folio AS id_pedido,
                    ...
                    IF(thsp.status != 'S', IFNULL((SELECT SUM(revisadas) FROM td_surtidopiezas WHERE fol_folio = tdsp.fol_folio AND Sufijo = tdsp.Sufijo), '--'), '--') AS cantidad_surtida, 
...
END AS disponible,
                    ...
                    th.Fec_Entrada AS Fec_Entrega_ord,
                ...
                FROM td_subpedido tdsp 
                  ...
                  WHERE thsp.Fol_folio != '' {$sql_union_search} 
...
/*
        const FOLIO                      = 0;
        const FECHA_PEDIDO               = 1;
        const CLAVE_CLIENTE              = 2;
        const CLAVE_CLIENTE_PROV         = 3;
        const DIR_ENTREGA_PRINCIPAL      = 4;
        const FECHA_ENTREGA              = 5;
        const CLAVE_VENDEDOR             = 6;
        const MESES                      = 7;
        const OBSERVACIONES              = 8;
        const FECHA_ENTRADA              = 9;
        const PRIORIDAD                  = 10;
        const BLOQUEADO                  = 11;
        const NUM_ORDEN_CLIENTE          = 12;
        const PICK_NUMBER                = 13;
        const CVE_ARTICULO               = 14;
        const NUM_CANTIDAD               = 15;
        const MESES_2                    = 16;
        const ITEM_NUM                   = 17;
        const CVE_DESTINATARIO           = 18;
        const RAZON_SOCIAL               = 19;
        const DIRECCION_DESTINATARIO     = 20;
        const COLONIA_DESTINATARIO       = 21;
        const CODIGO_POSTAL_DESTINATARIO = 22;
        const CIUDAD_DESTINATARIO        = 23;
        const ESTADO_DESTINATARIO        = 24;
        const CONTACTO                   = 25;
        const TELEF_DESTINATARIO         = 26;
        const EMAIL_DESTINATARIO         = 27;
        const LATITUD                    = 28;
        const LONGITUD                   = 29;
*/
        $errores = [];
