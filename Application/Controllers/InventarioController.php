...
*/
          ...
          //*******************************************************************************
            ...
            $sql_cerrar = "
                ...
                #IFNULL((SELECT DISTINCT GROUP_CONCAT(ID_Proveedor) FROM t_invpiezasciclico WHERE cve_articulo = tp.cve_articulo AND cve_lote = tp.cve_lote AND idy_ubica = tp.idy_ubica AND NConteo = 0 AND ID_PLAN = {$inventario}), (SELECT DISTINCT  FROM th_entalmacen WHERE fol_folio IN (SELECT distinct fol_folio FROM td_entalmacen WHERE cve_articulo = tp.cve_articulo AND cve_lote = tp.cve_lote) LIMIT 1)) AS proveedor
                ...
                #IFNULL((SELECT DISTINCT ID_Proveedor FROM t_invtarimaciclico WHERE cve_articulo = tt.cve_articulo AND cve_lote = tt.cve_lote AND idy_ubica = tt.idy_ubica AND NConteo = 0 AND ntarima = tt.ntarima AND ID_PLAN = {$inventario}), (SELECT DISTINCT  FROM th_entalmacen WHERE fol_folio IN (SELECT fol_folio FROM td_entalmacen WHERE cve_articulo = tt.cve_articulo AND cve_lote = tt.cve_lote) LIMIT 1)) AS proveedor
...
//***************************************************************************************************
    ...
    k.cve_articulo AS id_articulo, a.des_articulo AS des_articulo, k.cve_lote AS cve_lote, 
                ...
                #k.cantidad, 
                th_dest.Cve_clte as cliente,
                cliente.RazonSocial as nombre_cliente,
                ...
                LEFT JOIN c_cliente cliente ON cliente.Cve_Clte = th_dest.Cve_Clte AND th_dest.fol_folio is not null
...
function TrasladosMasivos()
{
    ...
    $nombre_unico = uniqid('importacion_', true) . '.' . $extension;
    // Construye la ruta de destino final
    $file = $dir_cache . $nombre_unico;
    ...
    try{
        ...
        $linea = 1; $productos = 0; $cajas_sin_lP = array(); $msj_cajas = ""; $msj_caja_no_existe = array();
        $lineas = $xlsx->rows();
        $lp_tipo_caja = array();
        $val_reg_almacen = array();//ubicaciones_pallets_articulo_lote_cantidad_con_almacen_diferente_sin_existencia
        $procesar_traslado = true;
        ...
        foreach ($xlsx->rows() as $row)
        {
            ...
            $cve_articulo = $this->limpiarDato($this->pSQL($row[self::CVE_ARTICULO_TR]));
            $cve_lote = $this->limpiarDato($this->pSQL($row[self::CVE_LOTE_TR]));
            $alm_origen  = $this->limpiarDato($this->pSQL($row[self::ALM_ORIGEN_TR]));
            $alm_destino = $this->limpiarDato($this->pSQL($row[self::ALM_DESTINO_TR]));
            $trasladar_en_cajas = $this->limpiarDato($this->pSQL($row[self::TRASLADAR_EN_CAJAS]));
            $cve_articulo_caja = "";
            $cve_lote_caja = "";
            $cantidad_caja = "";
            $traslado_almacen = 0;
            if($alm_origen == "") continue;
            ...
            $bl_origen  = $this->limpiarDato($this->pSQL($row[self::BL_ORIGEN_TR]));
            $bl_destino = $this->limpiarDato($this->pSQL($row[self::BL_DESTINO_TR]));

            $sql_doc = "SELECT idy_ubica FROM c_ubicacion WHERE CodigoCSD = '$bl_origen'";
            //if (!($res_doc = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
            $res_doc = $pdo->prepare($sql_doc);$res_doc->execute();
            //$row_doc = mysqli_fetch_array($res_doc);
            $row_doc = $res_doc->fetch();
            ...
            $sql_doc = "SELECT idy_ubica FROM c_ubicacion WHERE CodigoCSD = '$bl_origen' AND cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = $almacen_origen)";
            //if (!($res_doc = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
            $res_doc = $pdo->prepare($sql_doc);$res_doc->execute();
            //$row_doc = mysqli_fetch_array($res_doc);
            $row_doc = $res_doc->fetch();
            $idiorigen_existe_en_almacen = $row_doc['idy_ubica'];
            $res_doc->closeCursor();

            $sql_doc = "SELECT idy_ubica FROM c_ubicacion WHERE CodigoCSD = '$bl_destino' AND cve_almac IN (SELECT cve_almac FROM c_almacen WHERE cve_almacenp = $almacen_destino)";
            //if (!($res_doc = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
            $res_doc = $pdo->prepare($sql_doc);$res_doc->execute();
            //$row_doc = mysqli_fetch_array($res_doc);
            $row_doc = $res_doc->fetch();
            $ididestino_existe_en_almacen = $row_doc['idy_ubica'];
            $res_doc->closeCursor();

            $cantidad  = $this->limpiarDato($this->pSQL($row[self::CANTIDAD_TR]));

            $LP_ORIG  = $this->limpiarDato($this->pSQL($row[self::LP_TR_ORIG]));
            $LP_DEST  = $this->limpiarDato($this->pSQL($row[self::LP_TR_DEST]));
            ...
            if($LP_ORIG != '')
            {
                ...
                $sql_doc = "SELECT IDContenedor, tipo FROM c_charolas WHERE CveLP = '$LP_ORIG' AND cve_almac = $almacen_origen";
                //if (!($res_doc = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
                $res_doc = $pdo->prepare($sql_doc);$res_doc->execute();
                //$row_doc = mysqli_fetch_array($res_doc);
                $row_doc = $res_doc->fetch();
                $ntarima_almacen = $row_doc['IDContenedor'];
                $res_doc->closeCursor();

                if($ntarima == "") $val_reg_almacen[] = "El LP ORIGEN: ".$LP_ORIG. " No existe";
                if($ntarima_almacen == "") $val_reg_almacen[] = "El LP: ".$LP_ORIG. " No existe en el Almacén: ".$alm_origen;
                ...
                /*if($tipo == 'Caja')
                    $sql_doc = "SELECT PiezasXCaja as existencia, '' as ID_Proveedor FROM ts_existenciacajas WHERE idy_ubica = '$idiorigen' AND Id_Caja = (SELECT IDContenedor FROM c_charolas WHERE CveLP = '$LP_ORIG' AND IDContenedor in (SELECT ntarima from ts_existenciatarima where cve_almac = $almacen_origen) LIMIT 1) LIMIT 1";*/
            }
            //if (!($res_doc2 = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
            $res_doc2 = $pdo->prepare($sql_doc);$res_doc2->execute();

            //***************************************************************************************************************************
            // PROCESO PARA VERIFICAR QUE LOS BLS QUE SE COLOCAN EN LAS COLUMNAS PERTENECEN A SU ALMACÉN ESPECIFICADO EN ORIGEN/DESTINO
            //***************************************************************************************************************************

            //Recolecto datos de la ubicación origen y destino que no pertenece al almacén origen y destino especificado
            if($idiorigen_existe_en_almacen == "") $val_reg_almacen[] = "El BL: ".$bl_origen. " No existe en el Almacén: ".$alm_origen;
            if($ididestino_existe_en_almacen == "") $val_reg_almacen[] = "El BL: ".$bl_destino. " No existe en el Almacén: ".$alm_destino;
            //if(mysqli_num_rows($res_doc2) == 0) 
            if($res_doc2->rowCount() == 0 && $tipo != 'Caja') /*Esto mientras valido bien las cajas*/
            {
                if($LP_ORIG != '' && $cve_articulo == '')
                {
                    $val_reg_almacen[] = "El LP: ".$LP_ORIG. " No existe en el BL Origen: ".$bl_origen;
                }
                else if($LP_ORIG != '' && $cve_articulo != '')
                {
                    if($cve_lote)
                        $val_reg_almacen[] = "El LP: ".$LP_ORIG. ", Artículo: ".$cve_articulo." y Lote: ".$cve_lote." No existe en el BL Origen: ".$bl_origen;
                    else
                        $val_reg_almacen[] = "El LP: ".$LP_ORIG. ", Artículo: ".$cve_articulo." No existe en el BL Origen: ".$bl_origen;
                }
                else if($LP_ORIG == '' && $cve_articulo != '')
                {
                    if($cve_lote)
                        $val_reg_almacen[] = "El Artículo: ".$cve_articulo." y Lote: ".$cve_lote." No existe en el BL Origen: ".$bl_origen;
                    else
                        $val_reg_almacen[] = "El Artículo: ".$cve_articulo." No existe en el BL Origen: ".$bl_origen;
                }
            }

            //***************************************************************************************************************************

            if(count($val_reg_almacen) > 0) {$procesar_traslado = false; continue;}

            if($tipo == 'Caja' && $LP_ORIG != '')
            {
                if($LP_DEST == '') $LP_DEST = $LP_ORIG;
                if($LP_DEST == $LP_ORIG && $trasladar_en_cajas == 'S') $trasladar_en_cajas = '';//NO SE PUEDE REALIZAR PROCESO DE CONVERSIÓN DE CAJA A CAJA 
                $cantidad     = '';//----|
                $cve_articulo = '';//----> LA CANTIDAD, ARTICULO Y LOTE CUANDO ES POR CAJAS SIEMPRE SERÁ COMPLETA
                $cve_lote     = '';//----|
                $tipo_caja = $tipo;

                $sql_doc = "SELECT cve_articulo, cve_lote, PiezasXCaja FROM ts_existenciacajas WHERE Id_Caja = '$ntarima'";
                //if (!($res_doc = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
                $res_doc = $pdo->prepare($sql_doc);$res_doc->execute();
                //$row_doc = mysqli_fetch_array($res_doc);
                $row_doc = $res_doc->fetch();
                $cve_articulo_caja = $row_doc['cve_articulo'];
                $cve_lote_caja = $row_doc['cve_lote'];
                $cantidad_caja = $row_doc['PiezasXCaja'];
                $res_doc->closeCursor();

                //if(!mysqli_num_rows($res_doc)) {$msj_caja_no_existe[] = $LP_ORIG; continue;}
                if(!$res_doc->rowCount()) {$msj_caja_no_existe[] = $LP_ORIG; continue;}
            }
            ...
            if(@$res_doc2->rowCount() || $tipo == 'Caja') 
            {
                    ...
                    else if($LP_ORIG == $LP_DEST) //SI ES UNA CAJA Y SE VA A TRASLADAR EN PIEZAS
                    {
                        $sql_doc = "SELECT ID_Proveedor from ts_existenciapiezas where cve_articulo = '$cve_articulo_caja' AND cve_lote = '$cve_lote_caja' AND cve_almac = $almacen_origen AND idy_ubica = $idiorigen";
                        //if (!($res_doc3 = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
                        $res_doc3 = $pdo->prepare($sql_doc);$res_doc3->execute();
                        //$row_doc_caja = mysqli_fetch_array($res_doc3);
                        $row_doc_caja = $res_doc3->fetch();
                        $id_proveedor = $row_doc_caja['ID_Proveedor'];
                        $res_doc3->closeCursor();
                        //echo $sql_doc; exit;

                        if(!$id_proveedor)
                        {
                            $sql_doc = "SELECT  as ID_Proveedor from th_entalmacen where fol_folio in (select fol_folio from td_entalmacen where cve_articulo = '$cve_articulo_caja' AND cve_lote = '$cve_lote_caja' AND CantidadRecibida = CantidadUbicada) AND STATUS = 'T' LIMIT 1";
                            //if (!($res_doc3 = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
                            $res_doc3 = $pdo->prepare($sql_doc);$res_doc3->execute();
                            //$row_doc_caja = mysqli_fetch_array($res_doc3);
                            $row_doc_caja = $res_doc3->fetch();
                            ...
                            $sql_doc = "SELECT distinct Id_Proveedor as ID_Proveedor from rel_articulo_proveedor where cve_articulo = '$cve_articulo_caja' and Id_Proveedor IN (select Id_Proveedor from c_proveedores) LIMIT 1";
                            //if (!($res_doc3 = mysqli_query($conn, $sql_doc))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
                            $res_doc3 = $pdo->prepare($sql_doc);$res_doc3->execute();
                            //$row_doc_caja = mysqli_fetch_array($res_doc3);
                            $row_doc_caja = $res_doc3->fetch();
        ...
        */
            ...
            if($procesar_traslado == true)
            {
                ...
                $_POST['cantidad'] = ($cantidad_caja=="")?($cantidad):($cantidad_caja);
                ...
                $_POST['fusionar'] = 0;
                $_POST['cve_articulo'] = ($cve_articulo_caja=="")?($cve_articulo):($cve_articulo_caja);
                $_POST['cve_lote'] = ($cve_lote_caja=="")?($cve_lote):($cve_lote_caja);
                ...
                $post_ejecutados[] = $_POST;

                $data = $ga->moverTraslado($_POST, $pdo);
            }
        ...
        if(count($cajas_sin_lP) > 0 || $msj_cajas != '')
        {
            ...
            else
            {
                ...
                $msj = "Hay Cajas que no poseen Pallet Destino O Pallets que no existen: $cjs_msj";
            }
        }
        ...
        $msj_caja_cero = "";
        if(count($msj_caja_no_existe) > 0)
        {
            $success = 0;
            $msj_caja_cero = "";
            $cajasN = implode(", ", $msj_caja_no_existe);
            $limite = 100;
            if (strlen($cajasN) > $limite) {
              $msj_caja_cero = substr($cajasN, 0, $limite) . " ...";
            } else {
              $msj_caja_cero = $cajasN;
            }

            $msj = "Las Siguientes cajas no existen en el sistema: $msj_caja_cero";
        }

        if($procesar_traslado == false) 
        {
            $success = 2;
            $msj = implode("\n\n", $val_reg_almacen);
        }
        ...
        $this->response(200, [
            ...
            "success" => $success,
            "posts" => $_POST,
            "post_ejecutados" => $post_ejecutados,
