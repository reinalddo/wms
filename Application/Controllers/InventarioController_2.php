    ...
    public function paginate()
    {
        ...
        $sql = "SELECT * FROM(SELECT 
                    ...
                    (
                        ...
                        SELECT COUNT(*) FROM 
                        (
                          SELECT DISTINCT v.ID_Inventario, v.idy_ubica, v.cve_articulo, v.cve_lote FROM
                           t_invpiezas v 
                           WHERE v.NConteo = 0 AND CONCAT(v.idy_ubica, v.cve_articulo, v.cve_lote) NOT IN (SELECT CONCAT(idy_ubica, cve_articulo, cve_lote) FROM t_invtarima WHERE ID_Inventario = v.ID_Inventario)
                    ...
                    (
                        (
                        SELECT COUNT(DISTINCT iv.idy_ubica) FROM t_invpiezas iv
                        WHERE iv.ID_Inventario = inv.ID_Inventario AND NConteo = 0 
                        AND iv.idy_ubica NOT IN (SELECT idy_ubica FROM t_invpiezas WHERE ID_Inventario = inv.ID_Inventario AND NConteo > 0) 
                        AND ((SELECT SUM(Existencia) FROM V_ExistenciaGral WHERE tipo = 'ubicacion' AND cve_ubicacion = iv.idy_ubica) > 0)
                        AND iv.idy_ubica NOT IN (SELECT idy_ubica FROM t_invtarima WHERE ID_Inventario = inv.ID_Inventario AND NConteo > 0)
                        ) 
                        + 
                        (
                        SELECT COUNT(DISTINCT idy_ubica) FROM t_invtarima iv
                        WHERE iv.ID_Inventario = inv.ID_Inventario AND iv.NConteo = 0 
                        AND iv.idy_ubica NOT IN (SELECT idy_ubica FROM t_invtarima WHERE ID_Inventario = inv.ID_Inventario AND iv.NConteo > 0) 
                        AND iv.idy_ubica NOT IN (SELECT idy_ubica FROM t_invpiezas WHERE ID_Inventario = inv.ID_Inventario AND NConteo > 0)
                        AND ((SELECT SUM(Existencia) FROM V_ExistenciaGral WHERE tipo = 'ubicacion' AND cve_ubicacion = iv.idy_ubica) > 0)
                        )
                    ) AS num_bls_no_inv,
...
UNION
        ...
        SELECT DISTINCT 
            ...
            WHERE (inv.ID_Inventario = {$id_inventario} AND inv.cve_articulo = c_articulo.cve_articulo AND CONCAT(inv.cve_articulo,inv.cve_lote) NOT IN (SELECT CONCAT(cve_articulo,cve_lote) FROM t_invtarima WHERE ID_Inventario = {$id_inventario} AND NConteo = 0))
            GROUP BY LP, clave,cve_ubicacion,lote
...
*/
            ...
            $sql_cerrar = "SELECT NConteo, idy_ubica, cve_articulo, cve_lote, '' as ntarima, Cantidad FROM t_invpiezas WHERE ID_Inventario = {$inventario} AND NConteo > 0 AND CONCAT(idy_ubica, cve_articulo, cve_lote) NOT IN (SELECT CONCAT(idy_ubica, cve_articulo, Cve_Lote) FROM t_invtarima WHERE ID_Inventario = {$inventario} AND NConteo > 0)

                UNION 

                SELECT NConteo, idy_ubica, cve_articulo, cve_lote, ntarima, existencia AS Cantidad FROM t_invtarima WHERE ID_Inventario = {$inventario} AND NConteo > 0
...
*/
                ...
                $sql = "";
                if($LP)
                    $sql = "UPDATE ts_existenciatarima SET existencia = {$Cantidad} WHERE idy_ubica = '{$idy_ubica}' AND cve_articulo = '{$cve_articulo}' AND cve_lote = '{$cve_lote}' AND ntarima = '{$LP}'";
                else
                    $sql = "UPDATE ts_existenciapiezas SET Existencia = {$Cantidad} WHERE idy_ubica = '{$idy_ubica}' AND cve_articulo = '{$cve_articulo}' AND cve_lote = '{$cve_lote}'";
                $res = mysqli_query($conn, $sql);
              }
...
/*
    ...
    SELECT  DISTINCT
        ...
        FROM c_articulo a
            ...
            LEFT JOIN V_ExistenciaGral vg ON a.cve_articulo = vg.cve_articulo
        WHERE v.Cve_Almac = '{$almacen}' AND ad.cve_articulo != '' AND a.cve_articulo IN (SELECT cve_articulo FROM V_ExistenciaGralProduccion) AND vg.Id_Proveedor = p.ID_Proveedor 
        $ands
...
//***************************************************************************************************
    ...
    if($cve_proveedor) $sqlProveedor = !empty($cve_proveedor) ? "AND e.ID_Proveedor = '{$cve_proveedor}'" : "";
    ...
    $sql = "
      SELECT * FROM(
         SELECT DISTINCT 
            ...
            IFNULL(COALESCE(IF(IFNULL(a.Caduca, 'N') = 'S',IF(l.Caducidad = '0000-00-00','',DATE_FORMAT(l.Caducidad,'%d-%m-%Y')),'')), '') AS caducidad,
            COALESCE(IF(a.control_numero_series = 'S',e.cve_lote,'' ), ' ') AS nserie,
            e.Existencia as cantidad,
            #(SELECT fol_folio FROM td_entalmacen WHERE cve_articulo = e.cve_articulo AND cve_lote = e.cve_lote LIMIT 1) AS folio,
            #ent.fol_folio AS folio,
            ...
            LEFT JOIN c_almacenp ap ON ap.id = z.cve_almacenp
            LEFT JOIN c_charolas ch ON ch.clave_contenedor = e.Cve_Contenedor
            LEFT JOIN td_entalmacenxtarima ent ON ent.cve_articulo = e.cve_articulo AND ent.cve_lote = e.cve_lote AND ent.ClaveEtiqueta = ch.CveLP
