...
if($_GET['m']=='2'){ 
    ...
    $sql = "Select * from (SELECT
            ...
            COALESCE(e.Existencia, 0) AS existencia, IF(e.`Existencia`>a.`CapacidadMaxima`,'amarillo',IF(e.`Existencia`>a.`CapacidadMinima`,'verde',IF(e.`Existencia`<a.`CapacidadMinima`,'rojo','rojo')))AS sta
            FROM
            ts_ubicxart AS a
            ...
            where c_ubicacion.TECNOLOGIA='PTL' and c_ubicacion.picking <> 'N'
            GROUP BY a.`idy_ubica`,a.`cve_articulo`
            ORDER BY c_ubicacion.idy_ubica) x where x.reabastecer > 0 "  ; 
