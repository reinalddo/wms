...
if(isset($_POST) && !empty($_POST) && isset($_POST['action']) && $_POST['action'] === 'loadPedidosDashboard'){
    ...
    $sqlCount = "SELECT     COUNT(DISTINCT o.Fol_folio) AS cuenta 
                ...
                WHERE o.Activo = 1 AND o.status NOT IN('F', 'K', 'T')";
    if (!empty($almacen)) {
...
if (isset($_GET) && !empty($_GET) && isset($_GET['action']) && $_GET['action'] == '') {
    ...
    $sql = "SELECT  th_pedido.Fol_folio AS totalpedidos,
                    SUM(td_pedido.Num_cantidad * c_articulo.peso) AS pesototal,
                    SUM(td_pedido.Num_cantidad * ((c_articulo.alto/1000) * (c_articulo.ancho/1000) * (c_articulo.fondo/1000))) AS volumentotal
            FROM th_pedido 
                LEFT JOIN td_pedido  ON td_pedido.Fol_folio = th_pedido.Fol_folio
                LEFT JOIN c_articulo  ON a.cve_articulo = td_pedido.Cve_articulo
