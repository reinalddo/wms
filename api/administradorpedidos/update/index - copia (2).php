...
if($_POST['action'] === 'guardarSurtidoPorUbicacion')//1
{
    ...
    if($items["existencia"] > 0)
    {
        if(!$items["sufijo"])
            $items["sufijo"] = 0;
        $sql = "CALL SPWS_SurteArticulo('".$clave."','".$items["folio"]."',".$items["sufijo"].",'".$items["clave"]."',".$items["surtidas"].",1,0,".$items["idy_ubica"].",'".$items["surtidor"]."','".$items["lote"]."', 0);";
        ...
        $sql_insert = "INSERT INTO td_surtidopiezas(fol_folio, cve_almac, Sufijo, Cve_articulo, LOTE, Cantidad, revisadas, status) VALUES ('".$items["folio"]."', '".$almacen."', ".$items["sufijo"].",'".$items["clave"]."','".$items["lote"]."',".$items["existencia"].",".$items["surtidas"].", 'L');";
        ...
        //if (!($res2 = mysqli_multi_query($conn, $sql_insert))) {echo "Falló la preparación(2): (" . mysqli_error($conn) . ") ";}
        $res2 = mysqli_multi_query($conn, $sql_insert);
        mysqli_free_result($res2);

        $sql_update = "UPDATE ts_existenciapiezas SET Existencia = Existencia - ".$items["surtidas"]." WHERE idy_ubica = ".$items["idy_ubica"]." AND cve_almac = ".$almacen." AND cve_articulo = '".$items["clave"]."';
            UPDATE th_subpedido SET HIE = NOW(), Hora_inicio = NOW() WHERE fol_folio = '".$items["folio"]."' AND Sufijo = '".$items["sufijo"]."'";
        //if (!($res3 = mysqli_multi_query($conn, $sql_update))) {echo "Falló la preparación(3): (" . mysqli_error($conn) . ") ";}
        $res3 = mysqli_multi_query($conn, $sql_update);
...
if($_POST['action'] === 'cambiarStatus')
{
    ...
    if($motivo == "QASEND")
        ...
        UPDATE th_subpedido SET HFE = NOW(), Hora_Final = NOW() WHERE fol_folio = '{$folio}' AND Sufijo = '{$sufijo}' AND cve_almac = '{$almacen}';
        ";
    else
        $sql = "
        SELECT 
        ...
        case "S":
            switch($nuevo_status)
            {
                ...
                case "L":
                    ...
                    $sql = "
                        UPDATE th_subpedido set status = 'L', cve_usuario = '' where Fol_folio = '{$folio}' and Sufijo = '{$sufijo}' and cve_almac= '{$almacen}';
...
if( $_POST['action'] == 'loadArticulos' ) 
{
    ...
    if(isset($_POST['modo']))
    {
        $data = $ga->loadArticulos($pedidos, $_POST["almacen"], $_POST["status"], $_POST['sufijo'], $_POST['is_backorder']);
        if($data[3] == 'A')
        {
            $arr = array(
              "success"   =>  true,
                    ...
                    {//4
                       ...
                       $nivel = $row['nivel'];
                        ...
                        if($sufijo == 0 || $sufijo == '')
                        {
                          ...
                          $where_select = "WHERE cve_articulo IN ($cve_articulo)";
                        }
                          else if($nivel > 0)
                          {//3
                            /*
...
if( $_POST['action'] == 'asignar' ) //1
{
    ...
    //$res = mysqli_query($conn, $sql);
    if (!($res = mysqli_query($conn, $sql))) {
        //$sql_ejecutados .=  "c_almacenp: (" . mysqli_error($conn) . ") ";
    ...
    $sp = "SPWS_PreparaPedidoSurtido".((strpos($_SERVER['HTTP_HOST'], 'nikken') !== false)?"Nik_2":"");
    //$sp = "SPWS_PreparaPedidoSurtidoNik_2";
    $sql_ejecutados .= $sp." ------ ";
    //$res=array();
    $debug="";
    $call_sp_used = "";
...
//            @mysqli_close($conn);
            ...
            //{
                ...
                $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
                if (!($res2 = mysqli_query($conn, $sql))) {
                    //$sql_ejecutados .=  "t_recorrido_surtido: (" . mysqli_error($conn) . ") ";
                    echo "t_recorrido_surtido: (" . mysqli_error($conn) . ") ";
                }
                ...
                if($num_rows == 0)
                {
                    ...
                    if (!($res = mysqli_multi_query($conn, $sql))) {
                        //$sql_ejecutados .= " DELETE1: (" . mysqli_error($conn) . ") ";
                        echo " DELETE1: (" . mysqli_error($conn) . ") ";
                    }
                    @mysqli_close($conn);
                    ...
                    if (!($res = mysqli_multi_query($conn, $sql))) {
                        ...
                        echo " DELETE2: (" . mysqli_error($conn) . ") ";
                    }
                    @mysqli_close($conn);
                    ...
                    $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
                    if (!($res = mysqli_multi_query($conn, $sql))) {
                        //$sql_ejecutados .= " DELETE3: (" . mysqli_error($conn) . ") ";
                        echo " DELETE3: (" . mysqli_error($conn) . ") ";
                    }
            ...
            //}
                    ...
                    while($row = mysqli_fetch_array($res))
                    {
                        ...
                        $sql = "SELECT IFNULL(SUM(Existencia), 0) AS Existencia_Total FROM VS_ExistenciaParaSurtido WHERE VS_ExistenciaParaSurtido.cve_articulo = '$cve_articulo' AND cve_almac = '$almacen' ";//GROUP BY cve_articulo
                        $sql_ejecutados .= $sql." ------ ";
                        ...
                        if(!$existencia || $num_rows == 0 || $cantidad_bo > 0)
                        {
                            ...
                            if(!$listo_folio_backorder && !$folio_exist)
                            {
                                ...
                                $sql = "INSERT INTO th_backorder(Folio_BackO, Fol_Folio, Cve_Clte, Fec_Pedido, Fec_Entrega, Fec_BO, Pick_num, Status) VALUES ('$folio_backorder', '$pedidos', '$Cve_clte', '$fecha', '$Fec_Entrega', '$fecha_actual', '$Pick_Num', 'A') ";
                                $sql_ejecutados .= $sql." ------ ";
                            ...
                            {
                                $sql = "INSERT INTO td_backorder(Folio_BackO, Cve_Articulo, Cve_Lote, Cantidad_Pedido, Cantidad_BO, Status) VALUES ('$folio_backorder', '$cve_articulo', '$cve_lote', '$cantidad', '$cantidad_bo', 'A')";
                                $sql_ejecutados .= $sql." ------ ";
...
/*
                        ...
                        {
                            $sql = "UPDATE td_pedido SET status = 'S' WHERE id = $id";
                            $res = mysqli_query($conn, $sql);
                        }
*/
                    }
            ...
            /*
            $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
            $sql="UPDATE th_pedido SET status = 'S' WHERE Fol_folio = '$folios';";
            if (!($res = mysqli_multi_query($conn, $sql))) {
                //$sql_ejecutados .= "UPDATE1: (" . mysqli_error($conn) . ") ";
                echo "UPDATE1: (" . mysqli_error($conn) . ") ";
            }
            @mysqli_close($conn);
            */
            $folio = $pedidos;
/*
            ...
            $res = mysqli_multi_query($conn, $sql);
            $row = mysqli_fetch_array($res);
            $num_subpedidos1 = $row['num'];

            $sql="SELECT COUNT(*) num FROM th_subpedido th, td_subpedido td WHERE th.fol_folio = '{$folio}' AND th.fol_folio = td.fol_folio AND th.status = 'S' AND th.Sufijo = td.Sufijo AND th.cve_almac = '{$almacen}'";
            $res = mysqli_multi_query($conn, $sql);
            $row = mysqli_fetch_array($res);
            $num_subpedidos = $row['num'];
