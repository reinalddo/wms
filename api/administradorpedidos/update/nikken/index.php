...
if($_POST['action'] === 'guardarSurtidoPorUbicacion')//1
{
    ...
    {
        $sql = "CALL SPWS_SurteArticulo('".$row["clave"]."','".$items["folio"]."',".$items["sufijo"].",'".$items["clave"]."',".$items["surtidas"].",1,0,".$items["idy_ubica"].",'".$items["surtidor"]."','".$items["lote"]."', 0);";
        ...
        $res2 = mysqli_multi_query($conn, $sql);
        ...
        $res3 = mysqli_multi_query($conn, $sql);
        ...
        if($pedidos)
        {
            ...
            //}
                    ...
                    while($row = mysqli_fetch_array($res))
                    {
                        ...
                        $sql = "SELECT IFNULL(SUM(Existencia), 0) AS Existencia_Total FROM VS_ExistenciaParaSurtido WHERE VS_ExistenciaParaSurtido.cve_articulo = '$cve_articulo' GROUP BY cve_articulo";

                        if (!($res = mysqli_query($conn, $sql))) {
                            //$sql_ejecutados .= " DELETE2: (" . mysqli_error($conn) . ") ";
                            echo " Existencia_Total: (" . mysqli_error($conn) . ") ";
                        }
                        $existencia = mysqli_fetch_array($res)["Existencia_Total"];
                        ...
                        if(!$existencia || $num_rows == 0 || $cantidad_bo > 0)
                        {
                            ...
                            if(!$listo_folio_backorder)
                            {
                                ...
                                $res = mysqli_query($conn, $sql);
                                $row_pedido = mysqli_fetch_array($res);
                                $Cve_clte = $row_pedido["Cve_Clte"];
                                $Fec_Entrega = $row_pedido["Fec_Entrega"];
                                $Pick_Num = $row_pedido["Pick_Num"];
                                $fecha_actual = $ga->fecha_actual();

                                $sql = "INSERT INTO th_backorder(Folio_BackO, Fol_Folio, Cve_Clte, Fec_Pedido, Fec_Entrega, Fec_BO, Pick_num, Status) VALUES ('$folio_backorder', '$pedidos', '$Cve_clte', '$fecha', '$Fec_Entrega', '$fecha_actual', '$Pick_Num', 'S') ";
                                $res = mysqli_query($conn, $sql);
                                $listo_folio_backorder = true;
                            }

                            $sql = "INSERT INTO td_backorder(Folio_BackO, Cve_Articulo, Cve_Lote, Cantidad_Pedido, Cantidad_BO, Status) VALUES ('$folio_backorder', '$cve_articulo', '$cve_lote', '$cantidad', '$cantidad_bo', 'S')";
                            $res = mysqli_query($conn, $sql);
                        }
                        else
                        {
