...
if($_POST['action'] === 'guardarSurtidoPorUbicacion')//1
{
    ...
    {
        $sql = "CALL SPWS_SurteArticulo('".$row["clave"]."','".$items["folio"]."',".$items["sufijo"].",'".$items["clave"]."',".$items["surtidas"].",1,0,".$items["idy_ubica"].",'".$items["surtidor"]."','".$items["lote"]."');";
        if (!($res = mysqli_query($conn, $sql)))
        {
        ...
        if (!($res = mysqli_query($conn, $sql_insert))) {echo "Falló la preparación(1): (" . mysqli_error($conn) . ") ";}
        ...
        if (!($res = mysqli_query($conn, $sql_update))) {echo "Falló la preparación(1): (" . mysqli_error($conn) . ") ";}
    }
    $porcentaje = ($items["surtidas"] * 100)/$total;
