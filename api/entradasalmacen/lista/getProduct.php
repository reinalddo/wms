...
if (isset($_POST) && !empty($_POST)) {
    ...
    $sql = "SELECT
				...
				t_pendienteacomodo.Cantidad as CantidadaUbicar
				FROM
				c_articulo
				LEFT JOIN c_lotes ON c_articulo.cve_articulo = c_lotes.cve_articulo
				INNER JOIN t_pendienteacomodo ON c_lotes.LOTE = t_pendienteacomodo.cve_lote
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        $responce->rows[$i]['cell']=array($row['cve_articulo'], utf8_encode($row['des_articulo']), $lote, $CADUCIDAD, $row['num_multiplo'], $row['cve_umed'], $row['CantidadaUbicar']);
        $i++;
    }
