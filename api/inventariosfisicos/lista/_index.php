...
if(isset($_GET) && !empty($_GET) && $_GET['action'] === 'getAvailableUsers')
{
  ...
  mysqli_set_charset($conn, 'utf8');
  $usuarios = '';
  $sql = "SELECT cve_usuario, nombre_completo FROM c_usuario WHERE Activo = 1 AND cve_usuario NOT IN (SELECT cve_usuario FROM t_conteoinventario WHERE ID_Inventario = {$id}) ORDER BY nombre_completo ASC;";
