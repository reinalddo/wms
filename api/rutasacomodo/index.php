...
if(isset($_POST['action']) && !empty($_POST['action'])){ 
    ...
    switch ($_POST['action']) 
    {
        ...
        case 'save':
            ...
            $pre_sql = "INSERT INTO c_ubicacion (cve_almac, cve_pasillo, cve_rack, cve_nivel, num_ancho, num_largo, num_alto, picking, Seccion, Ubicacion, PesoMaximo, CodigoCSD, TECNOLOGIA, Tipo, AcomodoMixto, AreaProduccion, clasif_abc, Ptl, Activo, Maximo, Minimo) VALUES 
                        ('".strip_tags($_POST['zone'])."',
                        '".strip_tags($_POST['pasillo'])."',
                        '".strip_tags($_POST['rack'])."',
                        '_NIVEL_',
        ...
        case 'search-ubica':
                ...
                $sql = "SELECT a.idy_ubica, a.cve_almac, a.cve_pasillo, a.cve_rack, a.cve_nivel, a.num_ancho, a.num_largo, a.num_alto, a.Status, a.picking, a.Seccion, a.Ubicacion, a.orden_secuencia, a.PesoMaximo, a.PesoOcupado, a.claverp, a.CodigoCSD, a.TECNOLOGIA, a.Maneja_Cajas, a.Maneja_Piezas, a.Reabasto, a.Activo, a.Tipo, a.AcomodoMixto, a.AreaProduccion, a.Ptl, a.Maximo, a.Minimo, (b.des_almac)zona, (b.cve_almac)almacen, (c.clave)alma, IFNULL(a.clasif_abc, '') as clasif_abc FROM c_ubicacion a, c_almacen b, c_almacenp c WHERE a.cve_almac = b.cve_almac and b.cve_almacenp = c.id and a.idy_ubica = '".$_POST['idy_ubica']."'";
                $res = getArraySQL($sql);
        ...
        case 'generar-secuencia':
                ...
                $sql = 'DELETE FROM td_ruta_acomodo WHERE id_zona = "'.$_POST['zona'].'"';
                if($_POST['proveedor'])
                    $sql = 'DELETE r FROM td_ruta_acomodo r 
                            INNER JOIN c_ubicacion u ON r.idy_ubica = u.idy_ubica AND u.cve_almac = r.id_zona
                            INNER JOIN V_ExistenciaGralProduccion v ON v.cve_ubicacion = u.idy_ubica AND v.Id_Proveedor = "'.$_POST['proveedor'].'" WHERE r.id_zona = "'.$_POST['zona'].'"';

                $result = mysqli_query($conexion, $sql);

                $sql = 'INSERT IGNORE INTO td_ruta_acomodo(id_zona, idy_ubica, orden_secuencia) (SELECT "'.$_POST['zona'].'", idy_ubica, (@i:=@i+1) FROM c_ubicacion WHERE cve_almac = "'.$_POST['zona'].'" and cve_nivel != 1 and cve_nivel != 2 ORDER BY CodigoCSD ASC)';
                if($_POST['proveedor'])
                {
                    $sql = 'INSERT IGNORE INTO td_ruta_acomodo(id_zona, idy_ubica, orden_secuencia) (SELECT DISTINCT "'.$_POST['zona'].'", c.idy_ubica, (@i:=@i+1) FROM c_ubicacion c INNER JOIN V_ExistenciaGralProduccion v ON v.cve_ubicacion = c.idy_ubica AND v.tipo = "ubicacion" WHERE c.cve_almac = "'.$_POST['zona'].'" AND v.Id_Proveedor = "'.$_POST['proveedor'].'" AND c.Activo = 1 and c.cve_nivel != 1 and c.cve_nivel != 2 ORDER BY c.CodigoCSD ASC)';
                }
                $result = mysqli_query($conexion, $sql);
...
if (!($res_charset = mysqli_query($conexion, $sql_charset)))echo "Falló la preparación Charset: (" . mysqli_error($conexion) . ") ";
