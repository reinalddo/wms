...
if (isset($_POST) && !empty($_POST)) 
{
    ...
    if($_criterio != '')
    {
        $and =" and (ss.des_ssgpoart like '%".$_criterio."%' OR ss.cve_ssgpoart like '%".$_criterio."%')  ";
    }
...
*/
    ...
    $sql = "
        ...
        where ss.Activo = '1' #AND ss.id_almacen = '{$almacen}'
            ".$and." 
        order by ss.".$sidx." ".$sord."
    ";
    ...
    {
        $arr[] = $row;
        $responce["rows"][$i]['id']=utf8_encode($row['cve_ssgpoart']);
        $responce["rows"][$i]['cell']=array('', utf8_encode($row['cve_ssgpoart']), utf8_encode($row['des_sgpoart']),utf8_encode($row['des_ssgpoart']), $row['id']);
        $i++;
    }
