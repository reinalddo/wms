...
if (isset($_POST) && !empty($_POST)) {
	...
	$tipo_pallet = $_POST['vacio']; //Generico / No Generico
	$tipo 		 = $_POST['tipo']; //Pallet/Contenedor/Caja
	$status 	 = $_POST['status'];
	$activo 	 = $_POST['activo'];
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
*/
	if($activo != "")
	$sql_activo = " AND c_charolas.Activo = '$activo' ";

	if($status != "2")
	{
		if($status == '0') //ocupado
		{
			//$sqlStatus = " AND IFNULL(c_charolas.Clave_Contenedor, '') IN (SELECT Cve_Contenedor FROM V_ExistenciaGralProduccion WHERE Cve_Contenedor != '' AND Existencia > 0) AND IFNULL(tde.ClaveEtiqueta, '') != '' AND IFNULL(c_charolas.IDContenedor, '') IN (SELECT Id_Caja FROM ts_existenciacajas WHERE PiezasXCaja > 0) ";
			//$sqlStatus = " AND c_charolas.CveLP != c_charolas.clave_contenedor ";
			$sqlStatus = " WHERE lp.statu = 'Ocupado' ";
		}
		else //disponible
		{
			//$sqlStatus = " AND IFNULL(c_charolas.Clave_Contenedor, '') NOT IN (SELECT Cve_Contenedor FROM V_ExistenciaGralProduccion WHERE Cve_Contenedor != '' AND Existencia > 0) AND IFNULL(tde.ClaveEtiqueta, '') = '' AND IFNULL(c_charolas.IDContenedor, '') NOT IN (SELECT Id_Caja FROM ts_existenciacajas WHERE PiezasXCaja > 0) ";
			//$sqlStatus = " AND c_charolas.CveLP = c_charolas.clave_contenedor ";
			$sqlStatus = " WHERE lp.statu = 'Disponible' ";
		}
	}
    ...
    SELECT * FROM (
				SELECT
						...
						IF(IFNULL(c_charolas.clave_contenedor, '') NOT IN (SELECT IFNULL(Cve_Contenedor, '') FROM V_ExistenciaGralProduccion WHERE Cve_Contenedor != '')  AND IFNULL(c_charolas.IDContenedor, '') NOT IN (SELECT Id_Caja FROM ts_existenciacajas WHERE PiezasXCaja > 0) AND c_charolas.Activo = 1, 'Disponible', 'Ocupado') AS statu,
						...
						IFNULL(IF(tde.Ubicada = 'N' AND IFNULL(c_charolas.Clave_Contenedor, ''), 'RTM', u.CodigoCSD), '') AS BL,
						...
						c_charolas.CveLP,
						#CONCAT(c_charolas.CveLP, IF(IFNULL(cj.CveLP, '') != '', CONCAT('<br>',cj.Tipo,': ',cj.CveLP), '')) as CveLP,
						c_charolas.Activo,
				...
				FROM c_charolas
                        ...
                        #LEFT JOIN ts_existenciacajas ec ON ec.Id_Caja = c_charolas.IDContenedor 
                        #LEFT JOIN c_charolas cj ON cj.IDContenedor = ec.nTarima 
                        LEFT JOIN c_ubicacion u ON u.idy_ubica = ep.cve_ubicacion #IFNULL(ep.cve_ubicacion, ec.idy_ubica) = IFNULL(u.idy_ubica, ec.idy_ubica) 
