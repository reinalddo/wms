...
if (isset($_POST) && !empty($_POST)) {
    ...
    if(isset($_POST['criterio']))
    {
        ...
        {
            "and  concat(des_grupo,'',cve_grupo) like '%".$_criterio."%'"; 
        }
    }
    ...
    while ($row = mysqli_fetch_array($res)) {
        $arr[] = $row;
        $responce["rows"][$i]['id']=$row['cve_grupo'];
        $responce["rows"][$i]['cell']=array($row['cve_grupo'], utf8_encode($row['des_grupo']));
        $i++;
    }
