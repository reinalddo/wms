...
//****************************************************************************************
    ...
    if(count($arr_lectura) == 4)
    {
        ...
        {
            $resulCodigo = "LecturaIncorrectaOT";
            $codigo_correcto = false;
        }
...
else if($cantidad_art_compuesto > $cantidad_faltante)
{
...
*/
        ...
        while($row_art = mysqli_fetch_array($res_art))
        {
            ...
            $unidad_med_componente   = $row_art['unidad_med'];
            $peso_componente         = $row_art['peso'];

            if(!$peso_componente) $peso_componente = 1;

            if($control_peso_componente == 'S' && $unidad_med_componente == 'H87') $cantidad_requerida = round($cantidad_art_compuesto/$peso_componente, 3);
        ...
        if($acepto)
        {
            ...
            while($row_art_1 = mysqli_fetch_array($res_art))
            {
                ...
                if($control_peso_componente == 'S' && $unidad_med_componente == 'H87') $cantidad = round($cantidad_art_compuesto/$peso_componente, 3);
...
if($radio_select == 'pbase')
{
    ...
    if($unidad_medida == 'H87' && $control_peso_base == 'S' && $peso_componente > 0)
    ...
    $sql = "INSERT INTO t_cardex(cve_articulo, cve_lote, fecha, origen, destino, cantidad, id_TipoMovimiento, cve_usuario, Cve_Almac) VALUES ('{$cod_art_compuesto}', '{$lote_componente}', NOW(), 'PD_{$orden_id}', '{$idy_ubica}', {$cantidad_sobrante}, 8, '{$cve_usuario}','{$cve_almacen}')";
    ...
    if($existe_producto == 0)
    {
       ...
       $sql = "INSERT INTO ts_existenciapiezas(cve_almac, idy_ubica, cve_articulo, cve_lote, Existencia, ID_Proveedor) VALUES ('{$cve_almacen}', {$idy_ubica}, '{$art_no_compuesto_val_base}', '{$lote_compuesto}', {$cantidad_sobrante}, {$ID_Proveedor}) ON DUPLICATE KEY UPDATE Existencia = Existencia + {$cantidad_sobrante}";
    }
    if (!($res = mysqli_query($conn, $sql)))
        echo "Falló la preparación: 5(" . mysqli_error($conn) . ") --> ".$sql;
...
else
{
    ...
    if($unidad_medida_comp == 'H87' && $control_peso_comp == 'S' && $peso_componente > 0)
        $cantidad_componente = $cantidad_componente/$peso_componente;

    $sql = "UPDATE ts_existenciapiezas SET Existencia = Existencia - $cantidad_componente WHERE idy_ubica = '$idy_ubica' AND cve_articulo = '$componente_derivado_select' AND cve_lote = '{$lote_componente}'";
    ...
    if($existe_producto == 0)
    {
       ...
       $sql = "INSERT INTO ts_existenciapiezas(cve_almac, idy_ubica, cve_articulo, cve_lote, Existencia, ID_Proveedor) VALUES ('{$cve_almacen}', {$idy_ubica}, '{$art_sobrante}', '{$lote_compuesto_derivado}', {$cantidad_sobrante}, {$ID_Proveedor}) ON DUPLICATE KEY UPDATE Existencia = Existencia + {$cantidad_sobrante}";
    }
    if (!($res = mysqli_query($conn, $sql)))
        echo "Falló la preparación: 5(" . mysqli_error($conn) . ") --> ".$sql;
...
if( $_POST['action'] == 'terminar_produccion' ) 
{
    ...
    $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        ...
        if($Tipo == 'IMP_LP')
        {
            ...
            if (!$res = mysqli_query($conn, $sql)) 
            {
                echo "Falló la preparación1: (" . mysqli_error($conn) . ") ";
            }

            $sql = "UPDATE th_subpedido set status='T' WHERE fol_folio = '$folio'";
            if (!$res = mysqli_query($conn, $sql)) 
            {
                echo "Falló la preparación2: (" . mysqli_error($conn) . ") ";
            }
        }
      ...
      $query = mysqli_query($conn, $sql);
        ...
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación3: (" . mysqli_error($conn) . ") ";
        }
        ...
        if($folio_rel)
        {
            ...
            {
                echo "Falló la preparación4: (" . mysqli_error($conn) . ") ";
            }

            $sqlCount = "UPDATE td_pedido set cve_lote = '$folio' where Fol_folio = '$folio_rel'";
            if (!$res = mysqli_query($conn, $sqlCount)) 
            {
                echo "Falló la preparación5: (" . mysqli_error($conn) . ") ";
            }
        ...
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación8: (" . mysqli_error($conn) . ") ";
        }
        ...
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación9: (" . mysqli_error($conn) . ") ";
        }
        ...
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación10: (" . mysqli_error($conn) . ") ";
        }
        ...
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación11: (" . mysqli_error($conn) . ") ";
        }
        }
        //**************************************************************************************************
        //**************************************************************************************************
        ...
        $sql = "SELECT DISTINCT
            ...
            $sql1 = $sql;
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación12: (" . mysqli_error($conn) . ") ";
        }
        ...
        $sql = "SELECT DISTINCT
            ...
            LEFT JOIN c_articulo acf ON acf.clasificacion = a.clasificacion AND acf.cve_articulo != op.Cve_Articulo
            LEFT JOIN c_unimed um ON um.id_umed = a.unidadMedida
            ...
            $sql2 = $sql;
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación13: (" . mysqli_error($conn) . ") ";
        }
        ...
        $sql3 = $sql;
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación14: (" . mysqli_error($conn) . ") ";
        }
        ...
        $sql = "SELECT DISTINCT * FROM ts_existenciapiezas WHERE cve_articulo = '$Cve_Articulo_OT' AND cve_lote = '$Cve_Lote_OT' AND cve_almac = (SELECT cve_almac FROM t_ordenprod WHERE Folio_Pro = '{$folio}') AND idy_ubica = (SELECT idy_ubica FROM t_ordenprod WHERE Folio_Pro = '{$folio}')";
        $sql4 = $sql;
        if (!($res = mysqli_query($conn, $sql))) {
            echo "Falló la preparación15: (" . mysqli_error($conn) . ") ";
        }
        $row_existencia = mysqli_fetch_array($res);
        $existencia = $row_existencia['Existencia'];
...
if( $_POST['action'] == 'existencia_producto_terminado' ) 
{
    ...
    $sql = "SELECT Existencia FROM ts_existenciapiezas WHERE CONCAT(cve_almac, cve_articulo,cve_lote) = (SELECT CONCAT(cve_almac, Cve_Articulo, Cve_Lote) FROM t_ordenprod WHERE Folio_Pro = '$folio')";
    $query = mysqli_query($conn, $sql);
...
//echo $json;
  ...
  CURLOPT_URL => 'https://3.81.177.36:50000/b1s/v1/Login',
...
*/
/*
$response = $lic->Sql2JSONDameSurtOT();
