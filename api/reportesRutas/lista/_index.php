...
*/
    ...
    $SQLArticulo1 = ""; $SQLArticulo2 = ""; $SQLArticulo_Obseq = ""; //$SQLArticulo_Obseq2 = "";
    $SQLArticulo1_sub = ""; $SQLArticulo2_sub = "";$SQLArticulo3_sub = "";
    ...
    */
     ...
     {
        $SQLFecha = " AND (DATE(ventas.FechaBusq) >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') AND DATE(ventas.FechaBusq) <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y') OR DATE(ventas.FechaCob) >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') AND DATE(ventas.FechaCob) <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y'))";
        $SQLFechaCob = " AND DATE(DetalleCob.Fecha) >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') AND DATE(DetalleCob.Fecha) <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y') ";
        if($fecha_inicio == $fecha_fin)
            $SQLFecha = " AND (DATE(ventas.FechaBusq) = STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') OR DATE(ventas.FechaCob) = STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y'))";
     }
     else
     {
      ...
      else if (!empty($fecha_inicio)) 
      {
        ...
        if(isset($_POST['credito']))
        {
            $SQLFecha = " AND (ventas.FechaBusq >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') OR ventas.FechaCob >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y')) ";
            $SQLFechaCob = " AND DATE(DetalleCob.Fecha) >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') ";
        }
        else
        {
            $SQLFecha = " AND ventas.FechaBusq >= STR_TO_DATE('{$fecha_inicio}', '%d-%m-%Y') ";
      ...
      else if (!empty($fecha_fin)) 
      {
        ...
        if(isset($_POST['credito']))
        {
            $SQLFecha = " AND (ventas.FechaBusq <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y') OR ventas.FechaCob <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y'))";
            $SQLFechaCob = " AND DATE(DetalleCob.Fecha) <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y') ";
        }
        else
        {
    ...
    $SQLRuta = "";$SQLRutaIN = "";//$SQLRutaContador = "";
    if($ruta)
    {
    ...
    $SQLDiaO = "";$SQLDiaOIN = "";$SQLDiaOIN2 = ""; //$SQLDiaOContadorVenta = ""; $SQLDiaOContadorPreVenta = "";
    if($diao)
    {
        ...
        if(isset($_POST['credito']))
        {
            $SQLDiaO = " AND (ventas.DiaOperativo = '".$diao."' OR ventas.DiaOperativoCobranza = '".$diao."' OR '".$diao."' IN (SELECT dc.DiaO FROM DetalleCob dc WHERE dc.Documento = ventas.Documento)) ";
            $SQLDiaOIN = " AND RelOperaciones.DiaO = '".$diao."' ";
            $SQLDiaOIN2 = " AND (Venta.DiaO = '".$diao."' OR Cobranza.DiaO = '".$diao."' OR ('".$diao."' IN (SELECT dc.DiaO FROM DetalleCob dc WHERE dc.Documento = Venta.Documento))) ";
        }
        else
        {
            $SQLDiaO = " AND ventas.DiaOperativo = '".$diao."' ";
            $SQLDiaOIN = " AND RelOperaciones.DiaO = '".$diao."' ";
...
*/
    ...
    $field_liquidacion = "*"; $field_liquidacion1 = ""; $field_liquidacion2 = "";
    ...
    if(isset($_POST['liquidacion']))
    {
       ...
       SUM(ventas.tienepromo) AS tienepromo, ventas.Operacion, ventas.CveCliente ";
        $field_liquidacion1 = "IF(DetalleVet.Tipo = 0, (SELECT SUM(Pza) FROM DetalleVet WHERE Docto = Venta.Documento), SUM(IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)), 0))) AS cajas_total,
      IF(DetalleVet.Tipo = 0, 0, SUM(IFNULL(IF(um.mav_cveunimed != 'XBX', (DetalleVet.Pza - (c_articulo.num_multiplo*TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, DetalleVet.Pza, 0)), 0))) as piezas_total,
...
";
    ...
    0 AS Total,
      ...
      (IFNULL(Cobranza.Saldo, 0) - IFNULL((SELECT SUM(Abono) FROM DetalleCob WHERE Documento = Venta.Documento), 0)) AS saldoFinal,
      ...
      {$field_tipo_venta_preventa}
      {$field_liquidacion1}
      #IF(DetalleVet.Tipo = 0, DetalleVet.Pza, SUM(IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)), 0))) AS cajas_total,


      #IF(DetalleVet.Tipo = 0, (SELECT SUM(Pza) FROM DetalleVet WHERE Docto = Venta.Documento), SUM(IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)), 0))) AS cajas_total,
      #IF(DetalleVet.Tipo = 0, 0, SUM(IFNULL(IF(um.mav_cveunimed != 'XBX', (DetalleVet.Pza - (c_articulo.num_multiplo*TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, DetalleVet.Pza, 0)), 0))) as piezas_total,
      #IF(pr.Tipmed = 'Caja', (SELECT SUM(Cant) FROM PRegalado WHERE Docto = Venta.Documento), 0) AS PromoC,
      #IF(pr.Tipmed != 'Caja', (SELECT SUM(Cant) FROM PRegalado WHERE Docto = Venta.Documento), 0) AS PromoP,
      ...
      LEFT JOIN DetalleCob ON DetalleCob.IdCobranza = Cobranza.id {$SQLFechaCob}
...
SELECT DISTINCT
      ...
      IFNULL(Cobranza.Status, 1) AS StatusCobranza,
      Cobranza.Documento AS Documento,
      (IFNULL(Cobranza.Saldo, IFNULL(th_pedido.TotPedidas, 0)) - IFNULL((SELECT SUM(Abono) FROM DetalleCob WHERE Documento = th_pedido.Pedido), 0)) AS saldoFinal,
      #SUM(IFNULL(DetalleCob.Abono, 0)) AS Abono,
      ...
      {$SQLRutaIN} {$SQLDiaOIN} {$SQLArticulo2} {$SQLArticulo_Obseq}
      GROUP BY Folio
...
$envplastico = 0; $envcristal = 0; $envgarrafon = 0;
$ObsequioC = 0; $ObsequioP = 0; $cajas_total = ''; $piezas_total = ''; $promoC = ''; $promoP = '';
...
while ($row = mysqli_fetch_array($res)) {
    ...
    if($row['StatusCobranza'] == 2 && $row['Cancelada'] == 0)
    {
        ...
        if(isset($_POST['envase']) || isset($_POST['estado_cuenta']))
        {
            ...
            $envplastico += $cajas_total; $envcristal += $piezas_total; $envgarrafon += $promoC;
        ...
        $timporte = 0;$tiva = 0;$tdescuento = 0;$ttotal = 0;$ttotalc = 0;$ttotalp = 0;$tpromoc = 0;$tpromop = 0;$tobseqc = 0;$tobseqp = 0;
...
//EL 03-03-2023 ME DIJERON QUE REDONDEARA PARA QUE DE PROBLEMAS EN OTRAS INSTANCIAS Y LUEGO VOLVER A COLOCAR EL REDONDEO
        ...
        $responce->rows[$i]['cell']=array('', 
            ...
            number_format(round($row['Importe']+$row['IVA'], 2), 2), 
    ...
    if(!isset($_POST['estado_cuenta']))
    {
      ...
      $importe_venta2 = " th_pedido.SubTotPedidas AS Importe, ";
      $total_venta2 = " th_pedido.TotPedidas AS Total, ";
...
SELECT DISTINCT
      ...
      SUM(DetalleVet.IVA) AS IVA,
      SUM(IFNULL(DetalleVet.DescMon, 0.00)) AS Descuento,
      IF(FormasPag.Forma LIKE '%Efectivo%' AND Venta.TipoVta = 'Contado', SUM(DetalleVet.Importe)+SUM(DetalleVet.IVA)-SUM(IFNULL(DetalleVet.DescMon, 0.00)), 0) AS efectivo,
      IF(Venta.TipoVta = 'Credito', SUM(DetalleVet.Importe)+SUM(DetalleVet.IVA)-SUM(IFNULL(DetalleVet.DescMon, 0.00)), 0) AS credito,
      IF(Venta.TipoVta = 'Contado', SUM(DetalleVet.Importe)+SUM(DetalleVet.IVA)-SUM(IFNULL(DetalleVet.DescMon, 0.00)), 0) AS contado,
      {$total_venta}
        c_articulo.num_multiplo,
        um.mav_cveunimed,
      IF(DetalleVet.Tipo = 0, (SELECT SUM(Pza) FROM DetalleVet WHERE Docto = Venta.Documento {$SQLArticulo1_sub}), SUM(IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)), 0))) AS cajas_total,
      IF(DetalleVet.Tipo = 0, 0, SUM(IFNULL(IF(um.mav_cveunimed != 'XBX', (DetalleVet.Pza - (c_articulo.num_multiplo*TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, DetalleVet.Pza, 0)), 0))) as piezas_total,
      ...
      #IF(pr.Tipmed = 'Caja', SUM(pr.Cant), 0) as PromoC,
      #IF(pr.Tipmed != 'Caja', SUM(pr.Cant), 0) as PromoP,
      IF(Venta.TipoVta = 'Obsequio', SUM(IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)), 0)), 0) AS ObseqC,
      IF(Venta.TipoVta = 'Obsequio', SUM(IFNULL(IF(um.mav_cveunimed != 'XBX', (DetalleVet.Pza - (c_articulo.num_multiplo*TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, DetalleVet.Pza, 0)), 0)), 0) AS ObseqP
      FROM Venta
      LEFT JOIN th_pedido ON Venta.Documento LIKE CONCAT('%',th_pedido.Fol_folio)
      LEFT JOIN DetalleVet ON DetalleVet.Docto = Venta.Documento
      INNER JOIN t_ruta ON t_ruta.ID_Ruta = Venta.RutaId AND DetalleVet.RutaId = t_ruta.ID_Ruta 
...
SELECT DISTINCT
      ...
      th_pedido.TotIVAPedidas AS IVA,
      IFNULL(th_pedido.TotDescPedidas, 0.00) AS Descuento,
      IF(FormasPag.Forma LIKE '%Efectivo%' AND th_pedido.TipoPedido = 'Contado', th_pedido.SubTotPedidas+th_pedido.TotIVAPedidas-IFNULL(th_pedido.TotDescPedidas, 0.00), 0) AS efectivo,
      IF(th_pedido.TipoPedido = 'Credito', th_pedido.SubTotPedidas+th_pedido.TotIVAPedidas-IFNULL(th_pedido.TotDescPedidas, 0.00), 0) AS credito,
      IF(th_pedido.TipoPedido = 'Contado', th_pedido.SubTotPedidas+th_pedido.TotIVAPedidas, 0) AS contado,
      th_pedido.TotPedidas AS Total,
    c_articulo.num_multiplo,
    um.mav_cveunimed,
...
else if(isset($_POST['consolidado']))
{
    ...
    $sql = "
    SELECT DISTINCT
      ventas.sucursalNombre AS sucursalNombre,
      ventas.Sucursal AS Sucursal,
      ventas.Cliente AS Cliente,
      ventas.Responsable AS Responsable,
      ventas.limite_credito AS limite_credito,
      ventas.nombreComercial AS nombreComercial,
      ...
      ventas.saldoFinal AS saldoFinal,
      ventas.Abono AS Abono,
      SUM(ventas.saldoActual) AS saldoActual,
      SUM(ventas.cajas_total) AS cajas_total,
      SUM(ventas.piezas_total) AS piezas_total,
      SUM(ventas.PromoC) AS PromoC,
      SUM(ventas.PromoP) AS PromoP,
      ventas.tienepromo AS tienepromo, ventas.CveCliente
    FROM (
    SELECT DISTINCT
      c_almacenp.nombre AS sucursalNombre,
      Venta.IdEmpresa AS Sucursal,
      ...
      (Venta.Subtotal) AS Importe,
      (Venta.IVA) AS IVA,
      (IFNULL(DetalleVet.DescMon, 0.00)) AS Descuento,
      (Venta.TOTAL) AS Total,
      (IFNULL(Cobranza.Saldo, 0) - (SELECT SUM(Abono) FROM DetalleCob WHERE Documento = Venta.Documento)) AS saldoFinal,
      #SUM(IFNULL(DetalleCob.Abono, 0)) AS Abono,
      (SELECT SUM(Abono) FROM DetalleCob WHERE Documento = Venta.Documento) AS Abono,
      (IFNULL(Venta.Saldo, '0.00')) AS saldoActual,
      IF(DetalleVet.Tipo = 0, DetalleVet.Pza, (IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)), 0))) AS cajas_total,
      IF(DetalleVet.Tipo = 0, 0, (IFNULL(IF(um.mav_cveunimed != 'XBX', (DetalleVet.Pza - (c_articulo.num_multiplo*TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, DetalleVet.Pza, 0)), 0))) AS piezas_total,
      IF(pr.Tipmed = 'Caja', (pr.Cant), 0) AS PromoC,
      IF(pr.Tipmed != 'Caja', (pr.Cant), 0) AS PromoP,
      COUNT(pr.Docto) AS tienepromo
      FROM Venta
      ...
      LEFT JOIN c_unimed um ON um.id_umed = c_articulo.unidadMedida
      LEFT JOIN PRegalado pr ON pr.Docto = DetalleVet.Docto AND pr.RutaId = DetalleVet.RutaId AND pr.IdEmpresa = DetalleVet.IdEmpresa AND pr.Cliente = Venta.CodCliente
      WHERE 1 AND Venta.TipoVta = 'Credito' AND Venta.Cancelada = 0
      GROUP BY Venta.CodCliente
...
SELECT DISTINCT
      ...
      (IFNULL(th_pedido.TotDescPedidas, 0.00)) AS Descuento,
      (th_pedido.TotPedidas) AS Total,
      (IFNULL(Cobranza.Saldo, 0) - (SELECT SUM(Abono) FROM DetalleCob WHERE Documento = th_pedido.Pedido)) AS saldoFinal,
      #SUM(IFNULL(DetalleCob.Abono, 0)) AS Abono,
      (SELECT SUM(Abono) FROM DetalleCob WHERE Documento = th_pedido.Pedido) AS Abono,
      '' AS saldoActual,
      (IFNULL(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, td_pedido.Pedidas),TRUNCATE((td_pedido.Pedidas/c_articulo.num_multiplo), 0)), 0)) AS cajas_total,
      (IFNULL(IF(um.mav_cveunimed != 'XBX', (td_pedido.Pedidas - (c_articulo.num_multiplo*TRUNCATE((td_pedido.Pedidas/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, td_pedido.Pedidas, 0)), 0)) AS piezas_total,
      IF(pr.Tipmed = 'Caja', (pr.Cant), 0) AS PromoC,
      IF(pr.Tipmed != 'Caja', (pr.Cant), 0) AS PromoP,
      COUNT(pr.Docto) AS tienepromo
      FROM V_Cabecera_Pedido th_pedido
      ...
      AND th_pedido.TipoPedido = 'Credito' AND th_pedido.Cancelada = 0
      #GROUP BY th_pedido.Cve_Clte
      ) AS ventas WHERE 1 AND ventas.Cliente IS NOT NULL 
        {$SQLCriterio}
      GROUP BY Cliente
    ";
...
else if(isset($_POST['cobranzadet']) && $_POST['action'] == 'realizar_abono')
{
    ...
    $sql = "INSERT INTO DetalleCob(IdCobranza, Abono, Fecha, RutaId, SaldoAnt, Saldo, FormaP, DiaO, Documento, Cliente, IdEmpresa, Cancelada)
            VALUES((SELECT DISTINCT id FROM Cobranza WHERE Documento = '{$folio}'), '$abono', NOW(), (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$ruta}'), '{$saldo_restante}', ($saldo_restante-$abono),'{$forma_pago}', '{$diao}', '{$folio}', '{$cliente}', (SELECT clave FROM c_almacenp WHERE id = '{$almacen}'), 0)";
...
if(isset($_GET) && !empty($_GET) && $_GET['action'] == 'PromocionVenta'){
   ...
   $start = $limit*$page - $limit; // do not put $limit*($page - 1)
    ...
    $sql = "SELECT DISTINCT pr.SKU AS cve_articulo, a.des_articulo, CONCAT('(', r.cve_ruta, ') ', r.descripcion) AS ruta, 
                ...
                WHERE  al.id = {$almacen} AND '$folio' LIKE CONCAT('%',pr.Docto) AND IFNULL(d.razonsocial, '') != ''
    ...
    while($row = mysqli_fetch_array($res)){
        //$row=array_map('utf8_encode', $row);
        extract($row);
        $Total = ($Importe + $IVA - $Descuento); $Total = number_format($Total, 2);
   ...
   $start = $limit*$page - $limit; // do not put $limit*($page - 1)
    ...
    SELECT DISTINCT
      ...
      Venta.TOTAL AS Total,
      SUM(IFNULL(DetalleCob.Abono, 0)) AS Abono,
      (IFNULL(Cobranza.Saldo, 0) - SUM(IFNULL(DetalleCob.Abono, 0))) AS saldoFinal
      FROM Venta
      LEFT JOIN th_pedido ON Venta.Documento LIKE CONCAT('%',th_pedido.Fol_folio)
      LEFT JOIN DetalleVet ON DetalleVet.Docto = Venta.Documento
      INNER JOIN t_ruta ON t_ruta.ID_Ruta = Venta.RutaId AND DetalleVet.RutaId = t_ruta.ID_Ruta 
...
SELECT DISTINCT
      ...
      WHERE 1 AND th_pedido.Pedido NOT IN (SELECT Documento FROM Venta WHERE IdEmpresa = '{$almacen}')
      GROUP BY Folio
      ) AS ventas WHERE 1 AND ventas.cliente = '{$cliente}' AND ventas.Status = 1
    ...
    else
    {
        ...
        if(($id_venta != 'null' && $id_venta != '') || ($operacion!='' && $operacion == 'Venta'))
        {
          ...
          '' AS TotalPedidas,
          SUM(DetalleVet.Utilidad) AS Utilidad, SUM(c_articulo.num_multiplo) AS Cajas, SUM(DetalleVet.Pza) AS Piezas, Venta.Cancelada AS Cancelada, '' AS vendedorID,
          '' AS cveVendedor, 
          IFNULL(sh.Stock, '0') AS InvInicial,
          ...
          (IF(DetalleVet.Tipo = 0, DetalleVet.Pza,SUM(IF(um.mav_cveunimed = 'XBX', IF(c_articulo.num_multiplo = 1, 0, DetalleVet.Pza),TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0)))) - IF(pr.Tipmed = 'Caja', pr.Cant, 0)) AS cajas_total,
          (IF(DetalleVet.Tipo = 0, 0, SUM(IF(um.mav_cveunimed != 'XBX', (DetalleVet.Pza - (c_articulo.num_multiplo*TRUNCATE((DetalleVet.Pza/c_articulo.num_multiplo), 0))), IF(c_articulo.num_multiplo = 1, DetalleVet.Pza, 0)))) - IF(pr.Tipmed != 'Caja', pr.Cant, 0)) AS piezas_total,
    ...
    while($row = mysqli_fetch_array($res)){
        ...
        else if(isset($_GET['consolidado']))
        {
            ...
            $responce->rows[$i]['cell']=array(
                ...
                'abono' => number_format($Abono,2),
                'saldo' => number_format($saldoFinal,2)
            );
        }
        else
        {
