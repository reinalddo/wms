...
if (isset($_POST) && !empty($_POST) && !isset($_POST['consolidado']) && !isset($_POST['cobranzadet']) && !isset($_POST['bitacora']) && !isset($_POST['bitacoranv']) && !isset($_POST['noventas']) && $_POST['action'] != 'getDetallesFolioGrupo' && $_POST['action'] != 'getEnvases' && $_POST['action'] != 'getCobranzaCxC' && !isset($_POST['actionvp'])) {
    ...
    if ($ruta == '' && $criterio == '' && $fecha_inicio == '' && $fecha_fin == '') return;
    ...
    */
    ...
    if ($diao) {
        ...
        $SQLDiaOIN2 = " AND (Cob.DiaO = '" . $diao . "') ";
        $SQLDiaOIN3 = " AND (Venta.DiaO = '" . $diao . "') ";
...
//EL 03-03-2023 ME DIJERON QUE REDONDEARA PARA QUE DE PROBLEMAS EN OTRAS INSTANCIAS Y LUEGO VOLVER A COLOCAR EL REDONDEO
        ...
        $responce->rows[$i]['cell'] = array('',
                                            $row['idVenta'],
                                            $DiaOperativo,
                                            utf8_encode($rutaName),
                                            utf8_encode($row['Operacion']),
                                            $Fecha,
                                            $row['FechaCompromiso'],
                                            utf8_encode($row['Folio']),
                                            utf8_encode($row['Cliente']),
                                            utf8_encode($row['CveCliente']),
                                            utf8_encode($row['Responsable']),
                                            utf8_encode($row['nombreComercial']),
                                            utf8_encode($status),
                                            //$row['Articulo'],
                                            utf8_encode($row['Tipo']),
                                            utf8_encode($row['metodoPago']),
                                            ...
                                            $ObsequioC,
                                            $ObsequioP,
                                            utf8_encode($row['Vendedor']),
                                            utf8_encode($row['Ayudante1']),
                                            utf8_encode($row['Ayudante2']),
                                            '', //$row['Promociones'],
                                            $row['tienepromo'],
                                            $row['limite_credito']
        );
...
SELECT DISTINCT Venta.ID,
      ...
      {$SQLRutaIN} {$SQLDiaOIN3} {$SQLArticulo1} {$SQLArticulo_Obseq} 
      GROUP BY ID
...
SELECT DISTINCT
    ...
    if($_criterio)
        $SQLBusqueda = " AND (c_cliente.Cve_Clte LIKE '%$_criterio%' OR c_cliente.RazonComercial like '%$_criterio%' OR c_cliente.RazonSocial like '%$_criterio%' OR Vw_Cobranza.Documento like '%$_criterio%' OR t_ruta.cve_ruta LIKE '%$_criterio%' OR t_ruta.descripcion LIKE '%$_criterio%') ";
/*
...
from Vw_Cobranza
         ...
         LEFT JOIN c_cliente ON c_cliente.Cve_Clte = c_destinatarios.Cve_Clte
         LEFT JOIN c_almacenp on c_almacenp.clave = Vw_Cobranza.IdEmpresa
         LEFT JOIN t_ruta on t_ruta.ID_Ruta = Vw_Cobranza.Ruta
where Vw_Cobranza.Status = 1 {$SQLBusqueda}
group by c_cliente.Cve_Clte
";
    //}
...
WHERE r.cve_ruta = '{$ruta}' AND rdc.CodCli NOT IN (SELECT Codigo FROM BitacoraTiempos WHERE Visita = 1 AND DiaO = '{$diao}')";
    ...
    */
    ...
    $forma_pago = $_POST['forma_pago'];
      ...
      $sql = "SELECT IFNULL(IFNULL(DiaO, 1), '') AS DiaO, Fecha, CURDATE() AS F_Actual FROM DiasO WHERE RutaId = (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$ruta}') AND Id = (SELECT MAX(Id) FROM DiasO WHERE RutaId = (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$ruta}'))";
      ...
      {
        $DiaO++;
        $sql = "INSERT INTO DiasO (DiaO, Fecha, RutaId, IdEmpresa) VALUES 
                ($DiaO, CURDATE(), (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$ruta}'), (SELECT clave FROM c_almacenp a WHERE id = (SELECT cve_almacenp FROM t_ruta WHERE cve_ruta = '{$ruta}')))";
    ...
    $sql = "INSERT INTO DetalleCob(IdCobranza, Abono, Fecha, RutaId, SaldoAnt, Saldo, FormaP, DiaO, Documento, Cliente, IdEmpresa, Cancelada)
            VALUES((SELECT DISTINCT id FROM Cobranza WHERE Documento = '{$folio}' AND RutaId = (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$ruta}')), '$abono', NOW(), (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '{$ruta}'), '{$saldo_restante}', ($saldo_restante-$abono),'{$forma_pago}', '{$DiaO}', '{$folio}', (SELECT DISTINCT id_destinatario FROM c_destinatarios WHERE Cve_Clte = '{$cliente}' AND Activo = '1' LIMIT 1), (SELECT clave FROM c_almacenp WHERE id = '{$almacen}'), 0)";
...
group by c_cliente.Cve_Clte,Cob.Documento";
        ...
        $sql = "SELECT dc.Id, DATE_FORMAT(dc.Fecha, '%d-%m-%Y') as Fecha, r.cve_ruta AS Ruta, dc.SaldoAnt, dc.Abono, dc.Saldo, IFNULL(c.limite_credito, 0) as limiteCredito
                FROM DetalleCob dc
                #LEFT JOIN c_destinatarios d ON d.id_destinatario = dc.Cliente
                LEFT JOIN c_destinatarios d ON d.id_destinatario = IF(dc.Cliente != '', dc.Cliente, (SELECT Cliente FROM Cobranza WHERE Documento = dc.Documento))
                LEFT JOIN c_cliente c ON d.Cve_Clte = c.Cve_Clte
    ...
    if($count == 0)
    {
        $sql = "SELECT cob.id AS Id, DATE_FORMAT(cob.FechaReg, '%d-%m-%Y') AS Fecha, cob.Saldo AS SaldoAnt, 0 AS Abono, cob.Saldo, IFNULL(c.limite_credito, 0) AS limiteCredito
            FROM Cobranza cob
            LEFT JOIN c_destinatarios d ON d.id_destinatario = IF(cob.Cliente != '', cob.Cliente, (SELECT Cliente FROM Cobranza WHERE Documento = cob.Documento))
            LEFT JOIN c_cliente c ON d.Cve_Clte = c.Cve_Clte
            WHERE cob.Documento = '{$folio}' 
            ORDER BY cob.id
            ";
        if (!($res = mysqli_query($conn, $sql))) {
            echo "9Falló la preparación: (" . mysqli_error($conn) . ") " . $sql;
...
group by c_cliente.Cve_Clte,Cobranza.Documento";
    ...
    $sql = "SELECT DISTINCT GROUP_CONCAT(DISTINCT cob.Documento) AS Id, '' AS Fecha, 
                   SUM(cob.TOTAL) AS SaldoAnt, IFNULL(cob.Abono, 0) AS Abono, (SUM(cob.TOTAL)-IFNULL(cob.Abono, 0)) AS Saldo, cob.limite_credito AS limiteCredito, cob.Cve_Clte, cob.RazonSocial
            FROM (
                SELECT DISTINCT v.*, c.limite_credito, c.Cve_Clte, c.RazonSocial, (SELECT SUM(Abono) FROM DetalleCob WHERE Documento IN ({$folios_arr})) AS Abono
                        ...
                        LEFT JOIN c_cliente c ON d.Cve_Clte = c.Cve_Clte
                        WHERE v.Documento IN ({$folios_arr}) AND c.Cve_Clte = '$cve_cliente'
            ) AS cob
            ";
 ...
 else if (isset($_POST) && !empty($_POST) && $_POST['action'] == 'getEnvases') {
    ...
    $diao = $_POST['diao'];
    $almacen = $_POST['almacen'];
    $tipo_dev_envase = $_POST['tipo_dev_envase'];
   ...
   if($tipo_dev_envase != '')
    ...
    if($SQLRuta == "" &&  $SQLDiaO == "" &&  $SQLOperacion == "" &&  $SQLCriterio == "")
     $consultar = 0;

    $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);

    $sql = "SELECT v.Id, DATE_FORMAT(v.Fecha, '%d-%m-%Y') AS Fecha, ru.cve_ruta, e.DiaO , v.Documento, d.Cve_Clte, d.razonsocial, e.Envase, e.Tipo, e.Cantidad, 
            a.Tipo_Envase AS tipo_env_clave, IF(a.Tipo_Envase = 'G', 'Garrafón', IF(a.Tipo_Envase = 'P','Plástico', IF(a.Tipo_Envase = 'C', 'Cristal', ''))) AS Tipo_Envase 
            FROM DevEnvases e 
...
*/
    ...
    //header('Content-type: application/json');
    echo json_encode($responce);
} else if (isset($_POST) && !empty($_POST) && $_POST['action'] == 'getCobranzaCxC') {
    ...
    $sql = "CALL SPAD_ReporteCobranzaVtasH('$cve_almacen',$fecha_inicio,$fecha_fin,'$ruta',0,'$operacion','','','',$diao,'', $status,'$criterio')";
    if($operacion == 'PreVenta' || $operacion == 'Entrega')
        $sql = "CALL SPAD_ReportePedidosEntregasH('$cve_almacen',$fecha_inicio,$fecha_fin,'$ruta',0,'$operacion','','','',$diao,'','$criterio')";
...
/*
        $folio = $row['Folio'];
        $cve_ruta = $row['rutaName'];
    $sql_id_venta = "SELECT Id FROM Venta WHERE Documento = '$folio' AND RutaId = (SELECT ID_Ruta FROM t_ruta WHERE cve_ruta = '$cve_ruta' AND IdEmpresa = '$cve_almacen');";
    if (!($res_id_venta = mysqli_query($conn, $sql_id_venta))) echo "Falló la preparación Charset: (" . mysqli_error($conn) . ") ";
    $id_venta = mysqli_fetch_array($res_id_venta)['Id'];
        mysqli_free_result($res_id_venta);
*/
        //if($row['Tipo'] != 'Credito') continue;
        //$responce->rows[$i]['Id'] = $id_venta;
        ...
        if ($row['Status'] == 2 && $row['Cancelada'] == 'No') {
            $status = "<b style='color:blue;'>Pagada</b>";
            $status_calc = 0;
        }
...
//    $responce->tadeudo = $sql;
    ...
    //header('Content-type: application/json');
    echo json_encode($responce);
}else if (isset($_GET) && !empty($_GET) && $_GET['actionvp'] != 'getDetallesFolioSP') {
    $page = $_GET['page'];
    $rows = $_GET['rows'];
    $search = $_GET['search'];
...
if($_POST['actionvp'] == 'datosVentaPreventa')
{
    $page = $_POST['page'];  // get the requested page
    $limit = $_POST['rows']; // get how many rows we want to have into the grid
    $sidx = $_POST['sidx'];  // get index row - i.e. user click to sort
...
/*
SPAD_ReporteVtas
Almacen varchar(50), --> Este valor es obligatorio
FIni    Date, --> Poner NULL si no hay fecha inicial
FFin    Date, --> Poner NULL si no hay fecha final
CveRuta varchar(50), --> Dejar como cadena vacía si no se ocupa ruta
Vend    int, --> Dejar en cero (0) si no se ocupa vendedor
Oper    varchar(20), --> dejar cadena vacía si no se ocupa operación
TipoO   varchar(20), --> dejadejar cadena vacía si no se ocupa Tipo
Cte     varchar(50), --> dejadejar cadena vacía si no se ocupa Destinatario
CveCte  varchar(50), --> dejadejar cadena vacía si no se ocupa Cliente
DiaOp   int, --> Dejar en cero (0) si no se ocupa día operativo
CveArt  varchar(50) --> dejadejar cadena vacía si no se ocupa Clave de artículo
EL SP para el reporte de pedidos y entregas es el siguiente:
SPAD_ReportePedidosEntregas
Los parámetros son los mismo del reporte de ventas-devoluciones
...
*/
    ...
    $sql = "CALL SPAD_ReporteVtasH('$cve_almacen',$fecha_inicio,$fecha_fin,'$ruta',0,'$operacion','','','$cliente',$diao,'$articulos')";
    if($operacion == 'PreVenta' || $operacion == 'Entrega')
        $sql = "CALL SPAD_ReportePedidosEntregasH('$cve_almacen',$fecha_inicio,$fecha_fin,'$ruta',0,'$operacion','','','$cliente',$diao,'$articulos','$_criterio')";
    if (!($res = mysqli_query($conn, $sql))) echo "Falló la preparación SP: (" . mysqli_error($conn) . ") ";
...
/*
            var html = '<a class="btn btn-info" href="#" onclick="ver(\'' + id_venta + '\', \'' + folio + '\', \'' + cliente + '\', \'' + responsable + '\', \'' + operacion + '\', \'' + sucursal + '\', \'' + dia_operativo + '\', \'' + ruta + '\', \'' + vendedor + '\', \'' + tienepromo + '\')"><i class="fa fa-search" title="Ver Detalles"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
...
*/
/*
        $id_venta = ($row['Operacion'] == 'Venta')?(""):($row['Folio']);

        $html = "<a class='btn btn-info' href='#' onclick='ver('$id_venta', '".$row['Folio']."', '".$row['Cliente']."', '".$row['Responsable']."', '".$row['Operacion']."', '".$row['nombreComercial']."', '".$row['DiaO']."', '".$row['rutaName']."', '".$row['CveVendedor']."', '0')'><i class='fa fa-search' title='Ver Detalles'></i></a>&nbsp;&nbsp;&nbsp;&nbsp;";
*/
        $responce->rows[$i]['cell'] = array('',
                                            ($row['Operacion'] == 'Venta')?(""):($row['Folio']),
                                            $row['DiaO'],
                                            $row['rutaName'],
                                            $row['Operacion'],
                                            $row['Fecha'],
                                            ...
                                            $row['Responsable'],
                                            $row['nombreComercial'],
                                            ($row['Cancelada'] == 'Si')?("<span style='color:red;font-weight:bold;'>Cancelada</span>"):("<span style='color:green;font-weight:bold;'>Abierta</span>"),
                                            $row['Tipo'],
                                            $row['TipoPago'],
                                            $row['Importe'],
                                            $row['IVA'],
                                            $row['Descuento'],
                                            number_format($row['Importe'] + $row['IVA']-$row['Descuento'], 2),
                                            '',
                                            '',
                                            $row['Total_Cajas'],
                                            $row['Total_Piezas'],
...
//Otros Pagos
        ...
        $timporte = $row['Imp A/IVA'];
        $tiva = $row['Total IVA'];
        $tdescuento = $row['Descuentos'];
        $ttotal = $row['Total'];
        $ttotalc = $row['Total Cajas'];
        $ttotalp = $row['Total Piezas'];
        $tpromoc = $row['Total Cajas Promo'];
        $tpromop = $row['Total Piezas Promo'];
        $tefectivo = $row['Efectivo'];
        $tcontado = $row['Contado'];
        $tcredito = $row['Credito'];
        $tdevoluciones = $row['Devoluciones'];
        $totrospagos = $row['Otros Pagos'];
        $tcobranza = $row['Total Cobranza'];
        $tobseqc = $row['Total Cajas Obsequio'];
        $tobseqp = $row['Total Piezas Obsequio'];

    }
    if(!$tdevoluciones) $tdevoluciones = 0;
    if(!$tobseqc) $tobseqc = 0;
    if(!$tobseqp) $tobseqp = 0;
    if(!$tcredito) $tcredito = 0;
    if(!$totrospagos) $totrospagos = 0;
    if(!$tcontado) $tcontado = 0;
    if(!$tcobranza) $tcobranza = 0;

    $responce->timporte = number_format($timporte, 2);
    $responce->tiva = number_format($tiva, 2);
    $responce->tdescuento = number_format($tdescuento, 2);
    $responce->ttotal = number_format($ttotal, 2);
    $responce->ttotalc = $ttotalc;
    $responce->ttotalp = $ttotalp;
    $responce->tpromoc = $tpromoc;
    $responce->tpromop = $tpromop;
    $responce->tefectivo = number_format($tefectivo, 2);
    $responce->tcontado = number_format($tcontado, 2);
    $responce->tcredito = number_format($tcredito, 2);
    $responce->tdevoluciones = number_format($tdevoluciones, 2);
    $responce->totrospagos = number_format($totrospagos, 2);
    $responce->tcobranza = number_format($tcobranza, 2);
    $responce->tobseqc = number_format($tobseqc, 2);
    $responce->tobseqp = number_format($tobseqp, 2);
    $responce->ttliq = number_format($ttotal+$tcobranza-$tdevoluciones-$tcredito, 2);
...
if (isset($_GET) && !empty($_GET) && $_GET['actionvp'] == 'getDetallesFolioSP') {
    $page = $_GET['page'];
    $rows = $_GET['rows'];
    $limit = $_GET['rows']; // get how many rows we want to have into the grid
    //$search = $_GET['search'];
    ...
    $sql = "CALL SPAD_ReporteVtas('$cve_almacen', '$folio',$fecha_inicio,$fecha_fin,'$ruta',0,'$operacion','','','',$diao,'$cve_articulo')";
    if($operacion == 'PreVenta' || $operacion == 'Entrega')
        $sql = "CALL SPAD_ReportePedidosEntregas('$cve_almacen', '$folio',$fecha_inicio,$fecha_fin,'$ruta',0,'$operacion','','','',$diao,'$cve_articulo')";
  ...
  /*
        if($operacion == 'PreVenta' || $operacion == 'Entrega')
        {
            if(($Importe+$IVA) == 0) continue;
                //$responce->rows[$i]['Folio'] = $Pedido;
                $responce->rows[$i]['cell'] = array(
                    'clave'      => $Articulo,
                    'articulo'   => utf8_decode($Descripcion),
                    'InvInicial' => '',
                    'cajas'      => $cajas_total,
                    'piezas'     => $piezas_total,
                    'importe'    => $Importe,
                    ...
                    'promc'      => $PromoC,
                    'promp'      => $PromoP,
                    'tienepromo' => ($PromoC+$PromoP)
            );
        }
        else{
*/
            if(($Importe+$IVA) == 0) continue;
                $responce->rows[$i]['Folio'] = $Folio;
                $responce->rows[$i]['cell'] = array(
                    'clave'      => $cve_articulo,
