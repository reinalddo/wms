...
where tr.cve_almacenp = '$almacen'";
    ...
    if (!$result = mysqli_query($conn, $sqlSemaforo)) {
        echo json_encode(array('status' => 'error', 'data' => 'Error al obtener los datos', 'sql1'=>$sqlSemaforo));
        exit;
    }
    $semaforo = array();
    while ($row = mysqli_fetch_assoc($result)) {
...
GROUP BY Ruta";
    ...
    if (!$result = mysqli_query($conn, $sqlCajas)) {
        echo json_encode(array('status' => 'error', 'data' => 'Error al obtener los datos', 'sql2'=>$sqlCajas));
        exit;
    }
    $cajas = array();
    while ($row = mysqli_fetch_assoc($result)) {
...
from (SELECT SUM(PRegalado.Cant) as TotalCajasPromocion, t_ruta.cve_ruta
    ...
    if (!$result = mysqli_query($conn, $sqlCajasPromocion)) {
        echo json_encode(array('status' => 'error', 'data' => 'Error al obtener los datos', 'sql3'=>$sqlCajasPromocion));
        exit;
    }
...
group by rutaName";
    if (!$result = mysqli_query($conn, $sqlVentasPromocion)) {
        echo json_encode(array('status' => 'error', 'data' => 'Error al obtener los datos', 'sql4'=>$sqlVentasPromocion));
        exit;
    }
...
ORDER BY STR_TO_DATE(ventas.Fecha, '%d-%m-%Y') DESC, ventas.DiaOperativo DESC";
    if (!$result = mysqli_query($conn, $sqlEfectivoCreditoCobranza)) {
        echo json_encode(array('status' => 'error', 'data' => 'Error al obtener los datos', 'sql5'=>$sqlEfectivoCreditoCobranza));
        exit;
    }
...
";
    if (!$result = mysqli_query($conn, $sql3masvendidos)) {
        echo json_encode(array('status' => 'error', 'data' => 'Error al obtener los datos', 'sql6'=>$sqlVentasPromocion));
        exit;
    }
