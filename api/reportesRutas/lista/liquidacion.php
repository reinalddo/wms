...
GROUP BY liq.TipoOperacion, liq.cve_articulo, liq.Folio";
    ...
    $sqlCredito = "SELECT 
            ...
            0 AS saldo #Documento = cob.Documento AND
            FROM Cobranza cob
...
SELECT 
             cob.Documento, cte.Cve_Clte, cte.RazonSocial AS Cliente, (IFNULL(cob.Saldo, 0)) AS limite_credito, 
            SUM(IFNULL((SELECT SUM(Abono) FROM DetalleCob WHERE RutaId=r.ID_Ruta AND DiaO=$diao AND Documento = cob.Documento),0)) AS saldo
            FROM Cobranza cob
            LEFT JOIN DetalleCob dc ON dc.Documento = cob.Documento
            LEFT JOIN c_destinatarios d ON d.id_destinatario = cob.Cliente
            LEFT JOIN c_cliente cte ON cte.Cve_Clte = d.Cve_Clte
            LEFT JOIN t_ruta r ON r.ID_Ruta = cob.RutaId
            WHERE (cob.DiaO = $diao OR dc.DiaO = $diao) AND r.cve_ruta = '$ruta'  #AND cob.Status = 1
            GROUP BY Cve_Clte, Documento
