...
if (isset($_POST) && !empty($_POST) && $_POST['action'] === 'loadGrid') {
    ...
    $sql = "
        SELECT
            ...
            if(td_ajusteexist.ntarima = 0,'',td_ajusteexist.ntarima) AS contenedor,
            ...
            date_format(th_ajusteexist.fec_ajuste, '%d-%m-%Y %H:%i:%s') AS fecha,
			      IFNULL(c_motivo.Des_Motivo,'') AS motivo,
        ...
        FROM td_ajusteexist
            ...
            LEFT JOIN c_motivo on c_motivo.id = td_ajusteexist.Id_Motivo
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        $responce->rows[$i]['cell']=array("",$folio,$clave, $descripcion,$bl,$contenedor,$teorica,$fisica,$diferencia,$fecha,$motivo,$usuario,$almacen,$almacenaje);
        $i++;
    }
