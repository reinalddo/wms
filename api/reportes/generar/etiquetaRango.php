...
foreach (range($_SESSION["desde"], $_SESSION["hasta"]) as $number) {
    $pages[] = $number;
}
...
foreach ($data as $d) {
    ...
    for ($i=0;$i<$cajas;$i++) {
        ...
        <table>
            <tbody>
                <tr>
                    <td style=\"width: 384px; height: 60px; text-align: center;\">" . $d["Nom_CteCon"] . "</td>
                </tr>
                <tr>
                    <td style=\"width: 384px; height: 70px; text-align: center;\"><h3><strong>" . $d["RazonComercial"] . "</strong></h3></td>
                </tr>
                <tr>
...
for ($i=0;$i<count($contentArr);$i++) {
    if (in_array($c, $pages)) $contentArrOut[] = $contentArr[$i];
    $c++;
}
$content = "<style>
._divimg {
...
$contentE = join("<pagebreak />", $contentArrOut);
