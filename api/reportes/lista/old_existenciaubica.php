...
*/
    ...
    $sql = "
      SELECT x.acciones, x.cve_almacen, x.id_almacen, x.almacen, x.folio_OT, x.NCaja, x.zona, x.codigo, x.cve_ubicacion, x.zona_recepcion, x.QA, x.contenedor, x.LP, x.clave, x.descripcion, x.des_grupo, x.des_clasif, x.cajasxpallets, x.piezasxcajas, x.Pallet, x.Caja, x.Piezas, x.control_lotes, x.control_numero_series, x.lote, x.lote_alterno, x.caducidad, x.nserie, x.peso, (x.cantidad) AS cantidad, (x.cantidad_kg) AS cantidad_kg, x.id_proveedor, (x.proveedor) AS proveedor, (x.empresa_proveedor) AS empresa_proveedor, x.tipo_ubicacion, x.control_abc, x.clasif_abc, x.um, x.control_peso, x.referencia_well, x.pedimento_well, x.codigo_barras_pieza, x.ntarima,
            x.codigo_barras_caja, x.codigo_barras_pallet FROM(
         SELECT DISTINCT 
...
*/
    $sql2 = $sql;
    if (!($resc = mysqli_query($conn, $sql))) {echo json_encode(array( "error" => "2Error al procesar la petición: (" . mysqli_error($conn) . ") "));}
...
if ($_POST['action'] === 'loadDetalle') 
{
  ...
  $sql1 = "
SELECT u.CodigoCSD AS ubicacion, ch.CveLP AS LP, cj.CveLP AS Caja, a.cve_articulo, a.des_articulo, ec.cve_lote, 
...
WHERE ec.Cve_Almac = $almacen AND u.idy_ubica = '$idy_ubica' AND IFNULL(ch.CveLP, '') = '$lp' AND a.Cve_Articulo = '$clave' AND ec.cve_lote = '$lote'
      ";
