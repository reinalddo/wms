...
if (isset($_POST) && !empty($_POST) && $_POST['action'] === 'load') 
{
    ...
    if($search != "")
    {
        $and = " AND (modulo LIKE '%{$search}%' OR usuario LIKE '%{$search}%' OR dispositivo LIKE '%{$search}%' OR operacion LIKE '%{$search}%' OR observaciones LIKE '%{$search}%') ";
    }
    ...
    $sql_fecha = "";
      ...
      if (!empty($fecha_inicio) AND !empty($fecha_fin)) 
      {
        ...
        $sql_fecha = " AND IFNULL(fecha, '0000-00-00 00:00:00') BETWEEN STR_TO_DATE('$fecha_inicio', '%d-%m-%Y') AND STR_TO_DATE('$fecha_fin', '%d-%m-%Y') ";
      }
      elseif (!empty($fecha_inicio)) 
      {
        $fecha_inicio = date ("d-m-Y", strtotime($fecha_inicio));
        $sql_fecha = " AND IFNULL(fecha, '0000-00-00 00:00:00') >= STR_TO_DATE('$fecha_inicio', '%d-%m-%Y') ";
      }
      elseif (!empty($fecha_fin)) 
      {
        $fecha_fin = date ("d-m-Y", strtotime($fecha_fin));
        $sql_fecha = " AND IFNULL(fecha, '0000-00-00 00:00:00') <= STR_TO_DATE('{$fecha_fin}', '%d-%m-%Y') ";
      }
    ...
    $sql = "
              SELECT DISTINCT modulo, usuario, DATE_FORMAT(fecha, '%d-%m-%Y | %H:%i:%s') AS fecha, operacion, dispositivo, observaciones FROM t_log_operaciones WHERE usuario != 'wmsmaster' {$sql_fecha} {$and}
    ";
