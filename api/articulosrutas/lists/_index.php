...
SELECT  rutas.clave, rutas.descripcion_producto, SUM(rutas.cantidad_sin_conversion) AS cantidad_sin_conversion, 
    SUM(rutas.cantidad_final_sin_conversion) AS cantidad_final_sin_conversion, SUM(rutas.cantidad) AS cantidad, SUM(rutas.cantidad_final) AS cantidad_final, rutas.cve_ruta,
SUM(IFNULL(IF(rutas.mav_cveunimed = 'XBX', IF(rutas.num_multiplo = 1, 0,rutas.cantidad_final_sin_conversion),IF(rutas.num_multiplo = 1, 0, TRUNCATE((rutas.cantidad_final_sin_conversion/rutas.num_multiplo), 0))), 0)) AS cajas_total,
SUM(IFNULL(IF(rutas.mav_cveunimed != 'XBX', (rutas.cantidad_final_sin_conversion - IF(rutas.num_multiplo = 1, 0, (rutas.num_multiplo*TRUNCATE((rutas.cantidad_final_sin_conversion/rutas.num_multiplo), 0)))), IF(rutas.num_multiplo = 1, rutas.cantidad_final_sin_conversion, 0)), 0)) AS piezas_total
    ...
    LEFT JOIN th_ordenembarque oe ON oe.ID_OEmbarque = toe.ID_OEmbarque #IN (SELECT ID_OEmbarque FROM td_ordenembarque WHERE Fol_folio = caja.fol_folio)
    ...
    LEFT JOIN t_clientexruta ON t_clientexruta.clave_cliente = d.id_destinatario
    LEFT JOIN t_ruta ON t_ruta.ID_Ruta = t_clientexruta.clave_ruta      
    LEFT JOIN t_ruta ruta_pedido ON ruta_pedido.ID_Ruta = pd.ruta OR ruta_pedido.cve_ruta = pd.cve_ubicacion
...
FROM td_surtidopiezas st
    ...
    LEFT JOIN th_ordenembarque oet ON oet.ID_OEmbarque = toet.ID_OEmbarque #IN (SELECT ID_OEmbarque FROM td_ordenembarque WHERE Fol_folio = caja.fol_folio)
...
UNION
    ...
    #(IF(ar.control_peso = 'N',TRUNCATE(SUM(dv.Pza), 0),SUM(dv.Pza)) - (IFNULL(pr.Cant, 0))) as cantidad_sin_conversion,
    #(IF(ar.control_peso = 'N',TRUNCATE(SUM(dv.Pza), 0),SUM(dv.Pza)) - (IFNULL(pr.Cant, 0))) AS cantidad_final_sin_conversion,
    #IF(um.mav_cveunimed != 'XBX', IF(ar.control_peso = 'N',TRUNCATE(SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0)), 0),(SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0)))), IF(ar.control_peso = 'N',TRUNCATE((SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0)))*ar.num_multiplo, 0),(SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0)))*ar.num_multiplo)) AS cantidad, 
    #IF(um.mav_cveunimed != 'XBX', IF(ar.control_peso = 'N',TRUNCATE((SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0))), 0),SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0))), IF(ar.control_peso = 'N',TRUNCATE((SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0)))*ar.num_multiplo, 0),(SUM(dv.Pza) - SUM(IFNULL(pr.Cant, 0)))*ar.num_multiplo)) AS cantidad_final
    (IF(ar.control_peso = 'N',TRUNCATE((dv.Pza), 0),(dv.Pza)) - (IFNULL(pr.Cant, 0))) AS cantidad_sin_conversion,
    (IF(ar.control_peso = 'N',TRUNCATE((dv.Pza), 0),(dv.Pza)) - (IFNULL(pr.Cant, 0))) AS cantidad_final_sin_conversion,
    IF(um.mav_cveunimed != 'XBX', IF(ar.control_peso = 'N',TRUNCATE((dv.Pza) - (IFNULL(pr.Cant, 0)), 0),((dv.Pza) - (IFNULL(pr.Cant, 0)))), IF(ar.control_peso = 'N',TRUNCATE(((dv.Pza) - (IFNULL(pr.Cant, 0)))*ar.num_multiplo, 0),((dv.Pza) - (IFNULL(pr.Cant, 0)))*ar.num_multiplo)) AS cantidad, 
    IF(um.mav_cveunimed != 'XBX', IF(ar.control_peso = 'N',TRUNCATE(((dv.Pza) - (IFNULL(pr.Cant, 0))), 0),(dv.Pza) - (IFNULL(pr.Cant, 0))), IF(ar.control_peso = 'N',TRUNCATE(((dv.Pza) - (IFNULL(pr.Cant, 0)))*ar.num_multiplo, 0),((dv.Pza) - (IFNULL(pr.Cant, 0)))*ar.num_multiplo)) AS cantidad_final

    FROM Venta v
    LEFT JOIN PRegalado pr ON pr.Docto = v.Documento
