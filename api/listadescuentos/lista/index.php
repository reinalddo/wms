...
//<i class="fa fa-circle '+color+'" aria-hidden="true"></i>
        $sql = "SELECT  l.id AS id, 
                  IF(STR_TO_DATE(l.FechaFin, '%Y-%m-%d') >= STR_TO_DATE(CURDATE(), '%Y-%m-%d') OR l.Caduca = 0, '<i class=\'fa fa-circle\' style=\'color:green;\' aria-hidden=\'true\'></i>', '<i class=\'fa fa-circle\' style=\'color:red;\' aria-hidden=\'true\'></i>') AS status_fecha,
...
if (isset($_GET) && !empty($_GET) && $_GET['action'] === "getListadoClientes") 
{
   ...
   $start = $limit*$page - $limit; // do not put $limit*($page - 1)
    ...
    $sql = "SELECT DISTINCT d.id_destinatario AS Cve_Destinatario,
            ...
            WHERE r.ListaD = {$id} AND r.Id_Destinatario = d.id_destinatario";
...
if (isset($_GET) && !empty($_GET) && $_GET['action'] === "getDetallesFolio") 
{
   ...
   $start = $limit*$page - $limit; // do not put $limit*($page - 1)
    ...
    $sql = "SELECT l.Articulo, a.des_articulo, l.Minimo as PrecioMin, l.Maximo as PrecioMax, l.Factor as ComisionPor, l.FactorMax AS ComisionPorMax
            FROM detalleld l 
