...
if( $_POST['action'] == 'load_lista')
{
  ...
  $sql = "
    SELECT l.Lista, l.Tipo, DATE_FORMAT(l.FechaIni, '%d-%m-%Y') AS FechaIni, DATE_FORMAT(l.FechaFin, '%d-%m-%Y') AS FechaFin, c.clave, l.Caduca
    FROM listad l
  ...
  $sql = "
    SELECT l.Articulo as Cve_Articulo, a.des_articulo, l.Minimo as PrecioMin, l.Maximo as PrecioMax, a.costo, l.Factor as ComisionPor, l.FactorMax as FactorMax, l.Tipo
    FROM detalleld l 
...
if( $_POST['action'] == 'add' )
{
    ...
    $sql = "INSERT INTO listad(Lista, Tipo, Caduca, FechaIni, FechaFin, Cve_Almac) VALUES('{$nombre_lista}', {$tipo_lista}, {$caduca}, STR_TO_DATE('{$fechaini}', '%d-%m-%Y'), STR_TO_DATE('{$fechafin}', '%d-%m-%Y'), {$id_almac})";
    ...
    $sql = "SELECT IFNULL(MAX(id), 0) as id FROM listad";
    $res = "";
    if (!($res = mysqli_query($conn, $sql))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
    $id = mysqli_fetch_assoc($res)['id'];
      ...
      foreach($arrDetalle as $row)
      {
          ...
          $aplica_a_val = $row['aplica_a_val'];

          $sql = "INSERT INTO detalleld(ListaId, Articulo, Factor, FactorMax, Minimo, Maximo, Tipo, Cve_Almac) VALUES({$id}, '{$cve_articulo}', {$comisionporc}, {$descuentomax}, {$preciomin}, {$preciomax}, {$aplica_a_val}, {$id_almac})";
          if (!($res = mysqli_query($conn, $sql))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
      }
...
if( $_POST['action'] == 'edit' )
{
    ...
    $sql = "UPDATE listad SET Lista = '{$nombre_lista}', 
                              Tipo = {$tipo_lista}, 
    ...
    $sql = "DELETE FROM detalleld WHERE ListaId = {$id_lista}";
    $res = "";
    if (!($res = mysqli_query($conn, $sql))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
      ...
      foreach($arrDetalle as $row)
      {
          ...
          $sql = "INSERT INTO detalleld(ListaId, Articulo, Minimo, Maximo, Cve_Almac, Factor, FactorMax, Tipo) VALUES({$id_lista}, '{$cve_articulo}', {$preciomin}, {$preciomax}, {$id_almac}, {$comisionporc}, {$descuentomax}, {$aplica_a_val})";
          if (!($res = mysqli_query($conn, $sql))) {echo "Falló la preparación: (" . mysqli_error($conn) . ") ";}
      }
...
if( $_POST['action'] == 'asignar_destinatario_descuento' )
{
    ...
    $sql = "INSERT INTO RelCliLis(Id_Destinatario, ListaD) VALUES ({$id_destinatario}, {$id_lista})";

    if($existe)
      $sql = "UPDATE RelCliLis SET ListaD = {$id_lista} WHERE Id_Destinatario = {$id_destinatario}";
