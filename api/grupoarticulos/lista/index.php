...
if (isset($_POST) && !empty($_POST)) {
    ...
    if(isset($_POST['criterio']))
    {
        ...
        {
            $sqlBusq = " and  concat(des_gpoart,'',cve_gpoart) like '%".$_criterio."%' "; 
        }
    }
    ...
    $sqlCount = "
        Select 
            COUNT(*) as total
        from c_gpoarticulo 
        Where Activo = '1' ".$sqlBusq." #AND id_almacen = {$almacen}
        ;";
    ...
    while ($row = mysqli_fetch_array($res)) {
        $arr[] = $row;
        $responce["rows"][$i]['id']=$row['cve_gpoart'];
        $responce["rows"][$i]['cell']=array($row['cve_gpoart'], utf8_encode($row['des_gpoart']));
        $i++;
    }
