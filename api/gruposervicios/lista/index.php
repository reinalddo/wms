...
if (isset($_POST) && !empty($_POST)) {
    ...
    if(isset($_POST['criterio']))
    {
        ...
        {
            $sqlBusq = " and  concat(Des_GpoServicio,'',Cve_GpoServicio) like '%".$_criterio."%' "; 
        }
    }
    ...
    while ($row = mysqli_fetch_array($res)) {
        $arr[] = $row;
        $responce["rows"][$i]['Id_GpoServicio']=$row['Cve_GpoServicio'];
        $responce["rows"][$i]['cell']=array($row['Cve_GpoServicio'], utf8_encode($row['Des_GpoServicio']));
        $i++;
    }
