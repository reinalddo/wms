...
class PivotReader
{
    ...
    protected function buildIndexes($params) {
        ...
        foreach (array("row", "column") as $rc) {
            ...
            $dimension = Util::get($params[$rc], "dimension", $rc);
            $sort = Util::get($params[$rc], "sort", null);
    ...
    public function getHeader($params) {
        ...
        $fni = $this->fni;
        $header = array();
        $includeAll = Util::get($params, "includeAll", false);
        foreach (array("row", "column") as $rc) {
            if (! isset($params[$rc]))
                continue;
            $header[$rc] = array();
            $parent = Util::get($params[$rc], "parent", null);
            if (! isset($parent))
                continue;
            ...
            $getIndex = Util::get($params, "getHeaderIndex", false);
            foreach ($indexes as $i) {
                $node = $nodes[$i];
                if ($this->isChild($parent, $node)) {
                    $v = Util::get($node, $childField);
                    if ($includeAll || $v !== "{{all}}") {
    ...
    public function getDataStore($params) {
        ...
        $measures = Util::get($params, "measures", array());
        if (count($measures) > 1) {
            $columnHeader2 = array();
