...
FROM (SELECT DISTINCT c_almacenp.nombre                                                                       AS sucursalNombre,
      ...
      WHERE 1
        AND t_ruta.cve_ruta IN ($rutasCve)
        and th_pedido.Fec_Pedido = '$fecha'
...
foreach ($semaforo as $datos) {
    // Your existing logic to calculate the required values
    $efectividadxVisita = 0;
    $efectividadxVisitaxVenta = 0;
    if ($datos['visitas_programadas'] > 0 && $datos['visitas_clientes_programados'] > 0) {
        $efectividadxVisita = (100 / $datos['visitas_programadas']) * $datos['visitas_clientes_programados'];
    }
    if ($datos['visitas_realizadas'] > 0 && $datos['ClientesVisitaVenta'] > 0) {
        $efectividadxVisitaxVenta = ($datos['ClientesVisitaVenta'] * 100) / $datos['visitas_realizadas'];
    }
    $cajasPrevendidasDetalle = $datos['Cajas'] - $datos['CajasVentaProm'];

    $datosTablaSemaforo[] = [
        'Ruta'                                        => $datos['cve_ruta'],
        'Agente'                                      => $datos['nombre_completo'],
        'VisitasProgramadas'                          => $datos['visitas_programadas'],
        'VisitasRealizadas'                           => $datos['visitas_realizadas'],
        'VisitasClientesProgramados'                  => $datos['visitas_clientes_programados'],
        'VisitasClientesNoProgramados'                => $datos['visitas_realizadas'] - $datos['visitas_clientes_programados'],
        'ClientesVisitaVenta'                         => $datos['ClientesVisitaVenta'],
        'ClientesVisitaNoVenta'                       => $datos['visitas_realizadas'] - $datos['ClientesVisitaVenta'],
        'ClientesPromocion'                           => $datos['ClientesPromocion'],
        'ClientesSinPromocion'                        => $datos['ClientesVisitaVenta'] - $datos['ClientesPromocion'],
        'ClientesSinVisita'                           => $datos['visitas_programadas'] == 0 ? 0 : ($datos['visitas_programadas'] - $datos['visitas_clientes_programados']) * -1,
        'EfectividadxVisita'                          => $efectividadxVisita,
        'EfectividadxVisitaxVenta'                    => $efectividadxVisitaxVenta,
        'CajasPrevendidasVendidas'                    => $datos['Cajas'],
        ...
        'PresupuestoDiario'                           => 0,
        'Diferencia'                                  => 0,
        'Promociones'                                 => $datos['CajasPromocion'],
        'Obsequios'                                   => 0,
        'Pilones'                                     => 0,
        'CajasPrevendidasVendidasDetalle'             => $cajasPrevendidasDetalle,
        'PorcentajeCajasPrevendidasVendidas'          => ($cajasPrevendidasDetalle == 0 || $datos['Cajas'] == 0) ? 0 : ((float)$cajasPrevendidasDetalle * 100) / ((float)($datos['Cajas'])),
        'CajasPrevendidasVendidasPromocion'           => $datos['CajasVentaProm'],
        'PorcentajeCajasPrevendidasVendidasPromocion' => ($datos['CajasVentaProm'] == 0 || $datos['Cajas'] == 0) ? 0 : ((float)$datos['CajasVentaProm'] * 100) / ((float)($datos['Cajas'])),
        'TotalCajasZona'                              => 0,
        'TotalCargaKilos'                             => 0,
        'ProductosMasVendidos'                        => $datos['3MasVendidos'] ?? 'SIN PRODUCTOS MAS VENDIDOS',
        'SaboresMasVendidos'                          => $datos['3SaboresMasVendidos'] ?? 'SIN SABORES MAS VENDIDOS',
        'InicioOperativo'                             => $datos['InicioOperativo'],
        'InicioPrimerCliente'                         => $datos['InicioPrimerCliente'],
        'TiempoTranscurrido'                          => $datos['TiempoTranscurrido'],
        'UltimoCliente'                               => $datos['UltimoCliente'],
        'CierreOperativo'                             => $datos['CierreOperativo'],
        'UltimoTiempoTraslado'                        => $datos['UltimoTiempoTraslado'],
        'TiempoRuta'                                  => calcularTiempoRuta($datos['InicioOperativo'], $datos['CierreOperativo'], $datos['UltimoCliente'])
    ];
}

// Calculate $datosTotales
foreach ($datosTablaSemaforo as $datos) {
    $datosTotales['VisitasProgramadas'] += (int)$datos['VisitasProgramadas'];
    $datosTotales['VisitasRealizadas'] += (int)$datos['VisitasRealizadas'];
    $datosTotales['VisitasClientesProgramados'] += (int)$datos['VisitasClientesProgramados'];
    $datosTotales['VisitasClientesNoProgramados'] += (int)$datos['VisitasClientesNoProgramados'];
    $datosTotales['ClientesVisitaVenta'] += (int)$datos['ClientesVisitaVenta'];
    $datosTotales['ClientesVisitaNoVenta'] += (int)$datos['ClientesVisitaNoVenta'];
    $datosTotales['ClientesPromocion'] += (int)$datos['ClientesPromocion'];
    $datosTotales['ClientesSinPromocion'] += (int)$datos['ClientesSinPromocion'];
    $datosTotales['ClientesSinVisita'] += (int)$datos['ClientesSinVisita'];
    $datosTotales['EfectividadxVisita'] += (float)$datos['EfectividadxVisita'];
    $datosTotales['EfectividadxVisitaxVenta'] += (float)$datos['EfectividadxVisitaxVenta'];
    $datosTotales['CajasPrevendidasVendidas'] += (int)$datos['CajasPrevendidasVendidas'];
    $datosTotales['Efectivo'] += (float)$datos['Efectivo'];
    $datosTotales['Credito'] += (float)$datos['Credito'];
    $datosTotales['Cobranza'] += (float)$datos['Cobranza'];
    $datosTotales['Descuentos'] += (float)$datos['Descuentos'];
    $datosTotales['Promociones'] += (int)$datos['Promociones'];
    $datosTotales['CajasPrevendidasVendidasDetalle'] += (int)$datos['CajasPrevendidasVendidasDetalle'];
    $datosTotales['PorcentajeCajasPrevendidasVendidas'] += (float)$datos['PorcentajeCajasPrevendidasVendidas'];
    $datosTotales['CajasPrevendidasVendidasPromocion'] += (int)$datos['CajasPrevendidasVendidasPromocion'];
    $datosTotales['PorcentajeCajasPrevendidasVendidasPromocion'] += (float)$datos['PorcentajeCajasPrevendidasVendidasPromocion'];
    $datosTotales['TotalCajasZona'] += (int)$datos['TotalCajasZona'];
    $datosTotales['TotalCargaKilos'] += (int)$datos['TotalCargaKilos'];
}
$sqlAlmacen = "select nombre from c_almacenp where id = '$almacen'";
if (!$result = mysqli_query($conn, $sqlAlmacen)) {
    echo json_encode(array('status' => 'error', 'data' => 'Error al obtener el nombre del almacen'));
    exit;
}
$nombreAlmacen = mysqli_fetch_array($result)['nombre'];

$sqlEmpresa = "select des_cia from c_compania";
if (!$result = mysqli_query($conn, $sqlEmpresa)) {
    echo json_encode(array('status' => 'error', 'data' => 'Error al obtener el nombre de la empresa'));
    exit;
}
$nombreEmpresa = mysqli_fetch_array($result)['des_cia'];
$excel = new PHPExcel();
$excel->getProperties()->setCreator("ADL")
    ->setLastModifiedBy("ADL")
    ->setTitle("Semaforo de Ventas")
    ->setSubject("Semaforo de Ventas")
    ->setDescription("Semaforo de Ventas")
    ->setKeywords("office 2007 openxml php")
    ->setCategory("Semaforo de Ventas");
$colorBlanco = 'FFFFFF';
$colorAzulClaro = 'cae4ff';
$colorVerde = 'b3ffb3';
$colorNaranja = 'ffb768';
$colorAzulFuerte = '62a1ff';
$colorRojo = 'ff9999';
$colorAmarillo = 'f6da7e';
$excel->setActiveSheetIndex(0)
    ->setCellValue('A1', 'Empresa: ' . $nombreEmpresa)
    ->setCellValue('A2', 'Sucursal: ' . $nombreAlmacen)
    ->setCellValue('A3', 'Fecha: ' . date('d/m/Y'))
    ->setCellValue('A4', 'Reporte de Ventas y Preventas')
    ->setCellValue('A5', 'Rutas')//Celda color blanco
    ->setCellValue('B5', 'Agente') //Celda color azul claro
    ->setCellValue('C5', 'VISITAS PROGRAMADAS')//Celda Color Amarillo
    ->setCellValue('D5', 'VISITAS REALIZADAS')//Celda Color Amarillo
    ->setCellValue('E5', 'VISITAS A CLIENTES PROGRAMADOS')//Celda Color Amarillo
    ->setCellValue('F5', 'VISITAS A CLIENTES NO PROGRAMADOS')//Celda Color Amarillo
    ->setCellValue('G5', 'CLIENTES C/PEDIDO O VENTA')//Celda Color Amarillo
    ->setCellValue('H5', 'CLIENTES SIN VENTA O SIN PEDIDO')//Celda Color Amarillo
    ->setCellValue('I5', 'CLIENTES SIN PROMOCION') //Celda color verde
    ->setCellValue('J5', 'CLIENTES CON PROMOCION')//Celda color naranja
    ->setCellValue('K5', 'CLIENTES S/VISITA')//Celda color azul fuerte
    ->setCellValue('L5', 'EFECTIVIDAD * VISITAS')//Celda color azul fuerte
    ->setCellValue('M5', 'EFECTIVIDAD * VENTA * VISITA')//Celda color azul fuerte
    ->setCellValue('N5', 'CAJAS PREVENDIDAS o VENDIDAS')//Celda Color Amarillo
    ->setCellValue('O5', 'EFECTIVO')//Celda color azul claro
    ->setCellValue('P5', 'CREDITO')//Celda color azul claro
    ->setCellValue('Q5', 'COBRANZA')//Celda color azul claro
    ->setCellValue('R5', 'DESCUENTOS')//Celda color azul claro
    ->setCellValue('S5', 'PRESUPUESTO DIARIO')//Celda Color Amarillo
    ->setCellValue('T5', 'DIFERENCIA')//Celda Color Amarillo
    ->setCellValue('U5', 'PROMOCIONES')//Celda color naranja
                                       //    ->setCellValue('V5', 'OBSEQUIOS')//Celda color naranja
                                       //    ->setCellValue('W5', 'PILONES')//Celda Color Amarillo
    ->setCellValue('V5', 'CAJAS PREVENDIDASO VENDIDAS A DETALLE')//Celda color verde
    ->setCellValue('W5', '% DE PREVENTA A DETALLE')//Celda color verde
    ->setCellValue('X5', 'CAJAS PREVENDIDAS CON PROMOCION')//Celda color naranja
    ->setCellValue('Y5', '% DE CAJAS PREVENDIDAS CON PROMOCION')//Celda color naranja
    ->setCellValue('Z5', 'TOTAL DE CAJAS QUE LLEVA CADA ZONA')//Celda Color Amarillo
    ->setCellValue('AA5', 'Tolal Carga En Kilos')//Celda color verde
    ->setCellValue('AB5', 'Los 3 Producto mas vendido')//Celda color Rojo
    ->setCellValue('AC5', 'los 3 Saborer mas vendido')//Celda color Rojo
    ->setCellValue('AD5', 'Inicio_Operativo')//Celda color azul claro
    ->setCellValue('AE5', 'Inicio_Primercliente')//Celda color azul claro
    ->setCellValue('AF5', 'Tiempo_Traslado')//Celda color azul claro
    ->setCellValue('AG5', 'Cierre_UltimoCliente')//Celda color azul claro
    ->setCellValue('AH5', 'Cierre_Operativo')      //Celda color azul claro
    ->setCellValue('AI5', 'Ultimo_Tiempo_Traslado')//Celda color azul claro
    ->setCellValue('AJ5', 'Tiempo de ruta (Inicio de operación a Cierre de operación)');//Celda color azul claro;
$excel->getActiveSheet()->getStyle('A5:AJ5')->getFont()->setBold(true);
$excel->getActiveSheet()->getStyle('A5:AJ5')->getAlignment()->setWrapText(true);
$excel->getActiveSheet()->getStyle('A5:AJ5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$excel->getActiveSheet()->getStyle('A5:AJ5')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$excel->getActiveSheet()->getStyle('A5:AJ5')->getAlignment()->setTextRotation(90);

// Establecer estilos de celda
$cellStyleBlanco = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorBlanco),
    )
);

$cellStyleAzulClaro = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorAzulClaro),
    )
);

$cellStyleVerde = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorVerde),
    )
);

$cellStyleNaranja = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorNaranja),
    )
);

$cellStyleAzulFuerte = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorAzulFuerte),
    )
);
...
$cellStyleRojo = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorRojo),
    )
);

$cellStyleAmarillo = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => $colorAmarillo),
    )
);
$styleWhite = [
    'fill' => [
...
$styleRedLight = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => 'ff9999'), // Change to the appropriate color code
    ),
);

$styleGreenLight = array(
    'fill' => array(
        'type'  => PHPExcel_Style_Fill::FILL_SOLID,
        'color' => array('rgb' => 'b3ffb3'), // Change to the appropriate color code
    ),
);
// Aplicar estilos a las celdas
$excel->getActiveSheet()->getStyle('A5')->applyFromArray($cellStyleBlanco);
$excel->getActiveSheet()->getStyle('B5')->applyFromArray($cellStyleAzulClaro);
$excel->getActiveSheet()->getStyle('C5:F5')->applyFromArray($cellStyleAmarillo);
$excel->getActiveSheet()->getStyle('G5:H5')->applyFromArray($cellStyleAmarillo);
$excel->getActiveSheet()->getStyle('I5')->applyFromArray($cellStyleVerde);
$excel->getActiveSheet()->getStyle('J5:K5')->applyFromArray($cellStyleNaranja);
$excel->getActiveSheet()->getStyle('L5:M5')->applyFromArray($cellStyleAzulFuerte);
$excel->getActiveSheet()->getStyle('N5')->applyFromArray($cellStyleAmarillo);
$excel->getActiveSheet()->getStyle('O5:R5')->applyFromArray($cellStyleAzulClaro);
$excel->getActiveSheet()->getStyle('S5:T5')->applyFromArray($cellStyleAmarillo);
$excel->getActiveSheet()->getStyle('U5:V5')->applyFromArray($cellStyleNaranja);
$excel->getActiveSheet()->getStyle('W5')->applyFromArray($cellStyleAmarillo);
$excel->getActiveSheet()->getStyle('X5:Y5')->applyFromArray($cellStyleVerde);
$excel->getActiveSheet()->getStyle('Z5:AA5')->applyFromArray($cellStyleNaranja);
$excel->getActiveSheet()->getStyle('AB5')->applyFromArray($cellStyleAmarillo);
$excel->getActiveSheet()->getStyle('AC5')->applyFromArray($cellStyleVerde);
$excel->getActiveSheet()->getStyle('AD5:AE5')->applyFromArray($cellStyleRojo);
$excel->getActiveSheet()->getStyle('AF5:AJ5')->applyFromArray($cellStyleAzulClaro);
//agregar bordes a las celdas de a5 en adelante
$excel->getActiveSheet()->getStyle('A5:AJ5')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
//mezclar las celdas a1 hasta d1
$excel->getActiveSheet()->mergeCells('A1:N1');
$excel->getActiveSheet()->mergeCells('A2:N2');
$excel->getActiveSheet()->mergeCells('A3:N3');
$excel->getActiveSheet()->mergeCells('A4:N4');
...
$excel->getActiveSheet()->setTitle('Semaforo de Ventas');
$row = 6; // Fila inicial para los datos
foreach ($datosTablaSemaforo as $dato) {
    $excel->getActiveSheet()->setCellValue('A' . $row, $dato['Ruta']);
    $excel->getActiveSheet()->setCellValue('B' . $row, $dato['Agente']);
    $excel->getActiveSheet()->setCellValue('C' . $row, $dato['VisitasProgramadas']);
    $excel->getActiveSheet()->setCellValue('D' . $row, $dato['VisitasRealizadas']);
    $excel->getActiveSheet()->setCellValue('E' . $row, $dato['VisitasClientesProgramados']);
    $excel->getActiveSheet()->setCellValue('F' . $row, $dato['VisitasClientesNoProgramados']);
    $excel->getActiveSheet()->setCellValue('G' . $row, $dato['ClientesVisitaVenta']);
    $excel->getActiveSheet()->setCellValue('H' . $row, $dato['ClientesVisitaNoVenta']);
    $excel->getActiveSheet()->setCellValue('I' . $row, $dato['ClientesSinPromocion']);
    $excel->getActiveSheet()->setCellValue('J' . $row, $dato['ClientesPromocion']);
    // Apply styles to 'ClientesSinVisita' column
    $clientesSinVisitaValue = $dato['ClientesSinVisita'];
    $cellCoordinateClientesSinVisita = 'K' . $row;

    if ($clientesSinVisitaValue <= 0) {
        $excel->getActiveSheet()->getStyle($cellCoordinateClientesSinVisita)->applyFromArray($styleRedLight);
    } else {
        $excel->getActiveSheet()->getStyle($cellCoordinateClientesSinVisita)->applyFromArray($styleGreenLight);
    }

    // Set font color to black for 'ClientesSinVisita' cell
    $excel->getActiveSheet()->getStyle($cellCoordinateClientesSinVisita)->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_BLACK));

    // Apply styles to 'EfectividadxVisita' column
    $efectividadVisitaValue = $dato['EfectividadxVisita'];
    $cellCoordinateEfectividadVisita = 'L' . $row;

    if ($efectividadVisitaValue < 50) {
        $excel->getActiveSheet()->getStyle($cellCoordinateEfectividadVisita)->applyFromArray($styleRedLight);
    } else {
        $excel->getActiveSheet()->getStyle($cellCoordinateEfectividadVisita)->applyFromArray($styleGreenLight);
    }

    // Set font color to black for 'EfectividadxVisita' cell
    $excel->getActiveSheet()->getStyle($cellCoordinateEfectividadVisita)->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_BLACK));

    // Apply styles to 'EfectividadxVisitaxVenta' column
    $efectividadVisitaVentaValue = $dato['EfectividadxVisitaxVenta'];
    $cellCoordinateEfectividadVisitaVenta = 'M' . $row;

    if ($efectividadVisitaVentaValue < 50) {
        $excel->getActiveSheet()->getStyle($cellCoordinateEfectividadVisitaVenta)->applyFromArray($styleRedLight);
    } else {
        $excel->getActiveSheet()->getStyle($cellCoordinateEfectividadVisitaVenta)->applyFromArray($styleGreenLight);
    }

    $excel->getActiveSheet()->getStyle($cellCoordinateEfectividadVisitaVenta)->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_BLACK));
    $excel->getActiveSheet()->setCellValue('K' . $row, $dato['ClientesSinVisita']);
    $excel->getActiveSheet()->setCellValue('L' . $row, $dato['EfectividadxVisita']);
    $excel->getActiveSheet()->setCellValue('M' . $row, $dato['EfectividadxVisitaxVenta']);
    $excel->getActiveSheet()->setCellValue('N' . $row, $dato['CajasPrevendidasVendidas']);
    $excel->getActiveSheet()->setCellValue('O' . $row, $dato['Efectivo']);
    $excel->getActiveSheet()->setCellValue('P' . $row, $dato['Credito']);
    $excel->getActiveSheet()->setCellValue('Q' . $row, $dato['Cobranza']);
    $excel->getActiveSheet()->setCellValue('R' . $row, $dato['Descuentos']);
    $excel->getActiveSheet()->setCellValue('S' . $row, $dato['PresupuestoDiario']);
    $excel->getActiveSheet()->setCellValue('T' . $row, $dato['Diferencia']);
    $excel->getActiveSheet()->setCellValue('U' . $row, $dato['Promociones']);
    $excel->getActiveSheet()->setCellValue('V' . $row, $dato['CajasPrevendidasVendidasDetalle']);
    $excel->getActiveSheet()->setCellValue('W' . $row, $dato['PorcentajeCajasPrevendidasVendidas']);
    $excel->getActiveSheet()->setCellValue('X' . $row, $dato['CajasPrevendidasVendidasPromocion']);
    $excel->getActiveSheet()->setCellValue('Y' . $row, $dato['PorcentajeCajasPrevendidasVendidasPromocion']);
    $excel->getActiveSheet()->setCellValue('Z' . $row, $dato['TotalCajasZona']);
    $excel->getActiveSheet()->setCellValue('AA' . $row, $dato['TotalCargaKilos']);
    $excel->getActiveSheet()->setCellValue('AB' . $row, $dato['ProductosMasVendidos']);
    $excel->getActiveSheet()->setCellValue('AC' . $row, $dato['SaboresMasVendidos']);
    $excel->getActiveSheet()->setCellValue('AD' . $row, $dato['InicioOperativo']);
    $excel->getActiveSheet()->setCellValue('AE' . $row, $dato['InicioPrimerCliente']);
    $excel->getActiveSheet()->setCellValue('AF' . $row, $dato['TiempoTranscurrido']);
    $excel->getActiveSheet()->setCellValue('AG' . $row, $dato['UltimoCliente']);
    $excel->getActiveSheet()->setCellValue('AH' . $row, $dato['CierreOperativo']);
    $excel->getActiveSheet()->setCellValue('AI' . $row, $dato['UltimoTiempoTraslado']);
    $excel->getActiveSheet()->setCellValue('AJ' . $row, $dato['TiempoRuta']);
    $row++;
}
$excel->getActiveSheet()->setCellValue('A' . $row, 'Total');

// Merge the cells for the 'Total' row
$excel->getActiveSheet()->mergeCells('A' . $row . ':B' . $row);

// Set the values for the rest of the cells
$excel->getActiveSheet()->setCellValue('C' . $row, $datosTotales['VisitasProgramadas']);
$excel->getActiveSheet()->setCellValue('D' . $row, $datosTotales['VisitasRealizadas']);
$excel->getActiveSheet()->setCellValue('E' . $row, $datosTotales['VisitasClientesProgramados']);
$excel->getActiveSheet()->setCellValue('F' . $row, $datosTotales['VisitasClientesNoProgramados']);
$excel->getActiveSheet()->setCellValue('G' . $row, $datosTotales['ClientesVisitaVenta']);
$excel->getActiveSheet()->setCellValue('H' . $row, $datosTotales['ClientesVisitaNoVenta']);
$excel->getActiveSheet()->setCellValue('I' . $row, $datosTotales['ClientesSinPromocion']);
$excel->getActiveSheet()->setCellValue('J' . $row, $datosTotales['ClientesPromocion']);
$excel->getActiveSheet()->setCellValue('K' . $row, $datosTotales['ClientesSinVisita']);
$excel->getActiveSheet()->setCellValue('L' . $row, $datosTotales['EfectividadxVisita']);
$excel->getActiveSheet()->setCellValue('M' . $row, $datosTotales['EfectividadxVisitaxVenta']);
$excel->getActiveSheet()->setCellValue('N' . $row, $datosTotales['CajasPrevendidasVendidas']);
$excel->getActiveSheet()->setCellValue('O' . $row, $datosTotales['Efectivo']);
$excel->getActiveSheet()->setCellValue('P' . $row, $datosTotales['Credito']);
$excel->getActiveSheet()->setCellValue('Q' . $row, $datosTotales['Cobranza']);
$excel->getActiveSheet()->setCellValue('R' . $row, $datosTotales['Descuentos']);
$excel->getActiveSheet()->setCellValue('S' . $row, $datosTotales['PresupuestoDiario']);
$excel->getActiveSheet()->setCellValue('T' . $row, $datosTotales['Diferencia']);
$excel->getActiveSheet()->setCellValue('U' . $row, $datosTotales['Promociones']);
$excel->getActiveSheet()->setCellValue('V' . $row, $datosTotales['CajasPrevendidasVendidasDetalle']);
$excel->getActiveSheet()->setCellValue('W' . $row, $datosTotales['PorcentajeCajasPrevendidasVendidas']);
$excel->getActiveSheet()->setCellValue('X' . $row, $datosTotales['CajasPrevendidasVendidasPromocion']);
$excel->getActiveSheet()->setCellValue('Y' . $row, $datosTotales['PorcentajeCajasPrevendidasVendidasPromocion']);
$excel->getActiveSheet()->setCellValue('Z' . $row, $datosTotales['TotalCajasZona']);
$excel->getActiveSheet()->setCellValue('AA' . $row, $datosTotales['TotalCargaKilos']);
