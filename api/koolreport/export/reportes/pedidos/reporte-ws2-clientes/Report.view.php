...
?>
        <div class="col-lg-6 text-center">
                    ...
                    <h1><span lang="th"><b>Reporte Detalle de Ola por Clientes <?php echo $folio; ?></b><br></span></h1>
...
<?php 
    ...
    $sql_w2 = " SELECT DISTINCT c.Cve_CteProv, c.RazonSocial
...
?>
    <div class="row">
        <div class="col-12 text-left">
            <?php 
            //$folio_w1 = $row_w2['Fol_PedidoCon']; 
            $cve_cliente = $row_w2['Cve_CteProv']; 
            $nombre_cliente = $row_w2['RazonSocial']; 
            ?>
            <p style="font-size: 22px;"><b>Cliente <?php echo $cve_cliente." - ".$nombre_cliente; ?></b></p>
        </div>
    </div>
<?php 
/*
...
*/
        $sql_w1 = " SELECT DISTINCT t.Fol_PedidoCon
                    FROM t_consolidado t
                    LEFT JOIN th_pedido th ON t.Fol_Folio = th.Fol_folio
                    LEFT JOIN c_cliente c ON c.Cve_CteProv = IFNULL(th.Cve_CteProv, th.Cve_clte) 
                    WHERE c.Cve_CteProv = '{$cve_cliente}' AND t.Fol_Consolidado = '{$folio}'";
...
?>
            <div class="row">
                <div class="col-6 text-left">
                    <?php 
                        $folio_w0 = $row_w1['Fol_PedidoCon']; 
                    ?>
                    <p style="font-size: 20px;"><b>Ola <?php echo $folio_w0; ?></b></p>
                </div>
            </div>


<?php 
        $sql_wpedido = "SELECT DISTINCT t.Fol_Folio, c.Cve_CteProv, c.RazonSocial
                        FROM t_consolidado t
                        LEFT JOIN th_pedido th ON t.Fol_Folio = th.Fol_folio
                        LEFT JOIN c_cliente c ON c.Cve_CteProv = IFNULL(th.Cve_CteProv, th.Cve_clte) 
                        WHERE t.Fol_PedidoCon =  '{$folio_w0}' AND c.Cve_CteProv = '$cve_cliente'";
        if (!($res_wpedido = mysqli_query($conn, $sql_wpedido))){
            echo "Falló la preparación(2): (" . mysqli_error($conn) . ") ";
        }

            while ($row_wpedido = mysqli_fetch_array($res_wpedido)) 
            {
?>
            <div class="row">
                <div class="col-6 text-left">
                    <?php 
                        $folio_w1 = $row_wpedido['Fol_Folio']; 
                    ?>
                    <p style="font-size: 18px;"><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Pedido <?php echo $folio_w1; ?></b></p>
