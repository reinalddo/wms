    ...
    protected function runPhantom($script, $source, $output, $params)
    {
        if (!$this->phantomjs) {
            ...
            if (!is_file($this->phantomjs)) {
                $this->phantomjs.=".exe";
                if (!is_file($this->phantomjs)) {
                    throw new \Exception("Could not find phantomjs executed file in bin folder : ".realpath(dirname(__FILE__)));
                }
            }
        ...
        if (strpos($result, ";")===false) {
            throw new \Exception("Could not execute phantomjs --->\n\n Source = ".$source." --- FIN SOURCE --- ".$this->phantomjs." FIN PHANTOM ---- SCRIPT ---> ".$script." --- FIN SCRIPT ---- COMMAND ---> ".$command." ---- FIN COMMAND ------- RESULT ----".$result." -----FIN RESULT".realpath(dirname(__FILE__)) );
        }
