...
class Calculation
{
    ...
    private static function checkMatrixOperands(&$operand1, &$operand2, $resize = 1)
    {
        ...
        if (!is_array($operand1)) {
            ...
            $mm = self::getMatrixDimensions($operand2);
            $matrixRows = $mm[0];
            $matrixColumns = $mm[1];
            ...
            $mm = self::getMatrixDimensions($operand1);
            $matrixRows = $mm[0];
            $matrixColumns = $mm[1];
        ...
        $mm1 = self::getMatrixDimensions($operand1);
        $matrix1Rows = $mm1[0];
        $matrix1Columns = $mm1[1];
        //[$matrix2Rows, $matrix2Columns] = self::getMatrixDimensions($operand2);
        $mm2 = self::getMatrixDimensions($operand2);
        $matrix2Rows = $mm2[0];
        $matrix2Columns = $mm2[1];
            ...
            if ($opCharacter == '-' && !$expectingOperator) {                //    Is it a negation instead of a minus?
                ...
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $val, $matches)) {
                    ...
                    if ($testPrevOp !== null && $testPrevOp['value'] === ':') {
                        ...
                        $rs = Worksheet::extractSheetTitle($startRowColRef, true);
                        $rangeWS1 = $rs[0];
                        $startRowColRef = $rs[1];
                        ...
                        $rv = Worksheet::extractSheetTitle($val, true);
                        $rangeWS2 = $rv[0];
                        $val = $rv[1];
                ...
                switch ($token) {
                    ...
                    case ':':            //    Range
                        if (strpos($operand1Data['reference'], '!') !== false) {
                            ...
                            $so = Worksheet::extractSheetTitle($operand1Data['reference'], true);
                            $sheet1 = $so[0];
                            $operand1Data['reference'] = $so[1];
                        ...
                        $so = Worksheet::extractSheetTitle($operand2Data['reference'], true);
                        $sheet2 = $so[0];
                        $operand2Data['reference'] = $so[1];
                        if (empty($sheet2)) {
                            $sheet2 = $sheet1;
                        }
    ...
    public function extractCellRange(&$pRange = 'A1', Worksheet $pSheet = null, $resetLog = true)
    {
        ...
        if ($pSheet !== null) {
            ...
            if (strpos($pRange, '!') !== false) {
                ...
                $pp = Worksheet::extractSheetTitle($pRange, true);
                $pSheetName = $pp[0];
                $pRange = $pp[1];
    ...
    public function extractNamedRange(&$pRange = 'A1', Worksheet $pSheet = null, $resetLog = true)
    {
        ...
        if ($pSheet !== null) {
            ...
            if (!isset($aReferences[1])) {
                ...
                $cc = Coordinate::coordinateFromString($aReferences[0]);
                $currentCol = $cc[0];
                $currentRow = $cc[1];
                if ($pSheet->cellExists($aReferences[0])) {
                    $returnValue[$currentRow][$currentCol] = $pSheet->getCell($aReferences[0])->getCalculatedValue($resetLog);
                ...
                foreach ($aReferences as $reference) {
                    ...
                    $cc = Coordinate::coordinateFromString($reference);
                    $currentCol = $cc[0];
                    $currentRow = $cc[1];
                    if ($pSheet->cellExists($reference)) {
                        $returnValue[$currentRow][$currentCol] = $pSheet->getCell($reference)->getCalculatedValue($resetLog);
