...
?>
    ...
    <div cell="I1" excelStyle='<?php echo json_encode($styleArray); ?>'>Cajas</div>
    <div cell="J1" excelStyle='<?php echo json_encode($styleArray); ?>'>Piezas</div>
    <div cell="K1" excelStyle='<?php echo json_encode($styleArray); ?>'>PrCj</div>
    <div cell="L1" excelStyle='<?php echo json_encode($styleArray); ?>'>PrPz</div>
...
<?php 
    ...
    $sql = "SELECT DISTINCT DATE_FORMAT(th.Fec_Pedido, '%d/%m/%Y') as Fec_Pedido, DATE_FORMAT(th.Fec_Entrega, '%d/%m/%Y') as Fec_Entrega, r.cve_ruta, td.Fol_folio, c.Cve_Clte, c.RazonSocial, IFNULL(th.tipo_negociacion, 'Contado') AS tipo_negociacion, a.des_articulo, 
...
AND th.cve_almac = {$almacen}";
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        <div cell="I<?php echo $i; ?>"><?php echo ($cajas_total-$PrCaja); ?></div>
        <div cell="J<?php echo $i; ?>"><?php echo ($piezas_total-$PrPz); ?></div>
        <div cell="K<?php echo $i; ?>"><?php echo $PrCaja; ?></div>
        <div cell="L<?php echo $i; ?>"><?php echo $PrPz; ?></div>
        <?php 
        $i++;
