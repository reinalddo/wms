    ...
    dt.bindSearchOnEnter = function () {
        ...
        function strToRegexStr(str) {
            ...
            for (var i = 0; i < searches.length; i += 1) {
                var phrasesRegex = "";
                var phrases = [];
                // var phrases = typeof KRdt.searchMode.exact === 'undefined' ?
                //     strToPhrases(searches[i]) : [searches[i]];
                if (hasSearchMode('exact')) {
                    phrases = phrases.concat([searches[i]]);
                } else if (hasSearchMode('and')) {
                    var searchPart = searches[i];
                    searchPart = searchPart.trim();
                    searchPart = searchPart.replace(/^\s*and\s+/gi, "");
                    searchPart = searchPart.replace(/\s+and\s*$/gi, "");
                    searchPart = searchPart.replace(/\sand\s/gi, " and ");
                    var searchParts = searchPart.split(' and ');
                    for (var j = 0; j < searchParts.length; j += 1) {
                        var searchPartPhrases = strToPhrases(searchParts[j]);
                        phrases = phrases.concat(searchPartPhrases);
                    }
                } else {
                    phrases = phrases.concat(strToPhrases(searches[i]));
                }
                for (var j = 0; j < phrases.length; j += 1) {
                    phrasesRegex += hasSearchMode('exact') ?
                        "(?=.*" + phrases[j] + ")" : "(?=" + phrases[j] + ")";
                }
                if (typeof KRdt.searchMode.exact === 'undefined') phrasesRegex += ".+";
