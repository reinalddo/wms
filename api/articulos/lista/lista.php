...
*/
    ...
    $sql = "SELECT SUM(Existencia) AS existencia_total FROM V_ExistenciaGralProduccion WHERE cve_almac = (SELECT id FROM c_almacenp WHERE clave = '{$almacen}') AND tipo = 'ubicacion'";
    if (!($res = mysqli_query($conn, $sql))) {
        echo "Falló la preparación: (" . mysqli_error($conn) . ") ";
    }
    $totalExistencias = mysqli_fetch_array($res);
    $totalExistencias = $totalExistencias['existencia_total'];
